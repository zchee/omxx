// Code generated by go generate ./pkg/llm/codex; DO NOT EDIT.
// Source: codex app-server generate-json-schema --experimental
// Source binary: codex-cli 0.147.0-alpha.4

package codex

import (
	"github.com/go-json-experiment/json"
	"github.com/go-json-experiment/json/jsontext"
)

// AbsolutePathBuf represents a path that is guaranteed to be absolute and normalized (though it is not guaranteed to be canonicalized or exist on the filesystem).
//
// IMPORTANT: When deserializing an `AbsolutePathBuf`, a base path must be set using [AbsolutePathBufGuard::new]. If no base path is set, the deserialization will fail unless the path being deserialized is already absolute.
type AbsolutePathBuf = string

// Account is generated from the Account schema definition.
type Account interface {
	isAccount()
}

// RawAccount preserves an uninterpreted Account JSON value.
type RawAccount jsontext.Value

func (RawAccount) isAccount() {}

var (
	_ json.MarshalerTo     = RawAccount{}
	_ json.UnmarshalerFrom = (*RawAccount)(nil)
)

func (value RawAccount) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawAccount) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// APIKeyAccount is generated from the ApiKeyAccount schema definition.
type APIKeyAccount struct {
	Type string `json:"type"`
}

func (APIKeyAccount) isAccount() {}

// ChatGPTAccount is generated from the ChatgptAccount schema definition.
type ChatGPTAccount struct {
	Email    *string  `json:"email"`
	PlanType PlanType `json:"planType"`
	Type     string   `json:"type"`
}

func (ChatGPTAccount) isAccount() {}

// AmazonBedrockAccount is generated from the AmazonBedrockAccount schema definition.
type AmazonBedrockAccount struct {
	Type                        string `json:"type"`
	UsesCodexManagedCredentials bool   `json:"usesCodexManagedCredentials,omitzero"`
}

func (AmazonBedrockAccount) isAccount() {}

func decodeGeneratedAccount(raw jsontext.Value) (Account, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "apiKey":
			var value APIKeyAccount
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "chatgpt":
			var value ChatGPTAccount
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "amazonBedrock":
			var value AmazonBedrockAccount
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawAccount(raw), nil
	}
	return RawAccount(raw), nil
}

// AccountLoginCompletedNotification is generated from the AccountLoginCompletedNotification schema definition.
type AccountLoginCompletedNotification struct {
	Error   string `json:"error,omitzero"`
	LoginID string `json:"loginId,omitzero"`
	Success bool   `json:"success"`
}

// AccountRateLimitsUpdatedNotification sparse rolling rate-limit update.
//
// Clients should merge available values into the most recent `account/rateLimits/read` response or refetch that snapshot. Nullable account metadata may be unavailable in a rolling update and does not clear a previously observed value.
type AccountRateLimitsUpdatedNotification struct {
	RateLimits RateLimitSnapshot `json:"rateLimits"`
}

// AccountTokenUsageDailyBucket is generated from the AccountTokenUsageDailyBucket schema definition.
type AccountTokenUsageDailyBucket struct {
	StartDate string `json:"startDate"`
	Tokens    int64  `json:"tokens"`
}

// AccountTokenUsageSummary is generated from the AccountTokenUsageSummary schema definition.
type AccountTokenUsageSummary struct {
	CurrentStreakDays     int64 `json:"currentStreakDays,omitzero"`
	LifetimeTokens        int64 `json:"lifetimeTokens,omitzero"`
	LongestRunningTurnSec int64 `json:"longestRunningTurnSec,omitzero"`
	LongestStreakDays     int64 `json:"longestStreakDays,omitzero"`
	PeakDailyTokens       int64 `json:"peakDailyTokens,omitzero"`
}

// AccountUpdatedNotification is generated from the AccountUpdatedNotification schema definition.
type AccountUpdatedNotification struct {
	AuthMode AuthMode `json:"authMode,omitzero"`
	PlanType PlanType `json:"planType,omitzero"`
}

func (value *AccountUpdatedNotification) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		AuthMode jsontext.Value `json:"authMode,omitzero"`
		PlanType PlanType       `json:"planType,omitzero"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	if raw.AuthMode == nil {
		value.AuthMode = nil
	} else {
		decodedAuthMode, err := decodeGeneratedAuthMode(raw.AuthMode)
		if err != nil {
			return err
		}
		value.AuthMode = decodedAuthMode
	}
	value.PlanType = raw.PlanType
	return nil
}

// ActivePermissionProfile is generated from the ActivePermissionProfile schema definition.
type ActivePermissionProfile struct {
	// Extends parent profile identifier from the selected permissions profile's `extends` setting, when present.
	Extends string `json:"extends,omitzero"`

	// ID identifier from `default_permissions` or the implicit built-in default, such as `:workspace` or a user-defined `[permissions.<id>]` profile.
	ID string `json:"id"`
}

// AddCreditsNudgeCreditType is generated from the AddCreditsNudgeCreditType schema definition.
type AddCreditsNudgeCreditType string

const (
	// AddCreditsNudgeCreditTypeCredits is the "credits" AddCreditsNudgeCreditType value.
	AddCreditsNudgeCreditTypeCredits AddCreditsNudgeCreditType = "credits"
	// AddCreditsNudgeCreditTypeUsageLimit is the "usage_limit" AddCreditsNudgeCreditType value.
	AddCreditsNudgeCreditTypeUsageLimit AddCreditsNudgeCreditType = "usage_limit"
)

// AddCreditsNudgeEmailStatus is generated from the AddCreditsNudgeEmailStatus schema definition.
type AddCreditsNudgeEmailStatus string

const (
	// AddCreditsNudgeEmailStatusSent is the "sent" AddCreditsNudgeEmailStatus value.
	AddCreditsNudgeEmailStatusSent AddCreditsNudgeEmailStatus = "sent"
	// AddCreditsNudgeEmailStatusCooldownActive is the "cooldown_active" AddCreditsNudgeEmailStatus value.
	AddCreditsNudgeEmailStatusCooldownActive AddCreditsNudgeEmailStatus = "cooldown_active"
)

// AdditionalContextEntry is generated from the AdditionalContextEntry schema definition.
type AdditionalContextEntry struct {
	Kind  AdditionalContextKind `json:"kind"`
	Value string                `json:"value"`
}

// AdditionalContextKind is generated from the AdditionalContextKind schema definition.
type AdditionalContextKind string

const (
	// AdditionalContextKindUntrusted is the "untrusted" AdditionalContextKind value.
	AdditionalContextKindUntrusted AdditionalContextKind = "untrusted"
	// AdditionalContextKindApplication is the "application" AdditionalContextKind value.
	AdditionalContextKindApplication AdditionalContextKind = "application"
)

// AdditionalFileSystemPermissions is generated from the AdditionalFileSystemPermissions schema definition.
type AdditionalFileSystemPermissions struct {
	Entries          []FileSystemSandboxEntry `json:"entries,omitzero"`
	GlobScanMaxDepth uint                     `json:"globScanMaxDepth,omitzero"`

	// Read this will be removed in favor of `entries`.
	Read []LegacyAppPathString `json:"read,omitzero"`

	// Write this will be removed in favor of `entries`.
	Write []LegacyAppPathString `json:"write,omitzero"`
}

// AdditionalNetworkPermissions is generated from the AdditionalNetworkPermissions schema definition.
type AdditionalNetworkPermissions struct {
	Enabled bool `json:"enabled,omitzero"`
}

// AgentMessageDeltaNotification is generated from the AgentMessageDeltaNotification schema definition.
type AgentMessageDeltaNotification struct {
	Delta    string `json:"delta"`
	ItemID   string `json:"itemId"`
	ThreadID string `json:"threadId"`
	TurnID   string `json:"turnId"`
}

// AgentMessageInputContent is generated from the AgentMessageInputContent schema definition.
type AgentMessageInputContent interface {
	isAgentMessageInputContent()
}

// RawAgentMessageInputContent preserves an uninterpreted AgentMessageInputContent JSON value.
type RawAgentMessageInputContent jsontext.Value

func (RawAgentMessageInputContent) isAgentMessageInputContent() {}

var (
	_ json.MarshalerTo     = RawAgentMessageInputContent{}
	_ json.UnmarshalerFrom = (*RawAgentMessageInputContent)(nil)
)

func (value RawAgentMessageInputContent) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawAgentMessageInputContent) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// InputTextAgentMessageInputContent is generated from the InputTextAgentMessageInputContent schema definition.
type InputTextAgentMessageInputContent struct {
	Text string `json:"text"`
	Type string `json:"type"`
}

func (InputTextAgentMessageInputContent) isAgentMessageInputContent() {}

// EncryptedContentAgentMessageInputContent is generated from the EncryptedContentAgentMessageInputContent schema definition.
type EncryptedContentAgentMessageInputContent struct {
	EncryptedContent string `json:"encrypted_content"`
	Type             string `json:"type"`
}

func (EncryptedContentAgentMessageInputContent) isAgentMessageInputContent() {}

func decodeGeneratedAgentMessageInputContent(raw jsontext.Value) (AgentMessageInputContent, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "input_text":
			var value InputTextAgentMessageInputContent
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "encrypted_content":
			var value EncryptedContentAgentMessageInputContent
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawAgentMessageInputContent(raw), nil
	}
	return RawAgentMessageInputContent(raw), nil
}

// AgentPath is generated from the AgentPath schema definition.
type AgentPath = string

// AnalyticsConfig is generated from the AnalyticsConfig schema definition.
type AnalyticsConfig struct {
	Enabled bool `json:"enabled,omitzero"`
}

// AppBranding app metadata returned by app-list APIs.
type AppBranding struct {
	Category          string `json:"category,omitzero"`
	Developer         string `json:"developer,omitzero"`
	IsDiscoverableApp bool   `json:"isDiscoverableApp"`
	PrivacyPolicy     string `json:"privacyPolicy,omitzero"`
	TermsOfService    string `json:"termsOfService,omitzero"`
	Website           string `json:"website,omitzero"`
}

// AppConfig is generated from the AppConfig schema definition.
type AppConfig struct {
	ApprovalsReviewer        ApprovalsReviewer `json:"approvals_reviewer,omitzero"`
	DefaultToolsApprovalMode AppToolApproval   `json:"default_tools_approval_mode,omitzero"`
	DefaultToolsEnabled      bool              `json:"default_tools_enabled,omitzero"`
	DestructiveEnabled       bool              `json:"destructive_enabled,omitzero"`
	Enabled                  bool              `json:"enabled,omitzero"`
	OpenWorldEnabled         bool              `json:"open_world_enabled,omitzero"`
	Tools                    AppToolsConfig    `json:"tools,omitzero"`
}

// AppInfo app metadata returned by app-list APIs.
type AppInfo struct {
	AppMetadata         AppMetadata       `json:"appMetadata,omitzero"`
	Branding            AppBranding       `json:"branding,omitzero"`
	Description         string            `json:"description,omitzero"`
	DistributionChannel string            `json:"distributionChannel,omitzero"`
	IconAssets          map[string]string `json:"iconAssets,omitzero"`
	IconDarkAssets      map[string]string `json:"iconDarkAssets,omitzero"`
	ID                  string            `json:"id"`
	InstallURL          string            `json:"installUrl,omitzero"`
	IsAccessible        bool              `json:"isAccessible,omitzero"`

	// IsEnabled reports whether this app is enabled in config.toml. Example: ```toml [apps.bad_app] enabled = false ```
	IsEnabled          bool              `json:"isEnabled,omitzero"`
	Labels             map[string]string `json:"labels,omitzero"`
	LogoURL            string            `json:"logoUrl,omitzero"`
	LogoURLDark        string            `json:"logoUrlDark,omitzero"`
	Name               string            `json:"name"`
	PluginDisplayNames []string          `json:"pluginDisplayNames,omitzero"`
}

// AppListUpdatedNotification notification emitted when the app list changes.
type AppListUpdatedNotification struct {
	Data []AppInfo `json:"data"`
}

// AppMetadata is generated from the AppMetadata schema definition.
type AppMetadata struct {
	Categories                 []string        `json:"categories,omitzero"`
	Developer                  string          `json:"developer,omitzero"`
	FirstPartyRequiresInstall  bool            `json:"firstPartyRequiresInstall,omitzero"`
	Review                     AppReview       `json:"review,omitzero"`
	Screenshots                []AppScreenshot `json:"screenshots,omitzero"`
	SeoDescription             string          `json:"seoDescription,omitzero"`
	ShowInComposerWhenUnlinked bool            `json:"showInComposerWhenUnlinked,omitzero"`
	SubCategories              []string        `json:"subCategories,omitzero"`
	Version                    string          `json:"version,omitzero"`
	VersionID                  string          `json:"versionId,omitzero"`
	VersionNotes               string          `json:"versionNotes,omitzero"`
}

// AppReview is generated from the AppReview schema definition.
type AppReview struct {
	Status string `json:"status"`
}

// AppScreenshot is generated from the AppScreenshot schema definition.
type AppScreenshot struct {
	FileID     string `json:"fileId,omitzero"`
	URL        string `json:"url,omitzero"`
	UserPrompt string `json:"userPrompt"`
}

// AppSummary app metadata summary for plugin responses.
type AppSummary struct {
	Category    string `json:"category,omitzero"`
	Description string `json:"description,omitzero"`
	ID          string `json:"id"`
	InstallURL  string `json:"installUrl,omitzero"`
	Name        string `json:"name"`
}

// AppTemplateSummary is generated from the AppTemplateSummary schema definition.
type AppTemplateSummary struct {
	CanonicalConnectorID string                       `json:"canonicalConnectorId,omitzero"`
	Category             string                       `json:"category,omitzero"`
	Description          string                       `json:"description,omitzero"`
	LogoURL              string                       `json:"logoUrl,omitzero"`
	LogoURLDark          string                       `json:"logoUrlDark,omitzero"`
	MaterializedAppIDs   []string                     `json:"materializedAppIds"`
	Name                 string                       `json:"name"`
	Reason               AppTemplateUnavailableReason `json:"reason,omitzero"`
	TemplateID           string                       `json:"templateId"`
}

// AppTemplateUnavailableReason is generated from the AppTemplateUnavailableReason schema definition.
type AppTemplateUnavailableReason string

const (
	// AppTemplateUnavailableReasonNOTCONFIGUREDFORWORKSPACE is the "NOT_CONFIGURED_FOR_WORKSPACE" AppTemplateUnavailableReason value.
	AppTemplateUnavailableReasonNOTCONFIGUREDFORWORKSPACE AppTemplateUnavailableReason = "NOT_CONFIGURED_FOR_WORKSPACE"
	// AppTemplateUnavailableReasonNOACTIVEWORKSPACE is the "NO_ACTIVE_WORKSPACE" AppTemplateUnavailableReason value.
	AppTemplateUnavailableReasonNOACTIVEWORKSPACE AppTemplateUnavailableReason = "NO_ACTIVE_WORKSPACE"
)

// AppToolApproval is generated from the AppToolApproval schema definition.
type AppToolApproval string

const (
	// AppToolApprovalAuto is the "auto" AppToolApproval value.
	AppToolApprovalAuto AppToolApproval = "auto"
	// AppToolApprovalPrompt is the "prompt" AppToolApproval value.
	AppToolApprovalPrompt AppToolApproval = "prompt"
	// AppToolApprovalWrites is the "writes" AppToolApproval value.
	AppToolApprovalWrites AppToolApproval = "writes"
	// AppToolApprovalApprove is the "approve" AppToolApproval value.
	AppToolApprovalApprove AppToolApproval = "approve"
)

// AppToolConfig is generated from the AppToolConfig schema definition.
type AppToolConfig struct {
	ApprovalMode AppToolApproval `json:"approval_mode,omitzero"`
	Enabled      bool            `json:"enabled,omitzero"`
}

// AppToolSummary metadata returned by app/read.
type AppToolSummary struct {
	Description    string `json:"description"`
	DisabledReason string `json:"disabledReason,omitzero"`
	IsEnabled      bool   `json:"isEnabled,omitzero"`
	IsReadOnly     bool   `json:"isReadOnly,omitzero"`
	Name           string `json:"name"`
	Title          string `json:"title,omitzero"`
}

// AppToolsConfig is generated from the AppToolsConfig schema definition.
type AppToolsConfig struct{}

// ApprovalsReviewer configures who approval requests are routed to for review. Examples include sandbox escapes, blocked network access, MCP approval prompts, and ARC escalations. Defaults to `user`. `auto_review` uses a carefully prompted subagent to gather relevant context and apply a risk-based decision framework before approving or denying the request. The legacy value `guardian_subagent` is accepted for compatibility.
type ApprovalsReviewer string

const (
	// ApprovalsReviewerUser is the "user" ApprovalsReviewer value.
	ApprovalsReviewerUser ApprovalsReviewer = "user"
	// ApprovalsReviewerAutoReview is the "auto_review" ApprovalsReviewer value.
	ApprovalsReviewerAutoReview ApprovalsReviewer = "auto_review"
	// ApprovalsReviewerGuardianSubagent is the "guardian_subagent" ApprovalsReviewer value.
	ApprovalsReviewerGuardianSubagent ApprovalsReviewer = "guardian_subagent"
)

// AppsConfig is generated from the AppsConfig schema definition.
type AppsConfig struct {
	DefaultValue AppsDefaultConfig `json:"_default,omitzero"`
}

// AppsDefaultConfig is generated from the AppsDefaultConfig schema definition.
type AppsDefaultConfig struct {
	ApprovalsReviewer        ApprovalsReviewer `json:"approvals_reviewer,omitzero"`
	DefaultToolsApprovalMode AppToolApproval   `json:"default_tools_approval_mode,omitzero"`
	DestructiveEnabled       bool              `json:"destructive_enabled,omitzero"`
	Enabled                  bool              `json:"enabled,omitzero"`
	OpenWorldEnabled         bool              `json:"open_world_enabled,omitzero"`
}

// AppsInstalledParams reads the committed installed connector runtime snapshot.
type AppsInstalledParams struct {
	// ForceRefresh when true and Apps are permitted, refresh and publish the hosted connector runtime tool snapshot first.
	ForceRefresh bool `json:"forceRefresh,omitzero"`

	// ThreadID optional loaded thread id used to evaluate effective app configuration.
	ThreadID string `json:"threadId,omitzero"`
}

// AppsInstalledResponse represents an installed connectors in one committed runtime snapshot.
type AppsInstalledResponse struct {
	Apps []InstalledApp `json:"apps"`
}

// AppsListParams list available apps/connectors.
type AppsListParams struct {
	// Cursor opaque pagination cursor returned by a previous call.
	Cursor string `json:"cursor,omitzero"`

	// ForceRefetch when true, bypass app caches and fetch the latest data from sources.
	ForceRefetch bool `json:"forceRefetch,omitzero"`

	// Limit optional page size; defaults to a reasonable server-side value.
	Limit int32 `json:"limit,omitzero"`

	// ThreadID optional thread id used to evaluate app feature gating from that thread's config.
	ThreadID string `json:"threadId,omitzero"`
}

// AppsListResponse app list response.
type AppsListResponse struct {
	Data []AppInfo `json:"data"`

	// NextCursor opaque cursor to pass to the next call to continue after the last item. If None, there are no more items to return.
	NextCursor string `json:"nextCursor,omitzero"`
}

// AppsReadParams read metadata for specific apps/connectors.
type AppsReadParams struct {
	// AppIDs app ids to read. The server accepts at most 100 ids and deduplicates repeated ids while preserving their first-request order.
	AppIDs []string `json:"appIds"`

	// IncludeTools when true, include display-only public tool summaries in the returned metadata.
	IncludeTools bool `json:"includeTools,omitzero"`
}

// AppsReadResponse app/read response.
type AppsReadResponse struct {
	Apps          []ConnectorMetadata `json:"apps"`
	MissingAppIDs []string            `json:"missingAppIds"`
}

// AskForApproval is generated from the AskForApproval schema definition.
type AskForApproval interface {
	isAskForApproval()
}

// RawAskForApproval preserves an uninterpreted AskForApproval JSON value.
type RawAskForApproval jsontext.Value

func (RawAskForApproval) isAskForApproval() {}

var (
	_ json.MarshalerTo     = RawAskForApproval{}
	_ json.UnmarshalerFrom = (*RawAskForApproval)(nil)
)

func (value RawAskForApproval) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawAskForApproval) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// AskForApprovalValue is a string-valued AskForApproval variant.
type AskForApprovalValue string

func (AskForApprovalValue) isAskForApproval() {}

const (
	// AskForApprovalValueNever is the "never" AskForApprovalValue value.
	AskForApprovalValueNever AskForApprovalValue = "never"
	// AskForApprovalValueOnRequest is the "on-request" AskForApprovalValue value.
	AskForApprovalValueOnRequest AskForApprovalValue = "on-request"
	// AskForApprovalValueUntrusted is the "untrusted" AskForApprovalValue value.
	AskForApprovalValueUntrusted AskForApprovalValue = "untrusted"
)

// Granular is generated from the Granular schema definition.
type Granular struct {
	MCPElicitations    bool `json:"mcp_elicitations"`
	RequestPermissions bool `json:"request_permissions,omitzero"`
	Rules              bool `json:"rules"`
	SandboxApproval    bool `json:"sandbox_approval"`
	SkillApproval      bool `json:"skill_approval,omitzero"`
}

// GranularAskForApproval is generated from the GranularAskForApproval schema definition.
type GranularAskForApproval struct {
	Granular Granular `json:"granular"`
}

func (GranularAskForApproval) isAskForApproval() {}

func decodeGeneratedAskForApproval(raw jsontext.Value) (AskForApproval, error) {
	if raw == nil {
		return nil, nil
	}
	var text string
	if err := json.Unmarshal(raw, &text); err == nil {
		switch text {
		case "never":
			return AskForApprovalValue(text), nil
		case "on-request":
			return AskForApprovalValue(text), nil
		case "untrusted":
			return AskForApprovalValue(text), nil
		}
		return RawAskForApproval(raw), nil
	}
	var object struct {
		Granular jsontext.Value `json:"granular"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		if object.Granular != nil {
			var value GranularAskForApproval
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawAskForApproval(raw), nil
	}
	return RawAskForApproval(raw), nil
}

// AuthMode authentication mode for OpenAI-backed providers.
type AuthMode interface {
	isAuthMode()
}

// RawAuthMode preserves an uninterpreted AuthMode JSON value.
type RawAuthMode jsontext.Value

func (RawAuthMode) isAuthMode() {}

var (
	_ json.MarshalerTo     = RawAuthMode{}
	_ json.UnmarshalerFrom = (*RawAuthMode)(nil)
)

func (value RawAuthMode) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawAuthMode) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// AuthModeValue is a string-valued AuthMode variant.
type AuthModeValue string

func (AuthModeValue) isAuthMode() {}

const (
	// AuthModeValueAgentIDentity is the "agentIdentity" AuthModeValue value.
	AuthModeValueAgentIDentity AuthModeValue = "agentIdentity"
	// AuthModeValueApikey is the "apikey" AuthModeValue value.
	AuthModeValueApikey AuthModeValue = "apikey"
	// AuthModeValueBedrockAPIKey is the "bedrockApiKey" AuthModeValue value.
	AuthModeValueBedrockAPIKey AuthModeValue = "bedrockApiKey"
	// AuthModeValueChatGPT is the "chatgpt" AuthModeValue value.
	AuthModeValueChatGPT AuthModeValue = "chatgpt"
	// AuthModeValueChatGPTAuthTokens is the "chatgptAuthTokens" AuthModeValue value.
	AuthModeValueChatGPTAuthTokens AuthModeValue = "chatgptAuthTokens"
	// AuthModeValueHeaders is the "headers" AuthModeValue value.
	AuthModeValueHeaders AuthModeValue = "headers"
	// AuthModeValuePersonalAccessToken is the "personalAccessToken" AuthModeValue value.
	AuthModeValuePersonalAccessToken AuthModeValue = "personalAccessToken"
)

func decodeGeneratedAuthMode(raw jsontext.Value) (AuthMode, error) {
	if raw == nil {
		return nil, nil
	}
	var text string
	if err := json.Unmarshal(raw, &text); err == nil {
		switch text {
		case "agentIdentity":
			return AuthModeValue(text), nil
		case "apikey":
			return AuthModeValue(text), nil
		case "bedrockApiKey":
			return AuthModeValue(text), nil
		case "chatgpt":
			return AuthModeValue(text), nil
		case "chatgptAuthTokens":
			return AuthModeValue(text), nil
		case "headers":
			return AuthModeValue(text), nil
		case "personalAccessToken":
			return AuthModeValue(text), nil
		}
		return RawAuthMode(raw), nil
	}
	return RawAuthMode(raw), nil
}

// AutoCompactTokenLimitScope selects which part of the active context is charged against `model_auto_compact_token_limit`.
type AutoCompactTokenLimitScope interface {
	isAutoCompactTokenLimitScope()
}

// RawAutoCompactTokenLimitScope preserves an uninterpreted AutoCompactTokenLimitScope JSON value.
type RawAutoCompactTokenLimitScope jsontext.Value

func (RawAutoCompactTokenLimitScope) isAutoCompactTokenLimitScope() {}

var (
	_ json.MarshalerTo     = RawAutoCompactTokenLimitScope{}
	_ json.UnmarshalerFrom = (*RawAutoCompactTokenLimitScope)(nil)
)

func (value RawAutoCompactTokenLimitScope) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawAutoCompactTokenLimitScope) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// AutoCompactTokenLimitScopeValue is a string-valued AutoCompactTokenLimitScope variant.
type AutoCompactTokenLimitScopeValue string

func (AutoCompactTokenLimitScopeValue) isAutoCompactTokenLimitScope() {}

const (
	// AutoCompactTokenLimitScopeValueBodyAfterPrefix is the "body_after_prefix" AutoCompactTokenLimitScopeValue value.
	AutoCompactTokenLimitScopeValueBodyAfterPrefix AutoCompactTokenLimitScopeValue = "body_after_prefix"
	// AutoCompactTokenLimitScopeValueTotal is the "total" AutoCompactTokenLimitScopeValue value.
	AutoCompactTokenLimitScopeValueTotal AutoCompactTokenLimitScopeValue = "total"
)

func decodeGeneratedAutoCompactTokenLimitScope(raw jsontext.Value) (AutoCompactTokenLimitScope, error) {
	if raw == nil {
		return nil, nil
	}
	var text string
	if err := json.Unmarshal(raw, &text); err == nil {
		switch text {
		case "body_after_prefix":
			return AutoCompactTokenLimitScopeValue(text), nil
		case "total":
			return AutoCompactTokenLimitScopeValue(text), nil
		}
		return RawAutoCompactTokenLimitScope(raw), nil
	}
	return RawAutoCompactTokenLimitScope(raw), nil
}

// AutoReviewDecisionSource [UNSTABLE] Source that produced a terminal approval auto-review decision.
type AutoReviewDecisionSource string

const (
	// AutoReviewDecisionSourceAgent is the "agent" AutoReviewDecisionSource value.
	AutoReviewDecisionSourceAgent AutoReviewDecisionSource = "agent"
)

// BrowserUseRequirements is generated from the BrowserUseRequirements schema definition.
type BrowserUseRequirements struct {
	DisableAutoReview bool `json:"disableAutoReview,omitzero"`
}

// ByteRange is generated from the ByteRange schema definition.
type ByteRange struct {
	End   uint `json:"end"`
	Start uint `json:"start"`
}

// CancelLoginAccountParams is generated from the CancelLoginAccountParams schema definition.
type CancelLoginAccountParams struct {
	LoginID string `json:"loginId"`
}

// CancelLoginAccountResponse is generated from the CancelLoginAccountResponse schema definition.
type CancelLoginAccountResponse struct {
	Status CancelLoginAccountStatus `json:"status"`
}

// CancelLoginAccountStatus is generated from the CancelLoginAccountStatus schema definition.
type CancelLoginAccountStatus string

const (
	// CancelLoginAccountStatusCanceled is the "canceled" CancelLoginAccountStatus value.
	CancelLoginAccountStatusCanceled CancelLoginAccountStatus = "canceled"
	// CancelLoginAccountStatusNotFound is the "notFound" CancelLoginAccountStatus value.
	CancelLoginAccountStatusNotFound CancelLoginAccountStatus = "notFound"
)

// EnvironmentCapabilityRootLocation represents a path owned by an execution environment.
type EnvironmentCapabilityRootLocation struct {
	EnvironmentID string `json:"environmentId"`

	// Path absolute path for the root in the selected environment.
	Path string `json:"path"`
	Type string `json:"type"`
}

// CapabilityRootLocation location used to resolve a selected capability root.
type CapabilityRootLocation = EnvironmentCapabilityRootLocation

func (EnvironmentCapabilityRootLocation) isCapabilityRootLocation() {}

// ClientInfo is generated from the ClientInfo schema definition.
type ClientInfo struct {
	Name    string `json:"name"`
	Title   string `json:"title,omitzero"`
	Version string `json:"version"`
}

// ClientRequest requests from the client to the server.
type ClientRequest interface {
	isClientRequest()
}

// RawClientRequest preserves an uninterpreted ClientRequest JSON value.
type RawClientRequest jsontext.Value

func (RawClientRequest) isClientRequest() {}

var (
	_ json.MarshalerTo     = RawClientRequest{}
	_ json.UnmarshalerFrom = (*RawClientRequest)(nil)
)

func (value RawClientRequest) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawClientRequest) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

const (
	// RequestMethodInitialize is the "initialize" ClientRequest method.
	RequestMethodInitialize = "initialize"
	// RequestMethodThreadStart is the "thread/start" ClientRequest method.
	RequestMethodThreadStart = "thread/start"
	// RequestMethodThreadResume is the "thread/resume" ClientRequest method.
	RequestMethodThreadResume = "thread/resume"
	// RequestMethodThreadFork is the "thread/fork" ClientRequest method.
	RequestMethodThreadFork = "thread/fork"
	// RequestMethodThreadArchive is the "thread/archive" ClientRequest method.
	RequestMethodThreadArchive = "thread/archive"
	// RequestMethodThreadDelete is the "thread/delete" ClientRequest method.
	RequestMethodThreadDelete = "thread/delete"
	// RequestMethodThreadUnsubscribe is the "thread/unsubscribe" ClientRequest method.
	RequestMethodThreadUnsubscribe = "thread/unsubscribe"
	// RequestMethodThreadIncrementElicitation is the "thread/increment_elicitation" ClientRequest method.
	RequestMethodThreadIncrementElicitation = "thread/increment_elicitation"
	// RequestMethodThreadDecrementElicitation is the "thread/decrement_elicitation" ClientRequest method.
	RequestMethodThreadDecrementElicitation = "thread/decrement_elicitation"
	// RequestMethodThreadNameSet is the "thread/name/set" ClientRequest method.
	RequestMethodThreadNameSet = "thread/name/set"
	// RequestMethodThreadGoalSet is the "thread/goal/set" ClientRequest method.
	RequestMethodThreadGoalSet = "thread/goal/set"
	// RequestMethodThreadGoalGet is the "thread/goal/get" ClientRequest method.
	RequestMethodThreadGoalGet = "thread/goal/get"
	// RequestMethodThreadGoalClear is the "thread/goal/clear" ClientRequest method.
	RequestMethodThreadGoalClear = "thread/goal/clear"
	// RequestMethodThreadMetadataUpdate is the "thread/metadata/update" ClientRequest method.
	RequestMethodThreadMetadataUpdate = "thread/metadata/update"
	// RequestMethodThreadSectionMove is the "thread/section/move" ClientRequest method.
	RequestMethodThreadSectionMove = "thread/section/move"
	// RequestMethodThreadSettingsUpdate is the "thread/settings/update" ClientRequest method.
	RequestMethodThreadSettingsUpdate = "thread/settings/update"
	// RequestMethodThreadMemoryModeSet is the "thread/memoryMode/set" ClientRequest method.
	RequestMethodThreadMemoryModeSet = "thread/memoryMode/set"
	// RequestMethodMemoryReset is the "memory/reset" ClientRequest method.
	RequestMethodMemoryReset = "memory/reset"
	// RequestMethodThreadUnarchive is the "thread/unarchive" ClientRequest method.
	RequestMethodThreadUnarchive = "thread/unarchive"
	// RequestMethodThreadCompactStart is the "thread/compact/start" ClientRequest method.
	RequestMethodThreadCompactStart = "thread/compact/start"
	// RequestMethodThreadShellCommand is the "thread/shellCommand" ClientRequest method.
	RequestMethodThreadShellCommand = "thread/shellCommand"
	// RequestMethodThreadApproveGuardianDeniedAction is the "thread/approveGuardianDeniedAction" ClientRequest method.
	RequestMethodThreadApproveGuardianDeniedAction = "thread/approveGuardianDeniedAction"
	// RequestMethodThreadBackgroundTerminalsClean is the "thread/backgroundTerminals/clean" ClientRequest method.
	RequestMethodThreadBackgroundTerminalsClean = "thread/backgroundTerminals/clean"
	// RequestMethodThreadBackgroundTerminalsList is the "thread/backgroundTerminals/list" ClientRequest method.
	RequestMethodThreadBackgroundTerminalsList = "thread/backgroundTerminals/list"
	// RequestMethodThreadBackgroundTerminalsTerminate is the "thread/backgroundTerminals/terminate" ClientRequest method.
	RequestMethodThreadBackgroundTerminalsTerminate = "thread/backgroundTerminals/terminate"
	// RequestMethodThreadRollback is the "thread/rollback" ClientRequest method.
	RequestMethodThreadRollback = "thread/rollback"
	// RequestMethodThreadList is the "thread/list" ClientRequest method.
	RequestMethodThreadList = "thread/list"
	// RequestMethodThreadSectionList is the "threadSection/list" ClientRequest method.
	RequestMethodThreadSectionList = "threadSection/list"
	// RequestMethodThreadSearch is the "thread/search" ClientRequest method.
	RequestMethodThreadSearch = "thread/search"
	// RequestMethodThreadSearchOccurrences is the "thread/searchOccurrences" ClientRequest method.
	RequestMethodThreadSearchOccurrences = "thread/searchOccurrences"
	// RequestMethodThreadLoadedList is the "thread/loaded/list" ClientRequest method.
	RequestMethodThreadLoadedList = "thread/loaded/list"
	// RequestMethodThreadRead is the "thread/read" ClientRequest method.
	RequestMethodThreadRead = "thread/read"
	// RequestMethodThreadTurnsList is the "thread/turns/list" ClientRequest method.
	RequestMethodThreadTurnsList = "thread/turns/list"
	// RequestMethodThreadItemsList is the "thread/items/list" ClientRequest method.
	RequestMethodThreadItemsList = "thread/items/list"
	// RequestMethodThreadInjectItems is the "thread/inject_items" ClientRequest method.
	RequestMethodThreadInjectItems = "thread/inject_items"
	// RequestMethodSkillsList is the "skills/list" ClientRequest method.
	RequestMethodSkillsList = "skills/list"
	// RequestMethodSkillsExtraRootsSet is the "skills/extraRoots/set" ClientRequest method.
	RequestMethodSkillsExtraRootsSet = "skills/extraRoots/set"
	// RequestMethodHooksList is the "hooks/list" ClientRequest method.
	RequestMethodHooksList = "hooks/list"
	// RequestMethodMarketplaceAdd is the "marketplace/add" ClientRequest method.
	RequestMethodMarketplaceAdd = "marketplace/add"
	// RequestMethodMarketplaceRemove is the "marketplace/remove" ClientRequest method.
	RequestMethodMarketplaceRemove = "marketplace/remove"
	// RequestMethodMarketplaceUpgrade is the "marketplace/upgrade" ClientRequest method.
	RequestMethodMarketplaceUpgrade = "marketplace/upgrade"
	// RequestMethodPluginList is the "plugin/list" ClientRequest method.
	RequestMethodPluginList = "plugin/list"
	// RequestMethodPluginInstalled is the "plugin/installed" ClientRequest method.
	RequestMethodPluginInstalled = "plugin/installed"
	// RequestMethodPluginRead is the "plugin/read" ClientRequest method.
	RequestMethodPluginRead = "plugin/read"
	// RequestMethodPluginSkillRead is the "plugin/skill/read" ClientRequest method.
	RequestMethodPluginSkillRead = "plugin/skill/read"
	// RequestMethodPluginShareSave is the "plugin/share/save" ClientRequest method.
	RequestMethodPluginShareSave = "plugin/share/save"
	// RequestMethodPluginShareUpdateTargets is the "plugin/share/updateTargets" ClientRequest method.
	RequestMethodPluginShareUpdateTargets = "plugin/share/updateTargets"
	// RequestMethodPluginShareList is the "plugin/share/list" ClientRequest method.
	RequestMethodPluginShareList = "plugin/share/list"
	// RequestMethodPluginShareCheckout is the "plugin/share/checkout" ClientRequest method.
	RequestMethodPluginShareCheckout = "plugin/share/checkout"
	// RequestMethodPluginShareDelete is the "plugin/share/delete" ClientRequest method.
	RequestMethodPluginShareDelete = "plugin/share/delete"
	// RequestMethodAppRead is the "app/read" ClientRequest method.
	RequestMethodAppRead = "app/read"
	// RequestMethodAppList is the "app/list" ClientRequest method.
	RequestMethodAppList = "app/list"
	// RequestMethodAppInstalled is the "app/installed" ClientRequest method.
	RequestMethodAppInstalled = "app/installed"
	// RequestMethodFSReadFile is the "fs/readFile" ClientRequest method.
	RequestMethodFSReadFile = "fs/readFile"
	// RequestMethodFSWriteFile is the "fs/writeFile" ClientRequest method.
	RequestMethodFSWriteFile = "fs/writeFile"
	// RequestMethodFSCreateDirectory is the "fs/createDirectory" ClientRequest method.
	RequestMethodFSCreateDirectory = "fs/createDirectory"
	// RequestMethodFSGetMetadata is the "fs/getMetadata" ClientRequest method.
	RequestMethodFSGetMetadata = "fs/getMetadata"
	// RequestMethodFSReadDirectory is the "fs/readDirectory" ClientRequest method.
	RequestMethodFSReadDirectory = "fs/readDirectory"
	// RequestMethodFSRemove is the "fs/remove" ClientRequest method.
	RequestMethodFSRemove = "fs/remove"
	// RequestMethodFSCopy is the "fs/copy" ClientRequest method.
	RequestMethodFSCopy = "fs/copy"
	// RequestMethodFSWatch is the "fs/watch" ClientRequest method.
	RequestMethodFSWatch = "fs/watch"
	// RequestMethodFSUnwatch is the "fs/unwatch" ClientRequest method.
	RequestMethodFSUnwatch = "fs/unwatch"
	// RequestMethodSkillsConfigWrite is the "skills/config/write" ClientRequest method.
	RequestMethodSkillsConfigWrite = "skills/config/write"
	// RequestMethodPluginInstall is the "plugin/install" ClientRequest method.
	RequestMethodPluginInstall = "plugin/install"
	// RequestMethodPluginUninstall is the "plugin/uninstall" ClientRequest method.
	RequestMethodPluginUninstall = "plugin/uninstall"
	// RequestMethodTurnStart is the "turn/start" ClientRequest method.
	RequestMethodTurnStart = "turn/start"
	// RequestMethodTurnSteer is the "turn/steer" ClientRequest method.
	RequestMethodTurnSteer = "turn/steer"
	// RequestMethodTurnInterrupt is the "turn/interrupt" ClientRequest method.
	RequestMethodTurnInterrupt = "turn/interrupt"
	// RequestMethodThreadRealtimeStart is the "thread/realtime/start" ClientRequest method.
	RequestMethodThreadRealtimeStart = "thread/realtime/start"
	// RequestMethodThreadRealtimeAppendAudio is the "thread/realtime/appendAudio" ClientRequest method.
	RequestMethodThreadRealtimeAppendAudio = "thread/realtime/appendAudio"
	// RequestMethodThreadRealtimeAppendText is the "thread/realtime/appendText" ClientRequest method.
	RequestMethodThreadRealtimeAppendText = "thread/realtime/appendText"
	// RequestMethodThreadRealtimeAppendSpeech is the "thread/realtime/appendSpeech" ClientRequest method.
	RequestMethodThreadRealtimeAppendSpeech = "thread/realtime/appendSpeech"
	// RequestMethodThreadRealtimeStop is the "thread/realtime/stop" ClientRequest method.
	RequestMethodThreadRealtimeStop = "thread/realtime/stop"
	// RequestMethodThreadRealtimeListVoices is the "thread/realtime/listVoices" ClientRequest method.
	RequestMethodThreadRealtimeListVoices = "thread/realtime/listVoices"
	// RequestMethodReviewStart is the "review/start" ClientRequest method.
	RequestMethodReviewStart = "review/start"
	// RequestMethodModelList is the "model/list" ClientRequest method.
	RequestMethodModelList = "model/list"
	// RequestMethodModelProviderCapabilitiesRead is the "modelProvider/capabilities/read" ClientRequest method.
	RequestMethodModelProviderCapabilitiesRead = "modelProvider/capabilities/read"
	// RequestMethodExperimentalFeatureList is the "experimentalFeature/list" ClientRequest method.
	RequestMethodExperimentalFeatureList = "experimentalFeature/list"
	// RequestMethodPermissionProfileList is the "permissionProfile/list" ClientRequest method.
	RequestMethodPermissionProfileList = "permissionProfile/list"
	// RequestMethodExperimentalFeatureEnablementSet is the "experimentalFeature/enablement/set" ClientRequest method.
	RequestMethodExperimentalFeatureEnablementSet = "experimentalFeature/enablement/set"
	// RequestMethodRemoteControlEnable is the "remoteControl/enable" ClientRequest method.
	RequestMethodRemoteControlEnable = "remoteControl/enable"
	// RequestMethodRemoteControlDisable is the "remoteControl/disable" ClientRequest method.
	RequestMethodRemoteControlDisable = "remoteControl/disable"
	// RequestMethodRemoteControlStatusRead is the "remoteControl/status/read" ClientRequest method.
	RequestMethodRemoteControlStatusRead = "remoteControl/status/read"
	// RequestMethodRemoteControlPairingStart is the "remoteControl/pairing/start" ClientRequest method.
	RequestMethodRemoteControlPairingStart = "remoteControl/pairing/start"
	// RequestMethodRemoteControlPairingStatus is the "remoteControl/pairing/status" ClientRequest method.
	RequestMethodRemoteControlPairingStatus = "remoteControl/pairing/status"
	// RequestMethodRemoteControlClientList is the "remoteControl/client/list" ClientRequest method.
	RequestMethodRemoteControlClientList = "remoteControl/client/list"
	// RequestMethodRemoteControlClientRevoke is the "remoteControl/client/revoke" ClientRequest method.
	RequestMethodRemoteControlClientRevoke = "remoteControl/client/revoke"
	// RequestMethodCollaborationModeList is the "collaborationMode/list" ClientRequest method.
	RequestMethodCollaborationModeList = "collaborationMode/list"
	// RequestMethodMockExperimentalMethod is the "mock/experimentalMethod" ClientRequest method.
	RequestMethodMockExperimentalMethod = "mock/experimentalMethod"
	// RequestMethodEnvironmentAdd is the "environment/add" ClientRequest method.
	RequestMethodEnvironmentAdd = "environment/add"
	// RequestMethodEnvironmentInfo is the "environment/info" ClientRequest method.
	RequestMethodEnvironmentInfo = "environment/info"
	// RequestMethodEnvironmentStatus is the "environment/status" ClientRequest method.
	RequestMethodEnvironmentStatus = "environment/status"
	// RequestMethodMCPServerOAuthLogin is the "mcpServer/oauth/login" ClientRequest method.
	RequestMethodMCPServerOAuthLogin = "mcpServer/oauth/login"
	// RequestMethodConfigMCPServerReload is the "config/mcpServer/reload" ClientRequest method.
	RequestMethodConfigMCPServerReload = "config/mcpServer/reload"
	// RequestMethodMCPServerStatusList is the "mcpServerStatus/list" ClientRequest method.
	RequestMethodMCPServerStatusList = "mcpServerStatus/list"
	// RequestMethodMCPServerResourceRead is the "mcpServer/resource/read" ClientRequest method.
	RequestMethodMCPServerResourceRead = "mcpServer/resource/read"
	// RequestMethodMCPServerToolCall is the "mcpServer/tool/call" ClientRequest method.
	RequestMethodMCPServerToolCall = "mcpServer/tool/call"
	// RequestMethodWindowsSandboxSetupStart is the "windowsSandbox/setupStart" ClientRequest method.
	RequestMethodWindowsSandboxSetupStart = "windowsSandbox/setupStart"
	// RequestMethodWindowsSandboxReadiness is the "windowsSandbox/readiness" ClientRequest method.
	RequestMethodWindowsSandboxReadiness = "windowsSandbox/readiness"
	// RequestMethodAccountLoginStart is the "account/login/start" ClientRequest method.
	RequestMethodAccountLoginStart = "account/login/start"
	// RequestMethodAccountLoginCancel is the "account/login/cancel" ClientRequest method.
	RequestMethodAccountLoginCancel = "account/login/cancel"
	// RequestMethodAccountLogout is the "account/logout" ClientRequest method.
	RequestMethodAccountLogout = "account/logout"
	// RequestMethodAccountRateLimitsRead is the "account/rateLimits/read" ClientRequest method.
	RequestMethodAccountRateLimitsRead = "account/rateLimits/read"
	// RequestMethodAccountRateLimitResetCreditConsume is the "account/rateLimitResetCredit/consume" ClientRequest method.
	RequestMethodAccountRateLimitResetCreditConsume = "account/rateLimitResetCredit/consume"
	// RequestMethodAccountUsageRead is the "account/usage/read" ClientRequest method.
	RequestMethodAccountUsageRead = "account/usage/read"
	// RequestMethodAccountWorkspaceMessagesRead is the "account/workspaceMessages/read" ClientRequest method.
	RequestMethodAccountWorkspaceMessagesRead = "account/workspaceMessages/read"
	// RequestMethodAccountSendAddCreditsNudgeEmail is the "account/sendAddCreditsNudgeEmail" ClientRequest method.
	RequestMethodAccountSendAddCreditsNudgeEmail = "account/sendAddCreditsNudgeEmail"
	// RequestMethodFeedbackUpload is the "feedback/upload" ClientRequest method.
	RequestMethodFeedbackUpload = "feedback/upload"
	// RequestMethodCommandExec is the "command/exec" ClientRequest method.
	RequestMethodCommandExec = "command/exec"
	// RequestMethodCommandExecWrite is the "command/exec/write" ClientRequest method.
	RequestMethodCommandExecWrite = "command/exec/write"
	// RequestMethodCommandExecTerminate is the "command/exec/terminate" ClientRequest method.
	RequestMethodCommandExecTerminate = "command/exec/terminate"
	// RequestMethodCommandExecResize is the "command/exec/resize" ClientRequest method.
	RequestMethodCommandExecResize = "command/exec/resize"
	// RequestMethodProcessSpawn is the "process/spawn" ClientRequest method.
	RequestMethodProcessSpawn = "process/spawn"
	// RequestMethodProcessWriteStdin is the "process/writeStdin" ClientRequest method.
	RequestMethodProcessWriteStdin = "process/writeStdin"
	// RequestMethodProcessKill is the "process/kill" ClientRequest method.
	RequestMethodProcessKill = "process/kill"
	// RequestMethodProcessResizePty is the "process/resizePty" ClientRequest method.
	RequestMethodProcessResizePty = "process/resizePty"
	// RequestMethodConfigRead is the "config/read" ClientRequest method.
	RequestMethodConfigRead = "config/read"
	// RequestMethodExternalAgentConfigDetect is the "externalAgentConfig/detect" ClientRequest method.
	RequestMethodExternalAgentConfigDetect = "externalAgentConfig/detect"
	// RequestMethodExternalAgentConfigImport is the "externalAgentConfig/import" ClientRequest method.
	RequestMethodExternalAgentConfigImport = "externalAgentConfig/import"
	// RequestMethodExternalAgentConfigImportRecordHistory is the "externalAgentConfig/import/recordHistory" ClientRequest method.
	RequestMethodExternalAgentConfigImportRecordHistory = "externalAgentConfig/import/recordHistory"
	// RequestMethodExternalAgentConfigImportReadHistories is the "externalAgentConfig/import/readHistories" ClientRequest method.
	RequestMethodExternalAgentConfigImportReadHistories = "externalAgentConfig/import/readHistories"
	// RequestMethodConfigValueWrite is the "config/value/write" ClientRequest method.
	RequestMethodConfigValueWrite = "config/value/write"
	// RequestMethodConfigBatchWrite is the "config/batchWrite" ClientRequest method.
	RequestMethodConfigBatchWrite = "config/batchWrite"
	// RequestMethodConfigRequirementsRead is the "configRequirements/read" ClientRequest method.
	RequestMethodConfigRequirementsRead = "configRequirements/read"
	// RequestMethodAccountRead is the "account/read" ClientRequest method.
	RequestMethodAccountRead = "account/read"
	// RequestMethodFuzzyFileSearch is the "fuzzyFileSearch" ClientRequest method.
	RequestMethodFuzzyFileSearch = "fuzzyFileSearch"
	// RequestMethodFuzzyFileSearchSessionStart is the "fuzzyFileSearch/sessionStart" ClientRequest method.
	RequestMethodFuzzyFileSearchSessionStart = "fuzzyFileSearch/sessionStart"
	// RequestMethodFuzzyFileSearchSessionUpdate is the "fuzzyFileSearch/sessionUpdate" ClientRequest method.
	RequestMethodFuzzyFileSearchSessionUpdate = "fuzzyFileSearch/sessionUpdate"
	// RequestMethodFuzzyFileSearchSessionStop is the "fuzzyFileSearch/sessionStop" ClientRequest method.
	RequestMethodFuzzyFileSearchSessionStop = "fuzzyFileSearch/sessionStop"
)

// InitializeRequest is generated from the InitializeRequest schema definition.
type InitializeRequest struct {
	ID     string           `json:"id"`
	Method string           `json:"method"`
	Params InitializeParams `json:"params"`
}

func (InitializeRequest) isClientRequest() {}

// ThreadStartRequest NEW APIs.
type ThreadStartRequest struct {
	ID     string            `json:"id"`
	Method string            `json:"method"`
	Params ThreadStartParams `json:"params"`
}

func (ThreadStartRequest) isClientRequest() {}

// ThreadResumeRequest is generated from the Thread/resumeRequest schema definition.
type ThreadResumeRequest struct {
	ID     string             `json:"id"`
	Method string             `json:"method"`
	Params ThreadResumeParams `json:"params"`
}

func (ThreadResumeRequest) isClientRequest() {}

// ThreadForkRequest is generated from the Thread/forkRequest schema definition.
type ThreadForkRequest struct {
	ID     string           `json:"id"`
	Method string           `json:"method"`
	Params ThreadForkParams `json:"params"`
}

func (ThreadForkRequest) isClientRequest() {}

// ThreadArchiveRequest is generated from the Thread/archiveRequest schema definition.
type ThreadArchiveRequest struct {
	ID     string              `json:"id"`
	Method string              `json:"method"`
	Params ThreadArchiveParams `json:"params"`
}

func (ThreadArchiveRequest) isClientRequest() {}

// ThreadDeleteRequest is generated from the Thread/deleteRequest schema definition.
type ThreadDeleteRequest struct {
	ID     string             `json:"id"`
	Method string             `json:"method"`
	Params ThreadDeleteParams `json:"params"`
}

func (ThreadDeleteRequest) isClientRequest() {}

// ThreadUnsubscribeRequest is generated from the Thread/unsubscribeRequest schema definition.
type ThreadUnsubscribeRequest struct {
	ID     string                  `json:"id"`
	Method string                  `json:"method"`
	Params ThreadUnsubscribeParams `json:"params"`
}

func (ThreadUnsubscribeRequest) isClientRequest() {}

// ThreadIncrementElicitationRequest increments the thread-local out-of-band elicitation counter.
//
// This is used by external helpers to pause timeout accounting while a user approval or other elicitation is pending outside the app-server request flow.
type ThreadIncrementElicitationRequest struct {
	ID     string                           `json:"id"`
	Method string                           `json:"method"`
	Params ThreadIncrementElicitationParams `json:"params"`
}

func (ThreadIncrementElicitationRequest) isClientRequest() {}

// ThreadDecrementElicitationRequest decrement the thread-local out-of-band elicitation counter.
//
// When the count reaches zero, timeout accounting resumes for the thread.
type ThreadDecrementElicitationRequest struct {
	ID     string                           `json:"id"`
	Method string                           `json:"method"`
	Params ThreadDecrementElicitationParams `json:"params"`
}

func (ThreadDecrementElicitationRequest) isClientRequest() {}

// ThreadNameSetRequest is generated from the Thread/name/setRequest schema definition.
type ThreadNameSetRequest struct {
	ID     string              `json:"id"`
	Method string              `json:"method"`
	Params ThreadSetNameParams `json:"params"`
}

func (ThreadNameSetRequest) isClientRequest() {}

// ThreadGoalSetRequest is generated from the Thread/goal/setRequest schema definition.
type ThreadGoalSetRequest struct {
	ID     string              `json:"id"`
	Method string              `json:"method"`
	Params ThreadGoalSetParams `json:"params"`
}

func (ThreadGoalSetRequest) isClientRequest() {}

// ThreadGoalGetRequest is generated from the Thread/goal/getRequest schema definition.
type ThreadGoalGetRequest struct {
	ID     string              `json:"id"`
	Method string              `json:"method"`
	Params ThreadGoalGetParams `json:"params"`
}

func (ThreadGoalGetRequest) isClientRequest() {}

// ThreadGoalClearRequest is generated from the Thread/goal/clearRequest schema definition.
type ThreadGoalClearRequest struct {
	ID     string                `json:"id"`
	Method string                `json:"method"`
	Params ThreadGoalClearParams `json:"params"`
}

func (ThreadGoalClearRequest) isClientRequest() {}

// ThreadMetadataUpdateRequest is generated from the Thread/metadata/updateRequest schema definition.
type ThreadMetadataUpdateRequest struct {
	ID     string                     `json:"id"`
	Method string                     `json:"method"`
	Params ThreadMetadataUpdateParams `json:"params"`
}

func (ThreadMetadataUpdateRequest) isClientRequest() {}

// ThreadSectionMoveRequest is generated from the Thread/section/moveRequest schema definition.
type ThreadSectionMoveRequest struct {
	ID     string                  `json:"id"`
	Method string                  `json:"method"`
	Params ThreadSectionMoveParams `json:"params"`
}

func (ThreadSectionMoveRequest) isClientRequest() {}

// ThreadSettingsUpdateRequest is generated from the Thread/settings/updateRequest schema definition.
type ThreadSettingsUpdateRequest struct {
	ID     string                     `json:"id"`
	Method string                     `json:"method"`
	Params ThreadSettingsUpdateParams `json:"params"`
}

func (ThreadSettingsUpdateRequest) isClientRequest() {}

// ThreadMemoryModeSetRequest is generated from the Thread/memoryMode/setRequest schema definition.
type ThreadMemoryModeSetRequest struct {
	ID     string                    `json:"id"`
	Method string                    `json:"method"`
	Params ThreadMemoryModeSetParams `json:"params"`
}

func (ThreadMemoryModeSetRequest) isClientRequest() {}

// MemoryResetRequest is generated from the Memory/resetRequest schema definition.
type MemoryResetRequest struct {
	ID     string         `json:"id"`
	Method string         `json:"method"`
	Params jsontext.Value `json:"params,omitzero"`
}

func (MemoryResetRequest) isClientRequest() {}

// ThreadUnarchiveRequest is generated from the Thread/unarchiveRequest schema definition.
type ThreadUnarchiveRequest struct {
	ID     string                `json:"id"`
	Method string                `json:"method"`
	Params ThreadUnarchiveParams `json:"params"`
}

func (ThreadUnarchiveRequest) isClientRequest() {}

// ThreadCompactStartRequest is generated from the Thread/compact/startRequest schema definition.
type ThreadCompactStartRequest struct {
	ID     string                   `json:"id"`
	Method string                   `json:"method"`
	Params ThreadCompactStartParams `json:"params"`
}

func (ThreadCompactStartRequest) isClientRequest() {}

// ThreadShellCommandRequest is generated from the Thread/shellCommandRequest schema definition.
type ThreadShellCommandRequest struct {
	ID     string                   `json:"id"`
	Method string                   `json:"method"`
	Params ThreadShellCommandParams `json:"params"`
}

func (ThreadShellCommandRequest) isClientRequest() {}

// ThreadApproveGuardianDeniedActionRequest is generated from the Thread/approveGuardianDeniedActionRequest schema definition.
type ThreadApproveGuardianDeniedActionRequest struct {
	ID     string                                  `json:"id"`
	Method string                                  `json:"method"`
	Params ThreadApproveGuardianDeniedActionParams `json:"params"`
}

func (ThreadApproveGuardianDeniedActionRequest) isClientRequest() {}

// ThreadBackgroundTerminalsCleanRequest is generated from the Thread/backgroundTerminals/cleanRequest schema definition.
type ThreadBackgroundTerminalsCleanRequest struct {
	ID     string                               `json:"id"`
	Method string                               `json:"method"`
	Params ThreadBackgroundTerminalsCleanParams `json:"params"`
}

func (ThreadBackgroundTerminalsCleanRequest) isClientRequest() {}

// ThreadBackgroundTerminalsListRequest is generated from the Thread/backgroundTerminals/listRequest schema definition.
type ThreadBackgroundTerminalsListRequest struct {
	ID     string                              `json:"id"`
	Method string                              `json:"method"`
	Params ThreadBackgroundTerminalsListParams `json:"params"`
}

func (ThreadBackgroundTerminalsListRequest) isClientRequest() {}

// ThreadBackgroundTerminalsTerminateRequest is generated from the Thread/backgroundTerminals/terminateRequest schema definition.
type ThreadBackgroundTerminalsTerminateRequest struct {
	ID     string                                   `json:"id"`
	Method string                                   `json:"method"`
	Params ThreadBackgroundTerminalsTerminateParams `json:"params"`
}

func (ThreadBackgroundTerminalsTerminateRequest) isClientRequest() {}

// ThreadRollbackRequest is generated from the Thread/rollbackRequest schema definition.
type ThreadRollbackRequest struct {
	ID     string               `json:"id"`
	Method string               `json:"method"`
	Params ThreadRollbackParams `json:"params"`
}

func (ThreadRollbackRequest) isClientRequest() {}

// ThreadListRequest is generated from the Thread/listRequest schema definition.
type ThreadListRequest struct {
	ID     string           `json:"id"`
	Method string           `json:"method"`
	Params ThreadListParams `json:"params"`
}

func (ThreadListRequest) isClientRequest() {}

// ThreadSectionListRequest is generated from the ThreadSection/listRequest schema definition.
type ThreadSectionListRequest struct {
	ID     string                  `json:"id"`
	Method string                  `json:"method"`
	Params ThreadSectionListParams `json:"params"`
}

func (ThreadSectionListRequest) isClientRequest() {}

// ThreadSearchRequest is generated from the Thread/searchRequest schema definition.
type ThreadSearchRequest struct {
	ID     string             `json:"id"`
	Method string             `json:"method"`
	Params ThreadSearchParams `json:"params"`
}

func (ThreadSearchRequest) isClientRequest() {}

// ThreadSearchOccurrencesRequest is generated from the Thread/searchOccurrencesRequest schema definition.
type ThreadSearchOccurrencesRequest struct {
	ID     string                        `json:"id"`
	Method string                        `json:"method"`
	Params ThreadSearchOccurrencesParams `json:"params"`
}

func (ThreadSearchOccurrencesRequest) isClientRequest() {}

// ThreadLoadedListRequest is generated from the Thread/loaded/listRequest schema definition.
type ThreadLoadedListRequest struct {
	ID     string                 `json:"id"`
	Method string                 `json:"method"`
	Params ThreadLoadedListParams `json:"params"`
}

func (ThreadLoadedListRequest) isClientRequest() {}

// ThreadReadRequest is generated from the Thread/readRequest schema definition.
type ThreadReadRequest struct {
	ID     string           `json:"id"`
	Method string           `json:"method"`
	Params ThreadReadParams `json:"params"`
}

func (ThreadReadRequest) isClientRequest() {}

// ThreadTurnsListRequest is generated from the Thread/turns/listRequest schema definition.
type ThreadTurnsListRequest struct {
	ID     string                `json:"id"`
	Method string                `json:"method"`
	Params ThreadTurnsListParams `json:"params"`
}

func (ThreadTurnsListRequest) isClientRequest() {}

// ThreadItemsListRequest is generated from the Thread/items/listRequest schema definition.
type ThreadItemsListRequest struct {
	ID     string                `json:"id"`
	Method string                `json:"method"`
	Params ThreadItemsListParams `json:"params"`
}

func (ThreadItemsListRequest) isClientRequest() {}

// ThreadInjectItemsRequest appends raw Responses API items to the thread history without starting a user turn.
type ThreadInjectItemsRequest struct {
	ID     string                  `json:"id"`
	Method string                  `json:"method"`
	Params ThreadInjectItemsParams `json:"params"`
}

func (ThreadInjectItemsRequest) isClientRequest() {}

// SkillsListRequest is generated from the Skills/listRequest schema definition.
type SkillsListRequest struct {
	ID     string           `json:"id"`
	Method string           `json:"method"`
	Params SkillsListParams `json:"params"`
}

func (SkillsListRequest) isClientRequest() {}

// SkillsExtraRootsSetRequest is generated from the Skills/extraRoots/setRequest schema definition.
type SkillsExtraRootsSetRequest struct {
	ID     string                    `json:"id"`
	Method string                    `json:"method"`
	Params SkillsExtraRootsSetParams `json:"params"`
}

func (SkillsExtraRootsSetRequest) isClientRequest() {}

// HooksListRequest is generated from the Hooks/listRequest schema definition.
type HooksListRequest struct {
	ID     string          `json:"id"`
	Method string          `json:"method"`
	Params HooksListParams `json:"params"`
}

func (HooksListRequest) isClientRequest() {}

// MarketplaceAddRequest is generated from the Marketplace/addRequest schema definition.
type MarketplaceAddRequest struct {
	ID     string               `json:"id"`
	Method string               `json:"method"`
	Params MarketplaceAddParams `json:"params"`
}

func (MarketplaceAddRequest) isClientRequest() {}

// MarketplaceRemoveRequest is generated from the Marketplace/removeRequest schema definition.
type MarketplaceRemoveRequest struct {
	ID     string                  `json:"id"`
	Method string                  `json:"method"`
	Params MarketplaceRemoveParams `json:"params"`
}

func (MarketplaceRemoveRequest) isClientRequest() {}

// MarketplaceUpgradeRequest is generated from the Marketplace/upgradeRequest schema definition.
type MarketplaceUpgradeRequest struct {
	ID     string                   `json:"id"`
	Method string                   `json:"method"`
	Params MarketplaceUpgradeParams `json:"params"`
}

func (MarketplaceUpgradeRequest) isClientRequest() {}

// PluginListRequest is generated from the Plugin/listRequest schema definition.
type PluginListRequest struct {
	ID     string           `json:"id"`
	Method string           `json:"method"`
	Params PluginListParams `json:"params"`
}

func (PluginListRequest) isClientRequest() {}

// PluginInstalledRequest is generated from the Plugin/installedRequest schema definition.
type PluginInstalledRequest struct {
	ID     string                `json:"id"`
	Method string                `json:"method"`
	Params PluginInstalledParams `json:"params"`
}

func (PluginInstalledRequest) isClientRequest() {}

// PluginReadRequest is generated from the Plugin/readRequest schema definition.
type PluginReadRequest struct {
	ID     string           `json:"id"`
	Method string           `json:"method"`
	Params PluginReadParams `json:"params"`
}

func (PluginReadRequest) isClientRequest() {}

// PluginSkillReadRequest is generated from the Plugin/skill/readRequest schema definition.
type PluginSkillReadRequest struct {
	ID     string                `json:"id"`
	Method string                `json:"method"`
	Params PluginSkillReadParams `json:"params"`
}

func (PluginSkillReadRequest) isClientRequest() {}

// PluginShareSaveRequest is generated from the Plugin/share/saveRequest schema definition.
type PluginShareSaveRequest struct {
	ID     string                `json:"id"`
	Method string                `json:"method"`
	Params PluginShareSaveParams `json:"params"`
}

func (PluginShareSaveRequest) isClientRequest() {}

// PluginShareUpdateTargetsRequest is generated from the Plugin/share/updateTargetsRequest schema definition.
type PluginShareUpdateTargetsRequest struct {
	ID     string                         `json:"id"`
	Method string                         `json:"method"`
	Params PluginShareUpdateTargetsParams `json:"params"`
}

func (PluginShareUpdateTargetsRequest) isClientRequest() {}

// PluginShareListRequest is generated from the Plugin/share/listRequest schema definition.
type PluginShareListRequest struct {
	ID     string                `json:"id"`
	Method string                `json:"method"`
	Params PluginShareListParams `json:"params"`
}

func (PluginShareListRequest) isClientRequest() {}

// PluginShareCheckoutRequest is generated from the Plugin/share/checkoutRequest schema definition.
type PluginShareCheckoutRequest struct {
	ID     string                    `json:"id"`
	Method string                    `json:"method"`
	Params PluginShareCheckoutParams `json:"params"`
}

func (PluginShareCheckoutRequest) isClientRequest() {}

// PluginShareDeleteRequest is generated from the Plugin/share/deleteRequest schema definition.
type PluginShareDeleteRequest struct {
	ID     string                  `json:"id"`
	Method string                  `json:"method"`
	Params PluginShareDeleteParams `json:"params"`
}

func (PluginShareDeleteRequest) isClientRequest() {}

// AppReadRequest is generated from the App/readRequest schema definition.
type AppReadRequest struct {
	ID     string         `json:"id"`
	Method string         `json:"method"`
	Params AppsReadParams `json:"params"`
}

func (AppReadRequest) isClientRequest() {}

// AppListRequest is generated from the App/listRequest schema definition.
type AppListRequest struct {
	ID     string         `json:"id"`
	Method string         `json:"method"`
	Params AppsListParams `json:"params"`
}

func (AppListRequest) isClientRequest() {}

// AppInstalledRequest is generated from the App/installedRequest schema definition.
type AppInstalledRequest struct {
	ID     string              `json:"id"`
	Method string              `json:"method"`
	Params AppsInstalledParams `json:"params"`
}

func (AppInstalledRequest) isClientRequest() {}

// FSReadFileRequest is generated from the Fs/readFileRequest schema definition.
type FSReadFileRequest struct {
	ID     string           `json:"id"`
	Method string           `json:"method"`
	Params FSReadFileParams `json:"params"`
}

func (FSReadFileRequest) isClientRequest() {}

// FSWriteFileRequest is generated from the Fs/writeFileRequest schema definition.
type FSWriteFileRequest struct {
	ID     string            `json:"id"`
	Method string            `json:"method"`
	Params FSWriteFileParams `json:"params"`
}

func (FSWriteFileRequest) isClientRequest() {}

// FSCreateDirectoryRequest is generated from the Fs/createDirectoryRequest schema definition.
type FSCreateDirectoryRequest struct {
	ID     string                  `json:"id"`
	Method string                  `json:"method"`
	Params FSCreateDirectoryParams `json:"params"`
}

func (FSCreateDirectoryRequest) isClientRequest() {}

// FSGetMetadataRequest is generated from the Fs/getMetadataRequest schema definition.
type FSGetMetadataRequest struct {
	ID     string              `json:"id"`
	Method string              `json:"method"`
	Params FSGetMetadataParams `json:"params"`
}

func (FSGetMetadataRequest) isClientRequest() {}

// FSReadDirectoryRequest is generated from the Fs/readDirectoryRequest schema definition.
type FSReadDirectoryRequest struct {
	ID     string                `json:"id"`
	Method string                `json:"method"`
	Params FSReadDirectoryParams `json:"params"`
}

func (FSReadDirectoryRequest) isClientRequest() {}

// FSRemoveRequest is generated from the Fs/removeRequest schema definition.
type FSRemoveRequest struct {
	ID     string         `json:"id"`
	Method string         `json:"method"`
	Params FSRemoveParams `json:"params"`
}

func (FSRemoveRequest) isClientRequest() {}

// FSCopyRequest is generated from the Fs/copyRequest schema definition.
type FSCopyRequest struct {
	ID     string       `json:"id"`
	Method string       `json:"method"`
	Params FSCopyParams `json:"params"`
}

func (FSCopyRequest) isClientRequest() {}

// FSWatchRequest is generated from the Fs/watchRequest schema definition.
type FSWatchRequest struct {
	ID     string        `json:"id"`
	Method string        `json:"method"`
	Params FSWatchParams `json:"params"`
}

func (FSWatchRequest) isClientRequest() {}

// FSUnwatchRequest is generated from the Fs/unwatchRequest schema definition.
type FSUnwatchRequest struct {
	ID     string          `json:"id"`
	Method string          `json:"method"`
	Params FSUnwatchParams `json:"params"`
}

func (FSUnwatchRequest) isClientRequest() {}

// SkillsConfigWriteRequest is generated from the Skills/config/writeRequest schema definition.
type SkillsConfigWriteRequest struct {
	ID     string                  `json:"id"`
	Method string                  `json:"method"`
	Params SkillsConfigWriteParams `json:"params"`
}

func (SkillsConfigWriteRequest) isClientRequest() {}

// PluginInstallRequest is generated from the Plugin/installRequest schema definition.
type PluginInstallRequest struct {
	ID     string              `json:"id"`
	Method string              `json:"method"`
	Params PluginInstallParams `json:"params"`
}

func (PluginInstallRequest) isClientRequest() {}

// PluginUninstallRequest is generated from the Plugin/uninstallRequest schema definition.
type PluginUninstallRequest struct {
	ID     string                `json:"id"`
	Method string                `json:"method"`
	Params PluginUninstallParams `json:"params"`
}

func (PluginUninstallRequest) isClientRequest() {}

// TurnStartRequest is generated from the Turn/startRequest schema definition.
type TurnStartRequest struct {
	ID     string          `json:"id"`
	Method string          `json:"method"`
	Params TurnStartParams `json:"params"`
}

func (TurnStartRequest) isClientRequest() {}

// TurnSteerRequest is generated from the Turn/steerRequest schema definition.
type TurnSteerRequest struct {
	ID     string          `json:"id"`
	Method string          `json:"method"`
	Params TurnSteerParams `json:"params"`
}

func (TurnSteerRequest) isClientRequest() {}

// TurnInterruptRequest is generated from the Turn/interruptRequest schema definition.
type TurnInterruptRequest struct {
	ID     string              `json:"id"`
	Method string              `json:"method"`
	Params TurnInterruptParams `json:"params"`
}

func (TurnInterruptRequest) isClientRequest() {}

// ThreadRealtimeStartRequest is generated from the Thread/realtime/startRequest schema definition.
type ThreadRealtimeStartRequest struct {
	ID     string                    `json:"id"`
	Method string                    `json:"method"`
	Params ThreadRealtimeStartParams `json:"params"`
}

func (ThreadRealtimeStartRequest) isClientRequest() {}

// ThreadRealtimeAppendAudioRequest is generated from the Thread/realtime/appendAudioRequest schema definition.
type ThreadRealtimeAppendAudioRequest struct {
	ID     string                          `json:"id"`
	Method string                          `json:"method"`
	Params ThreadRealtimeAppendAudioParams `json:"params"`
}

func (ThreadRealtimeAppendAudioRequest) isClientRequest() {}

// ThreadRealtimeAppendTextRequest is generated from the Thread/realtime/appendTextRequest schema definition.
type ThreadRealtimeAppendTextRequest struct {
	ID     string                         `json:"id"`
	Method string                         `json:"method"`
	Params ThreadRealtimeAppendTextParams `json:"params"`
}

func (ThreadRealtimeAppendTextRequest) isClientRequest() {}

// ThreadRealtimeAppendSpeechRequest is generated from the Thread/realtime/appendSpeechRequest schema definition.
type ThreadRealtimeAppendSpeechRequest struct {
	ID     string                           `json:"id"`
	Method string                           `json:"method"`
	Params ThreadRealtimeAppendSpeechParams `json:"params"`
}

func (ThreadRealtimeAppendSpeechRequest) isClientRequest() {}

// ThreadRealtimeStopRequest is generated from the Thread/realtime/stopRequest schema definition.
type ThreadRealtimeStopRequest struct {
	ID     string                   `json:"id"`
	Method string                   `json:"method"`
	Params ThreadRealtimeStopParams `json:"params"`
}

func (ThreadRealtimeStopRequest) isClientRequest() {}

// ThreadRealtimeListVoicesRequest is generated from the Thread/realtime/listVoicesRequest schema definition.
type ThreadRealtimeListVoicesRequest struct {
	ID     string                         `json:"id"`
	Method string                         `json:"method"`
	Params ThreadRealtimeListVoicesParams `json:"params"`
}

func (ThreadRealtimeListVoicesRequest) isClientRequest() {}

// ReviewStartRequest is generated from the Review/startRequest schema definition.
type ReviewStartRequest struct {
	ID     string            `json:"id"`
	Method string            `json:"method"`
	Params ReviewStartParams `json:"params"`
}

func (ReviewStartRequest) isClientRequest() {}

// ModelListRequest is generated from the Model/listRequest schema definition.
type ModelListRequest struct {
	ID     string          `json:"id"`
	Method string          `json:"method"`
	Params ModelListParams `json:"params"`
}

func (ModelListRequest) isClientRequest() {}

// ModelProviderCapabilitiesReadRequest is generated from the ModelProvider/capabilities/readRequest schema definition.
type ModelProviderCapabilitiesReadRequest struct {
	ID     string                              `json:"id"`
	Method string                              `json:"method"`
	Params ModelProviderCapabilitiesReadParams `json:"params"`
}

func (ModelProviderCapabilitiesReadRequest) isClientRequest() {}

// ExperimentalFeatureListRequest is generated from the ExperimentalFeature/listRequest schema definition.
type ExperimentalFeatureListRequest struct {
	ID     string                        `json:"id"`
	Method string                        `json:"method"`
	Params ExperimentalFeatureListParams `json:"params"`
}

func (ExperimentalFeatureListRequest) isClientRequest() {}

// PermissionProfileListRequest is generated from the PermissionProfile/listRequest schema definition.
type PermissionProfileListRequest struct {
	ID     string                      `json:"id"`
	Method string                      `json:"method"`
	Params PermissionProfileListParams `json:"params"`
}

func (PermissionProfileListRequest) isClientRequest() {}

// ExperimentalFeatureEnablementSetRequest is generated from the ExperimentalFeature/enablement/setRequest schema definition.
type ExperimentalFeatureEnablementSetRequest struct {
	ID     string                                 `json:"id"`
	Method string                                 `json:"method"`
	Params ExperimentalFeatureEnablementSetParams `json:"params"`
}

func (ExperimentalFeatureEnablementSetRequest) isClientRequest() {}

// RemoteControlEnableRequest is generated from the RemoteControl/enableRequest schema definition.
type RemoteControlEnableRequest struct {
	ID     string                    `json:"id"`
	Method string                    `json:"method"`
	Params RemoteControlEnableParams `json:"params,omitzero"`
}

func (RemoteControlEnableRequest) isClientRequest() {}

// RemoteControlDisableRequest is generated from the RemoteControl/disableRequest schema definition.
type RemoteControlDisableRequest struct {
	ID     string                     `json:"id"`
	Method string                     `json:"method"`
	Params RemoteControlDisableParams `json:"params,omitzero"`
}

func (RemoteControlDisableRequest) isClientRequest() {}

// RemoteControlStatusReadRequest is generated from the RemoteControl/status/readRequest schema definition.
type RemoteControlStatusReadRequest struct {
	ID     string         `json:"id"`
	Method string         `json:"method"`
	Params jsontext.Value `json:"params,omitzero"`
}

func (RemoteControlStatusReadRequest) isClientRequest() {}

// RemoteControlPairingStartRequest is generated from the RemoteControl/pairing/startRequest schema definition.
type RemoteControlPairingStartRequest struct {
	ID     string                          `json:"id"`
	Method string                          `json:"method"`
	Params RemoteControlPairingStartParams `json:"params"`
}

func (RemoteControlPairingStartRequest) isClientRequest() {}

// RemoteControlPairingStatusRequest is generated from the RemoteControl/pairing/statusRequest schema definition.
type RemoteControlPairingStatusRequest struct {
	ID     string                           `json:"id"`
	Method string                           `json:"method"`
	Params RemoteControlPairingStatusParams `json:"params"`
}

func (RemoteControlPairingStatusRequest) isClientRequest() {}

// RemoteControlClientListRequest is generated from the RemoteControl/client/listRequest schema definition.
type RemoteControlClientListRequest struct {
	ID     string                         `json:"id"`
	Method string                         `json:"method"`
	Params RemoteControlClientsListParams `json:"params"`
}

func (RemoteControlClientListRequest) isClientRequest() {}

// RemoteControlClientRevokeRequest is generated from the RemoteControl/client/revokeRequest schema definition.
type RemoteControlClientRevokeRequest struct {
	ID     string                           `json:"id"`
	Method string                           `json:"method"`
	Params RemoteControlClientsRevokeParams `json:"params"`
}

func (RemoteControlClientRevokeRequest) isClientRequest() {}

// CollaborationModeListRequest lists collaboration mode presets.
type CollaborationModeListRequest struct {
	ID     string                      `json:"id"`
	Method string                      `json:"method"`
	Params CollaborationModeListParams `json:"params"`
}

func (CollaborationModeListRequest) isClientRequest() {}

// MockExperimentalMethodRequest test-only method used to validate experimental gating.
type MockExperimentalMethodRequest struct {
	ID     string                       `json:"id"`
	Method string                       `json:"method"`
	Params MockExperimentalMethodParams `json:"params"`
}

func (MockExperimentalMethodRequest) isClientRequest() {}

// EnvironmentAddRequest adds or replaces a remote environment by id for later selection.
type EnvironmentAddRequest struct {
	ID     string               `json:"id"`
	Method string               `json:"method"`
	Params EnvironmentAddParams `json:"params"`
}

func (EnvironmentAddRequest) isClientRequest() {}

// EnvironmentInfoRequest reads information from a configured execution environment.
type EnvironmentInfoRequest struct {
	ID     string                `json:"id"`
	Method string                `json:"method"`
	Params EnvironmentInfoParams `json:"params"`
}

func (EnvironmentInfoRequest) isClientRequest() {}

// EnvironmentStatusRequest reads the current status of a configured execution environment.
type EnvironmentStatusRequest struct {
	ID     string                  `json:"id"`
	Method string                  `json:"method"`
	Params EnvironmentStatusParams `json:"params"`
}

func (EnvironmentStatusRequest) isClientRequest() {}

// MCPServerOAuthLoginRequest is generated from the McpServer/oauth/loginRequest schema definition.
type MCPServerOAuthLoginRequest struct {
	ID     string                    `json:"id"`
	Method string                    `json:"method"`
	Params MCPServerOAuthLoginParams `json:"params"`
}

func (MCPServerOAuthLoginRequest) isClientRequest() {}

// ConfigMCPServerReloadRequest is generated from the Config/mcpServer/reloadRequest schema definition.
type ConfigMCPServerReloadRequest struct {
	ID     string         `json:"id"`
	Method string         `json:"method"`
	Params jsontext.Value `json:"params,omitzero"`
}

func (ConfigMCPServerReloadRequest) isClientRequest() {}

// MCPServerStatusListRequest is generated from the McpServerStatus/listRequest schema definition.
type MCPServerStatusListRequest struct {
	ID     string                    `json:"id"`
	Method string                    `json:"method"`
	Params ListMCPServerStatusParams `json:"params"`
}

func (MCPServerStatusListRequest) isClientRequest() {}

// MCPServerResourceReadRequest is generated from the McpServer/resource/readRequest schema definition.
type MCPServerResourceReadRequest struct {
	ID     string                `json:"id"`
	Method string                `json:"method"`
	Params MCPResourceReadParams `json:"params"`
}

func (MCPServerResourceReadRequest) isClientRequest() {}

// MCPServerToolCallRequest is generated from the McpServer/tool/callRequest schema definition.
type MCPServerToolCallRequest struct {
	ID     string                  `json:"id"`
	Method string                  `json:"method"`
	Params MCPServerToolCallParams `json:"params"`
}

func (MCPServerToolCallRequest) isClientRequest() {}

// WindowsSandboxSetupStartRequest is generated from the WindowsSandbox/setupStartRequest schema definition.
type WindowsSandboxSetupStartRequest struct {
	ID     string                         `json:"id"`
	Method string                         `json:"method"`
	Params WindowsSandboxSetupStartParams `json:"params"`
}

func (WindowsSandboxSetupStartRequest) isClientRequest() {}

// WindowsSandboxReadinessRequest is generated from the WindowsSandbox/readinessRequest schema definition.
type WindowsSandboxReadinessRequest struct {
	ID     string         `json:"id"`
	Method string         `json:"method"`
	Params jsontext.Value `json:"params,omitzero"`
}

func (WindowsSandboxReadinessRequest) isClientRequest() {}

// AccountLoginStartRequest is generated from the Account/login/startRequest schema definition.
type AccountLoginStartRequest struct {
	ID     string             `json:"id"`
	Method string             `json:"method"`
	Params LoginAccountParams `json:"params"`
}

func (value *AccountLoginStartRequest) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		ID     string         `json:"id"`
		Method string         `json:"method"`
		Params jsontext.Value `json:"params"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.ID = raw.ID
	value.Method = raw.Method
	if raw.Params == nil {
		value.Params = nil
	} else {
		decodedParams, err := decodeGeneratedLoginAccountParams(raw.Params)
		if err != nil {
			return err
		}
		value.Params = decodedParams
	}
	return nil
}

func (AccountLoginStartRequest) isClientRequest() {}

// AccountLoginCancelRequest is generated from the Account/login/cancelRequest schema definition.
type AccountLoginCancelRequest struct {
	ID     string                   `json:"id"`
	Method string                   `json:"method"`
	Params CancelLoginAccountParams `json:"params"`
}

func (AccountLoginCancelRequest) isClientRequest() {}

// AccountLogoutRequest is generated from the Account/logoutRequest schema definition.
type AccountLogoutRequest struct {
	ID     string         `json:"id"`
	Method string         `json:"method"`
	Params jsontext.Value `json:"params,omitzero"`
}

func (AccountLogoutRequest) isClientRequest() {}

// AccountRateLimitsReadRequest is generated from the Account/rateLimits/readRequest schema definition.
type AccountRateLimitsReadRequest struct {
	ID     string         `json:"id"`
	Method string         `json:"method"`
	Params jsontext.Value `json:"params,omitzero"`
}

func (AccountRateLimitsReadRequest) isClientRequest() {}

// AccountRateLimitResetCreditConsumeRequest is generated from the Account/rateLimitResetCredit/consumeRequest schema definition.
type AccountRateLimitResetCreditConsumeRequest struct {
	ID     string                                   `json:"id"`
	Method string                                   `json:"method"`
	Params ConsumeAccountRateLimitResetCreditParams `json:"params"`
}

func (AccountRateLimitResetCreditConsumeRequest) isClientRequest() {}

// AccountUsageReadRequest is generated from the Account/usage/readRequest schema definition.
type AccountUsageReadRequest struct {
	ID     string         `json:"id"`
	Method string         `json:"method"`
	Params jsontext.Value `json:"params,omitzero"`
}

func (AccountUsageReadRequest) isClientRequest() {}

// AccountWorkspaceMessagesReadRequest is generated from the Account/workspaceMessages/readRequest schema definition.
type AccountWorkspaceMessagesReadRequest struct {
	ID     string         `json:"id"`
	Method string         `json:"method"`
	Params jsontext.Value `json:"params,omitzero"`
}

func (AccountWorkspaceMessagesReadRequest) isClientRequest() {}

// AccountSendAddCreditsNudgeEmailRequest is generated from the Account/sendAddCreditsNudgeEmailRequest schema definition.
type AccountSendAddCreditsNudgeEmailRequest struct {
	ID     string                         `json:"id"`
	Method string                         `json:"method"`
	Params SendAddCreditsNudgeEmailParams `json:"params"`
}

func (AccountSendAddCreditsNudgeEmailRequest) isClientRequest() {}

// FeedbackUploadRequest is generated from the Feedback/uploadRequest schema definition.
type FeedbackUploadRequest struct {
	ID     string               `json:"id"`
	Method string               `json:"method"`
	Params FeedbackUploadParams `json:"params"`
}

func (FeedbackUploadRequest) isClientRequest() {}

// CommandExecRequest executes a standalone command (argv vector) under the server's sandbox.
type CommandExecRequest struct {
	ID     string            `json:"id"`
	Method string            `json:"method"`
	Params CommandExecParams `json:"params"`
}

func (CommandExecRequest) isClientRequest() {}

// CommandExecWriteRequest writes stdin bytes to a running `command/exec` session or close stdin.
type CommandExecWriteRequest struct {
	ID     string                 `json:"id"`
	Method string                 `json:"method"`
	Params CommandExecWriteParams `json:"params"`
}

func (CommandExecWriteRequest) isClientRequest() {}

// CommandExecTerminateRequest terminates a running `command/exec` session by client-supplied `processId`.
type CommandExecTerminateRequest struct {
	ID     string                     `json:"id"`
	Method string                     `json:"method"`
	Params CommandExecTerminateParams `json:"params"`
}

func (CommandExecTerminateRequest) isClientRequest() {}

// CommandExecResizeRequest resizes a running PTY-backed `command/exec` session by client-supplied `processId`.
type CommandExecResizeRequest struct {
	ID     string                  `json:"id"`
	Method string                  `json:"method"`
	Params CommandExecResizeParams `json:"params"`
}

func (CommandExecResizeRequest) isClientRequest() {}

// ProcessSpawnRequest spawn a standalone process (argv vector) without a Codex sandbox.
type ProcessSpawnRequest struct {
	ID     string             `json:"id"`
	Method string             `json:"method"`
	Params ProcessSpawnParams `json:"params"`
}

func (ProcessSpawnRequest) isClientRequest() {}

// ProcessWriteStdinRequest writes stdin bytes to a running `process/spawn` session or close stdin.
type ProcessWriteStdinRequest struct {
	ID     string                  `json:"id"`
	Method string                  `json:"method"`
	Params ProcessWriteStdinParams `json:"params"`
}

func (ProcessWriteStdinRequest) isClientRequest() {}

// ProcessKillRequest terminates a running `process/spawn` session by client-supplied `processHandle`.
type ProcessKillRequest struct {
	ID     string            `json:"id"`
	Method string            `json:"method"`
	Params ProcessKillParams `json:"params"`
}

func (ProcessKillRequest) isClientRequest() {}

// ProcessResizePtyRequest resizes a running PTY-backed `process/spawn` session by client-supplied `processHandle`.
type ProcessResizePtyRequest struct {
	ID     string                 `json:"id"`
	Method string                 `json:"method"`
	Params ProcessResizePtyParams `json:"params"`
}

func (ProcessResizePtyRequest) isClientRequest() {}

// ConfigReadRequest is generated from the Config/readRequest schema definition.
type ConfigReadRequest struct {
	ID     string           `json:"id"`
	Method string           `json:"method"`
	Params ConfigReadParams `json:"params"`
}

func (ConfigReadRequest) isClientRequest() {}

// ExternalAgentConfigDetectRequest is generated from the ExternalAgentConfig/detectRequest schema definition.
type ExternalAgentConfigDetectRequest struct {
	ID     string                          `json:"id"`
	Method string                          `json:"method"`
	Params ExternalAgentConfigDetectParams `json:"params"`
}

func (ExternalAgentConfigDetectRequest) isClientRequest() {}

// ExternalAgentConfigImportRequest is generated from the ExternalAgentConfig/importRequest schema definition.
type ExternalAgentConfigImportRequest struct {
	ID     string                          `json:"id"`
	Method string                          `json:"method"`
	Params ExternalAgentConfigImportParams `json:"params"`
}

func (ExternalAgentConfigImportRequest) isClientRequest() {}

// ExternalAgentConfigImportRecordHistoryRequest is generated from the ExternalAgentConfig/import/recordHistoryRequest schema definition.
type ExternalAgentConfigImportRecordHistoryRequest struct {
	ID     string                                       `json:"id"`
	Method string                                       `json:"method"`
	Params ExternalAgentConfigImportHistoryRecordParams `json:"params"`
}

func (ExternalAgentConfigImportRecordHistoryRequest) isClientRequest() {}

// ExternalAgentConfigImportReadHistoriesRequest is generated from the ExternalAgentConfig/import/readHistoriesRequest schema definition.
type ExternalAgentConfigImportReadHistoriesRequest struct {
	ID     string         `json:"id"`
	Method string         `json:"method"`
	Params jsontext.Value `json:"params,omitzero"`
}

func (ExternalAgentConfigImportReadHistoriesRequest) isClientRequest() {}

// ConfigValueWriteRequest is generated from the Config/value/writeRequest schema definition.
type ConfigValueWriteRequest struct {
	ID     string                 `json:"id"`
	Method string                 `json:"method"`
	Params ConfigValueWriteParams `json:"params"`
}

func (ConfigValueWriteRequest) isClientRequest() {}

// ConfigBatchWriteRequest is generated from the Config/batchWriteRequest schema definition.
type ConfigBatchWriteRequest struct {
	ID     string                 `json:"id"`
	Method string                 `json:"method"`
	Params ConfigBatchWriteParams `json:"params"`
}

func (ConfigBatchWriteRequest) isClientRequest() {}

// ConfigRequirementsReadRequest is generated from the ConfigRequirements/readRequest schema definition.
type ConfigRequirementsReadRequest struct {
	ID     string         `json:"id"`
	Method string         `json:"method"`
	Params jsontext.Value `json:"params,omitzero"`
}

func (ConfigRequirementsReadRequest) isClientRequest() {}

// AccountReadRequest is generated from the Account/readRequest schema definition.
type AccountReadRequest struct {
	ID     string           `json:"id"`
	Method string           `json:"method"`
	Params GetAccountParams `json:"params"`
}

func (AccountReadRequest) isClientRequest() {}

// FuzzyFileSearchRequest is generated from the FuzzyFileSearchRequest schema definition.
type FuzzyFileSearchRequest struct {
	ID     string                `json:"id"`
	Method string                `json:"method"`
	Params FuzzyFileSearchParams `json:"params"`
}

func (FuzzyFileSearchRequest) isClientRequest() {}

// FuzzyFileSearchSessionStartRequest is generated from the FuzzyFileSearch/sessionStartRequest schema definition.
type FuzzyFileSearchSessionStartRequest struct {
	ID     string                            `json:"id"`
	Method string                            `json:"method"`
	Params FuzzyFileSearchSessionStartParams `json:"params"`
}

func (FuzzyFileSearchSessionStartRequest) isClientRequest() {}

// FuzzyFileSearchSessionUpdateRequest is generated from the FuzzyFileSearch/sessionUpdateRequest schema definition.
type FuzzyFileSearchSessionUpdateRequest struct {
	ID     string                             `json:"id"`
	Method string                             `json:"method"`
	Params FuzzyFileSearchSessionUpdateParams `json:"params"`
}

func (FuzzyFileSearchSessionUpdateRequest) isClientRequest() {}

// FuzzyFileSearchSessionStopRequest is generated from the FuzzyFileSearch/sessionStopRequest schema definition.
type FuzzyFileSearchSessionStopRequest struct {
	ID     string                           `json:"id"`
	Method string                           `json:"method"`
	Params FuzzyFileSearchSessionStopParams `json:"params"`
}

func (FuzzyFileSearchSessionStopRequest) isClientRequest() {}

func decodeGeneratedClientRequest(raw jsontext.Value) (ClientRequest, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Method string `json:"method"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Method {
		case RequestMethodInitialize:
			var value InitializeRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadStart:
			var value ThreadStartRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadResume:
			var value ThreadResumeRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadFork:
			var value ThreadForkRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadArchive:
			var value ThreadArchiveRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadDelete:
			var value ThreadDeleteRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadUnsubscribe:
			var value ThreadUnsubscribeRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadIncrementElicitation:
			var value ThreadIncrementElicitationRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadDecrementElicitation:
			var value ThreadDecrementElicitationRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadNameSet:
			var value ThreadNameSetRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadGoalSet:
			var value ThreadGoalSetRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadGoalGet:
			var value ThreadGoalGetRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadGoalClear:
			var value ThreadGoalClearRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadMetadataUpdate:
			var value ThreadMetadataUpdateRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadSectionMove:
			var value ThreadSectionMoveRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadSettingsUpdate:
			var value ThreadSettingsUpdateRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadMemoryModeSet:
			var value ThreadMemoryModeSetRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodMemoryReset:
			var value MemoryResetRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadUnarchive:
			var value ThreadUnarchiveRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadCompactStart:
			var value ThreadCompactStartRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadShellCommand:
			var value ThreadShellCommandRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadApproveGuardianDeniedAction:
			var value ThreadApproveGuardianDeniedActionRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadBackgroundTerminalsClean:
			var value ThreadBackgroundTerminalsCleanRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadBackgroundTerminalsList:
			var value ThreadBackgroundTerminalsListRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadBackgroundTerminalsTerminate:
			var value ThreadBackgroundTerminalsTerminateRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadRollback:
			var value ThreadRollbackRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadList:
			var value ThreadListRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadSectionList:
			var value ThreadSectionListRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadSearch:
			var value ThreadSearchRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadSearchOccurrences:
			var value ThreadSearchOccurrencesRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadLoadedList:
			var value ThreadLoadedListRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadRead:
			var value ThreadReadRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadTurnsList:
			var value ThreadTurnsListRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadItemsList:
			var value ThreadItemsListRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadInjectItems:
			var value ThreadInjectItemsRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodSkillsList:
			var value SkillsListRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodSkillsExtraRootsSet:
			var value SkillsExtraRootsSetRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodHooksList:
			var value HooksListRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodMarketplaceAdd:
			var value MarketplaceAddRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodMarketplaceRemove:
			var value MarketplaceRemoveRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodMarketplaceUpgrade:
			var value MarketplaceUpgradeRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodPluginList:
			var value PluginListRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodPluginInstalled:
			var value PluginInstalledRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodPluginRead:
			var value PluginReadRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodPluginSkillRead:
			var value PluginSkillReadRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodPluginShareSave:
			var value PluginShareSaveRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodPluginShareUpdateTargets:
			var value PluginShareUpdateTargetsRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodPluginShareList:
			var value PluginShareListRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodPluginShareCheckout:
			var value PluginShareCheckoutRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodPluginShareDelete:
			var value PluginShareDeleteRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodAppRead:
			var value AppReadRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodAppList:
			var value AppListRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodAppInstalled:
			var value AppInstalledRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodFSReadFile:
			var value FSReadFileRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodFSWriteFile:
			var value FSWriteFileRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodFSCreateDirectory:
			var value FSCreateDirectoryRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodFSGetMetadata:
			var value FSGetMetadataRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodFSReadDirectory:
			var value FSReadDirectoryRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodFSRemove:
			var value FSRemoveRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodFSCopy:
			var value FSCopyRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodFSWatch:
			var value FSWatchRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodFSUnwatch:
			var value FSUnwatchRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodSkillsConfigWrite:
			var value SkillsConfigWriteRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodPluginInstall:
			var value PluginInstallRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodPluginUninstall:
			var value PluginUninstallRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodTurnStart:
			var value TurnStartRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodTurnSteer:
			var value TurnSteerRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodTurnInterrupt:
			var value TurnInterruptRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadRealtimeStart:
			var value ThreadRealtimeStartRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadRealtimeAppendAudio:
			var value ThreadRealtimeAppendAudioRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadRealtimeAppendText:
			var value ThreadRealtimeAppendTextRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadRealtimeAppendSpeech:
			var value ThreadRealtimeAppendSpeechRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadRealtimeStop:
			var value ThreadRealtimeStopRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodThreadRealtimeListVoices:
			var value ThreadRealtimeListVoicesRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodReviewStart:
			var value ReviewStartRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodModelList:
			var value ModelListRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodModelProviderCapabilitiesRead:
			var value ModelProviderCapabilitiesReadRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodExperimentalFeatureList:
			var value ExperimentalFeatureListRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodPermissionProfileList:
			var value PermissionProfileListRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodExperimentalFeatureEnablementSet:
			var value ExperimentalFeatureEnablementSetRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodRemoteControlEnable:
			var value RemoteControlEnableRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodRemoteControlDisable:
			var value RemoteControlDisableRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodRemoteControlStatusRead:
			var value RemoteControlStatusReadRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodRemoteControlPairingStart:
			var value RemoteControlPairingStartRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodRemoteControlPairingStatus:
			var value RemoteControlPairingStatusRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodRemoteControlClientList:
			var value RemoteControlClientListRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodRemoteControlClientRevoke:
			var value RemoteControlClientRevokeRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodCollaborationModeList:
			var value CollaborationModeListRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodMockExperimentalMethod:
			var value MockExperimentalMethodRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodEnvironmentAdd:
			var value EnvironmentAddRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodEnvironmentInfo:
			var value EnvironmentInfoRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodEnvironmentStatus:
			var value EnvironmentStatusRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodMCPServerOAuthLogin:
			var value MCPServerOAuthLoginRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodConfigMCPServerReload:
			var value ConfigMCPServerReloadRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodMCPServerStatusList:
			var value MCPServerStatusListRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodMCPServerResourceRead:
			var value MCPServerResourceReadRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodMCPServerToolCall:
			var value MCPServerToolCallRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodWindowsSandboxSetupStart:
			var value WindowsSandboxSetupStartRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodWindowsSandboxReadiness:
			var value WindowsSandboxReadinessRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodAccountLoginStart:
			var value AccountLoginStartRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodAccountLoginCancel:
			var value AccountLoginCancelRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodAccountLogout:
			var value AccountLogoutRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodAccountRateLimitsRead:
			var value AccountRateLimitsReadRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodAccountRateLimitResetCreditConsume:
			var value AccountRateLimitResetCreditConsumeRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodAccountUsageRead:
			var value AccountUsageReadRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodAccountWorkspaceMessagesRead:
			var value AccountWorkspaceMessagesReadRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodAccountSendAddCreditsNudgeEmail:
			var value AccountSendAddCreditsNudgeEmailRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodFeedbackUpload:
			var value FeedbackUploadRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodCommandExec:
			var value CommandExecRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodCommandExecWrite:
			var value CommandExecWriteRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodCommandExecTerminate:
			var value CommandExecTerminateRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodCommandExecResize:
			var value CommandExecResizeRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodProcessSpawn:
			var value ProcessSpawnRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodProcessWriteStdin:
			var value ProcessWriteStdinRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodProcessKill:
			var value ProcessKillRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodProcessResizePty:
			var value ProcessResizePtyRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodConfigRead:
			var value ConfigReadRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodExternalAgentConfigDetect:
			var value ExternalAgentConfigDetectRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodExternalAgentConfigImport:
			var value ExternalAgentConfigImportRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodExternalAgentConfigImportRecordHistory:
			var value ExternalAgentConfigImportRecordHistoryRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodExternalAgentConfigImportReadHistories:
			var value ExternalAgentConfigImportReadHistoriesRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodConfigValueWrite:
			var value ConfigValueWriteRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodConfigBatchWrite:
			var value ConfigBatchWriteRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodConfigRequirementsRead:
			var value ConfigRequirementsReadRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodAccountRead:
			var value AccountReadRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodFuzzyFileSearch:
			var value FuzzyFileSearchRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodFuzzyFileSearchSessionStart:
			var value FuzzyFileSearchSessionStartRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodFuzzyFileSearchSessionUpdate:
			var value FuzzyFileSearchSessionUpdateRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case RequestMethodFuzzyFileSearchSessionStop:
			var value FuzzyFileSearchSessionStopRequest
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawClientRequest(raw), nil
	}
	return RawClientRequest(raw), nil
}

// CodexErrorInfo this translation layer make sure that we expose codex error code in camel case.
//
// When an upstream HTTP status is available (for example, from the Responses API or a provider), it is forwarded in `httpStatusCode` on the relevant `codexErrorInfo` variant.
type CodexErrorInfo interface {
	isCodexErrorInfo()
}

// RawCodexErrorInfo preserves an uninterpreted CodexErrorInfo JSON value.
type RawCodexErrorInfo jsontext.Value

func (RawCodexErrorInfo) isCodexErrorInfo() {}

var (
	_ json.MarshalerTo     = RawCodexErrorInfo{}
	_ json.UnmarshalerFrom = (*RawCodexErrorInfo)(nil)
)

func (value RawCodexErrorInfo) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawCodexErrorInfo) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// CodexErrorInfoValue is a string-valued CodexErrorInfo variant.
type CodexErrorInfoValue string

func (CodexErrorInfoValue) isCodexErrorInfo() {}

const (
	// CodexErrorInfoValueBadRequest is the "badRequest" CodexErrorInfoValue value.
	CodexErrorInfoValueBadRequest CodexErrorInfoValue = "badRequest"
	// CodexErrorInfoValueContextWindowExceeded is the "contextWindowExceeded" CodexErrorInfoValue value.
	CodexErrorInfoValueContextWindowExceeded CodexErrorInfoValue = "contextWindowExceeded"
	// CodexErrorInfoValueCyberPolicy is the "cyberPolicy" CodexErrorInfoValue value.
	CodexErrorInfoValueCyberPolicy CodexErrorInfoValue = "cyberPolicy"
	// CodexErrorInfoValueInternalServerError is the "internalServerError" CodexErrorInfoValue value.
	CodexErrorInfoValueInternalServerError CodexErrorInfoValue = "internalServerError"
	// CodexErrorInfoValueOther is the "other" CodexErrorInfoValue value.
	CodexErrorInfoValueOther CodexErrorInfoValue = "other"
	// CodexErrorInfoValueSandboxError is the "sandboxError" CodexErrorInfoValue value.
	CodexErrorInfoValueSandboxError CodexErrorInfoValue = "sandboxError"
	// CodexErrorInfoValueServerOverloaded is the "serverOverloaded" CodexErrorInfoValue value.
	CodexErrorInfoValueServerOverloaded CodexErrorInfoValue = "serverOverloaded"
	// CodexErrorInfoValueSessionBudgetExceeded is the "sessionBudgetExceeded" CodexErrorInfoValue value.
	CodexErrorInfoValueSessionBudgetExceeded CodexErrorInfoValue = "sessionBudgetExceeded"
	// CodexErrorInfoValueThreadRollbackFailed is the "threadRollbackFailed" CodexErrorInfoValue value.
	CodexErrorInfoValueThreadRollbackFailed CodexErrorInfoValue = "threadRollbackFailed"
	// CodexErrorInfoValueUnauthorized is the "unauthorized" CodexErrorInfoValue value.
	CodexErrorInfoValueUnauthorized CodexErrorInfoValue = "unauthorized"
	// CodexErrorInfoValueUsageLimitExceeded is the "usageLimitExceeded" CodexErrorInfoValue value.
	CodexErrorInfoValueUsageLimitExceeded CodexErrorInfoValue = "usageLimitExceeded"
)

// HTTPConnectionFailed is generated from the HTTPConnectionFailed schema definition.
type HTTPConnectionFailed struct {
	HTTPStatusCode int64 `json:"httpStatusCode,omitzero"`
}

// HTTPConnectionFailedCodexErrorInfo is generated from the HttpConnectionFailedCodexErrorInfo schema definition.
type HTTPConnectionFailedCodexErrorInfo struct {
	HTTPConnectionFailed HTTPConnectionFailed `json:"httpConnectionFailed"`
}

func (HTTPConnectionFailedCodexErrorInfo) isCodexErrorInfo() {}

// ResponseStreamConnectionFailed is generated from the ResponseStreamConnectionFailed schema definition.
type ResponseStreamConnectionFailed struct {
	HTTPStatusCode int64 `json:"httpStatusCode,omitzero"`
}

// ResponseStreamConnectionFailedCodexErrorInfo failed to connect to the response SSE stream.
type ResponseStreamConnectionFailedCodexErrorInfo struct {
	ResponseStreamConnectionFailed ResponseStreamConnectionFailed `json:"responseStreamConnectionFailed"`
}

func (ResponseStreamConnectionFailedCodexErrorInfo) isCodexErrorInfo() {}

// ResponseStreamDisconnected is generated from the ResponseStreamDisconnected schema definition.
type ResponseStreamDisconnected struct {
	HTTPStatusCode int64 `json:"httpStatusCode,omitzero"`
}

// ResponseStreamDisconnectedCodexErrorInfo represents a response SSE stream disconnected in the middle of a turn before completion.
type ResponseStreamDisconnectedCodexErrorInfo struct {
	ResponseStreamDisconnected ResponseStreamDisconnected `json:"responseStreamDisconnected"`
}

func (ResponseStreamDisconnectedCodexErrorInfo) isCodexErrorInfo() {}

// ResponseTooManyFailedAttempts is generated from the ResponseTooManyFailedAttempts schema definition.
type ResponseTooManyFailedAttempts struct {
	HTTPStatusCode int64 `json:"httpStatusCode,omitzero"`
}

// ResponseTooManyFailedAttemptsCodexErrorInfo reached the retry limit for responses.
type ResponseTooManyFailedAttemptsCodexErrorInfo struct {
	ResponseTooManyFailedAttempts ResponseTooManyFailedAttempts `json:"responseTooManyFailedAttempts"`
}

func (ResponseTooManyFailedAttemptsCodexErrorInfo) isCodexErrorInfo() {}

// ActiveTurnNotSteerable is generated from the ActiveTurnNotSteerable schema definition.
type ActiveTurnNotSteerable struct {
	TurnKind NonSteerableTurnKind `json:"turnKind"`
}

// ActiveTurnNotSteerableCodexErrorInfo returned when `turn/start` or `turn/steer` is submitted while the current active turn cannot accept same-turn steering, for example `/review` or manual `/compact`.
type ActiveTurnNotSteerableCodexErrorInfo struct {
	ActiveTurnNotSteerable ActiveTurnNotSteerable `json:"activeTurnNotSteerable"`
}

func (ActiveTurnNotSteerableCodexErrorInfo) isCodexErrorInfo() {}

func decodeGeneratedCodexErrorInfo(raw jsontext.Value) (CodexErrorInfo, error) {
	if raw == nil {
		return nil, nil
	}
	var text string
	if err := json.Unmarshal(raw, &text); err == nil {
		switch text {
		case "badRequest":
			return CodexErrorInfoValue(text), nil
		case "contextWindowExceeded":
			return CodexErrorInfoValue(text), nil
		case "cyberPolicy":
			return CodexErrorInfoValue(text), nil
		case "internalServerError":
			return CodexErrorInfoValue(text), nil
		case "other":
			return CodexErrorInfoValue(text), nil
		case "sandboxError":
			return CodexErrorInfoValue(text), nil
		case "serverOverloaded":
			return CodexErrorInfoValue(text), nil
		case "sessionBudgetExceeded":
			return CodexErrorInfoValue(text), nil
		case "threadRollbackFailed":
			return CodexErrorInfoValue(text), nil
		case "unauthorized":
			return CodexErrorInfoValue(text), nil
		case "usageLimitExceeded":
			return CodexErrorInfoValue(text), nil
		}
		return RawCodexErrorInfo(raw), nil
	}
	var object struct {
		HTTPConnectionFailed           jsontext.Value `json:"httpConnectionFailed"`
		ResponseStreamConnectionFailed jsontext.Value `json:"responseStreamConnectionFailed"`
		ResponseStreamDisconnected     jsontext.Value `json:"responseStreamDisconnected"`
		ResponseTooManyFailedAttempts  jsontext.Value `json:"responseTooManyFailedAttempts"`
		ActiveTurnNotSteerable         jsontext.Value `json:"activeTurnNotSteerable"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		if object.HTTPConnectionFailed != nil {
			var value HTTPConnectionFailedCodexErrorInfo
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		if object.ResponseStreamConnectionFailed != nil {
			var value ResponseStreamConnectionFailedCodexErrorInfo
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		if object.ResponseStreamDisconnected != nil {
			var value ResponseStreamDisconnectedCodexErrorInfo
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		if object.ResponseTooManyFailedAttempts != nil {
			var value ResponseTooManyFailedAttemptsCodexErrorInfo
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		if object.ActiveTurnNotSteerable != nil {
			var value ActiveTurnNotSteerableCodexErrorInfo
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawCodexErrorInfo(raw), nil
	}
	return RawCodexErrorInfo(raw), nil
}

// CodexResponseHandoffMode is generated from the CodexResponseHandoffMode schema definition.
type CodexResponseHandoffMode string

const (
	// CodexResponseHandoffModeThinking is the "thinking" CodexResponseHandoffMode value.
	CodexResponseHandoffModeThinking CodexResponseHandoffMode = "thinking"
	// CodexResponseHandoffModeCommentary is the "commentary" CodexResponseHandoffMode value.
	CodexResponseHandoffModeCommentary CodexResponseHandoffMode = "commentary"
	// CodexResponseHandoffModeBemTags is the "bemTags" CodexResponseHandoffMode value.
	CodexResponseHandoffModeBemTags CodexResponseHandoffMode = "bemTags"
)

// CollabAgentState is generated from the CollabAgentState schema definition.
type CollabAgentState struct {
	Message string            `json:"message,omitzero"`
	Status  CollabAgentStatus `json:"status"`
}

// CollabAgentStatus is generated from the CollabAgentStatus schema definition.
type CollabAgentStatus string

const (
	// CollabAgentStatusPendingInit is the "pendingInit" CollabAgentStatus value.
	CollabAgentStatusPendingInit CollabAgentStatus = "pendingInit"
	// CollabAgentStatusRunning is the "running" CollabAgentStatus value.
	CollabAgentStatusRunning CollabAgentStatus = "running"
	// CollabAgentStatusInterrupted is the "interrupted" CollabAgentStatus value.
	CollabAgentStatusInterrupted CollabAgentStatus = "interrupted"
	// CollabAgentStatusCompleted is the "completed" CollabAgentStatus value.
	CollabAgentStatusCompleted CollabAgentStatus = "completed"
	// CollabAgentStatusErrored is the "errored" CollabAgentStatus value.
	CollabAgentStatusErrored CollabAgentStatus = "errored"
	// CollabAgentStatusShutdown is the "shutdown" CollabAgentStatus value.
	CollabAgentStatusShutdown CollabAgentStatus = "shutdown"
	// CollabAgentStatusNotFound is the "notFound" CollabAgentStatus value.
	CollabAgentStatusNotFound CollabAgentStatus = "notFound"
)

// CollabAgentTool is generated from the CollabAgentTool schema definition.
type CollabAgentTool string

const (
	// CollabAgentToolSpawnAgent is the "spawnAgent" CollabAgentTool value.
	CollabAgentToolSpawnAgent CollabAgentTool = "spawnAgent"
	// CollabAgentToolSendInput is the "sendInput" CollabAgentTool value.
	CollabAgentToolSendInput CollabAgentTool = "sendInput"
	// CollabAgentToolResumeAgent is the "resumeAgent" CollabAgentTool value.
	CollabAgentToolResumeAgent CollabAgentTool = "resumeAgent"
	// CollabAgentToolWait is the "wait" CollabAgentTool value.
	CollabAgentToolWait CollabAgentTool = "wait"
	// CollabAgentToolCloseAgent is the "closeAgent" CollabAgentTool value.
	CollabAgentToolCloseAgent CollabAgentTool = "closeAgent"
)

// CollabAgentToolCallStatus is generated from the CollabAgentToolCallStatus schema definition.
type CollabAgentToolCallStatus string

const (
	// CollabAgentToolCallStatusInProgress is the "inProgress" CollabAgentToolCallStatus value.
	CollabAgentToolCallStatusInProgress CollabAgentToolCallStatus = "inProgress"
	// CollabAgentToolCallStatusCompleted is the "completed" CollabAgentToolCallStatus value.
	CollabAgentToolCallStatusCompleted CollabAgentToolCallStatus = "completed"
	// CollabAgentToolCallStatusFailed is the "failed" CollabAgentToolCallStatus value.
	CollabAgentToolCallStatusFailed CollabAgentToolCallStatus = "failed"
)

// CollaborationMode collaboration mode for a Codex session.
type CollaborationMode struct {
	Mode     ModeKind `json:"mode"`
	Settings Settings `json:"settings"`
}

// CollaborationModeListParams list collaboration mode presets.
type CollaborationModeListParams struct{}

// CollaborationModeListResponse collaboration mode presets response.
type CollaborationModeListResponse struct {
	Data []CollaborationModeMask `json:"data"`
}

// CollaborationModeMask collaboration mode preset metadata for clients.
type CollaborationModeMask struct {
	Mode            ModeKind         `json:"mode,omitzero"`
	Model           string           `json:"model,omitzero"`
	Name            string           `json:"name"`
	ReasoningEffort *ReasoningEffort `json:"reasoning_effort,omitzero"`
}

// CommandAction is generated from the CommandAction schema definition.
type CommandAction interface {
	isCommandAction()
}

// RawCommandAction preserves an uninterpreted CommandAction JSON value.
type RawCommandAction jsontext.Value

func (RawCommandAction) isCommandAction() {}

var (
	_ json.MarshalerTo     = RawCommandAction{}
	_ json.UnmarshalerFrom = (*RawCommandAction)(nil)
)

func (value RawCommandAction) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawCommandAction) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// ReadCommandAction is generated from the ReadCommandAction schema definition.
type ReadCommandAction struct {
	Command string              `json:"command"`
	Name    string              `json:"name"`
	Path    LegacyAppPathString `json:"path"`
	Type    string              `json:"type"`
}

func (ReadCommandAction) isCommandAction() {}

// ListFilesCommandAction is generated from the ListFilesCommandAction schema definition.
type ListFilesCommandAction struct {
	Command string `json:"command"`
	Path    string `json:"path,omitzero"`
	Type    string `json:"type"`
}

func (ListFilesCommandAction) isCommandAction() {}

// SearchCommandAction is generated from the SearchCommandAction schema definition.
type SearchCommandAction struct {
	Command string `json:"command"`
	Path    string `json:"path,omitzero"`
	Query   string `json:"query,omitzero"`
	Type    string `json:"type"`
}

func (SearchCommandAction) isCommandAction() {}

// UnknownCommandAction is generated from the UnknownCommandAction schema definition.
type UnknownCommandAction struct {
	Command string `json:"command"`
	Type    string `json:"type"`
}

func (UnknownCommandAction) isCommandAction() {}

func decodeGeneratedCommandAction(raw jsontext.Value) (CommandAction, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "read":
			var value ReadCommandAction
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "listFiles":
			var value ListFilesCommandAction
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "search":
			var value SearchCommandAction
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "unknown":
			var value UnknownCommandAction
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawCommandAction(raw), nil
	}
	return RawCommandAction(raw), nil
}

// CommandExecOutputDeltaNotification base64-encoded output chunk emitted for a streaming `command/exec` request.
//
// These notifications are connection-scoped. If the originating connection closes, the server terminates the process.
type CommandExecOutputDeltaNotification struct {
	// CapReached `true` on the final streamed chunk for a stream when `outputBytesCap` truncated later output on that stream.
	CapReached bool `json:"capReached"`

	// DeltaBase64 base64-encoded output bytes.
	DeltaBase64 string `json:"deltaBase64"`

	// ProcessID client-supplied, connection-scoped `processId` from the original `command/exec` request.
	ProcessID string `json:"processId"`

	// Stream output stream for this chunk.
	Stream CommandExecOutputStream `json:"stream"`
}

func (value *CommandExecOutputDeltaNotification) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		CapReached  bool           `json:"capReached"`
		DeltaBase64 string         `json:"deltaBase64"`
		ProcessID   string         `json:"processId"`
		Stream      jsontext.Value `json:"stream"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.CapReached = raw.CapReached
	value.DeltaBase64 = raw.DeltaBase64
	value.ProcessID = raw.ProcessID
	if raw.Stream == nil {
		value.Stream = nil
	} else {
		decodedStream, err := decodeGeneratedCommandExecOutputStream(raw.Stream)
		if err != nil {
			return err
		}
		value.Stream = decodedStream
	}
	return nil
}

// CommandExecOutputStream streams label for `command/exec/outputDelta` notifications.
type CommandExecOutputStream interface {
	isCommandExecOutputStream()
}

// RawCommandExecOutputStream preserves an uninterpreted CommandExecOutputStream JSON value.
type RawCommandExecOutputStream jsontext.Value

func (RawCommandExecOutputStream) isCommandExecOutputStream() {}

var (
	_ json.MarshalerTo     = RawCommandExecOutputStream{}
	_ json.UnmarshalerFrom = (*RawCommandExecOutputStream)(nil)
)

func (value RawCommandExecOutputStream) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawCommandExecOutputStream) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// CommandExecOutputStreamValue is a string-valued CommandExecOutputStream variant.
type CommandExecOutputStreamValue string

func (CommandExecOutputStreamValue) isCommandExecOutputStream() {}

const (
	// CommandExecOutputStreamValueStderr is the "stderr" CommandExecOutputStreamValue value.
	CommandExecOutputStreamValueStderr CommandExecOutputStreamValue = "stderr"
	// CommandExecOutputStreamValueStdout is the "stdout" CommandExecOutputStreamValue value.
	CommandExecOutputStreamValueStdout CommandExecOutputStreamValue = "stdout"
)

func decodeGeneratedCommandExecOutputStream(raw jsontext.Value) (CommandExecOutputStream, error) {
	if raw == nil {
		return nil, nil
	}
	var text string
	if err := json.Unmarshal(raw, &text); err == nil {
		switch text {
		case "stderr":
			return CommandExecOutputStreamValue(text), nil
		case "stdout":
			return CommandExecOutputStreamValue(text), nil
		}
		return RawCommandExecOutputStream(raw), nil
	}
	return RawCommandExecOutputStream(raw), nil
}

// CommandExecParams runs a standalone command (argv vector) in the server sandbox without creating a thread or turn.
//
// The final `command/exec` response is deferred until the process exits and is sent only after all `command/exec/outputDelta` notifications for that connection have been emitted.
type CommandExecParams struct {
	// Command argv vector. Empty arrays are rejected.
	Command []string `json:"command"`

	// Cwd optional working directory. Defaults to the server cwd.
	Cwd string `json:"cwd,omitzero"`

	// DisableOutputCap disables stdout/stderr capture truncation for this request.
	//
	// Cannot be combined with `outputBytesCap`.
	DisableOutputCap bool `json:"disableOutputCap,omitzero"`

	// DisableTimeout disables the timeout entirely for this request.
	//
	// Cannot be combined with `timeoutMs`.
	DisableTimeout bool `json:"disableTimeout,omitzero"`

	// Env optional environment overrides merged into the server-computed environment.
	//
	// Matching names override inherited values. Set a key to `null` to unset an inherited variable.
	Env map[string]*string `json:"env,omitzero"`

	// OutputBytesCap optional per-stream stdout/stderr capture cap in bytes.
	//
	// When omitted, the server default applies. Cannot be combined with `disableOutputCap`.
	OutputBytesCap uint `json:"outputBytesCap,omitzero"`

	// PermissionProfile optional active permissions profile id for this command.
	//
	// Defaults to the user's configured permissions when omitted. Cannot be combined with `sandboxPolicy`.
	PermissionProfile string `json:"permissionProfile,omitzero"`

	// ProcessID optional client-supplied, connection-scoped process id.
	//
	// Required for `tty`, `streamStdin`, `streamStdoutStderr`, and follow-up `command/exec/write`, `command/exec/resize`, and `command/exec/terminate` calls. When omitted, buffered execution gets an internal id that is not exposed to the client.
	ProcessID string `json:"processId,omitzero"`

	// SandboxPolicy optional sandbox policy for this command.
	//
	// Uses the same shape as thread/turn execution sandbox configuration and defaults to the user's configured policy when omitted. Cannot be combined with `permissionProfile`.
	SandboxPolicy SandboxPolicy `json:"sandboxPolicy,omitzero"`

	// Size optional initial PTY size in character cells. Only valid when `tty` is true.
	Size CommandExecTerminalSize `json:"size,omitzero"`

	// StreamStdin allows follow-up `command/exec/write` requests to write stdin bytes.
	//
	// Requires a client-supplied `processId`.
	StreamStdin bool `json:"streamStdin,omitzero"`

	// StreamStdoutStderr streams stdout/stderr via `command/exec/outputDelta` notifications.
	//
	// Streamed bytes are not duplicated into the final response and require a client-supplied `processId`.
	StreamStdoutStderr bool `json:"streamStdoutStderr,omitzero"`

	// TimeoutMs optional timeout in milliseconds.
	//
	// When omitted, the server default applies. Cannot be combined with `disableTimeout`.
	TimeoutMs int64 `json:"timeoutMs,omitzero"`

	// TTY enables PTY mode.
	//
	// This implies `streamStdin` and `streamStdoutStderr`.
	TTY bool `json:"tty,omitzero"`
}

func (value *CommandExecParams) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Command            []string                `json:"command"`
		Cwd                string                  `json:"cwd,omitzero"`
		DisableOutputCap   bool                    `json:"disableOutputCap,omitzero"`
		DisableTimeout     bool                    `json:"disableTimeout,omitzero"`
		Env                map[string]*string      `json:"env,omitzero"`
		OutputBytesCap     uint                    `json:"outputBytesCap,omitzero"`
		PermissionProfile  string                  `json:"permissionProfile,omitzero"`
		ProcessID          string                  `json:"processId,omitzero"`
		SandboxPolicy      jsontext.Value          `json:"sandboxPolicy,omitzero"`
		Size               CommandExecTerminalSize `json:"size,omitzero"`
		StreamStdin        bool                    `json:"streamStdin,omitzero"`
		StreamStdoutStderr bool                    `json:"streamStdoutStderr,omitzero"`
		TimeoutMs          int64                   `json:"timeoutMs,omitzero"`
		TTY                bool                    `json:"tty,omitzero"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.Command = raw.Command
	value.Cwd = raw.Cwd
	value.DisableOutputCap = raw.DisableOutputCap
	value.DisableTimeout = raw.DisableTimeout
	value.Env = raw.Env
	value.OutputBytesCap = raw.OutputBytesCap
	value.PermissionProfile = raw.PermissionProfile
	value.ProcessID = raw.ProcessID
	if raw.SandboxPolicy == nil {
		value.SandboxPolicy = nil
	} else {
		decodedSandboxPolicy, err := decodeGeneratedSandboxPolicy(raw.SandboxPolicy)
		if err != nil {
			return err
		}
		value.SandboxPolicy = decodedSandboxPolicy
	}
	value.Size = raw.Size
	value.StreamStdin = raw.StreamStdin
	value.StreamStdoutStderr = raw.StreamStdoutStderr
	value.TimeoutMs = raw.TimeoutMs
	value.TTY = raw.TTY
	return nil
}

// CommandExecResizeParams resizes a running PTY-backed `command/exec` session.
type CommandExecResizeParams struct {
	// ProcessID client-supplied, connection-scoped `processId` from the original `command/exec` request.
	ProcessID string `json:"processId"`

	// Size new PTY size in character cells.
	Size CommandExecTerminalSize `json:"size"`
}

// CommandExecResizeResponse empty success response for `command/exec/resize`.
type CommandExecResizeResponse struct{}

// CommandExecResponse final buffered result for `command/exec`.
type CommandExecResponse struct {
	// ExitCode processes exit code.
	ExitCode int32 `json:"exitCode"`

	// Stderr buffered stderr capture.
	//
	// Empty when stderr was streamed via `command/exec/outputDelta`.
	Stderr string `json:"stderr"`

	// Stdout buffered stdout capture.
	//
	// Empty when stdout was streamed via `command/exec/outputDelta`.
	Stdout string `json:"stdout"`
}

// CommandExecTerminalSize PTY size in character cells for `command/exec` PTY sessions.
type CommandExecTerminalSize struct {
	// Cols terminal width in character cells.
	Cols int64 `json:"cols"`

	// Rows terminal height in character cells.
	Rows int64 `json:"rows"`
}

// CommandExecTerminateParams terminates a running `command/exec` session.
type CommandExecTerminateParams struct {
	// ProcessID client-supplied, connection-scoped `processId` from the original `command/exec` request.
	ProcessID string `json:"processId"`
}

// CommandExecTerminateResponse empty success response for `command/exec/terminate`.
type CommandExecTerminateResponse struct{}

// CommandExecWriteParams writes stdin bytes to a running `command/exec` session, close stdin, or both.
type CommandExecWriteParams struct {
	// CloseStdin closes stdin after writing `deltaBase64`, if present.
	CloseStdin bool `json:"closeStdin,omitzero"`

	// DeltaBase64 optional base64-encoded stdin bytes to write.
	DeltaBase64 string `json:"deltaBase64,omitzero"`

	// ProcessID client-supplied, connection-scoped `processId` from the original `command/exec` request.
	ProcessID string `json:"processId"`
}

// CommandExecWriteResponse empty success response for `command/exec/write`.
type CommandExecWriteResponse struct{}

// CommandExecutionOutputDeltaNotification is generated from the CommandExecutionOutputDeltaNotification schema definition.
type CommandExecutionOutputDeltaNotification struct {
	Delta    string `json:"delta"`
	ItemID   string `json:"itemId"`
	ThreadID string `json:"threadId"`
	TurnID   string `json:"turnId"`
}

// CommandExecutionSource is generated from the CommandExecutionSource schema definition.
type CommandExecutionSource string

const (
	// CommandExecutionSourceAgent is the "agent" CommandExecutionSource value.
	CommandExecutionSourceAgent CommandExecutionSource = "agent"
	// CommandExecutionSourceUserShell is the "userShell" CommandExecutionSource value.
	CommandExecutionSourceUserShell CommandExecutionSource = "userShell"
	// CommandExecutionSourceUnifiedExecStartup is the "unifiedExecStartup" CommandExecutionSource value.
	CommandExecutionSourceUnifiedExecStartup CommandExecutionSource = "unifiedExecStartup"
	// CommandExecutionSourceUnifiedExecInteraction is the "unifiedExecInteraction" CommandExecutionSource value.
	CommandExecutionSourceUnifiedExecInteraction CommandExecutionSource = "unifiedExecInteraction"
)

// CommandExecutionStatus is generated from the CommandExecutionStatus schema definition.
type CommandExecutionStatus string

const (
	// CommandExecutionStatusInProgress is the "inProgress" CommandExecutionStatus value.
	CommandExecutionStatusInProgress CommandExecutionStatus = "inProgress"
	// CommandExecutionStatusCompleted is the "completed" CommandExecutionStatus value.
	CommandExecutionStatusCompleted CommandExecutionStatus = "completed"
	// CommandExecutionStatusFailed is the "failed" CommandExecutionStatus value.
	CommandExecutionStatusFailed CommandExecutionStatus = "failed"
	// CommandExecutionStatusDeclined is the "declined" CommandExecutionStatus value.
	CommandExecutionStatusDeclined CommandExecutionStatus = "declined"
)

// CommandMigration is generated from the CommandMigration schema definition.
type CommandMigration struct {
	Name string `json:"name"`
}

// ComputerUseRequirements is generated from the ComputerUseRequirements schema definition.
type ComputerUseRequirements struct {
	AllowLockedComputerUse bool `json:"allowLockedComputerUse,omitzero"`
}

// ConfigPayload is generated from the Config schema definition.
type ConfigPayload struct {
	Analytics      AnalyticsConfig `json:"analytics,omitzero"`
	ApprovalPolicy AskForApproval  `json:"approval_policy,omitzero"`

	// ApprovalsReviewer [UNSTABLE] Optional default for where approval requests are routed for review.
	ApprovalsReviewer               ApprovalsReviewer          `json:"approvals_reviewer,omitzero"`
	Apps                            AppsConfig                 `json:"apps,omitzero"`
	CompactPrompt                   string                     `json:"compact_prompt,omitzero"`
	Desktop                         map[string]jsontext.Value  `json:"desktop,omitzero"`
	DeveloperInstructions           string                     `json:"developer_instructions,omitzero"`
	ForcedChatGPTWorkspaceID        ForcedChatGPTWorkspaceIDs  `json:"forced_chatgpt_workspace_id,omitzero"`
	ForcedLoginMethod               ForcedLoginMethod          `json:"forced_login_method,omitzero"`
	Instructions                    string                     `json:"instructions,omitzero"`
	Model                           string                     `json:"model,omitzero"`
	ModelAutoCompactTokenLimit      int64                      `json:"model_auto_compact_token_limit,omitzero"`
	ModelAutoCompactTokenLimitScope AutoCompactTokenLimitScope `json:"model_auto_compact_token_limit_scope,omitzero"`
	ModelContextWindow              int64                      `json:"model_context_window,omitzero"`
	ModelProvider                   string                     `json:"model_provider,omitzero"`
	ModelReasoningEffort            *ReasoningEffort           `json:"model_reasoning_effort,omitzero"`
	ModelReasoningSummary           ReasoningSummary           `json:"model_reasoning_summary,omitzero"`
	ModelVerbosity                  Verbosity                  `json:"model_verbosity,omitzero"`
	ReviewModel                     string                     `json:"review_model,omitzero"`
	SandboxMode                     SandboxMode                `json:"sandbox_mode,omitzero"`
	SandboxWorkspaceWrite           SandboxWorkspaceWrite      `json:"sandbox_workspace_write,omitzero"`
	ServiceTier                     string                     `json:"service_tier,omitzero"`
	Tools                           ToolsV2                    `json:"tools,omitzero"`
	WebSearch                       WebSearchMode              `json:"web_search,omitzero"`
}

func (value *ConfigPayload) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Analytics                       AnalyticsConfig           `json:"analytics,omitzero"`
		ApprovalPolicy                  jsontext.Value            `json:"approval_policy,omitzero"`
		ApprovalsReviewer               ApprovalsReviewer         `json:"approvals_reviewer,omitzero"`
		Apps                            AppsConfig                `json:"apps,omitzero"`
		CompactPrompt                   string                    `json:"compact_prompt,omitzero"`
		Desktop                         map[string]jsontext.Value `json:"desktop,omitzero"`
		DeveloperInstructions           string                    `json:"developer_instructions,omitzero"`
		ForcedChatGPTWorkspaceID        jsontext.Value            `json:"forced_chatgpt_workspace_id,omitzero"`
		ForcedLoginMethod               ForcedLoginMethod         `json:"forced_login_method,omitzero"`
		Instructions                    string                    `json:"instructions,omitzero"`
		Model                           string                    `json:"model,omitzero"`
		ModelAutoCompactTokenLimit      int64                     `json:"model_auto_compact_token_limit,omitzero"`
		ModelAutoCompactTokenLimitScope jsontext.Value            `json:"model_auto_compact_token_limit_scope,omitzero"`
		ModelContextWindow              int64                     `json:"model_context_window,omitzero"`
		ModelProvider                   string                    `json:"model_provider,omitzero"`
		ModelReasoningEffort            *ReasoningEffort          `json:"model_reasoning_effort,omitzero"`
		ModelReasoningSummary           jsontext.Value            `json:"model_reasoning_summary,omitzero"`
		ModelVerbosity                  Verbosity                 `json:"model_verbosity,omitzero"`
		ReviewModel                     string                    `json:"review_model,omitzero"`
		SandboxMode                     SandboxMode               `json:"sandbox_mode,omitzero"`
		SandboxWorkspaceWrite           SandboxWorkspaceWrite     `json:"sandbox_workspace_write,omitzero"`
		ServiceTier                     string                    `json:"service_tier,omitzero"`
		Tools                           ToolsV2                   `json:"tools,omitzero"`
		WebSearch                       WebSearchMode             `json:"web_search,omitzero"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.Analytics = raw.Analytics
	if raw.ApprovalPolicy == nil {
		value.ApprovalPolicy = nil
	} else {
		decodedApprovalPolicy, err := decodeGeneratedAskForApproval(raw.ApprovalPolicy)
		if err != nil {
			return err
		}
		value.ApprovalPolicy = decodedApprovalPolicy
	}
	value.ApprovalsReviewer = raw.ApprovalsReviewer
	value.Apps = raw.Apps
	value.CompactPrompt = raw.CompactPrompt
	value.Desktop = raw.Desktop
	value.DeveloperInstructions = raw.DeveloperInstructions
	if raw.ForcedChatGPTWorkspaceID == nil {
		value.ForcedChatGPTWorkspaceID = nil
	} else {
		decodedForcedChatGPTWorkspaceID, err := decodeGeneratedForcedChatGPTWorkspaceIDs(raw.ForcedChatGPTWorkspaceID)
		if err != nil {
			return err
		}
		value.ForcedChatGPTWorkspaceID = decodedForcedChatGPTWorkspaceID
	}
	value.ForcedLoginMethod = raw.ForcedLoginMethod
	value.Instructions = raw.Instructions
	value.Model = raw.Model
	value.ModelAutoCompactTokenLimit = raw.ModelAutoCompactTokenLimit
	if raw.ModelAutoCompactTokenLimitScope == nil {
		value.ModelAutoCompactTokenLimitScope = nil
	} else {
		decodedModelAutoCompactTokenLimitScope, err := decodeGeneratedAutoCompactTokenLimitScope(raw.ModelAutoCompactTokenLimitScope)
		if err != nil {
			return err
		}
		value.ModelAutoCompactTokenLimitScope = decodedModelAutoCompactTokenLimitScope
	}
	value.ModelContextWindow = raw.ModelContextWindow
	value.ModelProvider = raw.ModelProvider
	value.ModelReasoningEffort = raw.ModelReasoningEffort
	if raw.ModelReasoningSummary == nil {
		value.ModelReasoningSummary = nil
	} else {
		decodedModelReasoningSummary, err := decodeGeneratedReasoningSummary(raw.ModelReasoningSummary)
		if err != nil {
			return err
		}
		value.ModelReasoningSummary = decodedModelReasoningSummary
	}
	value.ModelVerbosity = raw.ModelVerbosity
	value.ReviewModel = raw.ReviewModel
	value.SandboxMode = raw.SandboxMode
	value.SandboxWorkspaceWrite = raw.SandboxWorkspaceWrite
	value.ServiceTier = raw.ServiceTier
	value.Tools = raw.Tools
	value.WebSearch = raw.WebSearch
	return nil
}

// ConfigBatchWriteParams is generated from the ConfigBatchWriteParams schema definition.
type ConfigBatchWriteParams struct {
	Edits           []ConfigEdit `json:"edits"`
	ExpectedVersion string       `json:"expectedVersion,omitzero"`

	// FilePath path to the config file to write; defaults to the user's `config.toml` when omitted.
	FilePath string `json:"filePath,omitzero"`

	// ReloadUserConfig when true, hot-reload updated runtime settings into loaded threads after writing. Session-static model, reasoning-effort, Plan-mode reasoning-effort, service-tier, and personality defaults are not reloaded.
	ReloadUserConfig bool `json:"reloadUserConfig,omitzero"`
}

// ConfigEdit is generated from the ConfigEdit schema definition.
type ConfigEdit struct {
	KeyPath       string         `json:"keyPath"`
	MergeStrategy MergeStrategy  `json:"mergeStrategy"`
	Value         jsontext.Value `json:"value"`
}

// ConfigLayer is generated from the ConfigLayer schema definition.
type ConfigLayer struct {
	Config         jsontext.Value    `json:"config"`
	DisabledReason string            `json:"disabledReason,omitzero"`
	Name           ConfigLayerSource `json:"name"`
	Version        string            `json:"version"`
}

func (value *ConfigLayer) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Config         jsontext.Value `json:"config"`
		DisabledReason string         `json:"disabledReason,omitzero"`
		Name           jsontext.Value `json:"name"`
		Version        string         `json:"version"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.Config = raw.Config
	value.DisabledReason = raw.DisabledReason
	if raw.Name == nil {
		value.Name = nil
	} else {
		decodedName, err := decodeGeneratedConfigLayerSource(raw.Name)
		if err != nil {
			return err
		}
		value.Name = decodedName
	}
	value.Version = raw.Version
	return nil
}

// ConfigLayerMetadata is generated from the ConfigLayerMetadata schema definition.
type ConfigLayerMetadata struct {
	Name    ConfigLayerSource `json:"name"`
	Version string            `json:"version"`
}

func (value *ConfigLayerMetadata) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Name    jsontext.Value `json:"name"`
		Version string         `json:"version"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	if raw.Name == nil {
		value.Name = nil
	} else {
		decodedName, err := decodeGeneratedConfigLayerSource(raw.Name)
		if err != nil {
			return err
		}
		value.Name = decodedName
	}
	value.Version = raw.Version
	return nil
}

// ConfigLayerSource is generated from the ConfigLayerSource schema definition.
type ConfigLayerSource interface {
	isConfigLayerSource()
}

// RawConfigLayerSource preserves an uninterpreted ConfigLayerSource JSON value.
type RawConfigLayerSource jsontext.Value

func (RawConfigLayerSource) isConfigLayerSource() {}

var (
	_ json.MarshalerTo     = RawConfigLayerSource{}
	_ json.UnmarshalerFrom = (*RawConfigLayerSource)(nil)
)

func (value RawConfigLayerSource) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawConfigLayerSource) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// MdmConfigLayerSource managed preferences layer delivered by MDM (macOS only).
type MdmConfigLayerSource struct {
	Domain string `json:"domain"`
	Key    string `json:"key"`
	Type   string `json:"type"`
}

func (MdmConfigLayerSource) isConfigLayerSource() {}

// SystemConfigLayerSource managed config layer from a file (usually `managed_config.toml`).
type SystemConfigLayerSource struct {
	// File this is the path to the system config.toml file, though it is not guaranteed to exist.
	File string `json:"file"`
	Type string `json:"type"`
}

func (SystemConfigLayerSource) isConfigLayerSource() {}

// EnterpriseManagedConfigLayerSource enterprise-managed config layer delivered by the cloud config bundle.
type EnterpriseManagedConfigLayerSource struct {
	// ID stable identifier for the delivered layer.
	ID string `json:"id"`

	// Name admin-facing name for the delivered layer. This is surfaced in diagnostics so users know which cloud layer needs administrator attention.
	Name string `json:"name"`
	Type string `json:"type"`
}

func (EnterpriseManagedConfigLayerSource) isConfigLayerSource() {}

// UserConfigLayerSource user config layer from $CODEX_HOME/config.toml. This layer is special in that it is expected to be: - writable by the user - generally outside the workspace directory.
type UserConfigLayerSource struct {
	// File this is the path to the user's config.toml file, though it is not guaranteed to exist.
	File string `json:"file"`

	// Profile names of the selected profile-v2 config layered on top of the base user config, when this layer represents one.
	Profile string `json:"profile,omitzero"`
	Type    string `json:"type"`
}

func (UserConfigLayerSource) isConfigLayerSource() {}

// ProjectConfigLayerSource path to a .codex/ folder within a project. There could be multiple of these between `cwd` and the project/repo root.
type ProjectConfigLayerSource struct {
	DotCodexFolder string `json:"dotCodexFolder"`
	Type           string `json:"type"`
}

func (ProjectConfigLayerSource) isConfigLayerSource() {}

// SessionFlagsConfigLayerSource session-layer overrides supplied via `-c`/`--config`.
type SessionFlagsConfigLayerSource struct {
	Type string `json:"type"`
}

func (SessionFlagsConfigLayerSource) isConfigLayerSource() {}

// LegacyManagedConfigTomlFromFileConfigLayerSource `managed_config.toml` was designed to be a config that was loaded as the last layer on top of everything else. This scheme did not quite work out as intended, but we keep this variant as a "best effort" while we phase out `managed_config.toml` in favor of `requirements.toml`.
type LegacyManagedConfigTomlFromFileConfigLayerSource struct {
	File string `json:"file"`
	Type string `json:"type"`
}

func (LegacyManagedConfigTomlFromFileConfigLayerSource) isConfigLayerSource() {}

// LegacyManagedConfigTomlFromMdmConfigLayerSource is generated from the LegacyManagedConfigTomlFromMdmConfigLayerSource schema definition.
type LegacyManagedConfigTomlFromMdmConfigLayerSource struct {
	Type string `json:"type"`
}

func (LegacyManagedConfigTomlFromMdmConfigLayerSource) isConfigLayerSource() {}

func decodeGeneratedConfigLayerSource(raw jsontext.Value) (ConfigLayerSource, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "mdm":
			var value MdmConfigLayerSource
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "system":
			var value SystemConfigLayerSource
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "enterpriseManaged":
			var value EnterpriseManagedConfigLayerSource
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "user":
			var value UserConfigLayerSource
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "project":
			var value ProjectConfigLayerSource
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "sessionFlags":
			var value SessionFlagsConfigLayerSource
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "legacyManagedConfigTomlFromFile":
			var value LegacyManagedConfigTomlFromFileConfigLayerSource
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "legacyManagedConfigTomlFromMdm":
			var value LegacyManagedConfigTomlFromMdmConfigLayerSource
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawConfigLayerSource(raw), nil
	}
	return RawConfigLayerSource(raw), nil
}

// ConfigReadParams is generated from the ConfigReadParams schema definition.
type ConfigReadParams struct {
	// Cwd optional working directory to resolve project config layers. If specified, return the effective config as seen from that directory (i.e., including any project layers between `cwd` and the project/repo root).
	Cwd           string `json:"cwd,omitzero"`
	IncludeLayers bool   `json:"includeLayers,omitzero"`
}

// ConfigReadResponse is generated from the ConfigReadResponse schema definition.
type ConfigReadResponse struct {
	Config  ConfigPayload                  `json:"config"`
	Layers  []ConfigLayer                  `json:"layers,omitzero"`
	Origins map[string]ConfigLayerMetadata `json:"origins"`
}

// ConfigRequirements is generated from the ConfigRequirements schema definition.
type ConfigRequirements struct {
	AllowAppshots                        bool                      `json:"allowAppshots,omitzero"`
	AllowLoginShell                      bool                      `json:"allowLoginShell,omitzero"`
	AllowManagedHooksOnly                bool                      `json:"allowManagedHooksOnly,omitzero"`
	AllowRemoteControl                   bool                      `json:"allowRemoteControl,omitzero"`
	AllowedApprovalPolicies              []AskForApproval          `json:"allowedApprovalPolicies,omitzero"`
	AllowedApprovalsReviewers            []ApprovalsReviewer       `json:"allowedApprovalsReviewers,omitzero"`
	AllowedPermissionProfiles            map[string]bool           `json:"allowedPermissionProfiles,omitzero"`
	AllowedSandboxModes                  []SandboxMode             `json:"allowedSandboxModes,omitzero"`
	AllowedWebSearchModes                []WebSearchMode           `json:"allowedWebSearchModes,omitzero"`
	AllowedWindowsSandboxImplementations []WindowsSandboxSetupMode `json:"allowedWindowsSandboxImplementations,omitzero"`
	BrowserUse                           BrowserUseRequirements    `json:"browserUse,omitzero"`
	CheckForUpdateOnStartup              bool                      `json:"checkForUpdateOnStartup,omitzero"`
	ComputerUse                          ComputerUseRequirements   `json:"computerUse,omitzero"`
	DefaultPermissions                   string                    `json:"defaultPermissions,omitzero"`
	EnforceResidency                     ResidencyRequirement      `json:"enforceResidency,omitzero"`
	FeatureRequirements                  map[string]bool           `json:"featureRequirements,omitzero"`
	Feedback                             FeedbackRequirements      `json:"feedback,omitzero"`
	Hooks                                ManagedHooksRequirements  `json:"hooks,omitzero"`
	LogDir                               string                    `json:"logDir,omitzero"`
	ModelCatalogJSON                     string                    `json:"modelCatalogJson,omitzero"`
	Models                               ModelsRequirements        `json:"models,omitzero"`
	Network                              NetworkRequirements       `json:"network,omitzero"`
	SQLiteHome                           string                    `json:"sqliteHome,omitzero"`
	WindowsSandboxPrivateDesktop         bool                      `json:"windowsSandboxPrivateDesktop,omitzero"`
}

func (value *ConfigRequirements) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		AllowAppshots                        bool                      `json:"allowAppshots,omitzero"`
		AllowLoginShell                      bool                      `json:"allowLoginShell,omitzero"`
		AllowManagedHooksOnly                bool                      `json:"allowManagedHooksOnly,omitzero"`
		AllowRemoteControl                   bool                      `json:"allowRemoteControl,omitzero"`
		AllowedApprovalPolicies              []jsontext.Value          `json:"allowedApprovalPolicies,omitzero"`
		AllowedApprovalsReviewers            []ApprovalsReviewer       `json:"allowedApprovalsReviewers,omitzero"`
		AllowedPermissionProfiles            map[string]bool           `json:"allowedPermissionProfiles,omitzero"`
		AllowedSandboxModes                  []SandboxMode             `json:"allowedSandboxModes,omitzero"`
		AllowedWebSearchModes                []WebSearchMode           `json:"allowedWebSearchModes,omitzero"`
		AllowedWindowsSandboxImplementations []WindowsSandboxSetupMode `json:"allowedWindowsSandboxImplementations,omitzero"`
		BrowserUse                           BrowserUseRequirements    `json:"browserUse,omitzero"`
		CheckForUpdateOnStartup              bool                      `json:"checkForUpdateOnStartup,omitzero"`
		ComputerUse                          ComputerUseRequirements   `json:"computerUse,omitzero"`
		DefaultPermissions                   string                    `json:"defaultPermissions,omitzero"`
		EnforceResidency                     ResidencyRequirement      `json:"enforceResidency,omitzero"`
		FeatureRequirements                  map[string]bool           `json:"featureRequirements,omitzero"`
		Feedback                             FeedbackRequirements      `json:"feedback,omitzero"`
		Hooks                                ManagedHooksRequirements  `json:"hooks,omitzero"`
		LogDir                               string                    `json:"logDir,omitzero"`
		ModelCatalogJSON                     string                    `json:"modelCatalogJson,omitzero"`
		Models                               ModelsRequirements        `json:"models,omitzero"`
		Network                              NetworkRequirements       `json:"network,omitzero"`
		SQLiteHome                           string                    `json:"sqliteHome,omitzero"`
		WindowsSandboxPrivateDesktop         bool                      `json:"windowsSandboxPrivateDesktop,omitzero"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.AllowAppshots = raw.AllowAppshots
	value.AllowLoginShell = raw.AllowLoginShell
	value.AllowManagedHooksOnly = raw.AllowManagedHooksOnly
	value.AllowRemoteControl = raw.AllowRemoteControl
	if raw.AllowedApprovalPolicies == nil {
		value.AllowedApprovalPolicies = nil
	} else {
		value.AllowedApprovalPolicies = make([]AskForApproval, len(raw.AllowedApprovalPolicies))
		for i, item := range raw.AllowedApprovalPolicies {
			if item != nil {
				decodedAllowedApprovalPolicies, err := decodeGeneratedAskForApproval(item)
				if err != nil {
					return err
				}
				value.AllowedApprovalPolicies[i] = decodedAllowedApprovalPolicies
			}
		}
	}
	value.AllowedApprovalsReviewers = raw.AllowedApprovalsReviewers
	value.AllowedPermissionProfiles = raw.AllowedPermissionProfiles
	value.AllowedSandboxModes = raw.AllowedSandboxModes
	value.AllowedWebSearchModes = raw.AllowedWebSearchModes
	value.AllowedWindowsSandboxImplementations = raw.AllowedWindowsSandboxImplementations
	value.BrowserUse = raw.BrowserUse
	value.CheckForUpdateOnStartup = raw.CheckForUpdateOnStartup
	value.ComputerUse = raw.ComputerUse
	value.DefaultPermissions = raw.DefaultPermissions
	value.EnforceResidency = raw.EnforceResidency
	value.FeatureRequirements = raw.FeatureRequirements
	value.Feedback = raw.Feedback
	value.Hooks = raw.Hooks
	value.LogDir = raw.LogDir
	value.ModelCatalogJSON = raw.ModelCatalogJSON
	value.Models = raw.Models
	value.Network = raw.Network
	value.SQLiteHome = raw.SQLiteHome
	value.WindowsSandboxPrivateDesktop = raw.WindowsSandboxPrivateDesktop
	return nil
}

// ConfigRequirementsReadResponse is generated from the ConfigRequirementsReadResponse schema definition.
type ConfigRequirementsReadResponse struct {
	// Requirements null if no requirements are configured (e.g. no requirements.toml/MDM entries).
	Requirements ConfigRequirements `json:"requirements,omitzero"`
}

// ConfigValueWriteParams is generated from the ConfigValueWriteParams schema definition.
type ConfigValueWriteParams struct {
	ExpectedVersion string `json:"expectedVersion,omitzero"`

	// FilePath path to the config file to write; defaults to the user's `config.toml` when omitted.
	FilePath      string         `json:"filePath,omitzero"`
	KeyPath       string         `json:"keyPath"`
	MergeStrategy MergeStrategy  `json:"mergeStrategy"`
	Value         jsontext.Value `json:"value"`
}

// ConfigWarningNotification is generated from the ConfigWarningNotification schema definition.
type ConfigWarningNotification struct {
	// Details optional extra guidance or error details.
	Details string `json:"details,omitzero"`

	// Path optional path to the config file that triggered the warning.
	Path string `json:"path,omitzero"`

	// RangeValue optional range for the error location inside the config file.
	RangeValue TextRange `json:"range,omitzero"`

	// Summary concise summary of the warning.
	Summary string `json:"summary"`
}

// ConfigWriteResponse is generated from the ConfigWriteResponse schema definition.
type ConfigWriteResponse struct {
	// FilePath canonical path to the config file that was written.
	FilePath           string             `json:"filePath"`
	OverriddenMetadata OverriddenMetadata `json:"overriddenMetadata,omitzero"`
	Status             WriteStatus        `json:"status"`
	Version            string             `json:"version"`
}

// ConfiguredHookHandler is generated from the ConfiguredHookHandler schema definition.
type ConfiguredHookHandler interface {
	isConfiguredHookHandler()
}

// RawConfiguredHookHandler preserves an uninterpreted ConfiguredHookHandler JSON value.
type RawConfiguredHookHandler jsontext.Value

func (RawConfiguredHookHandler) isConfiguredHookHandler() {}

var (
	_ json.MarshalerTo     = RawConfiguredHookHandler{}
	_ json.UnmarshalerFrom = (*RawConfiguredHookHandler)(nil)
)

func (value RawConfiguredHookHandler) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawConfiguredHookHandler) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// CommandConfiguredHookHandler is generated from the CommandConfiguredHookHandler schema definition.
type CommandConfiguredHookHandler struct {
	// AdditionalContextLimit approximate token threshold for spilling this hook's `additionalContext` to disk. `null` uses 2,500 tokens; `0` disables spilling for this hook. The threshold is evaluated against the original context; a spilled preview also includes recovery metadata.
	AdditionalContextLimit uint   `json:"additionalContextLimit,omitzero"`
	Async                  bool   `json:"async"`
	Command                string `json:"command"`
	CommandWindows         string `json:"commandWindows,omitzero"`
	StatusMessage          string `json:"statusMessage,omitzero"`
	TimeoutSec             uint64 `json:"timeoutSec,omitzero"`
	Type                   string `json:"type"`
}

func (CommandConfiguredHookHandler) isConfiguredHookHandler() {}

// PromptConfiguredHookHandler is generated from the PromptConfiguredHookHandler schema definition.
type PromptConfiguredHookHandler struct {
	Type string `json:"type"`
}

func (PromptConfiguredHookHandler) isConfiguredHookHandler() {}

// AgentConfiguredHookHandler is generated from the AgentConfiguredHookHandler schema definition.
type AgentConfiguredHookHandler struct {
	Type string `json:"type"`
}

func (AgentConfiguredHookHandler) isConfiguredHookHandler() {}

func decodeGeneratedConfiguredHookHandler(raw jsontext.Value) (ConfiguredHookHandler, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "command":
			var value CommandConfiguredHookHandler
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "prompt":
			var value PromptConfiguredHookHandler
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "agent":
			var value AgentConfiguredHookHandler
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawConfiguredHookHandler(raw), nil
	}
	return RawConfiguredHookHandler(raw), nil
}

// ConfiguredHookMatcherGroup is generated from the ConfiguredHookMatcherGroup schema definition.
type ConfiguredHookMatcherGroup struct {
	Hooks   []ConfiguredHookHandler `json:"hooks"`
	Matcher string                  `json:"matcher,omitzero"`
}

func (value *ConfiguredHookMatcherGroup) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Hooks   []jsontext.Value `json:"hooks"`
		Matcher string           `json:"matcher,omitzero"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	if raw.Hooks == nil {
		value.Hooks = nil
	} else {
		value.Hooks = make([]ConfiguredHookHandler, len(raw.Hooks))
		for i, item := range raw.Hooks {
			if item != nil {
				decodedHooks, err := decodeGeneratedConfiguredHookHandler(item)
				if err != nil {
					return err
				}
				value.Hooks[i] = decodedHooks
			}
		}
	}
	value.Matcher = raw.Matcher
	return nil
}

// ConnectorMetadata metadata returned by app/read.
type ConnectorMetadata struct {
	Description         string           `json:"description,omitzero"`
	DistributionChannel string           `json:"distributionChannel,omitzero"`
	IconURL             string           `json:"iconUrl,omitzero"`
	IconURLDark         string           `json:"iconUrlDark,omitzero"`
	ID                  string           `json:"id"`
	InstallURL          string           `json:"installUrl,omitzero"`
	Name                string           `json:"name"`
	PluginDisplayNames  []string         `json:"pluginDisplayNames,omitzero"`
	ToolSummaries       []AppToolSummary `json:"toolSummaries,omitzero"`
}

// ConsumeAccountRateLimitResetCreditOutcome is generated from the ConsumeAccountRateLimitResetCreditOutcome schema definition.
type ConsumeAccountRateLimitResetCreditOutcome interface {
	isConsumeAccountRateLimitResetCreditOutcome()
}

// RawConsumeAccountRateLimitResetCreditOutcome preserves an uninterpreted ConsumeAccountRateLimitResetCreditOutcome JSON value.
type RawConsumeAccountRateLimitResetCreditOutcome jsontext.Value

func (RawConsumeAccountRateLimitResetCreditOutcome) isConsumeAccountRateLimitResetCreditOutcome() {}

var (
	_ json.MarshalerTo     = RawConsumeAccountRateLimitResetCreditOutcome{}
	_ json.UnmarshalerFrom = (*RawConsumeAccountRateLimitResetCreditOutcome)(nil)
)

func (value RawConsumeAccountRateLimitResetCreditOutcome) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawConsumeAccountRateLimitResetCreditOutcome) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// ConsumeAccountRateLimitResetCreditOutcomeValue is a string-valued ConsumeAccountRateLimitResetCreditOutcome variant.
type ConsumeAccountRateLimitResetCreditOutcomeValue string

func (ConsumeAccountRateLimitResetCreditOutcomeValue) isConsumeAccountRateLimitResetCreditOutcome() {}

const (
	// ConsumeAccountRateLimitResetCreditOutcomeValueAlreadyRedeemed is the "alreadyRedeemed" ConsumeAccountRateLimitResetCreditOutcomeValue value.
	ConsumeAccountRateLimitResetCreditOutcomeValueAlreadyRedeemed ConsumeAccountRateLimitResetCreditOutcomeValue = "alreadyRedeemed"
	// ConsumeAccountRateLimitResetCreditOutcomeValueNoCredit is the "noCredit" ConsumeAccountRateLimitResetCreditOutcomeValue value.
	ConsumeAccountRateLimitResetCreditOutcomeValueNoCredit ConsumeAccountRateLimitResetCreditOutcomeValue = "noCredit"
	// ConsumeAccountRateLimitResetCreditOutcomeValueNothingToReset is the "nothingToReset" ConsumeAccountRateLimitResetCreditOutcomeValue value.
	ConsumeAccountRateLimitResetCreditOutcomeValueNothingToReset ConsumeAccountRateLimitResetCreditOutcomeValue = "nothingToReset"
	// ConsumeAccountRateLimitResetCreditOutcomeValueReset is the "reset" ConsumeAccountRateLimitResetCreditOutcomeValue value.
	ConsumeAccountRateLimitResetCreditOutcomeValueReset ConsumeAccountRateLimitResetCreditOutcomeValue = "reset"
)

func decodeGeneratedConsumeAccountRateLimitResetCreditOutcome(raw jsontext.Value) (ConsumeAccountRateLimitResetCreditOutcome, error) {
	if raw == nil {
		return nil, nil
	}
	var text string
	if err := json.Unmarshal(raw, &text); err == nil {
		switch text {
		case "alreadyRedeemed":
			return ConsumeAccountRateLimitResetCreditOutcomeValue(text), nil
		case "noCredit":
			return ConsumeAccountRateLimitResetCreditOutcomeValue(text), nil
		case "nothingToReset":
			return ConsumeAccountRateLimitResetCreditOutcomeValue(text), nil
		case "reset":
			return ConsumeAccountRateLimitResetCreditOutcomeValue(text), nil
		}
		return RawConsumeAccountRateLimitResetCreditOutcome(raw), nil
	}
	return RawConsumeAccountRateLimitResetCreditOutcome(raw), nil
}

// ConsumeAccountRateLimitResetCreditParams is generated from the ConsumeAccountRateLimitResetCreditParams schema definition.
type ConsumeAccountRateLimitResetCreditParams struct {
	// CreditID opaque reset-credit identifier to redeem. When omitted, the backend selects the next available credit.
	CreditID string `json:"creditId,omitzero"`

	// IDempotencyKey identifies one logical reset attempt. A UUID is recommended; reuse the same value when retrying that attempt.
	IDempotencyKey string `json:"idempotencyKey"`
}

// ConsumeAccountRateLimitResetCreditResponse is generated from the ConsumeAccountRateLimitResetCreditResponse schema definition.
type ConsumeAccountRateLimitResetCreditResponse struct {
	Outcome ConsumeAccountRateLimitResetCreditOutcome `json:"outcome"`
}

func (value *ConsumeAccountRateLimitResetCreditResponse) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Outcome jsontext.Value `json:"outcome"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	if raw.Outcome == nil {
		value.Outcome = nil
	} else {
		decodedOutcome, err := decodeGeneratedConsumeAccountRateLimitResetCreditOutcome(raw.Outcome)
		if err != nil {
			return err
		}
		value.Outcome = decodedOutcome
	}
	return nil
}

// ContentItem is generated from the ContentItem schema definition.
type ContentItem interface {
	isContentItem()
}

// RawContentItem preserves an uninterpreted ContentItem JSON value.
type RawContentItem jsontext.Value

func (RawContentItem) isContentItem() {}

var (
	_ json.MarshalerTo     = RawContentItem{}
	_ json.UnmarshalerFrom = (*RawContentItem)(nil)
)

func (value RawContentItem) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawContentItem) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// InputTextContentItem is generated from the InputTextContentItem schema definition.
type InputTextContentItem struct {
	Text string `json:"text"`
	Type string `json:"type"`
}

func (InputTextContentItem) isContentItem() {}

// InputImageContentItem is generated from the InputImageContentItem schema definition.
type InputImageContentItem struct {
	Detail   ImageDetail `json:"detail,omitzero"`
	ImageURL string      `json:"image_url"`
	Type     string      `json:"type"`
}

func (InputImageContentItem) isContentItem() {}

// InputAudioContentItem is generated from the InputAudioContentItem schema definition.
type InputAudioContentItem struct {
	AudioURL string `json:"audio_url"`
	Type     string `json:"type"`
}

func (InputAudioContentItem) isContentItem() {}

// OutputTextContentItem is generated from the OutputTextContentItem schema definition.
type OutputTextContentItem struct {
	Text string `json:"text"`
	Type string `json:"type"`
}

func (OutputTextContentItem) isContentItem() {}

func decodeGeneratedContentItem(raw jsontext.Value) (ContentItem, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "input_text":
			var value InputTextContentItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "input_image":
			var value InputImageContentItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "input_audio":
			var value InputAudioContentItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "output_text":
			var value OutputTextContentItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawContentItem(raw), nil
	}
	return RawContentItem(raw), nil
}

// ContextCompactedNotification deprecated: Use `ContextCompaction` item type instead.
type ContextCompactedNotification struct {
	ThreadID string `json:"threadId"`
	TurnID   string `json:"turnId"`
}

// ConversationTextRole is generated from the ConversationTextRole schema definition.
type ConversationTextRole string

const (
	// ConversationTextRoleUser is the "user" ConversationTextRole value.
	ConversationTextRoleUser ConversationTextRole = "user"
	// ConversationTextRoleDeveloper is the "developer" ConversationTextRole value.
	ConversationTextRoleDeveloper ConversationTextRole = "developer"
	// ConversationTextRoleAssistant is the "assistant" ConversationTextRole value.
	ConversationTextRoleAssistant ConversationTextRole = "assistant"
)

// CreditsSnapshot is generated from the CreditsSnapshot schema definition.
type CreditsSnapshot struct {
	Balance    string `json:"balance,omitzero"`
	HasCredits bool   `json:"hasCredits"`
	Unlimited  bool   `json:"unlimited"`
}

// DeprecationNoticeNotification is generated from the DeprecationNoticeNotification schema definition.
type DeprecationNoticeNotification struct {
	// Details optional extra guidance, such as migration steps or rationale.
	Details string `json:"details,omitzero"`

	// Summary concise summary of what is deprecated.
	Summary string `json:"summary"`
}

// DynamicToolCallOutputContentItem is generated from the DynamicToolCallOutputContentItem schema definition.
type DynamicToolCallOutputContentItem interface {
	isDynamicToolCallOutputContentItem()
}

// RawDynamicToolCallOutputContentItem preserves an uninterpreted DynamicToolCallOutputContentItem JSON value.
type RawDynamicToolCallOutputContentItem jsontext.Value

func (RawDynamicToolCallOutputContentItem) isDynamicToolCallOutputContentItem() {}

var (
	_ json.MarshalerTo     = RawDynamicToolCallOutputContentItem{}
	_ json.UnmarshalerFrom = (*RawDynamicToolCallOutputContentItem)(nil)
)

func (value RawDynamicToolCallOutputContentItem) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawDynamicToolCallOutputContentItem) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// InputTextDynamicToolCallOutputContentItem is generated from the InputTextDynamicToolCallOutputContentItem schema definition.
type InputTextDynamicToolCallOutputContentItem struct {
	Text string `json:"text"`
	Type string `json:"type"`
}

func (InputTextDynamicToolCallOutputContentItem) isDynamicToolCallOutputContentItem() {}

// InputImageDynamicToolCallOutputContentItem is generated from the InputImageDynamicToolCallOutputContentItem schema definition.
type InputImageDynamicToolCallOutputContentItem struct {
	ImageURL string `json:"imageUrl"`
	Type     string `json:"type"`
}

func (InputImageDynamicToolCallOutputContentItem) isDynamicToolCallOutputContentItem() {}

// InputAudioDynamicToolCallOutputContentItem is generated from the InputAudioDynamicToolCallOutputContentItem schema definition.
type InputAudioDynamicToolCallOutputContentItem struct {
	AudioURL string `json:"audioUrl"`
	Type     string `json:"type"`
}

func (InputAudioDynamicToolCallOutputContentItem) isDynamicToolCallOutputContentItem() {}

func decodeGeneratedDynamicToolCallOutputContentItem(raw jsontext.Value) (DynamicToolCallOutputContentItem, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "inputText":
			var value InputTextDynamicToolCallOutputContentItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "inputImage":
			var value InputImageDynamicToolCallOutputContentItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "inputAudio":
			var value InputAudioDynamicToolCallOutputContentItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawDynamicToolCallOutputContentItem(raw), nil
	}
	return RawDynamicToolCallOutputContentItem(raw), nil
}

// DynamicToolCallStatus is generated from the DynamicToolCallStatus schema definition.
type DynamicToolCallStatus string

const (
	// DynamicToolCallStatusInProgress is the "inProgress" DynamicToolCallStatus value.
	DynamicToolCallStatusInProgress DynamicToolCallStatus = "inProgress"
	// DynamicToolCallStatusCompleted is the "completed" DynamicToolCallStatus value.
	DynamicToolCallStatusCompleted DynamicToolCallStatus = "completed"
	// DynamicToolCallStatusFailed is the "failed" DynamicToolCallStatus value.
	DynamicToolCallStatusFailed DynamicToolCallStatus = "failed"
)

// FunctionDynamicToolNamespaceTool is generated from the FunctionDynamicToolNamespaceTool schema definition.
type FunctionDynamicToolNamespaceTool struct {
	DeferLoading bool           `json:"deferLoading,omitzero"`
	Description  string         `json:"description"`
	InputSchema  jsontext.Value `json:"inputSchema"`
	Name         string         `json:"name"`
	Type         string         `json:"type"`
}

// DynamicToolNamespaceTool is generated from the DynamicToolNamespaceTool schema definition.
type DynamicToolNamespaceTool = FunctionDynamicToolNamespaceTool

func (FunctionDynamicToolNamespaceTool) isDynamicToolNamespaceTool() {}

// DynamicToolSpec is generated from the DynamicToolSpec schema definition.
type DynamicToolSpec interface {
	isDynamicToolSpec()
}

// RawDynamicToolSpec preserves an uninterpreted DynamicToolSpec JSON value.
type RawDynamicToolSpec jsontext.Value

func (RawDynamicToolSpec) isDynamicToolSpec() {}

var (
	_ json.MarshalerTo     = RawDynamicToolSpec{}
	_ json.UnmarshalerFrom = (*RawDynamicToolSpec)(nil)
)

func (value RawDynamicToolSpec) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawDynamicToolSpec) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// FunctionDynamicToolSpec is generated from the FunctionDynamicToolSpec schema definition.
type FunctionDynamicToolSpec struct {
	DeferLoading bool           `json:"deferLoading,omitzero"`
	Description  string         `json:"description"`
	InputSchema  jsontext.Value `json:"inputSchema"`
	Name         string         `json:"name"`
	Type         string         `json:"type"`
}

func (FunctionDynamicToolSpec) isDynamicToolSpec() {}

// NamespaceDynamicToolSpec is generated from the NamespaceDynamicToolSpec schema definition.
type NamespaceDynamicToolSpec struct {
	Description string                     `json:"description"`
	Name        string                     `json:"name"`
	Tools       []DynamicToolNamespaceTool `json:"tools"`
	Type        string                     `json:"type"`
}

func (NamespaceDynamicToolSpec) isDynamicToolSpec() {}

func decodeGeneratedDynamicToolSpec(raw jsontext.Value) (DynamicToolSpec, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "function":
			var value FunctionDynamicToolSpec
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "namespace":
			var value NamespaceDynamicToolSpec
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawDynamicToolSpec(raw), nil
	}
	return RawDynamicToolSpec(raw), nil
}

// EnvironmentAddParams is generated from the EnvironmentAddParams schema definition.
type EnvironmentAddParams struct {
	// ConnectTimeoutMs optional WebSocket connection timeout. The server default applies when omitted.
	ConnectTimeoutMs uint64 `json:"connectTimeoutMs,omitzero"`
	EnvironmentID    string `json:"environmentId"`
	ExecServerURL    string `json:"execServerUrl"`
}

// EnvironmentAddResponse is generated from the EnvironmentAddResponse schema definition.
type EnvironmentAddResponse struct{}

// EnvironmentConnectionNotification is generated from the EnvironmentConnectionNotification schema definition.
type EnvironmentConnectionNotification struct {
	EnvironmentID string `json:"environmentId"`
	ThreadID      string `json:"threadId"`
}

// EnvironmentInfoParams is generated from the EnvironmentInfoParams schema definition.
type EnvironmentInfoParams struct {
	EnvironmentID string `json:"environmentId"`
}

// EnvironmentInfoResponse is generated from the EnvironmentInfoResponse schema definition.
type EnvironmentInfoResponse struct {
	// Cwd default working directory reported by the environment, as a canonical file URI.
	Cwd   *PathURI             `json:"cwd,omitzero"`
	Shell EnvironmentShellInfo `json:"shell"`
}

// EnvironmentShellInfo is generated from the EnvironmentShellInfo schema definition.
type EnvironmentShellInfo struct {
	// Name stable shell name, for example `zsh`, `bash`, `powershell`, `sh`, or `cmd`.
	Name string `json:"name"`

	// Path target-native shell executable path or command name.
	Path string `json:"path"`
}

// EnvironmentStatusKind current status observed by app-server without starting or recovering an environment.
//
// For a currently ready remote environment, app-server asks the existing exec-server connection for `environment/status` without allowing recovery.
type EnvironmentStatusKind interface {
	isEnvironmentStatusKind()
}

// RawEnvironmentStatusKind preserves an uninterpreted EnvironmentStatusKind JSON value.
type RawEnvironmentStatusKind jsontext.Value

func (RawEnvironmentStatusKind) isEnvironmentStatusKind() {}

var (
	_ json.MarshalerTo     = RawEnvironmentStatusKind{}
	_ json.UnmarshalerFrom = (*RawEnvironmentStatusKind)(nil)
)

func (value RawEnvironmentStatusKind) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawEnvironmentStatusKind) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// EnvironmentStatusKindValue is a string-valued EnvironmentStatusKind variant.
type EnvironmentStatusKindValue string

func (EnvironmentStatusKindValue) isEnvironmentStatusKind() {}

const (
	// EnvironmentStatusKindValueDisconnected is the "disconnected" EnvironmentStatusKindValue value.
	EnvironmentStatusKindValueDisconnected EnvironmentStatusKindValue = "disconnected"
	// EnvironmentStatusKindValuePending is the "pending" EnvironmentStatusKindValue value.
	EnvironmentStatusKindValuePending EnvironmentStatusKindValue = "pending"
	// EnvironmentStatusKindValueReady is the "ready" EnvironmentStatusKindValue value.
	EnvironmentStatusKindValueReady EnvironmentStatusKindValue = "ready"
	// EnvironmentStatusKindValueUnknown is the "unknown" EnvironmentStatusKindValue value.
	EnvironmentStatusKindValueUnknown EnvironmentStatusKindValue = "unknown"
)

func decodeGeneratedEnvironmentStatusKind(raw jsontext.Value) (EnvironmentStatusKind, error) {
	if raw == nil {
		return nil, nil
	}
	var text string
	if err := json.Unmarshal(raw, &text); err == nil {
		switch text {
		case "disconnected":
			return EnvironmentStatusKindValue(text), nil
		case "pending":
			return EnvironmentStatusKindValue(text), nil
		case "ready":
			return EnvironmentStatusKindValue(text), nil
		case "unknown":
			return EnvironmentStatusKindValue(text), nil
		}
		return RawEnvironmentStatusKind(raw), nil
	}
	return RawEnvironmentStatusKind(raw), nil
}

// EnvironmentStatusParams parameters for reading the current status of one configured environment.
type EnvironmentStatusParams struct {
	// EnvironmentID environment id to inspect.
	EnvironmentID string `json:"environmentId"`
}

// EnvironmentStatusResponse current status for the requested environment.
type EnvironmentStatusResponse struct {
	// Error human-readable detail for `disconnected` and `unknown`; omitted for other statuses.
	Error string `json:"error,omitzero"`

	// Status current status observed without starting or recovering the environment.
	Status EnvironmentStatusKind `json:"status"`
}

func (value *EnvironmentStatusResponse) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Error  string         `json:"error,omitzero"`
		Status jsontext.Value `json:"status"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.Error = raw.Error
	if raw.Status == nil {
		value.Status = nil
	} else {
		decodedStatus, err := decodeGeneratedEnvironmentStatusKind(raw.Status)
		if err != nil {
			return err
		}
		value.Status = decodedStatus
	}
	return nil
}

// ErrorNotification is generated from the ErrorNotification schema definition.
type ErrorNotification struct {
	Error     TurnError `json:"error"`
	ThreadID  string    `json:"threadId"`
	TurnID    string    `json:"turnId"`
	WillRetry bool      `json:"willRetry"`
}

// ExperimentalFeature is generated from the ExperimentalFeature schema definition.
type ExperimentalFeature struct {
	// Announcement copy shown to users when the feature is introduced. Null when this feature is not in beta.
	Announcement string `json:"announcement,omitzero"`

	// DefaultEnabled reports whether this feature is enabled by default.
	DefaultEnabled bool `json:"defaultEnabled"`

	// Description short summary describing what the feature does. Null when this feature is not in beta.
	Description string `json:"description,omitzero"`

	// DisplayName user-facing display name shown in the experimental features UI. Null when this feature is not in beta.
	DisplayName string `json:"displayName,omitzero"`

	// Enabled reports whether this feature is currently enabled in the loaded config.
	Enabled bool `json:"enabled"`

	// Name stable key used in config.toml and CLI flag toggles.
	Name string `json:"name"`

	// Stage lifecycle stage of this feature flag.
	Stage ExperimentalFeatureStage `json:"stage"`
}

func (value *ExperimentalFeature) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Announcement   string         `json:"announcement,omitzero"`
		DefaultEnabled bool           `json:"defaultEnabled"`
		Description    string         `json:"description,omitzero"`
		DisplayName    string         `json:"displayName,omitzero"`
		Enabled        bool           `json:"enabled"`
		Name           string         `json:"name"`
		Stage          jsontext.Value `json:"stage"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.Announcement = raw.Announcement
	value.DefaultEnabled = raw.DefaultEnabled
	value.Description = raw.Description
	value.DisplayName = raw.DisplayName
	value.Enabled = raw.Enabled
	value.Name = raw.Name
	if raw.Stage == nil {
		value.Stage = nil
	} else {
		decodedStage, err := decodeGeneratedExperimentalFeatureStage(raw.Stage)
		if err != nil {
			return err
		}
		value.Stage = decodedStage
	}
	return nil
}

// ExperimentalFeatureEnablementSetParams is generated from the ExperimentalFeatureEnablementSetParams schema definition.
type ExperimentalFeatureEnablementSetParams struct {
	// Enablement process-wide runtime feature enablement keyed by canonical feature name.
	//
	// Only named features are updated. Omitted features are left unchanged. Send an empty map for a no-op.
	Enablement map[string]bool `json:"enablement"`
}

// ExperimentalFeatureEnablementSetResponse is generated from the ExperimentalFeatureEnablementSetResponse schema definition.
type ExperimentalFeatureEnablementSetResponse struct {
	// Enablement feature enablement entries updated by this request.
	Enablement map[string]bool `json:"enablement"`
}

// ExperimentalFeatureListParams is generated from the ExperimentalFeatureListParams schema definition.
type ExperimentalFeatureListParams struct {
	// Cursor opaque pagination cursor returned by a previous call.
	Cursor string `json:"cursor,omitzero"`

	// Limit optional page size; defaults to a reasonable server-side value.
	Limit int32 `json:"limit,omitzero"`

	// ThreadID optional loaded thread id. Pass this when showing feature state for an existing thread so enablement is computed from that thread's refreshed config, including project-local config for the thread's cwd.
	ThreadID string `json:"threadId,omitzero"`
}

// ExperimentalFeatureListResponse is generated from the ExperimentalFeatureListResponse schema definition.
type ExperimentalFeatureListResponse struct {
	Data []ExperimentalFeature `json:"data"`

	// NextCursor opaque cursor to pass to the next call to continue after the last item. If None, there are no more items to return.
	NextCursor string `json:"nextCursor,omitzero"`
}

// ExperimentalFeatureStage is generated from the ExperimentalFeatureStage schema definition.
type ExperimentalFeatureStage interface {
	isExperimentalFeatureStage()
}

// RawExperimentalFeatureStage preserves an uninterpreted ExperimentalFeatureStage JSON value.
type RawExperimentalFeatureStage jsontext.Value

func (RawExperimentalFeatureStage) isExperimentalFeatureStage() {}

var (
	_ json.MarshalerTo     = RawExperimentalFeatureStage{}
	_ json.UnmarshalerFrom = (*RawExperimentalFeatureStage)(nil)
)

func (value RawExperimentalFeatureStage) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawExperimentalFeatureStage) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// ExperimentalFeatureStageValue is a string-valued ExperimentalFeatureStage variant.
type ExperimentalFeatureStageValue string

func (ExperimentalFeatureStageValue) isExperimentalFeatureStage() {}

const (
	// ExperimentalFeatureStageValueBeta is the "beta" ExperimentalFeatureStageValue value.
	ExperimentalFeatureStageValueBeta ExperimentalFeatureStageValue = "beta"
	// ExperimentalFeatureStageValueDeprecated is the "deprecated" ExperimentalFeatureStageValue value.
	ExperimentalFeatureStageValueDeprecated ExperimentalFeatureStageValue = "deprecated"
	// ExperimentalFeatureStageValueRemoved is the "removed" ExperimentalFeatureStageValue value.
	ExperimentalFeatureStageValueRemoved ExperimentalFeatureStageValue = "removed"
	// ExperimentalFeatureStageValueStable is the "stable" ExperimentalFeatureStageValue value.
	ExperimentalFeatureStageValueStable ExperimentalFeatureStageValue = "stable"
	// ExperimentalFeatureStageValueUnderDevelopment is the "underDevelopment" ExperimentalFeatureStageValue value.
	ExperimentalFeatureStageValueUnderDevelopment ExperimentalFeatureStageValue = "underDevelopment"
)

func decodeGeneratedExperimentalFeatureStage(raw jsontext.Value) (ExperimentalFeatureStage, error) {
	if raw == nil {
		return nil, nil
	}
	var text string
	if err := json.Unmarshal(raw, &text); err == nil {
		switch text {
		case "beta":
			return ExperimentalFeatureStageValue(text), nil
		case "deprecated":
			return ExperimentalFeatureStageValue(text), nil
		case "removed":
			return ExperimentalFeatureStageValue(text), nil
		case "stable":
			return ExperimentalFeatureStageValue(text), nil
		case "underDevelopment":
			return ExperimentalFeatureStageValue(text), nil
		}
		return RawExperimentalFeatureStage(raw), nil
	}
	return RawExperimentalFeatureStage(raw), nil
}

// ExternalAgentConfigDetectParams is generated from the ExternalAgentConfigDetectParams schema definition.
type ExternalAgentConfigDetectParams struct {
	// Cwds zero or more working directories to include for repo-scoped detection.
	Cwds []string `json:"cwds,omitzero"`

	// IncludeHome if true, include detection under the user's home directory.
	IncludeHome bool `json:"includeHome,omitzero"`

	// MaxSessionAgeDays maximum age in days for detected sessions. Missing values use the default limit.
	MaxSessionAgeDays int32 `json:"maxSessionAgeDays,omitzero"`

	// MaxSessions maximum number of sessions to detect. Missing values use the default limit.
	MaxSessions int32 `json:"maxSessions,omitzero"`

	// MigrationSource optional migration-source selector. Missing or unrecognized values use the default source.
	MigrationSource string `json:"migrationSource,omitzero"`

	// Source deprecated field retained for compatibility. This field is ignored; use `migrationSource` to select the migration source.
	Source string `json:"source,omitzero"`
}

// ExternalAgentConfigDetectResponse is generated from the ExternalAgentConfigDetectResponse schema definition.
type ExternalAgentConfigDetectResponse struct {
	Connectors []ExternalAgentDetectedConnectorCandidate `json:"connectors,omitzero"`
	Items      []ExternalAgentConfigMigrationItem        `json:"items"`
}

// ExternalAgentConfigImportCompletedNotification is generated from the ExternalAgentConfigImportCompletedNotification schema definition.
type ExternalAgentConfigImportCompletedNotification struct {
	ImportID        string                                `json:"importId"`
	ItemTypeResults []ExternalAgentConfigImportTypeResult `json:"itemTypeResults"`
}

// ExternalAgentConfigImportHistoriesReadResponse is generated from the ExternalAgentConfigImportHistoriesReadResponse schema definition.
type ExternalAgentConfigImportHistoriesReadResponse struct {
	Connectors []ExternalAgentImportedConnectorCandidate `json:"connectors"`
	Data       []ExternalAgentConfigImportHistory        `json:"data"`
}

// ExternalAgentConfigImportHistory is generated from the ExternalAgentConfigImportHistory schema definition.
type ExternalAgentConfigImportHistory struct {
	CompletedAtMs int64                                      `json:"completedAtMs"`
	Failures      []ExternalAgentConfigImportItemTypeFailure `json:"failures"`
	ImportID      string                                     `json:"importId"`
	ProviderID    string                                     `json:"providerId,omitzero"`
	Successes     []ExternalAgentConfigImportItemTypeSuccess `json:"successes"`
}

// ExternalAgentConfigImportHistoryRecordParams is generated from the ExternalAgentConfigImportHistoryRecordParams schema definition.
type ExternalAgentConfigImportHistoryRecordParams struct {
	// ItemTypeResults completed results grouped by imported item type.
	ItemTypeResults []ExternalAgentConfigImportHistoryRecordTypeResultParams `json:"itemTypeResults"`

	// ProviderID opaque provider identifier for the externally completed import.
	ProviderID string `json:"providerId"`
}

// ExternalAgentConfigImportHistoryRecordResponse is generated from the ExternalAgentConfigImportHistoryRecordResponse schema definition.
type ExternalAgentConfigImportHistoryRecordResponse struct {
	ImportID string `json:"importId"`
}

// ExternalAgentConfigImportHistoryRecordSuccessParams is generated from the ExternalAgentConfigImportHistoryRecordSuccessParams schema definition.
type ExternalAgentConfigImportHistoryRecordSuccessParams struct {
	Cwd      string                               `json:"cwd,omitzero"`
	ItemType ExternalAgentConfigMigrationItemType `json:"itemType"`
	Source   string                               `json:"source,omitzero"`
	Target   string                               `json:"target,omitzero"`

	// Title original title for an imported session, when available.
	Title string `json:"title,omitzero"`
}

// ExternalAgentConfigImportHistoryRecordTypeResultParams is generated from the ExternalAgentConfigImportHistoryRecordTypeResultParams schema definition.
type ExternalAgentConfigImportHistoryRecordTypeResultParams struct {
	Failures  []ExternalAgentConfigImportItemTypeFailure            `json:"failures"`
	ItemType  ExternalAgentConfigMigrationItemType                  `json:"itemType"`
	Successes []ExternalAgentConfigImportHistoryRecordSuccessParams `json:"successes"`
}

// ExternalAgentConfigImportItemTypeFailure is generated from the ExternalAgentConfigImportItemTypeFailure schema definition.
type ExternalAgentConfigImportItemTypeFailure struct {
	Cwd          string                               `json:"cwd,omitzero"`
	ErrorType    string                               `json:"errorType,omitzero"`
	FailureStage string                               `json:"failureStage"`
	ItemType     ExternalAgentConfigMigrationItemType `json:"itemType"`
	Message      string                               `json:"message"`
	Source       string                               `json:"source,omitzero"`
	SubErrorType string                               `json:"subErrorType,omitzero"`
}

// ExternalAgentConfigImportItemTypeSuccess is generated from the ExternalAgentConfigImportItemTypeSuccess schema definition.
type ExternalAgentConfigImportItemTypeSuccess struct {
	Cwd      string                               `json:"cwd,omitzero"`
	ItemType ExternalAgentConfigMigrationItemType `json:"itemType"`
	Source   string                               `json:"source,omitzero"`
	Target   string                               `json:"target,omitzero"`

	// Title original title for an imported session; null for other item types.
	Title string `json:"title,omitzero"`
}

// ExternalAgentConfigImportParams is generated from the ExternalAgentConfigImportParams schema definition.
type ExternalAgentConfigImportParams struct {
	MigrationItems []ExternalAgentConfigMigrationItem `json:"migrationItems"`

	// MigrationSource migration-source selector used to produce the migration items. Pass the same value to detection and import; missing or unrecognized values use the default source.
	MigrationSource string `json:"migrationSource,omitzero"`

	// ProviderID opaque provider identifier supplied by the caller for analytics attribution and import history display. This does not select the migration source.
	ProviderID string `json:"providerId,omitzero"`

	// Source optional identifier for the product that initiated the import.
	Source string `json:"source,omitzero"`
}

// ExternalAgentConfigImportProgressNotification is generated from the ExternalAgentConfigImportProgressNotification schema definition.
type ExternalAgentConfigImportProgressNotification struct {
	ImportID        string                                `json:"importId"`
	ItemTypeResults []ExternalAgentConfigImportTypeResult `json:"itemTypeResults"`
}

// ExternalAgentConfigImportResponse is generated from the ExternalAgentConfigImportResponse schema definition.
type ExternalAgentConfigImportResponse struct {
	ImportID string `json:"importId"`
}

// ExternalAgentConfigImportTypeResult is generated from the ExternalAgentConfigImportTypeResult schema definition.
type ExternalAgentConfigImportTypeResult struct {
	Failures  []ExternalAgentConfigImportItemTypeFailure `json:"failures"`
	ItemType  ExternalAgentConfigMigrationItemType       `json:"itemType"`
	Successes []ExternalAgentConfigImportItemTypeSuccess `json:"successes"`
}

// ExternalAgentConfigMigrationItem is generated from the ExternalAgentConfigMigrationItem schema definition.
type ExternalAgentConfigMigrationItem struct {
	// Cwd null or empty means home-scoped migration; non-empty means repo-scoped migration.
	Cwd         string                               `json:"cwd,omitzero"`
	Description string                               `json:"description"`
	Details     MigrationDetails                     `json:"details,omitzero"`
	ItemType    ExternalAgentConfigMigrationItemType `json:"itemType"`
}

// ExternalAgentConfigMigrationItemType is generated from the ExternalAgentConfigMigrationItemType schema definition.
type ExternalAgentConfigMigrationItemType string

const (
	// ExternalAgentConfigMigrationItemTypeAGENTSMD is the "AGENTS_MD" ExternalAgentConfigMigrationItemType value.
	ExternalAgentConfigMigrationItemTypeAGENTSMD ExternalAgentConfigMigrationItemType = "AGENTS_MD"
	// ExternalAgentConfigMigrationItemTypeCONFIG is the "CONFIG" ExternalAgentConfigMigrationItemType value.
	ExternalAgentConfigMigrationItemTypeCONFIG ExternalAgentConfigMigrationItemType = "CONFIG"
	// ExternalAgentConfigMigrationItemTypeSKILLS is the "SKILLS" ExternalAgentConfigMigrationItemType value.
	ExternalAgentConfigMigrationItemTypeSKILLS ExternalAgentConfigMigrationItemType = "SKILLS"
	// ExternalAgentConfigMigrationItemTypePLUGINS is the "PLUGINS" ExternalAgentConfigMigrationItemType value.
	ExternalAgentConfigMigrationItemTypePLUGINS ExternalAgentConfigMigrationItemType = "PLUGINS"
	// ExternalAgentConfigMigrationItemTypeMCPSERVERCONFIG is the "MCP_SERVER_CONFIG" ExternalAgentConfigMigrationItemType value.
	ExternalAgentConfigMigrationItemTypeMCPSERVERCONFIG ExternalAgentConfigMigrationItemType = "MCP_SERVER_CONFIG"
	// ExternalAgentConfigMigrationItemTypeSUBAGENTS is the "SUBAGENTS" ExternalAgentConfigMigrationItemType value.
	ExternalAgentConfigMigrationItemTypeSUBAGENTS ExternalAgentConfigMigrationItemType = "SUBAGENTS"
	// ExternalAgentConfigMigrationItemTypeHOOKS is the "HOOKS" ExternalAgentConfigMigrationItemType value.
	ExternalAgentConfigMigrationItemTypeHOOKS ExternalAgentConfigMigrationItemType = "HOOKS"
	// ExternalAgentConfigMigrationItemTypeCOMMANDS is the "COMMANDS" ExternalAgentConfigMigrationItemType value.
	ExternalAgentConfigMigrationItemTypeCOMMANDS ExternalAgentConfigMigrationItemType = "COMMANDS"
	// ExternalAgentConfigMigrationItemTypeMEMORY is the "MEMORY" ExternalAgentConfigMigrationItemType value.
	ExternalAgentConfigMigrationItemTypeMEMORY ExternalAgentConfigMigrationItemType = "MEMORY"
	// ExternalAgentConfigMigrationItemTypeSESSIONS is the "SESSIONS" ExternalAgentConfigMigrationItemType value.
	ExternalAgentConfigMigrationItemTypeSESSIONS ExternalAgentConfigMigrationItemType = "SESSIONS"
)

// ExternalAgentDetectedConnectorCandidate is generated from the ExternalAgentDetectedConnectorCandidate schema definition.
type ExternalAgentDetectedConnectorCandidate struct {
	Name         string                               `json:"name"`
	SessionCount int32                                `json:"sessionCount"`
	Source       ExternalAgentDetectedConnectorSource `json:"source"`
}

// ExternalAgentDetectedConnectorSource is generated from the ExternalAgentDetectedConnectorSource schema definition.
type ExternalAgentDetectedConnectorSource string

const (
	// ExternalAgentDetectedConnectorSourceRemoteMCPServersConfig is the "remoteMcpServersConfig" ExternalAgentDetectedConnectorSource value.
	ExternalAgentDetectedConnectorSourceRemoteMCPServersConfig ExternalAgentDetectedConnectorSource = "remoteMcpServersConfig"
	// ExternalAgentDetectedConnectorSourceSessionToolUse is the "sessionToolUse" ExternalAgentDetectedConnectorSource value.
	ExternalAgentDetectedConnectorSourceSessionToolUse ExternalAgentDetectedConnectorSource = "sessionToolUse"
)

// ExternalAgentImportedConnectorCandidate is generated from the ExternalAgentImportedConnectorCandidate schema definition.
type ExternalAgentImportedConnectorCandidate struct {
	Name         string                               `json:"name"`
	SessionCount int32                                `json:"sessionCount"`
	Source       ExternalAgentImportedConnectorSource `json:"source"`
}

// ExternalAgentImportedConnectorSource is generated from the ExternalAgentImportedConnectorSource schema definition.
type ExternalAgentImportedConnectorSource string

const (
	// ExternalAgentImportedConnectorSourceRemoteMCPServersConfig is the "remoteMcpServersConfig" ExternalAgentImportedConnectorSource value.
	ExternalAgentImportedConnectorSourceRemoteMCPServersConfig ExternalAgentImportedConnectorSource = "remoteMcpServersConfig"
)

// FeedbackRequirements is generated from the FeedbackRequirements schema definition.
type FeedbackRequirements struct {
	Enabled bool `json:"enabled,omitzero"`
}

// FeedbackUploadParams is generated from the FeedbackUploadParams schema definition.
type FeedbackUploadParams struct {
	Classification string            `json:"classification"`
	ExtraLogFiles  []string          `json:"extraLogFiles,omitzero"`
	IncludeLogs    bool              `json:"includeLogs,omitzero"`
	Reason         string            `json:"reason,omitzero"`
	Tags           map[string]string `json:"tags,omitzero"`
	ThreadID       string            `json:"threadId,omitzero"`
}

// FeedbackUploadResponse is generated from the FeedbackUploadResponse schema definition.
type FeedbackUploadResponse struct {
	ThreadID string `json:"threadId"`
}

// FileChangeOutputDeltaNotification deprecated legacy notification for `apply_patch` textual output.
//
// The server no longer emits this notification.
type FileChangeOutputDeltaNotification struct {
	Delta    string `json:"delta"`
	ItemID   string `json:"itemId"`
	ThreadID string `json:"threadId"`
	TurnID   string `json:"turnId"`
}

// FileChangePatchUpdatedNotification is generated from the FileChangePatchUpdatedNotification schema definition.
type FileChangePatchUpdatedNotification struct {
	Changes  []FileUpdateChange `json:"changes"`
	ItemID   string             `json:"itemId"`
	ThreadID string             `json:"threadId"`
	TurnID   string             `json:"turnId"`
}

// FileSystemAccessMode is generated from the FileSystemAccessMode schema definition.
type FileSystemAccessMode string

const (
	// FileSystemAccessModeRead is the "read" FileSystemAccessMode value.
	FileSystemAccessModeRead FileSystemAccessMode = "read"
	// FileSystemAccessModeWrite is the "write" FileSystemAccessMode value.
	FileSystemAccessModeWrite FileSystemAccessMode = "write"
	// FileSystemAccessModeDeny is the "deny" FileSystemAccessMode value.
	FileSystemAccessModeDeny FileSystemAccessMode = "deny"
)

// FileSystemPath is generated from the FileSystemPath schema definition.
type FileSystemPath interface {
	isFileSystemPath()
}

// RawFileSystemPath preserves an uninterpreted FileSystemPath JSON value.
type RawFileSystemPath jsontext.Value

func (RawFileSystemPath) isFileSystemPath() {}

var (
	_ json.MarshalerTo     = RawFileSystemPath{}
	_ json.UnmarshalerFrom = (*RawFileSystemPath)(nil)
)

func (value RawFileSystemPath) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawFileSystemPath) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// PathFileSystemPath is generated from the PathFileSystemPath schema definition.
type PathFileSystemPath struct {
	Path LegacyAppPathString `json:"path"`
	Type string              `json:"type"`
}

func (PathFileSystemPath) isFileSystemPath() {}

// GlobPatternFileSystemPath is generated from the GlobPatternFileSystemPath schema definition.
type GlobPatternFileSystemPath struct {
	Pattern string `json:"pattern"`
	Type    string `json:"type"`
}

func (GlobPatternFileSystemPath) isFileSystemPath() {}

// SpecialFileSystemPath is generated from the SpecialFileSystemPath schema definition.
type SpecialFileSystemPath struct {
	Type  string                `json:"type"`
	Value FileSystemSpecialPath `json:"value"`
}

func (value *SpecialFileSystemPath) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Type  string         `json:"type"`
		Value jsontext.Value `json:"value"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.Type = raw.Type
	if raw.Value == nil {
		value.Value = nil
	} else {
		decodedValue, err := decodeGeneratedFileSystemSpecialPath(raw.Value)
		if err != nil {
			return err
		}
		value.Value = decodedValue
	}
	return nil
}

func (SpecialFileSystemPath) isFileSystemPath() {}

func decodeGeneratedFileSystemPath(raw jsontext.Value) (FileSystemPath, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "path":
			var value PathFileSystemPath
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "glob_pattern":
			var value GlobPatternFileSystemPath
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "special":
			var value SpecialFileSystemPath
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawFileSystemPath(raw), nil
	}
	return RawFileSystemPath(raw), nil
}

// FileSystemSandboxEntry is generated from the FileSystemSandboxEntry schema definition.
type FileSystemSandboxEntry struct {
	Access FileSystemAccessMode `json:"access"`
	Path   FileSystemPath       `json:"path"`
}

func (value *FileSystemSandboxEntry) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Access FileSystemAccessMode `json:"access"`
		Path   jsontext.Value       `json:"path"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.Access = raw.Access
	if raw.Path == nil {
		value.Path = nil
	} else {
		decodedPath, err := decodeGeneratedFileSystemPath(raw.Path)
		if err != nil {
			return err
		}
		value.Path = decodedPath
	}
	return nil
}

// FileSystemSpecialPath is generated from the FileSystemSpecialPath schema definition.
type FileSystemSpecialPath interface {
	isFileSystemSpecialPath()
}

// RawFileSystemSpecialPath preserves an uninterpreted FileSystemSpecialPath JSON value.
type RawFileSystemSpecialPath jsontext.Value

func (RawFileSystemSpecialPath) isFileSystemSpecialPath() {}

var (
	_ json.MarshalerTo     = RawFileSystemSpecialPath{}
	_ json.UnmarshalerFrom = (*RawFileSystemSpecialPath)(nil)
)

func (value RawFileSystemSpecialPath) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawFileSystemSpecialPath) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// RootFileSystemSpecialPath is generated from the RootFileSystemSpecialPath schema definition.
type RootFileSystemSpecialPath struct {
	Kind string `json:"kind"`
}

func (RootFileSystemSpecialPath) isFileSystemSpecialPath() {}

// MinimalFileSystemSpecialPath is generated from the MinimalFileSystemSpecialPath schema definition.
type MinimalFileSystemSpecialPath struct {
	Kind string `json:"kind"`
}

func (MinimalFileSystemSpecialPath) isFileSystemSpecialPath() {}

// KindFileSystemSpecialPath is generated from the KindFileSystemSpecialPath schema definition.
type KindFileSystemSpecialPath struct {
	Kind    string               `json:"kind"`
	Subpath *LegacyAppPathString `json:"subpath,omitzero"`
}

func (KindFileSystemSpecialPath) isFileSystemSpecialPath() {}

// TmpdirFileSystemSpecialPath is generated from the TmpdirFileSystemSpecialPath schema definition.
type TmpdirFileSystemSpecialPath struct {
	Kind string `json:"kind"`
}

func (TmpdirFileSystemSpecialPath) isFileSystemSpecialPath() {}

// SlashTmpFileSystemSpecialPath is generated from the SlashTmpFileSystemSpecialPath schema definition.
type SlashTmpFileSystemSpecialPath struct {
	Kind string `json:"kind"`
}

func (SlashTmpFileSystemSpecialPath) isFileSystemSpecialPath() {}

// FileSystemSpecialPathKind is generated from the FileSystemSpecialPathKind schema definition.
type FileSystemSpecialPathKind struct {
	Kind    string               `json:"kind"`
	Path    string               `json:"path"`
	Subpath *LegacyAppPathString `json:"subpath,omitzero"`
}

func (FileSystemSpecialPathKind) isFileSystemSpecialPath() {}

func decodeGeneratedFileSystemSpecialPath(raw jsontext.Value) (FileSystemSpecialPath, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Kind string `json:"kind"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Kind {
		case "root":
			var value RootFileSystemSpecialPath
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "minimal":
			var value MinimalFileSystemSpecialPath
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "project_roots":
			var value KindFileSystemSpecialPath
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "tmpdir":
			var value TmpdirFileSystemSpecialPath
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "slash_tmp":
			var value SlashTmpFileSystemSpecialPath
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "unknown":
			var value FileSystemSpecialPathKind
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawFileSystemSpecialPath(raw), nil
	}
	return RawFileSystemSpecialPath(raw), nil
}

// FileUpdateChange is generated from the FileUpdateChange schema definition.
type FileUpdateChange struct {
	Diff string          `json:"diff"`
	Kind PatchChangeKind `json:"kind"`
	Path string          `json:"path"`
}

func (value *FileUpdateChange) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Diff string         `json:"diff"`
		Kind jsontext.Value `json:"kind"`
		Path string         `json:"path"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.Diff = raw.Diff
	if raw.Kind == nil {
		value.Kind = nil
	} else {
		decodedKind, err := decodeGeneratedPatchChangeKind(raw.Kind)
		if err != nil {
			return err
		}
		value.Kind = decodedKind
	}
	value.Path = raw.Path
	return nil
}

// ForcedChatGPTWorkspaceIDs backward-compatible API shape for ChatGPT workspace login restrictions.
type ForcedChatGPTWorkspaceIDs interface {
	isForcedChatGPTWorkspaceIDs()
}

// RawForcedChatGPTWorkspaceIDs preserves an uninterpreted ForcedChatGPTWorkspaceIDs JSON value.
type RawForcedChatGPTWorkspaceIDs jsontext.Value

func (RawForcedChatGPTWorkspaceIDs) isForcedChatGPTWorkspaceIDs() {}

var (
	_ json.MarshalerTo     = RawForcedChatGPTWorkspaceIDs{}
	_ json.UnmarshalerFrom = (*RawForcedChatGPTWorkspaceIDs)(nil)
)

func (value RawForcedChatGPTWorkspaceIDs) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawForcedChatGPTWorkspaceIDs) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// ForcedChatGPTWorkspaceIDsString is a ForcedChatGPTWorkspaceIDs variant.
type ForcedChatGPTWorkspaceIDsString string

func (ForcedChatGPTWorkspaceIDsString) isForcedChatGPTWorkspaceIDs() {}

// ForcedChatGPTWorkspaceIDsItems is a ForcedChatGPTWorkspaceIDs variant.
type ForcedChatGPTWorkspaceIDsItems []string

func (ForcedChatGPTWorkspaceIDsItems) isForcedChatGPTWorkspaceIDs() {}

func decodeGeneratedForcedChatGPTWorkspaceIDs(raw jsontext.Value) (ForcedChatGPTWorkspaceIDs, error) {
	if raw == nil {
		return nil, nil
	}
	var text string
	if err := json.Unmarshal(raw, &text); err == nil {
		return ForcedChatGPTWorkspaceIDsString(text), nil
	}
	var value ForcedChatGPTWorkspaceIDsItems
	if err := json.Unmarshal(raw, &value); err == nil {
		return value, nil
	}
	return RawForcedChatGPTWorkspaceIDs(raw), nil
}

// ForcedLoginMethod is generated from the ForcedLoginMethod schema definition.
type ForcedLoginMethod string

const (
	// ForcedLoginMethodChatGPT is the "chatgpt" ForcedLoginMethod value.
	ForcedLoginMethodChatGPT ForcedLoginMethod = "chatgpt"
	// ForcedLoginMethodAPI is the "api" ForcedLoginMethod value.
	ForcedLoginMethodAPI ForcedLoginMethod = "api"
)

// FSChangedNotification filesystem watch notification emitted for `fs/watch` subscribers.
type FSChangedNotification struct {
	// ChangedPaths file or directory paths associated with this event.
	ChangedPaths []string `json:"changedPaths"`

	// WatchID watches identifier previously provided to `fs/watch`.
	WatchID string `json:"watchId"`
}

// FSCopyParams copies a file or directory tree on the host filesystem.
type FSCopyParams struct {
	// DestinationPath absolute destination path.
	DestinationPath string `json:"destinationPath"`

	// Recursive required for directory copies; ignored for file copies.
	Recursive bool `json:"recursive,omitzero"`

	// SourcePath absolute source path.
	SourcePath string `json:"sourcePath"`
}

// FSCopyResponse successful response for `fs/copy`.
type FSCopyResponse struct{}

// FSCreateDirectoryParams creates a directory on the host filesystem.
type FSCreateDirectoryParams struct {
	// Path absolute directory path to create.
	Path string `json:"path"`

	// Recursive reports whether parent directories should also be created. Defaults to `true`.
	Recursive bool `json:"recursive,omitzero"`
}

// FSCreateDirectoryResponse successful response for `fs/createDirectory`.
type FSCreateDirectoryResponse struct{}

// FSGetMetadataParams requests metadata for an absolute path.
type FSGetMetadataParams struct {
	// Path absolute path to inspect.
	Path string `json:"path"`
}

// FSGetMetadataResponse metadata returned by `fs/getMetadata`.
type FSGetMetadataResponse struct {
	// CreatedAtMs file creation time in Unix milliseconds when available, otherwise `0`.
	CreatedAtMs int64 `json:"createdAtMs"`

	// IsDirectory reports whether the path resolves to a directory.
	IsDirectory bool `json:"isDirectory"`

	// IsFile reports whether the path resolves to a regular file.
	IsFile bool `json:"isFile"`

	// IsSymlink reports whether the path itself is a symbolic link.
	IsSymlink bool `json:"isSymlink"`

	// ModifiedAtMs file modification time in Unix milliseconds when available, otherwise `0`.
	ModifiedAtMs int64 `json:"modifiedAtMs"`
}

// FSReadDirectoryEntry represents a directory entry returned by `fs/readDirectory`.
type FSReadDirectoryEntry struct {
	// FileName direct child entry name only, not an absolute or relative path.
	FileName string `json:"fileName"`

	// IsDirectory reports whether this entry resolves to a directory.
	IsDirectory bool `json:"isDirectory"`

	// IsFile reports whether this entry resolves to a regular file.
	IsFile bool `json:"isFile"`
}

// FSReadDirectoryParams lists direct child names for a directory.
type FSReadDirectoryParams struct {
	// Path absolute directory path to read.
	Path string `json:"path"`
}

// FSReadDirectoryResponse directory entries returned by `fs/readDirectory`.
type FSReadDirectoryResponse struct {
	// Entries direct child entries in the requested directory.
	Entries []FSReadDirectoryEntry `json:"entries"`
}

// FSReadFileParams reads a file from the host filesystem.
type FSReadFileParams struct {
	// Path absolute path to read.
	Path string `json:"path"`
}

// FSReadFileResponse base64-encoded file contents returned by `fs/readFile`.
type FSReadFileResponse struct {
	// DataBase64 file contents encoded as base64.
	DataBase64 string `json:"dataBase64"`
}

// FSRemoveParams removes a file or directory tree from the host filesystem.
type FSRemoveParams struct {
	// Force reports whether missing paths should be ignored. Defaults to `true`.
	Force bool `json:"force,omitzero"`

	// Path absolute path to remove.
	Path string `json:"path"`

	// Recursive reports whether directory removal should recurse. Defaults to `true`.
	Recursive bool `json:"recursive,omitzero"`
}

// FSRemoveResponse successful response for `fs/remove`.
type FSRemoveResponse struct{}

// FSUnwatchParams stops filesystem watch notifications for a prior `fs/watch`.
type FSUnwatchParams struct {
	// WatchID watches identifier previously provided to `fs/watch`.
	WatchID string `json:"watchId"`
}

// FSUnwatchResponse successful response for `fs/unwatch`.
type FSUnwatchResponse struct{}

// FSWatchParams starts filesystem watch notifications for an absolute path.
type FSWatchParams struct {
	// Path absolute file or directory path to watch.
	Path string `json:"path"`

	// WatchID connection-scoped watch identifier used for `fs/unwatch` and `fs/changed`.
	WatchID string `json:"watchId"`
}

// FSWatchResponse successful response for `fs/watch`.
type FSWatchResponse struct {
	// Path canonicalized path associated with the watch.
	Path string `json:"path"`
}

// FSWriteFileParams writes a file on the host filesystem.
type FSWriteFileParams struct {
	// DataBase64 file contents encoded as base64.
	DataBase64 string `json:"dataBase64"`

	// Path absolute path to write.
	Path string `json:"path"`
}

// FSWriteFileResponse successful response for `fs/writeFile`.
type FSWriteFileResponse struct{}

// FunctionCallOutputBody is generated from the FunctionCallOutputBody schema definition.
type FunctionCallOutputBody interface {
	isFunctionCallOutputBody()
}

// RawFunctionCallOutputBody preserves an uninterpreted FunctionCallOutputBody JSON value.
type RawFunctionCallOutputBody jsontext.Value

func (RawFunctionCallOutputBody) isFunctionCallOutputBody() {}

var (
	_ json.MarshalerTo     = RawFunctionCallOutputBody{}
	_ json.UnmarshalerFrom = (*RawFunctionCallOutputBody)(nil)
)

func (value RawFunctionCallOutputBody) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawFunctionCallOutputBody) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// FunctionCallOutputBodyString is a FunctionCallOutputBody variant.
type FunctionCallOutputBodyString string

func (FunctionCallOutputBodyString) isFunctionCallOutputBody() {}

// FunctionCallOutputBodyItems is a FunctionCallOutputBody variant.
type FunctionCallOutputBodyItems []FunctionCallOutputContentItem

func (FunctionCallOutputBodyItems) isFunctionCallOutputBody() {}

func decodeGeneratedFunctionCallOutputBody(raw jsontext.Value) (FunctionCallOutputBody, error) {
	if raw == nil {
		return nil, nil
	}
	var text string
	if err := json.Unmarshal(raw, &text); err == nil {
		return FunctionCallOutputBodyString(text), nil
	}
	var rawItems []jsontext.Value
	if err := json.Unmarshal(raw, &rawItems); err == nil {
		value := make(FunctionCallOutputBodyItems, len(rawItems))
		for i, item := range rawItems {
			if item == nil {
				continue
			}
			decodedItem, err := decodeGeneratedFunctionCallOutputContentItem(item)
			if err != nil {
				return nil, err
			}
			value[i] = decodedItem
		}
		return value, nil
	}
	return RawFunctionCallOutputBody(raw), nil
}

// FunctionCallOutputContentItem responses API compatible content items that can be returned by a tool call. This is a subset of ContentItem with the types we support as function call outputs.
type FunctionCallOutputContentItem interface {
	isFunctionCallOutputContentItem()
}

// RawFunctionCallOutputContentItem preserves an uninterpreted FunctionCallOutputContentItem JSON value.
type RawFunctionCallOutputContentItem jsontext.Value

func (RawFunctionCallOutputContentItem) isFunctionCallOutputContentItem() {}

var (
	_ json.MarshalerTo     = RawFunctionCallOutputContentItem{}
	_ json.UnmarshalerFrom = (*RawFunctionCallOutputContentItem)(nil)
)

func (value RawFunctionCallOutputContentItem) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawFunctionCallOutputContentItem) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// InputTextFunctionCallOutputContentItem is generated from the InputTextFunctionCallOutputContentItem schema definition.
type InputTextFunctionCallOutputContentItem struct {
	Text string `json:"text"`
	Type string `json:"type"`
}

func (InputTextFunctionCallOutputContentItem) isFunctionCallOutputContentItem() {}

// InputImageFunctionCallOutputContentItem is generated from the InputImageFunctionCallOutputContentItem schema definition.
type InputImageFunctionCallOutputContentItem struct {
	Detail   ImageDetail `json:"detail,omitzero"`
	ImageURL string      `json:"image_url"`
	Type     string      `json:"type"`
}

func (InputImageFunctionCallOutputContentItem) isFunctionCallOutputContentItem() {}

// InputAudioFunctionCallOutputContentItem is generated from the InputAudioFunctionCallOutputContentItem schema definition.
type InputAudioFunctionCallOutputContentItem struct {
	AudioURL string `json:"audio_url"`
	Type     string `json:"type"`
}

func (InputAudioFunctionCallOutputContentItem) isFunctionCallOutputContentItem() {}

// EncryptedContentFunctionCallOutputContentItem is generated from the EncryptedContentFunctionCallOutputContentItem schema definition.
type EncryptedContentFunctionCallOutputContentItem struct {
	EncryptedContent string `json:"encrypted_content"`
	Type             string `json:"type"`
}

func (EncryptedContentFunctionCallOutputContentItem) isFunctionCallOutputContentItem() {}

func decodeGeneratedFunctionCallOutputContentItem(raw jsontext.Value) (FunctionCallOutputContentItem, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "input_text":
			var value InputTextFunctionCallOutputContentItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "input_image":
			var value InputImageFunctionCallOutputContentItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "input_audio":
			var value InputAudioFunctionCallOutputContentItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "encrypted_content":
			var value EncryptedContentFunctionCallOutputContentItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawFunctionCallOutputContentItem(raw), nil
	}
	return RawFunctionCallOutputContentItem(raw), nil
}

// FuzzyFileSearchMatchType is generated from the FuzzyFileSearchMatchType schema definition.
type FuzzyFileSearchMatchType string

const (
	// FuzzyFileSearchMatchTypeFile is the "file" FuzzyFileSearchMatchType value.
	FuzzyFileSearchMatchTypeFile FuzzyFileSearchMatchType = "file"
	// FuzzyFileSearchMatchTypeDirectory is the "directory" FuzzyFileSearchMatchType value.
	FuzzyFileSearchMatchTypeDirectory FuzzyFileSearchMatchType = "directory"
)

// FuzzyFileSearchParams is generated from the FuzzyFileSearchParams schema definition.
type FuzzyFileSearchParams struct {
	CancellationToken string   `json:"cancellationToken,omitzero"`
	Query             string   `json:"query"`
	Roots             []string `json:"roots"`
}

// FuzzyFileSearchResult superset of [`codex_file_search::FileMatch`].
type FuzzyFileSearchResult struct {
	FileName  string                   `json:"file_name"`
	Indices   []int32                  `json:"indices,omitzero"`
	MatchType FuzzyFileSearchMatchType `json:"match_type"`
	Path      string                   `json:"path"`
	Root      string                   `json:"root"`
	Score     int32                    `json:"score"`
}

// FuzzyFileSearchSessionCompletedNotification is generated from the FuzzyFileSearchSessionCompletedNotification schema definition.
type FuzzyFileSearchSessionCompletedNotification struct {
	SessionID string `json:"sessionId"`
}

// FuzzyFileSearchSessionStartParams is generated from the FuzzyFileSearchSessionStartParams schema definition.
type FuzzyFileSearchSessionStartParams struct {
	Roots     []string `json:"roots"`
	SessionID string   `json:"sessionId"`
}

// FuzzyFileSearchSessionStopParams is generated from the FuzzyFileSearchSessionStopParams schema definition.
type FuzzyFileSearchSessionStopParams struct {
	SessionID string `json:"sessionId"`
}

// FuzzyFileSearchSessionUpdateParams is generated from the FuzzyFileSearchSessionUpdateParams schema definition.
type FuzzyFileSearchSessionUpdateParams struct {
	Query     string `json:"query"`
	SessionID string `json:"sessionId"`
}

// FuzzyFileSearchSessionUpdatedNotification is generated from the FuzzyFileSearchSessionUpdatedNotification schema definition.
type FuzzyFileSearchSessionUpdatedNotification struct {
	Files     []FuzzyFileSearchResult `json:"files"`
	Query     string                  `json:"query"`
	SessionID string                  `json:"sessionId"`
}

// GetAccountParams is generated from the GetAccountParams schema definition.
type GetAccountParams struct {
	// RefreshToken when `true`, requests a proactive token refresh before returning.
	//
	// In managed auth mode this triggers the normal refresh-token flow. In external auth mode this flag is ignored. Clients should refresh tokens themselves and call `account/login/start` with `chatgptAuthTokens`.
	RefreshToken bool `json:"refreshToken,omitzero"`
}

// GetAccountRateLimitsResponse is generated from the GetAccountRateLimitsResponse schema definition.
type GetAccountRateLimitsResponse struct {
	RateLimitResetCredits RateLimitResetCreditsSummary `json:"rateLimitResetCredits,omitzero"`

	// RateLimits backward-compatible single-bucket view; mirrors the historical payload.
	RateLimits RateLimitSnapshot `json:"rateLimits"`

	// RateLimitsByLimitID multi-bucket view keyed by metered `limit_id` (for example, `codex`).
	RateLimitsByLimitID map[string]RateLimitSnapshot `json:"rateLimitsByLimitId,omitzero"`
}

// GetAccountResponse is generated from the GetAccountResponse schema definition.
type GetAccountResponse struct {
	Account            Account `json:"account,omitzero"`
	RequiresOpenaiAuth bool    `json:"requiresOpenaiAuth"`
}

func (value *GetAccountResponse) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Account            jsontext.Value `json:"account,omitzero"`
		RequiresOpenaiAuth bool           `json:"requiresOpenaiAuth"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	if raw.Account == nil {
		value.Account = nil
	} else {
		decodedAccount, err := decodeGeneratedAccount(raw.Account)
		if err != nil {
			return err
		}
		value.Account = decodedAccount
	}
	value.RequiresOpenaiAuth = raw.RequiresOpenaiAuth
	return nil
}

// GetAccountTokenUsageResponse is generated from the GetAccountTokenUsageResponse schema definition.
type GetAccountTokenUsageResponse struct {
	DailyUsageBuckets []AccountTokenUsageDailyBucket `json:"dailyUsageBuckets,omitzero"`
	Summary           AccountTokenUsageSummary       `json:"summary"`
}

// GetWorkspaceMessagesResponse is generated from the GetWorkspaceMessagesResponse schema definition.
type GetWorkspaceMessagesResponse struct {
	// FeatureEnabled reports whether the workspace-message backend route is available for this client.
	FeatureEnabled bool `json:"featureEnabled"`

	// Messages active workspace messages returned by the backend.
	Messages []WorkspaceMessage `json:"messages"`
}

// GitInfo is generated from the GitInfo schema definition.
type GitInfo struct {
	Branch    string `json:"branch,omitzero"`
	OriginURL string `json:"originUrl,omitzero"`
	Sha       string `json:"sha,omitzero"`
}

// GuardianApprovalReview [UNSTABLE] Temporary approval auto-review payload used by `item/autoApprovalReview/*` notifications. This shape is expected to change soon.
type GuardianApprovalReview struct {
	Rationale         string                       `json:"rationale,omitzero"`
	RiskLevel         GuardianRiskLevel            `json:"riskLevel,omitzero"`
	Status            GuardianApprovalReviewStatus `json:"status"`
	UserAuthorization GuardianUserAuthorization    `json:"userAuthorization,omitzero"`
}

// GuardianApprovalReviewAction is generated from the GuardianApprovalReviewAction schema definition.
type GuardianApprovalReviewAction interface {
	isGuardianApprovalReviewAction()
}

// RawGuardianApprovalReviewAction preserves an uninterpreted GuardianApprovalReviewAction JSON value.
type RawGuardianApprovalReviewAction jsontext.Value

func (RawGuardianApprovalReviewAction) isGuardianApprovalReviewAction() {}

var (
	_ json.MarshalerTo     = RawGuardianApprovalReviewAction{}
	_ json.UnmarshalerFrom = (*RawGuardianApprovalReviewAction)(nil)
)

func (value RawGuardianApprovalReviewAction) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawGuardianApprovalReviewAction) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// CommandGuardianApprovalReviewAction is generated from the CommandGuardianApprovalReviewAction schema definition.
type CommandGuardianApprovalReviewAction struct {
	Command string                `json:"command"`
	Cwd     string                `json:"cwd"`
	Source  GuardianCommandSource `json:"source"`
	Type    string                `json:"type"`
}

func (CommandGuardianApprovalReviewAction) isGuardianApprovalReviewAction() {}

// ExecveGuardianApprovalReviewAction is generated from the ExecveGuardianApprovalReviewAction schema definition.
type ExecveGuardianApprovalReviewAction struct {
	Argv    []string              `json:"argv"`
	Cwd     string                `json:"cwd"`
	Program string                `json:"program"`
	Source  GuardianCommandSource `json:"source"`
	Type    string                `json:"type"`
}

func (ExecveGuardianApprovalReviewAction) isGuardianApprovalReviewAction() {}

// ApplyPatchGuardianApprovalReviewAction is generated from the ApplyPatchGuardianApprovalReviewAction schema definition.
type ApplyPatchGuardianApprovalReviewAction struct {
	Cwd   string   `json:"cwd"`
	Files []string `json:"files"`
	Type  string   `json:"type"`
}

func (ApplyPatchGuardianApprovalReviewAction) isGuardianApprovalReviewAction() {}

// NetworkAccessGuardianApprovalReviewAction is generated from the NetworkAccessGuardianApprovalReviewAction schema definition.
type NetworkAccessGuardianApprovalReviewAction struct {
	Host     string                  `json:"host"`
	Port     int64                   `json:"port"`
	Protocol NetworkApprovalProtocol `json:"protocol"`
	Target   string                  `json:"target"`
	Type     string                  `json:"type"`
}

func (NetworkAccessGuardianApprovalReviewAction) isGuardianApprovalReviewAction() {}

// MCPToolCallGuardianApprovalReviewAction is generated from the McpToolCallGuardianApprovalReviewAction schema definition.
type MCPToolCallGuardianApprovalReviewAction struct {
	ConnectorID   string `json:"connectorId,omitzero"`
	ConnectorName string `json:"connectorName,omitzero"`
	Server        string `json:"server"`
	ToolName      string `json:"toolName"`
	ToolTitle     string `json:"toolTitle,omitzero"`
	Type          string `json:"type"`
}

func (MCPToolCallGuardianApprovalReviewAction) isGuardianApprovalReviewAction() {}

// RequestPermissionsGuardianApprovalReviewAction is generated from the RequestPermissionsGuardianApprovalReviewAction schema definition.
type RequestPermissionsGuardianApprovalReviewAction struct {
	Permissions RequestPermissionProfile `json:"permissions"`
	Reason      string                   `json:"reason,omitzero"`
	Type        string                   `json:"type"`
}

func (RequestPermissionsGuardianApprovalReviewAction) isGuardianApprovalReviewAction() {}

func decodeGeneratedGuardianApprovalReviewAction(raw jsontext.Value) (GuardianApprovalReviewAction, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "command":
			var value CommandGuardianApprovalReviewAction
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "execve":
			var value ExecveGuardianApprovalReviewAction
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "applyPatch":
			var value ApplyPatchGuardianApprovalReviewAction
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "networkAccess":
			var value NetworkAccessGuardianApprovalReviewAction
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "mcpToolCall":
			var value MCPToolCallGuardianApprovalReviewAction
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "requestPermissions":
			var value RequestPermissionsGuardianApprovalReviewAction
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawGuardianApprovalReviewAction(raw), nil
	}
	return RawGuardianApprovalReviewAction(raw), nil
}

// GuardianApprovalReviewStatus [UNSTABLE] Lifecycle state for an approval auto-review.
type GuardianApprovalReviewStatus string

const (
	// GuardianApprovalReviewStatusInProgress is the "inProgress" GuardianApprovalReviewStatus value.
	GuardianApprovalReviewStatusInProgress GuardianApprovalReviewStatus = "inProgress"
	// GuardianApprovalReviewStatusApproved is the "approved" GuardianApprovalReviewStatus value.
	GuardianApprovalReviewStatusApproved GuardianApprovalReviewStatus = "approved"
	// GuardianApprovalReviewStatusDenied is the "denied" GuardianApprovalReviewStatus value.
	GuardianApprovalReviewStatusDenied GuardianApprovalReviewStatus = "denied"
	// GuardianApprovalReviewStatusTimedOut is the "timedOut" GuardianApprovalReviewStatus value.
	GuardianApprovalReviewStatusTimedOut GuardianApprovalReviewStatus = "timedOut"
	// GuardianApprovalReviewStatusAborted is the "aborted" GuardianApprovalReviewStatus value.
	GuardianApprovalReviewStatusAborted GuardianApprovalReviewStatus = "aborted"
)

// GuardianCommandSource is generated from the GuardianCommandSource schema definition.
type GuardianCommandSource string

const (
	// GuardianCommandSourceShell is the "shell" GuardianCommandSource value.
	GuardianCommandSourceShell GuardianCommandSource = "shell"
	// GuardianCommandSourceUnifiedExec is the "unifiedExec" GuardianCommandSource value.
	GuardianCommandSourceUnifiedExec GuardianCommandSource = "unifiedExec"
)

// GuardianRiskLevel [UNSTABLE] Risk level assigned by approval auto-review.
type GuardianRiskLevel string

const (
	// GuardianRiskLevelLow is the "low" GuardianRiskLevel value.
	GuardianRiskLevelLow GuardianRiskLevel = "low"
	// GuardianRiskLevelMedium is the "medium" GuardianRiskLevel value.
	GuardianRiskLevelMedium GuardianRiskLevel = "medium"
	// GuardianRiskLevelHigh is the "high" GuardianRiskLevel value.
	GuardianRiskLevelHigh GuardianRiskLevel = "high"
	// GuardianRiskLevelCritical is the "critical" GuardianRiskLevel value.
	GuardianRiskLevelCritical GuardianRiskLevel = "critical"
)

// GuardianUserAuthorization [UNSTABLE] Authorization level assigned by approval auto-review.
type GuardianUserAuthorization string

const (
	// GuardianUserAuthorizationUnknown is the "unknown" GuardianUserAuthorization value.
	GuardianUserAuthorizationUnknown GuardianUserAuthorization = "unknown"
	// GuardianUserAuthorizationLow is the "low" GuardianUserAuthorization value.
	GuardianUserAuthorizationLow GuardianUserAuthorization = "low"
	// GuardianUserAuthorizationMedium is the "medium" GuardianUserAuthorization value.
	GuardianUserAuthorizationMedium GuardianUserAuthorization = "medium"
	// GuardianUserAuthorizationHigh is the "high" GuardianUserAuthorization value.
	GuardianUserAuthorizationHigh GuardianUserAuthorization = "high"
)

// GuardianWarningNotification is generated from the GuardianWarningNotification schema definition.
type GuardianWarningNotification struct {
	// Message concise guardian warning message for the user.
	Message string `json:"message"`

	// ThreadID thread target for the guardian warning.
	ThreadID string `json:"threadId"`
}

// HookCompletedNotification is generated from the HookCompletedNotification schema definition.
type HookCompletedNotification struct {
	Run      HookRunSummary `json:"run"`
	ThreadID string         `json:"threadId"`
	TurnID   string         `json:"turnId,omitzero"`
}

// HookErrorInfo is generated from the HookErrorInfo schema definition.
type HookErrorInfo struct {
	Message string `json:"message"`
	Path    string `json:"path"`
}

// HookEventName is generated from the HookEventName schema definition.
type HookEventName string

const (
	// HookEventNamePreToolUse is the "preToolUse" HookEventName value.
	HookEventNamePreToolUse HookEventName = "preToolUse"
	// HookEventNamePermissionRequest is the "permissionRequest" HookEventName value.
	HookEventNamePermissionRequest HookEventName = "permissionRequest"
	// HookEventNamePostToolUse is the "postToolUse" HookEventName value.
	HookEventNamePostToolUse HookEventName = "postToolUse"
	// HookEventNamePreCompact is the "preCompact" HookEventName value.
	HookEventNamePreCompact HookEventName = "preCompact"
	// HookEventNamePostCompact is the "postCompact" HookEventName value.
	HookEventNamePostCompact HookEventName = "postCompact"
	// HookEventNameSessionStart is the "sessionStart" HookEventName value.
	HookEventNameSessionStart HookEventName = "sessionStart"
	// HookEventNameSessionEnd is the "sessionEnd" HookEventName value.
	HookEventNameSessionEnd HookEventName = "sessionEnd"
	// HookEventNameUserPromptSubmit is the "userPromptSubmit" HookEventName value.
	HookEventNameUserPromptSubmit HookEventName = "userPromptSubmit"
	// HookEventNameSubagentStart is the "subagentStart" HookEventName value.
	HookEventNameSubagentStart HookEventName = "subagentStart"
	// HookEventNameSubagentStop is the "subagentStop" HookEventName value.
	HookEventNameSubagentStop HookEventName = "subagentStop"
	// HookEventNameStop is the "stop" HookEventName value.
	HookEventNameStop HookEventName = "stop"
)

// HookExecutionMode is generated from the HookExecutionMode schema definition.
type HookExecutionMode string

const (
	// HookExecutionModeSync is the "sync" HookExecutionMode value.
	HookExecutionModeSync HookExecutionMode = "sync"
	// HookExecutionModeAsync is the "async" HookExecutionMode value.
	HookExecutionModeAsync HookExecutionMode = "async"
)

// HookHandlerType is generated from the HookHandlerType schema definition.
type HookHandlerType string

const (
	// HookHandlerTypeCommand is the "command" HookHandlerType value.
	HookHandlerTypeCommand HookHandlerType = "command"
	// HookHandlerTypePrompt is the "prompt" HookHandlerType value.
	HookHandlerTypePrompt HookHandlerType = "prompt"
	// HookHandlerTypeAgent is the "agent" HookHandlerType value.
	HookHandlerTypeAgent HookHandlerType = "agent"
)

// HookMetadata is generated from the HookMetadata schema definition.
type HookMetadata struct {
	// AdditionalContextLimit configured `additionalContext` spill threshold. `null` uses 2,500 tokens; `0` disables spilling.
	AdditionalContextLimit uint            `json:"additionalContextLimit,omitzero"`
	Command                string          `json:"command,omitzero"`
	CurrentHash            string          `json:"currentHash"`
	DisplayOrder           int64           `json:"displayOrder"`
	Enabled                bool            `json:"enabled"`
	EventName              HookEventName   `json:"eventName"`
	HandlerType            HookHandlerType `json:"handlerType"`
	IsManaged              bool            `json:"isManaged"`
	Key                    string          `json:"key"`
	Matcher                string          `json:"matcher,omitzero"`
	PluginID               string          `json:"pluginId,omitzero"`
	Source                 HookSource      `json:"source"`
	SourcePath             string          `json:"sourcePath"`
	StatusMessage          string          `json:"statusMessage,omitzero"`
	TimeoutSec             uint64          `json:"timeoutSec"`
	TrustStatus            HookTrustStatus `json:"trustStatus"`
}

// HookMigration is generated from the HookMigration schema definition.
type HookMigration struct {
	Name string `json:"name"`
}

// HookOutputEntry is generated from the HookOutputEntry schema definition.
type HookOutputEntry struct {
	Kind HookOutputEntryKind `json:"kind"`
	Text string              `json:"text"`
}

// HookOutputEntryKind is generated from the HookOutputEntryKind schema definition.
type HookOutputEntryKind string

const (
	// HookOutputEntryKindWarning is the "warning" HookOutputEntryKind value.
	HookOutputEntryKindWarning HookOutputEntryKind = "warning"
	// HookOutputEntryKindStop is the "stop" HookOutputEntryKind value.
	HookOutputEntryKindStop HookOutputEntryKind = "stop"
	// HookOutputEntryKindFeedback is the "feedback" HookOutputEntryKind value.
	HookOutputEntryKindFeedback HookOutputEntryKind = "feedback"
	// HookOutputEntryKindContext is the "context" HookOutputEntryKind value.
	HookOutputEntryKindContext HookOutputEntryKind = "context"
	// HookOutputEntryKindError is the "error" HookOutputEntryKind value.
	HookOutputEntryKindError HookOutputEntryKind = "error"
)

// HookPromptFragment is generated from the HookPromptFragment schema definition.
type HookPromptFragment struct {
	HookRunID string `json:"hookRunId"`
	Text      string `json:"text"`
}

// HookRunStatus is generated from the HookRunStatus schema definition.
type HookRunStatus string

const (
	// HookRunStatusRunning is the "running" HookRunStatus value.
	HookRunStatusRunning HookRunStatus = "running"
	// HookRunStatusCompleted is the "completed" HookRunStatus value.
	HookRunStatusCompleted HookRunStatus = "completed"
	// HookRunStatusFailed is the "failed" HookRunStatus value.
	HookRunStatusFailed HookRunStatus = "failed"
	// HookRunStatusBlocked is the "blocked" HookRunStatus value.
	HookRunStatusBlocked HookRunStatus = "blocked"
	// HookRunStatusStopped is the "stopped" HookRunStatus value.
	HookRunStatusStopped HookRunStatus = "stopped"
)

// HookRunSummary is generated from the HookRunSummary schema definition.
type HookRunSummary struct {
	CompletedAt   int64             `json:"completedAt,omitzero"`
	DisplayOrder  int64             `json:"displayOrder"`
	DurationMs    int64             `json:"durationMs,omitzero"`
	Entries       []HookOutputEntry `json:"entries"`
	EventName     HookEventName     `json:"eventName"`
	ExecutionMode HookExecutionMode `json:"executionMode"`
	HandlerType   HookHandlerType   `json:"handlerType"`
	ID            string            `json:"id"`
	Scope         HookScope         `json:"scope"`
	Source        HookSource        `json:"source,omitzero"`
	SourcePath    string            `json:"sourcePath"`
	StartedAt     int64             `json:"startedAt"`
	Status        HookRunStatus     `json:"status"`
	StatusMessage string            `json:"statusMessage,omitzero"`
}

// HookScope is generated from the HookScope schema definition.
type HookScope string

const (
	// HookScopeThread is the "thread" HookScope value.
	HookScopeThread HookScope = "thread"
	// HookScopeTurn is the "turn" HookScope value.
	HookScopeTurn HookScope = "turn"
)

// HookSource is generated from the HookSource schema definition.
type HookSource string

const (
	// HookSourceSystem is the "system" HookSource value.
	HookSourceSystem HookSource = "system"
	// HookSourceUser is the "user" HookSource value.
	HookSourceUser HookSource = "user"
	// HookSourceProject is the "project" HookSource value.
	HookSourceProject HookSource = "project"
	// HookSourceMdm is the "mdm" HookSource value.
	HookSourceMdm HookSource = "mdm"
	// HookSourceSessionFlags is the "sessionFlags" HookSource value.
	HookSourceSessionFlags HookSource = "sessionFlags"
	// HookSourcePlugin is the "plugin" HookSource value.
	HookSourcePlugin HookSource = "plugin"
	// HookSourceCloudRequirements is the "cloudRequirements" HookSource value.
	HookSourceCloudRequirements HookSource = "cloudRequirements"
	// HookSourceCloudManagedConfig is the "cloudManagedConfig" HookSource value.
	HookSourceCloudManagedConfig HookSource = "cloudManagedConfig"
	// HookSourceLegacyManagedConfigFile is the "legacyManagedConfigFile" HookSource value.
	HookSourceLegacyManagedConfigFile HookSource = "legacyManagedConfigFile"
	// HookSourceLegacyManagedConfigMdm is the "legacyManagedConfigMdm" HookSource value.
	HookSourceLegacyManagedConfigMdm HookSource = "legacyManagedConfigMdm"
	// HookSourceUnknown is the "unknown" HookSource value.
	HookSourceUnknown HookSource = "unknown"
)

// HookStartedNotification is generated from the HookStartedNotification schema definition.
type HookStartedNotification struct {
	Run      HookRunSummary `json:"run"`
	ThreadID string         `json:"threadId"`
	TurnID   string         `json:"turnId,omitzero"`
}

// HookTrustStatus is generated from the HookTrustStatus schema definition.
type HookTrustStatus string

const (
	// HookTrustStatusManaged is the "managed" HookTrustStatus value.
	HookTrustStatusManaged HookTrustStatus = "managed"
	// HookTrustStatusUntrusted is the "untrusted" HookTrustStatus value.
	HookTrustStatusUntrusted HookTrustStatus = "untrusted"
	// HookTrustStatusTrusted is the "trusted" HookTrustStatus value.
	HookTrustStatusTrusted HookTrustStatus = "trusted"
	// HookTrustStatusModified is the "modified" HookTrustStatus value.
	HookTrustStatusModified HookTrustStatus = "modified"
)

// HooksListEntry is generated from the HooksListEntry schema definition.
type HooksListEntry struct {
	Cwd      string          `json:"cwd"`
	Errors   []HookErrorInfo `json:"errors"`
	Hooks    []HookMetadata  `json:"hooks"`
	Warnings []string        `json:"warnings"`
}

// HooksListParams is generated from the HooksListParams schema definition.
type HooksListParams struct {
	// Cwds when empty, defaults to the current session working directory.
	Cwds []string `json:"cwds,omitzero"`
}

// HooksListResponse is generated from the HooksListResponse schema definition.
type HooksListResponse struct {
	Data []HooksListEntry `json:"data"`
}

// ImageDetail is generated from the ImageDetail schema definition.
type ImageDetail string

const (
	// ImageDetailAuto is the "auto" ImageDetail value.
	ImageDetailAuto ImageDetail = "auto"
	// ImageDetailLow is the "low" ImageDetail value.
	ImageDetailLow ImageDetail = "low"
	// ImageDetailHigh is the "high" ImageDetail value.
	ImageDetailHigh ImageDetail = "high"
	// ImageDetailOriginal is the "original" ImageDetail value.
	ImageDetailOriginal ImageDetail = "original"
)

// InitializeCapabilities client-declared capabilities negotiated during initialize.
type InitializeCapabilities struct {
	// ExperimentalAPI opt into receiving experimental API methods and fields.
	ExperimentalAPI bool `json:"experimentalApi,omitzero"`

	// MCPServerOpenaiFormElicitation allows downstream MCP servers to request OpenAI extended form elicitations.
	MCPServerOpenaiFormElicitation bool `json:"mcpServerOpenaiFormElicitation,omitzero"`

	// OptOutNotificationMethods exact notification method names that should be suppressed for this connection (for example `thread/started`).
	OptOutNotificationMethods []string `json:"optOutNotificationMethods,omitzero"`

	// RequestAttestation opt into `attestation/generate` requests for upstream `x-oai-attestation`.
	RequestAttestation bool `json:"requestAttestation,omitzero"`
}

// InitializeParams is generated from the InitializeParams schema definition.
type InitializeParams struct {
	Capabilities InitializeCapabilities `json:"capabilities,omitzero"`
	ClientInfo   ClientInfo             `json:"clientInfo"`
}

// InputModality canonical user-input modality tags advertised by a model.
type InputModality interface {
	isInputModality()
}

// RawInputModality preserves an uninterpreted InputModality JSON value.
type RawInputModality jsontext.Value

func (RawInputModality) isInputModality() {}

var (
	_ json.MarshalerTo     = RawInputModality{}
	_ json.UnmarshalerFrom = (*RawInputModality)(nil)
)

func (value RawInputModality) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawInputModality) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// InputModalityValue is a string-valued InputModality variant.
type InputModalityValue string

func (InputModalityValue) isInputModality() {}

const (
	// InputModalityValueAudio is the "audio" InputModalityValue value.
	InputModalityValueAudio InputModalityValue = "audio"
	// InputModalityValueImage is the "image" InputModalityValue value.
	InputModalityValueImage InputModalityValue = "image"
	// InputModalityValueText is the "text" InputModalityValue value.
	InputModalityValueText InputModalityValue = "text"
)

func decodeGeneratedInputModality(raw jsontext.Value) (InputModality, error) {
	if raw == nil {
		return nil, nil
	}
	var text string
	if err := json.Unmarshal(raw, &text); err == nil {
		switch text {
		case "audio":
			return InputModalityValue(text), nil
		case "image":
			return InputModalityValue(text), nil
		case "text":
			return InputModalityValue(text), nil
		}
		return RawInputModality(raw), nil
	}
	return RawInputModality(raw), nil
}

// InstalledApp installed connector runtime state.
type InstalledApp struct {
	// Callable reports whether the connector is enabled and has a non-synthetic, model-visible tool allowed by effective MCP and app/tool policy in the committed runtime snapshot.
	Callable bool `json:"callable"`

	// Enabled effective enabled state after applying global, workspace, local, and managed configuration at read time.
	Enabled bool   `json:"enabled"`
	ID      string `json:"id"`

	// RuntimeName best-effort name carried by the runtime tool catalog. Canonical app metadata remains owned by `app/read`.
	RuntimeName string `json:"runtimeName,omitzero"`
}

// InternalChatMessageMetadataPassthrough internal Responses API passthrough metadata copied into underlying chat messages.
//
// Responses API strongly types this payload. Do not modify it without first getting API approval and making the corresponding Responses API change.
type InternalChatMessageMetadataPassthrough struct {
	TurnID string `json:"turn_id,omitzero"`
}

// ItemCompletedNotification is generated from the ItemCompletedNotification schema definition.
type ItemCompletedNotification struct {
	// CompletedAtMs unix timestamp (in milliseconds) when this item lifecycle completed.
	CompletedAtMs int64      `json:"completedAtMs"`
	Item          ThreadItem `json:"item"`
	ThreadID      string     `json:"threadId"`
	TurnID        string     `json:"turnId"`
}

func (value *ItemCompletedNotification) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		CompletedAtMs int64          `json:"completedAtMs"`
		Item          jsontext.Value `json:"item"`
		ThreadID      string         `json:"threadId"`
		TurnID        string         `json:"turnId"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.CompletedAtMs = raw.CompletedAtMs
	if raw.Item == nil {
		value.Item = nil
	} else {
		decodedItem, err := decodeGeneratedThreadItem(raw.Item)
		if err != nil {
			return err
		}
		value.Item = decodedItem
	}
	value.ThreadID = raw.ThreadID
	value.TurnID = raw.TurnID
	return nil
}

// ItemGuardianApprovalReviewCompletedNotification [UNSTABLE] Temporary notification payload for approval auto-review. This shape is expected to change soon.
type ItemGuardianApprovalReviewCompletedNotification struct {
	Action GuardianApprovalReviewAction `json:"action"`

	// CompletedAtMs unix timestamp (in milliseconds) when this review completed.
	CompletedAtMs  int64                    `json:"completedAtMs"`
	DecisionSource AutoReviewDecisionSource `json:"decisionSource"`
	Review         GuardianApprovalReview   `json:"review"`

	// ReviewID stable identifier for this review.
	ReviewID string `json:"reviewId"`

	// StartedAtMs unix timestamp (in milliseconds) when this review started.
	StartedAtMs int64 `json:"startedAtMs"`

	// TargetItemID identifier for the reviewed item or tool call when one exists.
	//
	// In most cases, one review maps to one target item. The exceptions are - execve reviews, where a single command may contain multiple execve calls to review (only possible when using the shell_zsh_fork feature) - network policy reviews, where there is no target item
	//
	// A network call is triggered by a CommandExecution item, so having a target_item_id set to the CommandExecution item would be misleading because the review is about the network call, not the command execution. Therefore, target_item_id is set to None for network policy reviews.
	TargetItemID string `json:"targetItemId,omitzero"`
	ThreadID     string `json:"threadId"`
	TurnID       string `json:"turnId"`
}

func (value *ItemGuardianApprovalReviewCompletedNotification) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Action         jsontext.Value           `json:"action"`
		CompletedAtMs  int64                    `json:"completedAtMs"`
		DecisionSource AutoReviewDecisionSource `json:"decisionSource"`
		Review         GuardianApprovalReview   `json:"review"`
		ReviewID       string                   `json:"reviewId"`
		StartedAtMs    int64                    `json:"startedAtMs"`
		TargetItemID   string                   `json:"targetItemId,omitzero"`
		ThreadID       string                   `json:"threadId"`
		TurnID         string                   `json:"turnId"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	if raw.Action == nil {
		value.Action = nil
	} else {
		decodedAction, err := decodeGeneratedGuardianApprovalReviewAction(raw.Action)
		if err != nil {
			return err
		}
		value.Action = decodedAction
	}
	value.CompletedAtMs = raw.CompletedAtMs
	value.DecisionSource = raw.DecisionSource
	value.Review = raw.Review
	value.ReviewID = raw.ReviewID
	value.StartedAtMs = raw.StartedAtMs
	value.TargetItemID = raw.TargetItemID
	value.ThreadID = raw.ThreadID
	value.TurnID = raw.TurnID
	return nil
}

// ItemGuardianApprovalReviewStartedNotification [UNSTABLE] Temporary notification payload for approval auto-review. This shape is expected to change soon.
type ItemGuardianApprovalReviewStartedNotification struct {
	Action GuardianApprovalReviewAction `json:"action"`
	Review GuardianApprovalReview       `json:"review"`

	// ReviewID stable identifier for this review.
	ReviewID string `json:"reviewId"`

	// StartedAtMs unix timestamp (in milliseconds) when this review started.
	StartedAtMs int64 `json:"startedAtMs"`

	// TargetItemID identifier for the reviewed item or tool call when one exists.
	//
	// In most cases, one review maps to one target item. The exceptions are - execve reviews, where a single command may contain multiple execve calls to review (only possible when using the shell_zsh_fork feature) - network policy reviews, where there is no target item
	//
	// A network call is triggered by a CommandExecution item, so having a target_item_id set to the CommandExecution item would be misleading because the review is about the network call, not the command execution. Therefore, target_item_id is set to None for network policy reviews.
	TargetItemID string `json:"targetItemId,omitzero"`
	ThreadID     string `json:"threadId"`
	TurnID       string `json:"turnId"`
}

func (value *ItemGuardianApprovalReviewStartedNotification) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Action       jsontext.Value         `json:"action"`
		Review       GuardianApprovalReview `json:"review"`
		ReviewID     string                 `json:"reviewId"`
		StartedAtMs  int64                  `json:"startedAtMs"`
		TargetItemID string                 `json:"targetItemId,omitzero"`
		ThreadID     string                 `json:"threadId"`
		TurnID       string                 `json:"turnId"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	if raw.Action == nil {
		value.Action = nil
	} else {
		decodedAction, err := decodeGeneratedGuardianApprovalReviewAction(raw.Action)
		if err != nil {
			return err
		}
		value.Action = decodedAction
	}
	value.Review = raw.Review
	value.ReviewID = raw.ReviewID
	value.StartedAtMs = raw.StartedAtMs
	value.TargetItemID = raw.TargetItemID
	value.ThreadID = raw.ThreadID
	value.TurnID = raw.TurnID
	return nil
}

// ItemStartedNotification is generated from the ItemStartedNotification schema definition.
type ItemStartedNotification struct {
	Item ThreadItem `json:"item"`

	// StartedAtMs unix timestamp (in milliseconds) when this item lifecycle started.
	StartedAtMs int64  `json:"startedAtMs"`
	ThreadID    string `json:"threadId"`
	TurnID      string `json:"turnId"`
}

func (value *ItemStartedNotification) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Item        jsontext.Value `json:"item"`
		StartedAtMs int64          `json:"startedAtMs"`
		ThreadID    string         `json:"threadId"`
		TurnID      string         `json:"turnId"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	if raw.Item == nil {
		value.Item = nil
	} else {
		decodedItem, err := decodeGeneratedThreadItem(raw.Item)
		if err != nil {
			return err
		}
		value.Item = decodedItem
	}
	value.StartedAtMs = raw.StartedAtMs
	value.ThreadID = raw.ThreadID
	value.TurnID = raw.TurnID
	return nil
}

// LegacyAppPathString is generated from the LegacyAppPathString schema definition.
type LegacyAppPathString = string

// ListMCPServerStatusParams is generated from the ListMcpServerStatusParams schema definition.
type ListMCPServerStatusParams struct {
	// Cursor opaque pagination cursor returned by a previous call.
	Cursor string `json:"cursor,omitzero"`

	// Detail controls how much MCP inventory data to fetch for each server. Defaults to `Full` when omitted.
	Detail MCPServerStatusDetail `json:"detail,omitzero"`

	// Limit optional page size; defaults to a server-defined value.
	Limit    int32  `json:"limit,omitzero"`
	ThreadID string `json:"threadId,omitzero"`
}

// ListMCPServerStatusResponse is generated from the ListMcpServerStatusResponse schema definition.
type ListMCPServerStatusResponse struct {
	Data []MCPServerStatus `json:"data"`

	// NextCursor opaque cursor to pass to the next call to continue after the last item. If None, there are no more items to return.
	NextCursor string `json:"nextCursor,omitzero"`
}

// ExecLocalShellAction is generated from the ExecLocalShellAction schema definition.
type ExecLocalShellAction struct {
	Command          []string          `json:"command"`
	Env              map[string]string `json:"env,omitzero"`
	TimeoutMs        uint64            `json:"timeout_ms,omitzero"`
	Type             string            `json:"type"`
	User             string            `json:"user,omitzero"`
	WorkingDirectory string            `json:"working_directory,omitzero"`
}

// LocalShellAction is generated from the LocalShellAction schema definition.
type LocalShellAction = ExecLocalShellAction

func (ExecLocalShellAction) isLocalShellAction() {}

// LocalShellStatus is generated from the LocalShellStatus schema definition.
type LocalShellStatus string

const (
	// LocalShellStatusCompleted is the "completed" LocalShellStatus value.
	LocalShellStatusCompleted LocalShellStatus = "completed"
	// LocalShellStatusInProgress is the "in_progress" LocalShellStatus value.
	LocalShellStatusInProgress LocalShellStatus = "in_progress"
	// LocalShellStatusIncomplete is the "incomplete" LocalShellStatus value.
	LocalShellStatusIncomplete LocalShellStatus = "incomplete"
)

// LoginAccountParams is generated from the LoginAccountParams schema definition.
type LoginAccountParams interface {
	isLoginAccountParams()
}

// RawLoginAccountParams preserves an uninterpreted LoginAccountParams JSON value.
type RawLoginAccountParams jsontext.Value

func (RawLoginAccountParams) isLoginAccountParams() {}

var (
	_ json.MarshalerTo     = RawLoginAccountParams{}
	_ json.UnmarshalerFrom = (*RawLoginAccountParams)(nil)
)

func (value RawLoginAccountParams) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawLoginAccountParams) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// APIKeyv2LoginAccountParams is generated from the ApiKeyv2::LoginAccountParams schema definition.
type APIKeyv2LoginAccountParams struct {
	APIKey string `json:"apiKey"`
	Type   string `json:"type"`
}

func (APIKeyv2LoginAccountParams) isLoginAccountParams() {}

// ChatGPTv2LoginAccountParams is generated from the Chatgptv2::LoginAccountParams schema definition.
type ChatGPTv2LoginAccountParams struct {
	AppBrand                  LoginAppBrand `json:"appBrand,omitzero"`
	CodexStreamlinedLogin     bool          `json:"codexStreamlinedLogin,omitzero"`
	Type                      string        `json:"type"`
	UseHostedLoginSuccessPage bool          `json:"useHostedLoginSuccessPage,omitzero"`
}

func (ChatGPTv2LoginAccountParams) isLoginAccountParams() {}

// ChatGPTDeviceCodev2LoginAccountParams is generated from the ChatgptDeviceCodev2::LoginAccountParams schema definition.
type ChatGPTDeviceCodev2LoginAccountParams struct {
	Type string `json:"type"`
}

func (ChatGPTDeviceCodev2LoginAccountParams) isLoginAccountParams() {}

// ChatGPTAuthTokensv2LoginAccountParams [UNSTABLE] FOR OPENAI INTERNAL USE ONLY - DO NOT USE. The access token must contain the same scopes that Codex-managed ChatGPT auth tokens have.
type ChatGPTAuthTokensv2LoginAccountParams struct {
	// AccessToken access token (JWT) supplied by the client. This token is used for backend API requests and email extraction.
	AccessToken string `json:"accessToken"`

	// ChatGPTAccountID workspace/account identifier supplied by the client.
	ChatGPTAccountID string `json:"chatgptAccountId"`

	// ChatGPTPlanType optional plan type supplied by the client.
	//
	// When `null`, Codex attempts to derive the plan type from access-token claims. If unavailable, the plan defaults to `unknown`.
	ChatGPTPlanType string `json:"chatgptPlanType,omitzero"`
	Type            string `json:"type"`
}

func (ChatGPTAuthTokensv2LoginAccountParams) isLoginAccountParams() {}

// AmazonBedrockv2LoginAccountParams [UNSTABLE] Managed Amazon Bedrock login is experimental.
type AmazonBedrockv2LoginAccountParams struct {
	APIKey string `json:"apiKey"`
	Region string `json:"region"`
	Type   string `json:"type"`
}

func (AmazonBedrockv2LoginAccountParams) isLoginAccountParams() {}

func decodeGeneratedLoginAccountParams(raw jsontext.Value) (LoginAccountParams, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "apiKey":
			var value APIKeyv2LoginAccountParams
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "chatgpt":
			var value ChatGPTv2LoginAccountParams
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "chatgptDeviceCode":
			var value ChatGPTDeviceCodev2LoginAccountParams
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "chatgptAuthTokens":
			var value ChatGPTAuthTokensv2LoginAccountParams
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "amazonBedrock":
			var value AmazonBedrockv2LoginAccountParams
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawLoginAccountParams(raw), nil
	}
	return RawLoginAccountParams(raw), nil
}

// LoginAccountResponse is generated from the LoginAccountResponse schema definition.
type LoginAccountResponse interface {
	isLoginAccountResponse()
}

// RawLoginAccountResponse preserves an uninterpreted LoginAccountResponse JSON value.
type RawLoginAccountResponse jsontext.Value

func (RawLoginAccountResponse) isLoginAccountResponse() {}

var (
	_ json.MarshalerTo     = RawLoginAccountResponse{}
	_ json.UnmarshalerFrom = (*RawLoginAccountResponse)(nil)
)

func (value RawLoginAccountResponse) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawLoginAccountResponse) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// APIKeyv2LoginAccountResponse is generated from the ApiKeyv2::LoginAccountResponse schema definition.
type APIKeyv2LoginAccountResponse struct {
	Type string `json:"type"`
}

func (APIKeyv2LoginAccountResponse) isLoginAccountResponse() {}

// ChatGPTv2LoginAccountResponse is generated from the Chatgptv2::LoginAccountResponse schema definition.
type ChatGPTv2LoginAccountResponse struct {
	// AuthURL URL the client should open in a browser to initiate the OAuth flow.
	AuthURL string `json:"authUrl"`
	LoginID string `json:"loginId"`
	Type    string `json:"type"`
}

func (ChatGPTv2LoginAccountResponse) isLoginAccountResponse() {}

// ChatGPTDeviceCodev2LoginAccountResponse is generated from the ChatgptDeviceCodev2::LoginAccountResponse schema definition.
type ChatGPTDeviceCodev2LoginAccountResponse struct {
	LoginID string `json:"loginId"`
	Type    string `json:"type"`

	// UserCode one-time code the user must enter after signing in.
	UserCode string `json:"userCode"`

	// VerificationURL URL the client should open in a browser to complete device code authorization.
	VerificationURL string `json:"verificationUrl"`
}

func (ChatGPTDeviceCodev2LoginAccountResponse) isLoginAccountResponse() {}

// ChatGPTAuthTokensv2LoginAccountResponse is generated from the ChatgptAuthTokensv2::LoginAccountResponse schema definition.
type ChatGPTAuthTokensv2LoginAccountResponse struct {
	Type string `json:"type"`
}

func (ChatGPTAuthTokensv2LoginAccountResponse) isLoginAccountResponse() {}

// AmazonBedrockv2LoginAccountResponse is generated from the AmazonBedrockv2::LoginAccountResponse schema definition.
type AmazonBedrockv2LoginAccountResponse struct {
	Type string `json:"type"`
}

func (AmazonBedrockv2LoginAccountResponse) isLoginAccountResponse() {}

func decodeGeneratedLoginAccountResponse(raw jsontext.Value) (LoginAccountResponse, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "apiKey":
			var value APIKeyv2LoginAccountResponse
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "chatgpt":
			var value ChatGPTv2LoginAccountResponse
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "chatgptDeviceCode":
			var value ChatGPTDeviceCodev2LoginAccountResponse
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "chatgptAuthTokens":
			var value ChatGPTAuthTokensv2LoginAccountResponse
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "amazonBedrock":
			var value AmazonBedrockv2LoginAccountResponse
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawLoginAccountResponse(raw), nil
	}
	return RawLoginAccountResponse(raw), nil
}

// LoginAppBrand is generated from the LoginAppBrand schema definition.
type LoginAppBrand string

const (
	// LoginAppBrandCodex is the "codex" LoginAppBrand value.
	LoginAppBrandCodex LoginAppBrand = "codex"
	// LoginAppBrandChatGPT is the "chatgpt" LoginAppBrand value.
	LoginAppBrandChatGPT LoginAppBrand = "chatgpt"
)

// LogoutAccountResponse is generated from the LogoutAccountResponse schema definition.
type LogoutAccountResponse struct{}

// ManagedHooksRequirements is generated from the ManagedHooksRequirements schema definition.
type ManagedHooksRequirements struct {
	PermissionRequest []ConfiguredHookMatcherGroup `json:"PermissionRequest"`
	PostCompact       []ConfiguredHookMatcherGroup `json:"PostCompact"`
	PostToolUse       []ConfiguredHookMatcherGroup `json:"PostToolUse"`
	PreCompact        []ConfiguredHookMatcherGroup `json:"PreCompact"`
	PreToolUse        []ConfiguredHookMatcherGroup `json:"PreToolUse"`
	SessionEnd        []ConfiguredHookMatcherGroup `json:"SessionEnd,omitzero"`
	SessionStart      []ConfiguredHookMatcherGroup `json:"SessionStart"`
	Stop              []ConfiguredHookMatcherGroup `json:"Stop"`
	SubagentStart     []ConfiguredHookMatcherGroup `json:"SubagentStart"`
	SubagentStop      []ConfiguredHookMatcherGroup `json:"SubagentStop"`
	UserPromptSubmit  []ConfiguredHookMatcherGroup `json:"UserPromptSubmit"`
	ManagedDir        string                       `json:"managedDir,omitzero"`
	WindowsManagedDir string                       `json:"windowsManagedDir,omitzero"`
}

// MarketplaceAddParams is generated from the MarketplaceAddParams schema definition.
type MarketplaceAddParams struct {
	RefName     string   `json:"refName,omitzero"`
	Source      string   `json:"source"`
	SparsePaths []string `json:"sparsePaths,omitzero"`
}

// MarketplaceAddResponse is generated from the MarketplaceAddResponse schema definition.
type MarketplaceAddResponse struct {
	AlreadyAdded    bool   `json:"alreadyAdded"`
	InstalledRoot   string `json:"installedRoot"`
	MarketplaceName string `json:"marketplaceName"`
}

// MarketplaceInterface is generated from the MarketplaceInterface schema definition.
type MarketplaceInterface struct {
	DisplayName string `json:"displayName,omitzero"`
}

// MarketplaceLoadErrorInfo is generated from the MarketplaceLoadErrorInfo schema definition.
type MarketplaceLoadErrorInfo struct {
	MarketplacePath string `json:"marketplacePath"`
	Message         string `json:"message"`
}

// MarketplaceRemoveParams is generated from the MarketplaceRemoveParams schema definition.
type MarketplaceRemoveParams struct {
	MarketplaceName string `json:"marketplaceName"`
}

// MarketplaceRemoveResponse is generated from the MarketplaceRemoveResponse schema definition.
type MarketplaceRemoveResponse struct {
	InstalledRoot   string `json:"installedRoot,omitzero"`
	MarketplaceName string `json:"marketplaceName"`
}

// MarketplaceUpgradeErrorInfo is generated from the MarketplaceUpgradeErrorInfo schema definition.
type MarketplaceUpgradeErrorInfo struct {
	MarketplaceName string `json:"marketplaceName"`
	Message         string `json:"message"`
}

// MarketplaceUpgradeParams is generated from the MarketplaceUpgradeParams schema definition.
type MarketplaceUpgradeParams struct {
	MarketplaceName string `json:"marketplaceName,omitzero"`
}

// MarketplaceUpgradeResponse is generated from the MarketplaceUpgradeResponse schema definition.
type MarketplaceUpgradeResponse struct {
	Errors               []MarketplaceUpgradeErrorInfo `json:"errors"`
	SelectedMarketplaces []string                      `json:"selectedMarketplaces"`
	UpgradedRoots        []string                      `json:"upgradedRoots"`
}

// MCPAuthStatus is generated from the McpAuthStatus schema definition.
type MCPAuthStatus string

const (
	// MCPAuthStatusUnknown is the "unknown" MCPAuthStatus value.
	MCPAuthStatusUnknown MCPAuthStatus = "unknown"
	// MCPAuthStatusUnsupported is the "unsupported" MCPAuthStatus value.
	MCPAuthStatusUnsupported MCPAuthStatus = "unsupported"
	// MCPAuthStatusNotLoggedIn is the "notLoggedIn" MCPAuthStatus value.
	MCPAuthStatusNotLoggedIn MCPAuthStatus = "notLoggedIn"
	// MCPAuthStatusBearerToken is the "bearerToken" MCPAuthStatus value.
	MCPAuthStatusBearerToken MCPAuthStatus = "bearerToken"
	// MCPAuthStatusOAuth is the "oAuth" MCPAuthStatus value.
	MCPAuthStatusOAuth MCPAuthStatus = "oAuth"
)

// MCPResourceReadParams is generated from the McpResourceReadParams schema definition.
type MCPResourceReadParams struct {
	Server   string `json:"server"`
	ThreadID string `json:"threadId,omitzero"`
	URI      string `json:"uri"`
}

// MCPResourceReadResponse is generated from the McpResourceReadResponse schema definition.
type MCPResourceReadResponse struct {
	Contents []ResourceContent `json:"contents"`
}

func (value *MCPResourceReadResponse) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Contents []jsontext.Value `json:"contents"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	if raw.Contents == nil {
		value.Contents = nil
	} else {
		value.Contents = make([]ResourceContent, len(raw.Contents))
		for i, item := range raw.Contents {
			if item != nil {
				decodedContents, err := decodeGeneratedResourceContent(item)
				if err != nil {
					return err
				}
				value.Contents[i] = decodedContents
			}
		}
	}
	return nil
}

// MCPServerInfo presentation metadata advertised by an initialized MCP server.
type MCPServerInfo struct {
	Description string           `json:"description,omitzero"`
	Icons       []jsontext.Value `json:"icons,omitzero"`
	Name        string           `json:"name"`
	Title       string           `json:"title,omitzero"`
	Version     string           `json:"version"`
	WebsiteURL  string           `json:"websiteUrl,omitzero"`
}

// MCPServerMigration is generated from the McpServerMigration schema definition.
type MCPServerMigration struct {
	Name string `json:"name"`
}

// MCPServerOAuthLoginCompletedNotification is generated from the McpServerOauthLoginCompletedNotification schema definition.
type MCPServerOAuthLoginCompletedNotification struct {
	Error    string `json:"error,omitzero"`
	Name     string `json:"name"`
	Success  bool   `json:"success"`
	ThreadID string `json:"threadId,omitzero"`
}

// MCPServerOAuthLoginParams is generated from the McpServerOauthLoginParams schema definition.
type MCPServerOAuthLoginParams struct {
	Name        string   `json:"name"`
	Scopes      []string `json:"scopes,omitzero"`
	ThreadID    string   `json:"threadId,omitzero"`
	TimeoutSecs int64    `json:"timeoutSecs,omitzero"`
}

// MCPServerOAuthLoginResponse is generated from the McpServerOauthLoginResponse schema definition.
type MCPServerOAuthLoginResponse struct {
	AuthorizationURL string `json:"authorizationUrl"`
}

// MCPServerRefreshResponse is generated from the McpServerRefreshResponse schema definition.
type MCPServerRefreshResponse struct{}

// MCPServerStartupFailureReason is generated from the McpServerStartupFailureReason schema definition.
type MCPServerStartupFailureReason string

const (
	// MCPServerStartupFailureReasonReauthenticationRequired is the "reauthenticationRequired" MCPServerStartupFailureReason value.
	MCPServerStartupFailureReasonReauthenticationRequired MCPServerStartupFailureReason = "reauthenticationRequired"
)

// MCPServerStartupState is generated from the McpServerStartupState schema definition.
type MCPServerStartupState string

const (
	// MCPServerStartupStateStarting is the "starting" MCPServerStartupState value.
	MCPServerStartupStateStarting MCPServerStartupState = "starting"
	// MCPServerStartupStateReady is the "ready" MCPServerStartupState value.
	MCPServerStartupStateReady MCPServerStartupState = "ready"
	// MCPServerStartupStateFailed is the "failed" MCPServerStartupState value.
	MCPServerStartupStateFailed MCPServerStartupState = "failed"
	// MCPServerStartupStateCancelled is the "cancelled" MCPServerStartupState value.
	MCPServerStartupStateCancelled MCPServerStartupState = "cancelled"
)

// MCPServerStatus is generated from the McpServerStatus schema definition.
type MCPServerStatus struct {
	AuthStatus        MCPAuthStatus      `json:"authStatus"`
	Name              string             `json:"name"`
	ResourceTemplates []ResourceTemplate `json:"resourceTemplates"`
	Resources         []Resource         `json:"resources"`
	ServerInfo        MCPServerInfo      `json:"serverInfo,omitzero"`
	Tools             map[string]Tool    `json:"tools"`
}

// MCPServerStatusDetail is generated from the McpServerStatusDetail schema definition.
type MCPServerStatusDetail string

const (
	// MCPServerStatusDetailFull is the "full" MCPServerStatusDetail value.
	MCPServerStatusDetailFull MCPServerStatusDetail = "full"
	// MCPServerStatusDetailToolsAndAuthOnly is the "toolsAndAuthOnly" MCPServerStatusDetail value.
	MCPServerStatusDetailToolsAndAuthOnly MCPServerStatusDetail = "toolsAndAuthOnly"
)

// MCPServerStatusUpdatedNotification is generated from the McpServerStatusUpdatedNotification schema definition.
type MCPServerStatusUpdatedNotification struct {
	Error         string                        `json:"error,omitzero"`
	FailureReason MCPServerStartupFailureReason `json:"failureReason,omitzero"`
	Name          string                        `json:"name"`
	Status        MCPServerStartupState         `json:"status"`
	ThreadID      string                        `json:"threadId,omitzero"`
}

// MCPServerToolCallParams is generated from the McpServerToolCallParams schema definition.
type MCPServerToolCallParams struct {
	Meta      jsontext.Value `json:"_meta,omitzero"`
	Arguments jsontext.Value `json:"arguments,omitzero"`
	Server    string         `json:"server"`
	ThreadID  string         `json:"threadId"`
	Tool      string         `json:"tool"`
}

// MCPServerToolCallResponse is generated from the McpServerToolCallResponse schema definition.
type MCPServerToolCallResponse struct {
	Meta              jsontext.Value   `json:"_meta,omitzero"`
	Content           []jsontext.Value `json:"content"`
	IsError           bool             `json:"isError,omitzero"`
	StructuredContent jsontext.Value   `json:"structuredContent,omitzero"`
}

// MCPToolCallAppContext is generated from the McpToolCallAppContext schema definition.
type MCPToolCallAppContext struct {
	ActionName  string `json:"actionName,omitzero"`
	AppName     string `json:"appName,omitzero"`
	ConnectorID string `json:"connectorId"`
	LinkID      string `json:"linkId,omitzero"`
	ResourceURI string `json:"resourceUri,omitzero"`
}

// MCPToolCallError is generated from the McpToolCallError schema definition.
type MCPToolCallError struct {
	Message string `json:"message"`
}

// MCPToolCallProgressNotification is generated from the McpToolCallProgressNotification schema definition.
type MCPToolCallProgressNotification struct {
	ItemID   string `json:"itemId"`
	Message  string `json:"message"`
	ThreadID string `json:"threadId"`
	TurnID   string `json:"turnId"`
}

// MCPToolCallResult is generated from the McpToolCallResult schema definition.
type MCPToolCallResult struct {
	Meta              jsontext.Value   `json:"_meta,omitzero"`
	Content           []jsontext.Value `json:"content"`
	StructuredContent jsontext.Value   `json:"structuredContent,omitzero"`
}

// MCPToolCallStatus is generated from the McpToolCallStatus schema definition.
type MCPToolCallStatus string

const (
	// MCPToolCallStatusInProgress is the "inProgress" MCPToolCallStatus value.
	MCPToolCallStatusInProgress MCPToolCallStatus = "inProgress"
	// MCPToolCallStatusCompleted is the "completed" MCPToolCallStatus value.
	MCPToolCallStatusCompleted MCPToolCallStatus = "completed"
	// MCPToolCallStatusFailed is the "failed" MCPToolCallStatus value.
	MCPToolCallStatusFailed MCPToolCallStatus = "failed"
)

// MemoryCitation is generated from the MemoryCitation schema definition.
type MemoryCitation struct {
	Entries   []MemoryCitationEntry `json:"entries"`
	ThreadIDs []string              `json:"threadIds"`
}

// MemoryCitationEntry is generated from the MemoryCitationEntry schema definition.
type MemoryCitationEntry struct {
	LineEnd   int32  `json:"lineEnd"`
	LineStart int32  `json:"lineStart"`
	Note      string `json:"note"`
	Path      string `json:"path"`
}

// MemoryResetResponse is generated from the MemoryResetResponse schema definition.
type MemoryResetResponse struct{}

// MergeStrategy is generated from the MergeStrategy schema definition.
type MergeStrategy string

const (
	// MergeStrategyReplace is the "replace" MergeStrategy value.
	MergeStrategyReplace MergeStrategy = "replace"
	// MergeStrategyUpsert is the "upsert" MergeStrategy value.
	MergeStrategyUpsert MergeStrategy = "upsert"
)

// MessagePhase classifies an assistant message as interim commentary or final answer text.
//
// Providers do not emit this consistently, so callers must treat `None` as "phase unknown" and keep compatibility behavior for legacy models.
type MessagePhase interface {
	isMessagePhase()
}

// RawMessagePhase preserves an uninterpreted MessagePhase JSON value.
type RawMessagePhase jsontext.Value

func (RawMessagePhase) isMessagePhase() {}

var (
	_ json.MarshalerTo     = RawMessagePhase{}
	_ json.UnmarshalerFrom = (*RawMessagePhase)(nil)
)

func (value RawMessagePhase) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawMessagePhase) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// MessagePhaseValue is a string-valued MessagePhase variant.
type MessagePhaseValue string

func (MessagePhaseValue) isMessagePhase() {}

const (
	// MessagePhaseValueCommentary is the "commentary" MessagePhaseValue value.
	MessagePhaseValueCommentary MessagePhaseValue = "commentary"
	// MessagePhaseValueFinalAnswer is the "final_answer" MessagePhaseValue value.
	MessagePhaseValueFinalAnswer MessagePhaseValue = "final_answer"
)

func decodeGeneratedMessagePhase(raw jsontext.Value) (MessagePhase, error) {
	if raw == nil {
		return nil, nil
	}
	var text string
	if err := json.Unmarshal(raw, &text); err == nil {
		switch text {
		case "commentary":
			return MessagePhaseValue(text), nil
		case "final_answer":
			return MessagePhaseValue(text), nil
		}
		return RawMessagePhase(raw), nil
	}
	return RawMessagePhase(raw), nil
}

// MigrationDetails is generated from the MigrationDetails schema definition.
type MigrationDetails struct {
	Commands   []CommandMigration   `json:"commands,omitzero"`
	Hooks      []HookMigration      `json:"hooks,omitzero"`
	MCPServers []MCPServerMigration `json:"mcpServers,omitzero"`
	Memory     []string             `json:"memory,omitzero"`
	Plugins    []PluginsMigration   `json:"plugins,omitzero"`
	Sessions   []SessionMigration   `json:"sessions,omitzero"`
	Skills     []SkillMigration     `json:"skills,omitzero"`
	Subagents  []SubagentMigration  `json:"subagents,omitzero"`
}

// MockExperimentalMethodParams is generated from the MockExperimentalMethodParams schema definition.
type MockExperimentalMethodParams struct {
	// Value test-only payload field.
	Value string `json:"value,omitzero"`
}

// MockExperimentalMethodResponse is generated from the MockExperimentalMethodResponse schema definition.
type MockExperimentalMethodResponse struct {
	// Echoed echoes the input `value`.
	Echoed string `json:"echoed,omitzero"`
}

// ModeKind initial collaboration mode to use when the TUI starts.
type ModeKind string

const (
	// ModeKindPlan is the "plan" ModeKind value.
	ModeKindPlan ModeKind = "plan"
	// ModeKindDefaultValue is the "default" ModeKind value.
	ModeKindDefaultValue ModeKind = "default"
)

// Model is generated from the Model schema definition.
type Model struct {
	// AdditionalSpeedTiers deprecated: use `serviceTiers` instead.
	AdditionalSpeedTiers   []string             `json:"additionalSpeedTiers,omitzero"`
	AvailabilityNux        ModelAvailabilityNux `json:"availabilityNux,omitzero"`
	DefaultReasoningEffort ReasoningEffort      `json:"defaultReasoningEffort"`

	// DefaultServiceTier catalog default service tier id for this model, when one is configured.
	DefaultServiceTier        string                  `json:"defaultServiceTier,omitzero"`
	Description               string                  `json:"description"`
	DisplayName               string                  `json:"displayName"`
	Hidden                    bool                    `json:"hidden"`
	ID                        string                  `json:"id"`
	InputModalities           []InputModality         `json:"inputModalities,omitzero"`
	IsDefault                 bool                    `json:"isDefault"`
	Model                     string                  `json:"model"`
	ServiceTiers              []ModelServiceTier      `json:"serviceTiers,omitzero"`
	SupportedReasoningEfforts []ReasoningEffortOption `json:"supportedReasoningEfforts"`
	SupportsPersonality       bool                    `json:"supportsPersonality,omitzero"`
	Upgrade                   string                  `json:"upgrade,omitzero"`
	UpgradeInfo               ModelUpgradeInfo        `json:"upgradeInfo,omitzero"`
}

func (value *Model) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		AdditionalSpeedTiers      []string                `json:"additionalSpeedTiers,omitzero"`
		AvailabilityNux           ModelAvailabilityNux    `json:"availabilityNux,omitzero"`
		DefaultReasoningEffort    ReasoningEffort         `json:"defaultReasoningEffort"`
		DefaultServiceTier        string                  `json:"defaultServiceTier,omitzero"`
		Description               string                  `json:"description"`
		DisplayName               string                  `json:"displayName"`
		Hidden                    bool                    `json:"hidden"`
		ID                        string                  `json:"id"`
		InputModalities           []jsontext.Value        `json:"inputModalities,omitzero"`
		IsDefault                 bool                    `json:"isDefault"`
		Model                     string                  `json:"model"`
		ServiceTiers              []ModelServiceTier      `json:"serviceTiers,omitzero"`
		SupportedReasoningEfforts []ReasoningEffortOption `json:"supportedReasoningEfforts"`
		SupportsPersonality       bool                    `json:"supportsPersonality,omitzero"`
		Upgrade                   string                  `json:"upgrade,omitzero"`
		UpgradeInfo               ModelUpgradeInfo        `json:"upgradeInfo,omitzero"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.AdditionalSpeedTiers = raw.AdditionalSpeedTiers
	value.AvailabilityNux = raw.AvailabilityNux
	value.DefaultReasoningEffort = raw.DefaultReasoningEffort
	value.DefaultServiceTier = raw.DefaultServiceTier
	value.Description = raw.Description
	value.DisplayName = raw.DisplayName
	value.Hidden = raw.Hidden
	value.ID = raw.ID
	if raw.InputModalities == nil {
		value.InputModalities = nil
	} else {
		value.InputModalities = make([]InputModality, len(raw.InputModalities))
		for i, item := range raw.InputModalities {
			if item != nil {
				decodedInputModalities, err := decodeGeneratedInputModality(item)
				if err != nil {
					return err
				}
				value.InputModalities[i] = decodedInputModalities
			}
		}
	}
	value.IsDefault = raw.IsDefault
	value.Model = raw.Model
	value.ServiceTiers = raw.ServiceTiers
	value.SupportedReasoningEfforts = raw.SupportedReasoningEfforts
	value.SupportsPersonality = raw.SupportsPersonality
	value.Upgrade = raw.Upgrade
	value.UpgradeInfo = raw.UpgradeInfo
	return nil
}

// ModelAvailabilityNux is generated from the ModelAvailabilityNux schema definition.
type ModelAvailabilityNux struct {
	Message string `json:"message"`
}

// ModelListParams is generated from the ModelListParams schema definition.
type ModelListParams struct {
	// Cursor opaque pagination cursor returned by a previous call.
	Cursor string `json:"cursor,omitzero"`

	// IncludeHidden when true, include models that are hidden from the default picker list.
	IncludeHidden bool `json:"includeHidden,omitzero"`

	// Limit optional page size; defaults to a reasonable server-side value.
	Limit int32 `json:"limit,omitzero"`
}

// ModelListResponse is generated from the ModelListResponse schema definition.
type ModelListResponse struct {
	Data []Model `json:"data"`

	// NextCursor opaque cursor to pass to the next call to continue after the last item. If None, there are no more items to return.
	NextCursor string `json:"nextCursor,omitzero"`
}

// ModelProviderCapabilitiesReadParams is generated from the ModelProviderCapabilitiesReadParams schema definition.
type ModelProviderCapabilitiesReadParams struct{}

// ModelProviderCapabilitiesReadResponse is generated from the ModelProviderCapabilitiesReadResponse schema definition.
type ModelProviderCapabilitiesReadResponse struct {
	ImageGeneration bool `json:"imageGeneration"`
	NamespaceTools  bool `json:"namespaceTools"`
	WebSearch       bool `json:"webSearch"`
}

// ModelRerouteReason is generated from the ModelRerouteReason schema definition.
type ModelRerouteReason string

const (
	// ModelRerouteReasonHighRiskCyberActivity is the "highRiskCyberActivity" ModelRerouteReason value.
	ModelRerouteReasonHighRiskCyberActivity ModelRerouteReason = "highRiskCyberActivity"
)

// ModelReroutedNotification is generated from the ModelReroutedNotification schema definition.
type ModelReroutedNotification struct {
	FromModel string             `json:"fromModel"`
	Reason    ModelRerouteReason `json:"reason"`
	ThreadID  string             `json:"threadId"`
	ToModel   string             `json:"toModel"`
	TurnID    string             `json:"turnId"`
}

// ModelSafetyBufferingUpdatedNotification is generated from the ModelSafetyBufferingUpdatedNotification schema definition.
type ModelSafetyBufferingUpdatedNotification struct {
	FasterModel     string   `json:"fasterModel,omitzero"`
	Model           string   `json:"model"`
	Reasons         []string `json:"reasons"`
	ShowBufferingUI bool     `json:"showBufferingUi"`
	ThreadID        string   `json:"threadId"`
	TurnID          string   `json:"turnId"`
	UseCases        []string `json:"useCases"`
}

// ModelServiceTier is generated from the ModelServiceTier schema definition.
type ModelServiceTier struct {
	Description string `json:"description"`
	ID          string `json:"id"`
	Name        string `json:"name"`
}

// ModelUpgradeInfo is generated from the ModelUpgradeInfo schema definition.
type ModelUpgradeInfo struct {
	MigrationMarkdown string `json:"migrationMarkdown,omitzero"`
	Model             string `json:"model"`
	ModelLink         string `json:"modelLink,omitzero"`
	UpgradeCopy       string `json:"upgradeCopy,omitzero"`
}

// ModelVerification is generated from the ModelVerification schema definition.
type ModelVerification string

const (
	// ModelVerificationTrustedAccessForCyber is the "trustedAccessForCyber" ModelVerification value.
	ModelVerificationTrustedAccessForCyber ModelVerification = "trustedAccessForCyber"
)

// ModelVerificationNotification is generated from the ModelVerificationNotification schema definition.
type ModelVerificationNotification struct {
	ThreadID      string              `json:"threadId"`
	TurnID        string              `json:"turnId"`
	Verifications []ModelVerification `json:"verifications"`
}

// ModelsRequirements is generated from the ModelsRequirements schema definition.
type ModelsRequirements struct {
	NewThread NewThreadModelDefaults `json:"newThread,omitzero"`
}

// MultiAgentMode controls the effective multi-agent delegation instructions for a turn. `custom` means the configured mode hint defines the policy instead of a built-in policy.
type MultiAgentMode interface {
	isMultiAgentMode()
}

// RawMultiAgentMode preserves an uninterpreted MultiAgentMode JSON value.
type RawMultiAgentMode jsontext.Value

func (RawMultiAgentMode) isMultiAgentMode() {}

var (
	_ json.MarshalerTo     = RawMultiAgentMode{}
	_ json.UnmarshalerFrom = (*RawMultiAgentMode)(nil)
)

func (value RawMultiAgentMode) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawMultiAgentMode) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// MultiAgentModeValue is a string-valued MultiAgentMode variant.
type MultiAgentModeValue string

func (MultiAgentModeValue) isMultiAgentMode() {}

const (
	// MultiAgentModeValueExplicitRequestOnly is the "explicitRequestOnly" MultiAgentModeValue value.
	MultiAgentModeValueExplicitRequestOnly MultiAgentModeValue = "explicitRequestOnly"
	// MultiAgentModeValueProactive is the "proactive" MultiAgentModeValue value.
	MultiAgentModeValueProactive MultiAgentModeValue = "proactive"
)

// CustomMultiAgentMode is generated from the CustomMultiAgentMode schema definition.
type CustomMultiAgentMode struct {
	Custom string `json:"custom"`
}

func (CustomMultiAgentMode) isMultiAgentMode() {}

func decodeGeneratedMultiAgentMode(raw jsontext.Value) (MultiAgentMode, error) {
	if raw == nil {
		return nil, nil
	}
	var text string
	if err := json.Unmarshal(raw, &text); err == nil {
		switch text {
		case "explicitRequestOnly":
			return MultiAgentModeValue(text), nil
		case "proactive":
			return MultiAgentModeValue(text), nil
		}
		return RawMultiAgentMode(raw), nil
	}
	var object struct {
		Custom jsontext.Value `json:"custom"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		if object.Custom != nil {
			var value CustomMultiAgentMode
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawMultiAgentMode(raw), nil
	}
	return RawMultiAgentMode(raw), nil
}

// NetworkAccess is generated from the NetworkAccess schema definition.
type NetworkAccess string

const (
	// NetworkAccessRestricted is the "restricted" NetworkAccess value.
	NetworkAccessRestricted NetworkAccess = "restricted"
	// NetworkAccessEnabled is the "enabled" NetworkAccess value.
	NetworkAccessEnabled NetworkAccess = "enabled"
)

// NetworkApprovalProtocol is generated from the NetworkApprovalProtocol schema definition.
type NetworkApprovalProtocol string

const (
	// NetworkApprovalProtocolHTTP is the "http" NetworkApprovalProtocol value.
	NetworkApprovalProtocolHTTP NetworkApprovalProtocol = "http"
	// NetworkApprovalProtocolHTTPS is the "https" NetworkApprovalProtocol value.
	NetworkApprovalProtocolHTTPS NetworkApprovalProtocol = "https"
	// NetworkApprovalProtocolSocks5TCP is the "socks5Tcp" NetworkApprovalProtocol value.
	NetworkApprovalProtocolSocks5TCP NetworkApprovalProtocol = "socks5Tcp"
	// NetworkApprovalProtocolSocks5Udp is the "socks5Udp" NetworkApprovalProtocol value.
	NetworkApprovalProtocolSocks5Udp NetworkApprovalProtocol = "socks5Udp"
)

// NetworkDomainPermission is generated from the NetworkDomainPermission schema definition.
type NetworkDomainPermission string

const (
	// NetworkDomainPermissionAllow is the "allow" NetworkDomainPermission value.
	NetworkDomainPermissionAllow NetworkDomainPermission = "allow"
	// NetworkDomainPermissionDeny is the "deny" NetworkDomainPermission value.
	NetworkDomainPermissionDeny NetworkDomainPermission = "deny"
)

// NetworkRequirements is generated from the NetworkRequirements schema definition.
type NetworkRequirements struct {
	AllowLocalBinding bool `json:"allowLocalBinding,omitzero"`

	// AllowUnixSockets legacy compatibility view derived from `unix_sockets`.
	AllowUnixSockets   []string `json:"allowUnixSockets,omitzero"`
	AllowUpstreamProxy bool     `json:"allowUpstreamProxy,omitzero"`

	// AllowedDomains legacy compatibility view derived from `domains`.
	AllowedDomains                   []string `json:"allowedDomains,omitzero"`
	DangerouslyAllowAllUnixSockets   bool     `json:"dangerouslyAllowAllUnixSockets,omitzero"`
	DangerouslyAllowNonLoopbackProxy bool     `json:"dangerouslyAllowNonLoopbackProxy,omitzero"`

	// DeniedDomains legacy compatibility view derived from `domains`.
	DeniedDomains []string `json:"deniedDomains,omitzero"`

	// Domains canonical network permission map for `experimental_network`.
	Domains  map[string]NetworkDomainPermission `json:"domains,omitzero"`
	Enabled  bool                               `json:"enabled,omitzero"`
	HTTPPort int64                              `json:"httpPort,omitzero"`

	// ManagedAllowedDomainsOnly when true, only managed allowlist entries are respected while managed network enforcement is active.
	ManagedAllowedDomainsOnly bool  `json:"managedAllowedDomainsOnly,omitzero"`
	SocksPort                 int64 `json:"socksPort,omitzero"`

	// UnixSockets canonical unix socket permission map for `experimental_network`.
	UnixSockets map[string]NetworkUnixSocketPermission `json:"unixSockets,omitzero"`
}

// NetworkUnixSocketPermission is generated from the NetworkUnixSocketPermission schema definition.
type NetworkUnixSocketPermission string

const (
	// NetworkUnixSocketPermissionAllow is the "allow" NetworkUnixSocketPermission value.
	NetworkUnixSocketPermissionAllow NetworkUnixSocketPermission = "allow"
	// NetworkUnixSocketPermissionDeny is the "deny" NetworkUnixSocketPermission value.
	NetworkUnixSocketPermissionDeny NetworkUnixSocketPermission = "deny"
)

// NewThreadModelDefaults is generated from the NewThreadModelDefaults schema definition.
type NewThreadModelDefaults struct {
	Model                string           `json:"model,omitzero"`
	ModelReasoningEffort *ReasoningEffort `json:"modelReasoningEffort,omitzero"`
	ServiceTier          string           `json:"serviceTier,omitzero"`
}

// NonSteerableTurnKind is generated from the NonSteerableTurnKind schema definition.
type NonSteerableTurnKind string

const (
	// NonSteerableTurnKindReview is the "review" NonSteerableTurnKind value.
	NonSteerableTurnKindReview NonSteerableTurnKind = "review"
	// NonSteerableTurnKindCompact is the "compact" NonSteerableTurnKind value.
	NonSteerableTurnKindCompact NonSteerableTurnKind = "compact"
)

// NullableRemoteControlDisableParams is generated from the NullableRemoteControlDisableParams schema definition.
type NullableRemoteControlDisableParams = *RemoteControlDisableParams

// NullableRemoteControlEnableParams is generated from the NullableRemoteControlEnableParams schema definition.
type NullableRemoteControlEnableParams = *RemoteControlEnableParams

// OverriddenMetadata is generated from the OverriddenMetadata schema definition.
type OverriddenMetadata struct {
	EffectiveValue  jsontext.Value      `json:"effectiveValue"`
	Message         string              `json:"message"`
	OverridingLayer ConfigLayerMetadata `json:"overridingLayer"`
}

// PatchApplyStatus is generated from the PatchApplyStatus schema definition.
type PatchApplyStatus string

const (
	// PatchApplyStatusInProgress is the "inProgress" PatchApplyStatus value.
	PatchApplyStatusInProgress PatchApplyStatus = "inProgress"
	// PatchApplyStatusCompleted is the "completed" PatchApplyStatus value.
	PatchApplyStatusCompleted PatchApplyStatus = "completed"
	// PatchApplyStatusFailed is the "failed" PatchApplyStatus value.
	PatchApplyStatusFailed PatchApplyStatus = "failed"
	// PatchApplyStatusDeclined is the "declined" PatchApplyStatus value.
	PatchApplyStatusDeclined PatchApplyStatus = "declined"
)

// PatchChangeKind is generated from the PatchChangeKind schema definition.
type PatchChangeKind interface {
	isPatchChangeKind()
}

// RawPatchChangeKind preserves an uninterpreted PatchChangeKind JSON value.
type RawPatchChangeKind jsontext.Value

func (RawPatchChangeKind) isPatchChangeKind() {}

var (
	_ json.MarshalerTo     = RawPatchChangeKind{}
	_ json.UnmarshalerFrom = (*RawPatchChangeKind)(nil)
)

func (value RawPatchChangeKind) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawPatchChangeKind) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// AddPatchChangeKind is generated from the AddPatchChangeKind schema definition.
type AddPatchChangeKind struct {
	Type string `json:"type"`
}

func (AddPatchChangeKind) isPatchChangeKind() {}

// DeletePatchChangeKind is generated from the DeletePatchChangeKind schema definition.
type DeletePatchChangeKind struct {
	Type string `json:"type"`
}

func (DeletePatchChangeKind) isPatchChangeKind() {}

// UpdatePatchChangeKind is generated from the UpdatePatchChangeKind schema definition.
type UpdatePatchChangeKind struct {
	MovePath string `json:"move_path,omitzero"`
	Type     string `json:"type"`
}

func (UpdatePatchChangeKind) isPatchChangeKind() {}

func decodeGeneratedPatchChangeKind(raw jsontext.Value) (PatchChangeKind, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "add":
			var value AddPatchChangeKind
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "delete":
			var value DeletePatchChangeKind
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "update":
			var value UpdatePatchChangeKind
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawPatchChangeKind(raw), nil
	}
	return RawPatchChangeKind(raw), nil
}

// PathURI is generated from the PathUri schema definition.
type PathURI = string

// PermissionProfileListParams is generated from the PermissionProfileListParams schema definition.
type PermissionProfileListParams struct {
	// Cursor opaque pagination cursor returned by a previous call.
	Cursor string `json:"cursor,omitzero"`

	// Cwd optional working directory to resolve project config layers.
	Cwd string `json:"cwd,omitzero"`

	// Limit optional page size; defaults to the full result set.
	Limit int32 `json:"limit,omitzero"`
}

// PermissionProfileListResponse is generated from the PermissionProfileListResponse schema definition.
type PermissionProfileListResponse struct {
	Data []PermissionProfileSummary `json:"data"`

	// NextCursor opaque cursor to pass to the next call to continue after the last item. If None, there are no more items to return.
	NextCursor string `json:"nextCursor,omitzero"`
}

// PermissionProfileSummary is generated from the PermissionProfileSummary schema definition.
type PermissionProfileSummary struct {
	// Allowed reports whether the effective requirements allow selecting this profile.
	Allowed bool `json:"allowed"`

	// Description optional user-facing description for display in clients.
	Description string `json:"description,omitzero"`

	// ID available permission profile identifier.
	ID string `json:"id"`
}

// Personality is generated from the Personality schema definition.
type Personality string

const (
	// PersonalityNone is the "none" Personality value.
	PersonalityNone Personality = "none"
	// PersonalityFriendly is the "friendly" Personality value.
	PersonalityFriendly Personality = "friendly"
	// PersonalityPragmatic is the "pragmatic" Personality value.
	PersonalityPragmatic Personality = "pragmatic"
)

// PlanDeltaNotification proposed plan streaming deltas for plan items. Clients should not assume concatenated deltas match the completed plan item content.
type PlanDeltaNotification struct {
	Delta    string `json:"delta"`
	ItemID   string `json:"itemId"`
	ThreadID string `json:"threadId"`
	TurnID   string `json:"turnId"`
}

// PlanType is generated from the PlanType schema definition.
type PlanType string

const (
	// PlanTypeFree is the "free" PlanType value.
	PlanTypeFree PlanType = "free"
	// PlanTypeGoValue is the "go" PlanType value.
	PlanTypeGoValue PlanType = "go"
	// PlanTypePlus is the "plus" PlanType value.
	PlanTypePlus PlanType = "plus"
	// PlanTypePro is the "pro" PlanType value.
	PlanTypePro PlanType = "pro"
	// PlanTypeProlite is the "prolite" PlanType value.
	PlanTypeProlite PlanType = "prolite"
	// PlanTypeTeam is the "team" PlanType value.
	PlanTypeTeam PlanType = "team"
	// PlanTypeSelfServeBusinessProlite is the "self_serve_business_prolite" PlanType value.
	PlanTypeSelfServeBusinessProlite PlanType = "self_serve_business_prolite"
	// PlanTypeSelfServeBusinessUsageBased is the "self_serve_business_usage_based" PlanType value.
	PlanTypeSelfServeBusinessUsageBased PlanType = "self_serve_business_usage_based"
	// PlanTypeBusiness is the "business" PlanType value.
	PlanTypeBusiness PlanType = "business"
	// PlanTypeEnt26 is the "ent26" PlanType value.
	PlanTypeEnt26 PlanType = "ent26"
	// PlanTypeEnterpriseCbpAutomation is the "enterprise_cbp_automation" PlanType value.
	PlanTypeEnterpriseCbpAutomation PlanType = "enterprise_cbp_automation"
	// PlanTypeEnterpriseCbpUsageBased is the "enterprise_cbp_usage_based" PlanType value.
	PlanTypeEnterpriseCbpUsageBased PlanType = "enterprise_cbp_usage_based"
	// PlanTypeEnterprise is the "enterprise" PlanType value.
	PlanTypeEnterprise PlanType = "enterprise"
	// PlanTypeEdu is the "edu" PlanType value.
	PlanTypeEdu PlanType = "edu"
	// PlanTypeUnknown is the "unknown" PlanType value.
	PlanTypeUnknown PlanType = "unknown"
)

// PluginAuthPolicy is generated from the PluginAuthPolicy schema definition.
type PluginAuthPolicy string

const (
	// PluginAuthPolicyONINSTALL is the "ON_INSTALL" PluginAuthPolicy value.
	PluginAuthPolicyONINSTALL PluginAuthPolicy = "ON_INSTALL"
	// PluginAuthPolicyONUSE is the "ON_USE" PluginAuthPolicy value.
	PluginAuthPolicyONUSE PluginAuthPolicy = "ON_USE"
)

// PluginAvailability is generated from the PluginAvailability schema definition.
type PluginAvailability interface {
	isPluginAvailability()
}

// RawPluginAvailability preserves an uninterpreted PluginAvailability JSON value.
type RawPluginAvailability jsontext.Value

func (RawPluginAvailability) isPluginAvailability() {}

var (
	_ json.MarshalerTo     = RawPluginAvailability{}
	_ json.UnmarshalerFrom = (*RawPluginAvailability)(nil)
)

func (value RawPluginAvailability) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawPluginAvailability) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// PluginAvailabilityValue is a string-valued PluginAvailability variant.
type PluginAvailabilityValue string

func (PluginAvailabilityValue) isPluginAvailability() {}

const (
	// PluginAvailabilityValueAVAILABLE is the "AVAILABLE" PluginAvailabilityValue value.
	PluginAvailabilityValueAVAILABLE PluginAvailabilityValue = "AVAILABLE"
	// PluginAvailabilityValueDISABLEDBYADMIN is the "DISABLED_BY_ADMIN" PluginAvailabilityValue value.
	PluginAvailabilityValueDISABLEDBYADMIN PluginAvailabilityValue = "DISABLED_BY_ADMIN"
)

func decodeGeneratedPluginAvailability(raw jsontext.Value) (PluginAvailability, error) {
	if raw == nil {
		return nil, nil
	}
	var text string
	if err := json.Unmarshal(raw, &text); err == nil {
		switch text {
		case "AVAILABLE":
			return PluginAvailabilityValue(text), nil
		case "DISABLED_BY_ADMIN":
			return PluginAvailabilityValue(text), nil
		}
		return RawPluginAvailability(raw), nil
	}
	return RawPluginAvailability(raw), nil
}

// PluginDetail is generated from the PluginDetail schema definition.
type PluginDetail struct {
	AppTemplates    []AppTemplateSummary   `json:"appTemplates"`
	Apps            []AppSummary           `json:"apps"`
	Description     string                 `json:"description,omitzero"`
	Hooks           []PluginHookSummary    `json:"hooks"`
	MarketplaceName string                 `json:"marketplaceName"`
	MarketplacePath string                 `json:"marketplacePath,omitzero"`
	MCPServers      []string               `json:"mcpServers"`
	ScheduledTasks  []ScheduledTaskSummary `json:"scheduledTasks,omitzero"`
	ShareURL        string                 `json:"shareUrl,omitzero"`
	Skills          []SkillSummary         `json:"skills"`
	Summary         PluginSummary          `json:"summary"`
}

// PluginDisabledReason is generated from the PluginDisabledReason schema definition.
type PluginDisabledReason string

const (
	// PluginDisabledReasonDisabledByAdmin is the "disabled_by_admin" PluginDisabledReason value.
	PluginDisabledReasonDisabledByAdmin PluginDisabledReason = "disabled_by_admin"
	// PluginDisabledReasonPlanNotEligible is the "plan_not_eligible" PluginDisabledReason value.
	PluginDisabledReasonPlanNotEligible PluginDisabledReason = "plan_not_eligible"
	// PluginDisabledReasonRequiredAppUnavailable is the "required_app_unavailable" PluginDisabledReason value.
	PluginDisabledReasonRequiredAppUnavailable PluginDisabledReason = "required_app_unavailable"
	// PluginDisabledReasonUnknown is the "unknown" PluginDisabledReason value.
	PluginDisabledReasonUnknown PluginDisabledReason = "unknown"
)

// PluginHookSummary is generated from the PluginHookSummary schema definition.
type PluginHookSummary struct {
	EventName HookEventName `json:"eventName"`
	Key       string        `json:"key"`
}

// PluginInstallParams is generated from the PluginInstallParams schema definition.
type PluginInstallParams struct {
	MarketplacePath       string `json:"marketplacePath,omitzero"`
	PluginName            string `json:"pluginName"`
	RemoteMarketplaceName string `json:"remoteMarketplaceName,omitzero"`
}

// PluginInstallPolicy is generated from the PluginInstallPolicy schema definition.
type PluginInstallPolicy string

const (
	// PluginInstallPolicyNOTAVAILABLE is the "NOT_AVAILABLE" PluginInstallPolicy value.
	PluginInstallPolicyNOTAVAILABLE PluginInstallPolicy = "NOT_AVAILABLE"
	// PluginInstallPolicyAVAILABLE is the "AVAILABLE" PluginInstallPolicy value.
	PluginInstallPolicyAVAILABLE PluginInstallPolicy = "AVAILABLE"
	// PluginInstallPolicyINSTALLEDBYDEFAULT is the "INSTALLED_BY_DEFAULT" PluginInstallPolicy value.
	PluginInstallPolicyINSTALLEDBYDEFAULT PluginInstallPolicy = "INSTALLED_BY_DEFAULT"
)

// PluginInstallPolicySource is generated from the PluginInstallPolicySource schema definition.
type PluginInstallPolicySource string

const (
	// PluginInstallPolicySourceWORKSPACESETTING is the "WORKSPACE_SETTING" PluginInstallPolicySource value.
	PluginInstallPolicySourceWORKSPACESETTING PluginInstallPolicySource = "WORKSPACE_SETTING"
	// PluginInstallPolicySourceIMPLICITCANONICALAPP is the "IMPLICIT_CANONICAL_APP" PluginInstallPolicySource value.
	PluginInstallPolicySourceIMPLICITCANONICALAPP PluginInstallPolicySource = "IMPLICIT_CANONICAL_APP"
)

// PluginInstallResponse is generated from the PluginInstallResponse schema definition.
type PluginInstallResponse struct {
	AppsNeedingAuth []AppSummary     `json:"appsNeedingAuth"`
	AuthPolicy      PluginAuthPolicy `json:"authPolicy"`
}

// PluginInstalledParams is generated from the PluginInstalledParams schema definition.
type PluginInstalledParams struct {
	// Cwds optional working directories used to discover repo marketplaces.
	Cwds []string `json:"cwds,omitzero"`

	// InstallSuggestionPluginNames additional uninstalled plugin names that should be returned when present locally. This is used by mention surfaces that intentionally expose install entrypoints.
	InstallSuggestionPluginNames []string `json:"installSuggestionPluginNames,omitzero"`
}

// PluginInstalledResponse is generated from the PluginInstalledResponse schema definition.
type PluginInstalledResponse struct {
	MarketplaceLoadErrors []MarketplaceLoadErrorInfo `json:"marketplaceLoadErrors,omitzero"`
	Marketplaces          []PluginMarketplaceEntry   `json:"marketplaces"`
}

// PluginInterface is generated from the PluginInterface schema definition.
type PluginInterface struct {
	BrandColor   string   `json:"brandColor,omitzero"`
	Capabilities []string `json:"capabilities"`
	Category     string   `json:"category,omitzero"`

	// ComposerIcon local composer icon path, resolved from the installed plugin package.
	ComposerIcon string `json:"composerIcon,omitzero"`

	// ComposerIconURL remote composer icon URL from the plugin catalog.
	ComposerIconURL string `json:"composerIconUrl,omitzero"`

	// DefaultPrompt starter prompts for the plugin. Capped at 3 entries with a maximum of 128 characters per entry.
	DefaultPrompt []string `json:"defaultPrompt,omitzero"`
	DeveloperName string   `json:"developerName,omitzero"`
	DisplayName   string   `json:"displayName,omitzero"`

	// Logo local logo path, resolved from the installed plugin package.
	Logo string `json:"logo,omitzero"`

	// LogoDark local dark-mode logo path, resolved from the installed plugin package.
	LogoDark string `json:"logoDark,omitzero"`

	// LogoURL remote logo URL from the plugin catalog.
	LogoURL string `json:"logoUrl,omitzero"`

	// LogoURLDark remote dark-mode logo URL from the plugin catalog.
	LogoURLDark      string `json:"logoUrlDark,omitzero"`
	LongDescription  string `json:"longDescription,omitzero"`
	PrivacyPolicyURL string `json:"privacyPolicyUrl,omitzero"`

	// ScreenshotURLs remote screenshot URLs from the plugin catalog.
	ScreenshotURLs []string `json:"screenshotUrls"`

	// Screenshots local screenshot paths, resolved from the installed plugin package.
	Screenshots       []string `json:"screenshots"`
	ShortDescription  string   `json:"shortDescription,omitzero"`
	TermsOfServiceURL string   `json:"termsOfServiceUrl,omitzero"`
	WebsiteURL        string   `json:"websiteUrl,omitzero"`
}

// PluginListMarketplaceKind is generated from the PluginListMarketplaceKind schema definition.
type PluginListMarketplaceKind string

const (
	// PluginListMarketplaceKindLocal is the "local" PluginListMarketplaceKind value.
	PluginListMarketplaceKindLocal PluginListMarketplaceKind = "local"
	// PluginListMarketplaceKindVertical is the "vertical" PluginListMarketplaceKind value.
	PluginListMarketplaceKindVertical PluginListMarketplaceKind = "vertical"
	// PluginListMarketplaceKindWorkspaceDirectory is the "workspace-directory" PluginListMarketplaceKind value.
	PluginListMarketplaceKindWorkspaceDirectory PluginListMarketplaceKind = "workspace-directory"
	// PluginListMarketplaceKindSharedWithMe is the "shared-with-me" PluginListMarketplaceKind value.
	PluginListMarketplaceKindSharedWithMe PluginListMarketplaceKind = "shared-with-me"
	// PluginListMarketplaceKindCreatedByMeRemote is the "created-by-me-remote" PluginListMarketplaceKind value.
	PluginListMarketplaceKindCreatedByMeRemote PluginListMarketplaceKind = "created-by-me-remote"
)

// PluginListParams is generated from the PluginListParams schema definition.
type PluginListParams struct {
	// Cwds optional working directories used to discover repo marketplaces. When omitted, only home-scoped marketplaces and the official curated marketplace are considered.
	Cwds []string `json:"cwds,omitzero"`

	// ForceRefetch reports whether the client requests a fresh remote plugin catalog fetch.
	ForceRefetch bool `json:"forceRefetch,omitzero"`

	// MarketplaceKinds optional marketplace kind filter. When omitted, only local marketplaces are queried, plus the default remote catalog when enabled by feature flag.
	MarketplaceKinds []PluginListMarketplaceKind `json:"marketplaceKinds,omitzero"`
}

// PluginListResponse is generated from the PluginListResponse schema definition.
type PluginListResponse struct {
	FeaturedPluginIDs     []string                   `json:"featuredPluginIds,omitzero"`
	MarketplaceLoadErrors []MarketplaceLoadErrorInfo `json:"marketplaceLoadErrors,omitzero"`
	Marketplaces          []PluginMarketplaceEntry   `json:"marketplaces"`
}

// PluginMarketplaceEntry is generated from the PluginMarketplaceEntry schema definition.
type PluginMarketplaceEntry struct {
	InterfaceValue MarketplaceInterface `json:"interface,omitzero"`
	Name           string               `json:"name"`

	// Path local marketplace file path when the marketplace is backed by a local file. Remote-only catalog marketplaces do not have a local path.
	Path    string          `json:"path,omitzero"`
	Plugins []PluginSummary `json:"plugins"`
}

// PluginReadParams is generated from the PluginReadParams schema definition.
type PluginReadParams struct {
	MarketplacePath       string `json:"marketplacePath,omitzero"`
	PluginName            string `json:"pluginName"`
	RemoteMarketplaceName string `json:"remoteMarketplaceName,omitzero"`
}

// PluginReadResponse is generated from the PluginReadResponse schema definition.
type PluginReadResponse struct {
	Plugin PluginDetail `json:"plugin"`
}

// PluginShareCheckoutParams is generated from the PluginShareCheckoutParams schema definition.
type PluginShareCheckoutParams struct {
	RemotePluginID string `json:"remotePluginId"`
}

// PluginShareCheckoutResponse is generated from the PluginShareCheckoutResponse schema definition.
type PluginShareCheckoutResponse struct {
	MarketplaceName string `json:"marketplaceName"`
	MarketplacePath string `json:"marketplacePath"`
	PluginID        string `json:"pluginId"`
	PluginName      string `json:"pluginName"`
	PluginPath      string `json:"pluginPath"`
	RemotePluginID  string `json:"remotePluginId"`
	RemoteVersion   string `json:"remoteVersion,omitzero"`
}

// PluginShareContext is generated from the PluginShareContext schema definition.
type PluginShareContext struct {
	CanPublishToWorkspace bool                       `json:"canPublishToWorkspace,omitzero"`
	CreatorAccountUserID  string                     `json:"creatorAccountUserId,omitzero"`
	CreatorName           string                     `json:"creatorName,omitzero"`
	Discoverability       PluginShareDiscoverability `json:"discoverability,omitzero"`
	RemotePluginID        string                     `json:"remotePluginId"`

	// RemoteVersion version of the remote shared plugin release when available.
	RemoteVersion   string                 `json:"remoteVersion,omitzero"`
	SharePrincipals []PluginSharePrincipal `json:"sharePrincipals,omitzero"`
	ShareURL        string                 `json:"shareUrl,omitzero"`
}

// PluginShareDeleteParams is generated from the PluginShareDeleteParams schema definition.
type PluginShareDeleteParams struct {
	RemotePluginID string `json:"remotePluginId"`
}

// PluginShareDeleteResponse is generated from the PluginShareDeleteResponse schema definition.
type PluginShareDeleteResponse struct{}

// PluginShareDiscoverability is generated from the PluginShareDiscoverability schema definition.
type PluginShareDiscoverability string

const (
	// PluginShareDiscoverabilityLISTED is the "LISTED" PluginShareDiscoverability value.
	PluginShareDiscoverabilityLISTED PluginShareDiscoverability = "LISTED"
	// PluginShareDiscoverabilityUNLISTED is the "UNLISTED" PluginShareDiscoverability value.
	PluginShareDiscoverabilityUNLISTED PluginShareDiscoverability = "UNLISTED"
	// PluginShareDiscoverabilityPRIVATE is the "PRIVATE" PluginShareDiscoverability value.
	PluginShareDiscoverabilityPRIVATE PluginShareDiscoverability = "PRIVATE"
)

// PluginShareListItem is generated from the PluginShareListItem schema definition.
type PluginShareListItem struct {
	LocalPluginPath string        `json:"localPluginPath,omitzero"`
	Plugin          PluginSummary `json:"plugin"`
}

// PluginShareListParams is generated from the PluginShareListParams schema definition.
type PluginShareListParams struct{}

// PluginShareListResponse is generated from the PluginShareListResponse schema definition.
type PluginShareListResponse struct {
	Data []PluginShareListItem `json:"data"`
}

// PluginSharePrincipal is generated from the PluginSharePrincipal schema definition.
type PluginSharePrincipal struct {
	Name          string                   `json:"name"`
	PrincipalID   string                   `json:"principalId"`
	PrincipalType PluginSharePrincipalType `json:"principalType"`
	Role          PluginSharePrincipalRole `json:"role"`
}

// PluginSharePrincipalRole is generated from the PluginSharePrincipalRole schema definition.
type PluginSharePrincipalRole string

const (
	// PluginSharePrincipalRoleReader is the "reader" PluginSharePrincipalRole value.
	PluginSharePrincipalRoleReader PluginSharePrincipalRole = "reader"
	// PluginSharePrincipalRoleEditor is the "editor" PluginSharePrincipalRole value.
	PluginSharePrincipalRoleEditor PluginSharePrincipalRole = "editor"
	// PluginSharePrincipalRoleOwner is the "owner" PluginSharePrincipalRole value.
	PluginSharePrincipalRoleOwner PluginSharePrincipalRole = "owner"
)

// PluginSharePrincipalType is generated from the PluginSharePrincipalType schema definition.
type PluginSharePrincipalType string

const (
	// PluginSharePrincipalTypeUser is the "user" PluginSharePrincipalType value.
	PluginSharePrincipalTypeUser PluginSharePrincipalType = "user"
	// PluginSharePrincipalTypeGroup is the "group" PluginSharePrincipalType value.
	PluginSharePrincipalTypeGroup PluginSharePrincipalType = "group"
	// PluginSharePrincipalTypeWorkspace is the "workspace" PluginSharePrincipalType value.
	PluginSharePrincipalTypeWorkspace PluginSharePrincipalType = "workspace"
)

// PluginShareSaveParams is generated from the PluginShareSaveParams schema definition.
type PluginShareSaveParams struct {
	Discoverability PluginShareDiscoverability `json:"discoverability,omitzero"`
	PluginPath      string                     `json:"pluginPath"`
	RemotePluginID  string                     `json:"remotePluginId,omitzero"`
	ShareTargets    []PluginShareTarget        `json:"shareTargets,omitzero"`
}

// PluginShareSaveResponse is generated from the PluginShareSaveResponse schema definition.
type PluginShareSaveResponse struct {
	CanPublishToWorkspace bool   `json:"canPublishToWorkspace,omitzero"`
	RemotePluginID        string `json:"remotePluginId"`
	ShareURL              string `json:"shareUrl"`
}

// PluginShareTarget is generated from the PluginShareTarget schema definition.
type PluginShareTarget struct {
	PrincipalID   string                   `json:"principalId"`
	PrincipalType PluginSharePrincipalType `json:"principalType"`
	Role          PluginShareTargetRole    `json:"role"`
}

// PluginShareTargetRole is generated from the PluginShareTargetRole schema definition.
type PluginShareTargetRole string

const (
	// PluginShareTargetRoleReader is the "reader" PluginShareTargetRole value.
	PluginShareTargetRoleReader PluginShareTargetRole = "reader"
	// PluginShareTargetRoleEditor is the "editor" PluginShareTargetRole value.
	PluginShareTargetRoleEditor PluginShareTargetRole = "editor"
)

// PluginShareUpdateDiscoverability is generated from the PluginShareUpdateDiscoverability schema definition.
type PluginShareUpdateDiscoverability string

const (
	// PluginShareUpdateDiscoverabilityUNLISTED is the "UNLISTED" PluginShareUpdateDiscoverability value.
	PluginShareUpdateDiscoverabilityUNLISTED PluginShareUpdateDiscoverability = "UNLISTED"
	// PluginShareUpdateDiscoverabilityPRIVATE is the "PRIVATE" PluginShareUpdateDiscoverability value.
	PluginShareUpdateDiscoverabilityPRIVATE PluginShareUpdateDiscoverability = "PRIVATE"
	// PluginShareUpdateDiscoverabilityLISTED is the "LISTED" PluginShareUpdateDiscoverability value.
	PluginShareUpdateDiscoverabilityLISTED PluginShareUpdateDiscoverability = "LISTED"
)

// PluginShareUpdateTargetsParams is generated from the PluginShareUpdateTargetsParams schema definition.
type PluginShareUpdateTargetsParams struct {
	Discoverability PluginShareUpdateDiscoverability `json:"discoverability"`
	RemotePluginID  string                           `json:"remotePluginId"`
	ShareTargets    []PluginShareTarget              `json:"shareTargets"`
}

// PluginShareUpdateTargetsResponse is generated from the PluginShareUpdateTargetsResponse schema definition.
type PluginShareUpdateTargetsResponse struct {
	Discoverability PluginShareDiscoverability `json:"discoverability"`
	Principals      []PluginSharePrincipal     `json:"principals"`
}

// PluginSkillReadParams is generated from the PluginSkillReadParams schema definition.
type PluginSkillReadParams struct {
	RemoteMarketplaceName string `json:"remoteMarketplaceName"`
	RemotePluginID        string `json:"remotePluginId"`
	SkillName             string `json:"skillName"`
}

// PluginSkillReadResponse is generated from the PluginSkillReadResponse schema definition.
type PluginSkillReadResponse struct {
	Contents string `json:"contents,omitzero"`
}

// PluginSource is generated from the PluginSource schema definition.
type PluginSource interface {
	isPluginSource()
}

// RawPluginSource preserves an uninterpreted PluginSource JSON value.
type RawPluginSource jsontext.Value

func (RawPluginSource) isPluginSource() {}

var (
	_ json.MarshalerTo     = RawPluginSource{}
	_ json.UnmarshalerFrom = (*RawPluginSource)(nil)
)

func (value RawPluginSource) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawPluginSource) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// LocalPluginSource is generated from the LocalPluginSource schema definition.
type LocalPluginSource struct {
	Path string `json:"path"`
	Type string `json:"type"`
}

func (LocalPluginSource) isPluginSource() {}

// GitPluginSource is generated from the GitPluginSource schema definition.
type GitPluginSource struct {
	Path    string `json:"path,omitzero"`
	RefName string `json:"refName,omitzero"`
	Sha     string `json:"sha,omitzero"`
	Type    string `json:"type"`
	URL     string `json:"url"`
}

func (GitPluginSource) isPluginSource() {}

// NpmPluginSource is generated from the NpmPluginSource schema definition.
type NpmPluginSource struct {
	PackageValue string `json:"package"`

	// Registry optional HTTPS registry URL. Authentication stays in the user's npm config.
	Registry string `json:"registry,omitzero"`
	Type     string `json:"type"`

	// Version optional npm version or version range.
	Version string `json:"version,omitzero"`
}

func (NpmPluginSource) isPluginSource() {}

// RemotePluginSource represents a plugin is available in the remote catalog. Download metadata is kept server-side and is not exposed through the app-server API.
type RemotePluginSource struct {
	Type string `json:"type"`
}

func (RemotePluginSource) isPluginSource() {}

func decodeGeneratedPluginSource(raw jsontext.Value) (PluginSource, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "local":
			var value LocalPluginSource
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "git":
			var value GitPluginSource
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "npm":
			var value NpmPluginSource
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "remote":
			var value RemotePluginSource
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawPluginSource(raw), nil
	}
	return RawPluginSource(raw), nil
}

// PluginSummary is generated from the PluginSummary schema definition.
type PluginSummary struct {
	AuthPolicy PluginAuthPolicy `json:"authPolicy"`

	// Availability state for installing and using the plugin.
	Availability PluginAvailability `json:"availability,omitzero"`

	// DisabledReason why the remote plugin is unavailable, when provided by plugin-service.
	DisabledReason PluginDisabledReason `json:"disabledReason,omitzero"`

	// EligiblePlanTypes raw plugin-service plan identifiers eligible to install the plugin.
	EligiblePlanTypes   []string                  `json:"eligiblePlanTypes,omitzero"`
	Enabled             bool                      `json:"enabled"`
	ID                  string                    `json:"id"`
	InstallPolicy       PluginInstallPolicy       `json:"installPolicy"`
	InstallPolicySource PluginInstallPolicySource `json:"installPolicySource,omitzero"`
	Installed           bool                      `json:"installed"`

	// InstalledAt unix timestamp in seconds when the remote plugin was installed, when available.
	InstalledAt    int64           `json:"installedAt,omitzero"`
	InterfaceValue PluginInterface `json:"interface,omitzero"`
	Keywords       []string        `json:"keywords,omitzero"`

	// LocalVersion version of the locally materialized plugin package when available.
	LocalVersion                     string `json:"localVersion,omitzero"`
	MustShowInstallationInterstitial bool   `json:"mustShowInstallationInterstitial,omitzero"`
	Name                             string `json:"name"`

	// RemotePluginID backend remote plugin identifier when available.
	RemotePluginID string `json:"remotePluginId,omitzero"`

	// ShareContext remote sharing context associated with this plugin when available.
	ShareContext PluginShareContext `json:"shareContext,omitzero"`
	Source       PluginSource       `json:"source"`

	// Version advertised by the remote marketplace backend when available.
	Version string `json:"version,omitzero"`
}

func (value *PluginSummary) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		AuthPolicy                       PluginAuthPolicy          `json:"authPolicy"`
		Availability                     jsontext.Value            `json:"availability,omitzero"`
		DisabledReason                   PluginDisabledReason      `json:"disabledReason,omitzero"`
		EligiblePlanTypes                []string                  `json:"eligiblePlanTypes,omitzero"`
		Enabled                          bool                      `json:"enabled"`
		ID                               string                    `json:"id"`
		InstallPolicy                    PluginInstallPolicy       `json:"installPolicy"`
		InstallPolicySource              PluginInstallPolicySource `json:"installPolicySource,omitzero"`
		Installed                        bool                      `json:"installed"`
		InstalledAt                      int64                     `json:"installedAt,omitzero"`
		InterfaceValue                   PluginInterface           `json:"interface,omitzero"`
		Keywords                         []string                  `json:"keywords,omitzero"`
		LocalVersion                     string                    `json:"localVersion,omitzero"`
		MustShowInstallationInterstitial bool                      `json:"mustShowInstallationInterstitial,omitzero"`
		Name                             string                    `json:"name"`
		RemotePluginID                   string                    `json:"remotePluginId,omitzero"`
		ShareContext                     PluginShareContext        `json:"shareContext,omitzero"`
		Source                           jsontext.Value            `json:"source"`
		Version                          string                    `json:"version,omitzero"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.AuthPolicy = raw.AuthPolicy
	if raw.Availability == nil {
		value.Availability = nil
	} else {
		decodedAvailability, err := decodeGeneratedPluginAvailability(raw.Availability)
		if err != nil {
			return err
		}
		value.Availability = decodedAvailability
	}
	value.DisabledReason = raw.DisabledReason
	value.EligiblePlanTypes = raw.EligiblePlanTypes
	value.Enabled = raw.Enabled
	value.ID = raw.ID
	value.InstallPolicy = raw.InstallPolicy
	value.InstallPolicySource = raw.InstallPolicySource
	value.Installed = raw.Installed
	value.InstalledAt = raw.InstalledAt
	value.InterfaceValue = raw.InterfaceValue
	value.Keywords = raw.Keywords
	value.LocalVersion = raw.LocalVersion
	value.MustShowInstallationInterstitial = raw.MustShowInstallationInterstitial
	value.Name = raw.Name
	value.RemotePluginID = raw.RemotePluginID
	value.ShareContext = raw.ShareContext
	if raw.Source == nil {
		value.Source = nil
	} else {
		decodedSource, err := decodeGeneratedPluginSource(raw.Source)
		if err != nil {
			return err
		}
		value.Source = decodedSource
	}
	value.Version = raw.Version
	return nil
}

// PluginUninstallParams is generated from the PluginUninstallParams schema definition.
type PluginUninstallParams struct {
	PluginID string `json:"pluginId"`
}

// PluginUninstallResponse is generated from the PluginUninstallResponse schema definition.
type PluginUninstallResponse struct{}

// PluginsMigration is generated from the PluginsMigration schema definition.
type PluginsMigration struct {
	MarketplaceName string   `json:"marketplaceName"`
	PluginNames     []string `json:"pluginNames"`
}

// ProcessExitedNotification final process exit notification for `process/spawn`.
type ProcessExitedNotification struct {
	// ExitCode processes exit code.
	ExitCode int32 `json:"exitCode"`

	// ProcessHandle client-supplied, connection-scoped `processHandle` from `process/spawn`.
	ProcessHandle string `json:"processHandle"`

	// Stderr buffered stderr capture.
	//
	// Empty when stderr was streamed via `process/outputDelta`.
	Stderr string `json:"stderr"`

	// StderrCapReached reports whether stderr reached `outputBytesCap`.
	//
	// In streaming mode, stderr is empty and cap state is also reported on the final stderr `process/outputDelta` notification.
	StderrCapReached bool `json:"stderrCapReached"`

	// Stdout buffered stdout capture.
	//
	// Empty when stdout was streamed via `process/outputDelta`.
	Stdout string `json:"stdout"`

	// StdoutCapReached reports whether stdout reached `outputBytesCap`.
	//
	// In streaming mode, stdout is empty and cap state is also reported on the final stdout `process/outputDelta` notification.
	StdoutCapReached bool `json:"stdoutCapReached"`
}

// ProcessKillParams terminates a running `process/spawn` session.
type ProcessKillParams struct {
	// ProcessHandle client-supplied, connection-scoped `processHandle` from `process/spawn`.
	ProcessHandle string `json:"processHandle"`
}

// ProcessKillResponse empty success response for `process/kill`.
type ProcessKillResponse struct{}

// ProcessOutputDeltaNotification base64-encoded output chunk emitted for a streaming `process/spawn` request.
type ProcessOutputDeltaNotification struct {
	// CapReached true on the final streamed chunk for this stream when output was truncated by `outputBytesCap`.
	CapReached bool `json:"capReached"`

	// DeltaBase64 base64-encoded output bytes.
	DeltaBase64 string `json:"deltaBase64"`

	// ProcessHandle client-supplied, connection-scoped `processHandle` from `process/spawn`.
	ProcessHandle string `json:"processHandle"`

	// Stream output stream this chunk belongs to.
	Stream ProcessOutputStream `json:"stream"`
}

func (value *ProcessOutputDeltaNotification) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		CapReached    bool           `json:"capReached"`
		DeltaBase64   string         `json:"deltaBase64"`
		ProcessHandle string         `json:"processHandle"`
		Stream        jsontext.Value `json:"stream"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.CapReached = raw.CapReached
	value.DeltaBase64 = raw.DeltaBase64
	value.ProcessHandle = raw.ProcessHandle
	if raw.Stream == nil {
		value.Stream = nil
	} else {
		decodedStream, err := decodeGeneratedProcessOutputStream(raw.Stream)
		if err != nil {
			return err
		}
		value.Stream = decodedStream
	}
	return nil
}

// ProcessOutputStream streams label for `process/outputDelta` notifications.
type ProcessOutputStream interface {
	isProcessOutputStream()
}

// RawProcessOutputStream preserves an uninterpreted ProcessOutputStream JSON value.
type RawProcessOutputStream jsontext.Value

func (RawProcessOutputStream) isProcessOutputStream() {}

var (
	_ json.MarshalerTo     = RawProcessOutputStream{}
	_ json.UnmarshalerFrom = (*RawProcessOutputStream)(nil)
)

func (value RawProcessOutputStream) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawProcessOutputStream) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// ProcessOutputStreamValue is a string-valued ProcessOutputStream variant.
type ProcessOutputStreamValue string

func (ProcessOutputStreamValue) isProcessOutputStream() {}

const (
	// ProcessOutputStreamValueStderr is the "stderr" ProcessOutputStreamValue value.
	ProcessOutputStreamValueStderr ProcessOutputStreamValue = "stderr"
	// ProcessOutputStreamValueStdout is the "stdout" ProcessOutputStreamValue value.
	ProcessOutputStreamValueStdout ProcessOutputStreamValue = "stdout"
)

func decodeGeneratedProcessOutputStream(raw jsontext.Value) (ProcessOutputStream, error) {
	if raw == nil {
		return nil, nil
	}
	var text string
	if err := json.Unmarshal(raw, &text); err == nil {
		switch text {
		case "stderr":
			return ProcessOutputStreamValue(text), nil
		case "stdout":
			return ProcessOutputStreamValue(text), nil
		}
		return RawProcessOutputStream(raw), nil
	}
	return RawProcessOutputStream(raw), nil
}

// ProcessResizePtyParams resizes a running PTY-backed `process/spawn` session.
type ProcessResizePtyParams struct {
	// ProcessHandle client-supplied, connection-scoped `processHandle` from `process/spawn`.
	ProcessHandle string `json:"processHandle"`

	// Size new PTY size in character cells.
	Size ProcessTerminalSize `json:"size"`
}

// ProcessResizePtyResponse empty success response for `process/resizePty`.
type ProcessResizePtyResponse struct{}

// ProcessSpawnParams spawn a standalone process (argv vector) without a Codex sandbox on the host where the app server is running.
//
// `process/spawn` returns after the process has started and the connection-scoped `processHandle` has been registered. Process output and exit are reported via `process/outputDelta` and `process/exited` notifications.
type ProcessSpawnParams struct {
	// Command argv vector. Empty arrays are rejected.
	Command []string `json:"command"`

	// Cwd absolute working directory for the process.
	Cwd string `json:"cwd"`

	// Env optional environment overrides merged into the app-server process environment.
	//
	// Matching names override inherited values. Set a key to `null` to unset an inherited variable.
	Env map[string]*string `json:"env,omitzero"`

	// OutputBytesCap optional per-stream stdout/stderr capture cap in bytes.
	//
	// When omitted, the server default applies. Set to `null` to disable the cap.
	OutputBytesCap uint `json:"outputBytesCap,omitzero"`

	// ProcessHandle client-supplied, connection-scoped process handle.
	//
	// Duplicate active handles are rejected on the same connection. The same handle can be reused after the prior process exits.
	ProcessHandle string `json:"processHandle"`

	// Size optional initial PTY size in character cells. Only valid when `tty` is true.
	Size ProcessTerminalSize `json:"size,omitzero"`

	// StreamStdin allows follow-up `process/writeStdin` requests to write stdin bytes.
	StreamStdin bool `json:"streamStdin,omitzero"`

	// StreamStdoutStderr streams stdout/stderr via `process/outputDelta` notifications.
	//
	// Streamed bytes are not duplicated into the `process/exited` notification.
	StreamStdoutStderr bool `json:"streamStdoutStderr,omitzero"`

	// TimeoutMs optional timeout in milliseconds.
	//
	// When omitted, the server default applies. Set to `null` to disable the timeout.
	TimeoutMs int64 `json:"timeoutMs,omitzero"`

	// TTY enables PTY mode.
	//
	// This implies `streamStdin` and `streamStdoutStderr`.
	TTY bool `json:"tty,omitzero"`
}

// ProcessSpawnResponse successful response for `process/spawn`.
type ProcessSpawnResponse struct{}

// ProcessTerminalSize PTY size in character cells for `process/spawn` PTY sessions.
type ProcessTerminalSize struct {
	// Cols terminal width in character cells.
	Cols int64 `json:"cols"`

	// Rows terminal height in character cells.
	Rows int64 `json:"rows"`
}

// ProcessWriteStdinParams writes stdin bytes to a running `process/spawn` session, close stdin, or both.
type ProcessWriteStdinParams struct {
	// CloseStdin closes stdin after writing `deltaBase64`, if present.
	CloseStdin bool `json:"closeStdin,omitzero"`

	// DeltaBase64 optional base64-encoded stdin bytes to write.
	DeltaBase64 string `json:"deltaBase64,omitzero"`

	// ProcessHandle client-supplied, connection-scoped `processHandle` from `process/spawn`.
	ProcessHandle string `json:"processHandle"`
}

// ProcessWriteStdinResponse empty success response for `process/writeStdin`.
type ProcessWriteStdinResponse struct{}

// RateLimitReachedType is generated from the RateLimitReachedType schema definition.
type RateLimitReachedType string

const (
	// RateLimitReachedTypeRateLimitReached is the "rate_limit_reached" RateLimitReachedType value.
	RateLimitReachedTypeRateLimitReached RateLimitReachedType = "rate_limit_reached"
	// RateLimitReachedTypeWorkspaceOwnerCreditsDepleted is the "workspace_owner_credits_depleted" RateLimitReachedType value.
	RateLimitReachedTypeWorkspaceOwnerCreditsDepleted RateLimitReachedType = "workspace_owner_credits_depleted"
	// RateLimitReachedTypeWorkspaceMemberCreditsDepleted is the "workspace_member_credits_depleted" RateLimitReachedType value.
	RateLimitReachedTypeWorkspaceMemberCreditsDepleted RateLimitReachedType = "workspace_member_credits_depleted"
	// RateLimitReachedTypeWorkspaceOwnerUsageLimitReached is the "workspace_owner_usage_limit_reached" RateLimitReachedType value.
	RateLimitReachedTypeWorkspaceOwnerUsageLimitReached RateLimitReachedType = "workspace_owner_usage_limit_reached"
	// RateLimitReachedTypeWorkspaceMemberUsageLimitReached is the "workspace_member_usage_limit_reached" RateLimitReachedType value.
	RateLimitReachedTypeWorkspaceMemberUsageLimitReached RateLimitReachedType = "workspace_member_usage_limit_reached"
)

// RateLimitResetCredit is generated from the RateLimitResetCredit schema definition.
type RateLimitResetCredit struct {
	// Description backend-provided display description for this credit, or `null` when unavailable.
	Description string `json:"description,omitzero"`

	// ExpiresAt unix timestamp in seconds when the credit expires, or `null` if it does not expire.
	ExpiresAt int64 `json:"expiresAt,omitzero"`

	// GrantedAt unix timestamp in seconds when the credit was granted.
	GrantedAt int64 `json:"grantedAt"`

	// ID opaque backend identifier for this reset credit.
	ID        string                     `json:"id"`
	ResetType RateLimitResetType         `json:"resetType"`
	Status    RateLimitResetCreditStatus `json:"status"`

	// Title backend-provided display title for this credit, or `null` when unavailable.
	Title string `json:"title,omitzero"`
}

// RateLimitResetCreditStatus is generated from the RateLimitResetCreditStatus schema definition.
type RateLimitResetCreditStatus string

const (
	// RateLimitResetCreditStatusAvailable is the "available" RateLimitResetCreditStatus value.
	RateLimitResetCreditStatusAvailable RateLimitResetCreditStatus = "available"
	// RateLimitResetCreditStatusRedeeming is the "redeeming" RateLimitResetCreditStatus value.
	RateLimitResetCreditStatusRedeeming RateLimitResetCreditStatus = "redeeming"
	// RateLimitResetCreditStatusRedeemed is the "redeemed" RateLimitResetCreditStatus value.
	RateLimitResetCreditStatusRedeemed RateLimitResetCreditStatus = "redeemed"
	// RateLimitResetCreditStatusUnknown is the "unknown" RateLimitResetCreditStatus value.
	RateLimitResetCreditStatusUnknown RateLimitResetCreditStatus = "unknown"
)

// RateLimitResetCreditsSummary is generated from the RateLimitResetCreditsSummary schema definition.
type RateLimitResetCreditsSummary struct {
	AvailableCount int64 `json:"availableCount"`

	// Credits detail rows for available reset credits, when the backend provides them.
	//
	// `null` means only `availableCount` is known, while an empty array means details were fetched and no available credits were returned. The backend may cap this list, so its length can be less than `availableCount`.
	Credits []RateLimitResetCredit `json:"credits,omitzero"`
}

// RateLimitResetType is generated from the RateLimitResetType schema definition.
type RateLimitResetType string

const (
	// RateLimitResetTypeCodexRateLimits is the "codexRateLimits" RateLimitResetType value.
	RateLimitResetTypeCodexRateLimits RateLimitResetType = "codexRateLimits"
	// RateLimitResetTypeUnknown is the "unknown" RateLimitResetType value.
	RateLimitResetTypeUnknown RateLimitResetType = "unknown"
)

// RateLimitSnapshot is generated from the RateLimitSnapshot schema definition.
type RateLimitSnapshot struct {
	Credits              CreditsSnapshot           `json:"credits,omitzero"`
	IndividualLimit      SpendControlLimitSnapshot `json:"individualLimit,omitzero"`
	LimitID              string                    `json:"limitId,omitzero"`
	LimitName            string                    `json:"limitName,omitzero"`
	PlanType             PlanType                  `json:"planType,omitzero"`
	Primary              RateLimitWindow           `json:"primary,omitzero"`
	RateLimitReachedType RateLimitReachedType      `json:"rateLimitReachedType,omitzero"`
	Secondary            RateLimitWindow           `json:"secondary,omitzero"`

	// SpendControlReached backend-reported spend-control state. `None` is unavailable, not a sparse-update recovery.
	SpendControlReached bool `json:"spendControlReached,omitzero"`
}

// RateLimitWindow is generated from the RateLimitWindow schema definition.
type RateLimitWindow struct {
	ResetsAt           int64 `json:"resetsAt,omitzero"`
	UsedPercent        int32 `json:"usedPercent"`
	WindowDurationMins int64 `json:"windowDurationMins,omitzero"`
}

// RawResponseCompletedNotification internal-only notification containing the exact usage from one upstream Responses API completion.
type RawResponseCompletedNotification struct {
	ResponseID string              `json:"responseId"`
	ThreadID   string              `json:"threadId"`
	TurnID     string              `json:"turnId"`
	Usage      TokenUsageBreakdown `json:"usage,omitzero"`
}

// RawResponseItemCompletedNotification is generated from the RawResponseItemCompletedNotification schema definition.
type RawResponseItemCompletedNotification struct {
	Item     ResponseItem `json:"item"`
	ThreadID string       `json:"threadId"`
	TurnID   string       `json:"turnId"`
}

func (value *RawResponseItemCompletedNotification) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Item     jsontext.Value `json:"item"`
		ThreadID string         `json:"threadId"`
		TurnID   string         `json:"turnId"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	if raw.Item == nil {
		value.Item = nil
	} else {
		decodedItem, err := decodeGeneratedResponseItem(raw.Item)
		if err != nil {
			return err
		}
		value.Item = decodedItem
	}
	value.ThreadID = raw.ThreadID
	value.TurnID = raw.TurnID
	return nil
}

// RealtimeConversationVersion is generated from the RealtimeConversationVersion schema definition.
type RealtimeConversationVersion string

const (
	// RealtimeConversationVersionV1 is the "v1" RealtimeConversationVersion value.
	RealtimeConversationVersionV1 RealtimeConversationVersion = "v1"
	// RealtimeConversationVersionV2 is the "v2" RealtimeConversationVersion value.
	RealtimeConversationVersionV2 RealtimeConversationVersion = "v2"
	// RealtimeConversationVersionV3 is the "v3" RealtimeConversationVersion value.
	RealtimeConversationVersionV3 RealtimeConversationVersion = "v3"
)

// RealtimeOutputModality is generated from the RealtimeOutputModality schema definition.
type RealtimeOutputModality string

const (
	// RealtimeOutputModalityText is the "text" RealtimeOutputModality value.
	RealtimeOutputModalityText RealtimeOutputModality = "text"
	// RealtimeOutputModalityAudio is the "audio" RealtimeOutputModality value.
	RealtimeOutputModalityAudio RealtimeOutputModality = "audio"
)

// RealtimeVoice is generated from the RealtimeVoice schema definition.
type RealtimeVoice string

const (
	// RealtimeVoiceAlloy is the "alloy" RealtimeVoice value.
	RealtimeVoiceAlloy RealtimeVoice = "alloy"
	// RealtimeVoiceArbor is the "arbor" RealtimeVoice value.
	RealtimeVoiceArbor RealtimeVoice = "arbor"
	// RealtimeVoiceAsh is the "ash" RealtimeVoice value.
	RealtimeVoiceAsh RealtimeVoice = "ash"
	// RealtimeVoiceBallad is the "ballad" RealtimeVoice value.
	RealtimeVoiceBallad RealtimeVoice = "ballad"
	// RealtimeVoiceBreeze is the "breeze" RealtimeVoice value.
	RealtimeVoiceBreeze RealtimeVoice = "breeze"
	// RealtimeVoiceCedar is the "cedar" RealtimeVoice value.
	RealtimeVoiceCedar RealtimeVoice = "cedar"
	// RealtimeVoiceCoral is the "coral" RealtimeVoice value.
	RealtimeVoiceCoral RealtimeVoice = "coral"
	// RealtimeVoiceCove is the "cove" RealtimeVoice value.
	RealtimeVoiceCove RealtimeVoice = "cove"
	// RealtimeVoiceEcho is the "echo" RealtimeVoice value.
	RealtimeVoiceEcho RealtimeVoice = "echo"
	// RealtimeVoiceEmber is the "ember" RealtimeVoice value.
	RealtimeVoiceEmber RealtimeVoice = "ember"
	// RealtimeVoiceJuniper is the "juniper" RealtimeVoice value.
	RealtimeVoiceJuniper RealtimeVoice = "juniper"
	// RealtimeVoiceMaple is the "maple" RealtimeVoice value.
	RealtimeVoiceMaple RealtimeVoice = "maple"
	// RealtimeVoiceMarin is the "marin" RealtimeVoice value.
	RealtimeVoiceMarin RealtimeVoice = "marin"
	// RealtimeVoiceSage is the "sage" RealtimeVoice value.
	RealtimeVoiceSage RealtimeVoice = "sage"
	// RealtimeVoiceShimmer is the "shimmer" RealtimeVoice value.
	RealtimeVoiceShimmer RealtimeVoice = "shimmer"
	// RealtimeVoiceSol is the "sol" RealtimeVoice value.
	RealtimeVoiceSol RealtimeVoice = "sol"
	// RealtimeVoiceSpruce is the "spruce" RealtimeVoice value.
	RealtimeVoiceSpruce RealtimeVoice = "spruce"
	// RealtimeVoiceVale is the "vale" RealtimeVoice value.
	RealtimeVoiceVale RealtimeVoice = "vale"
	// RealtimeVoiceVerse is the "verse" RealtimeVoice value.
	RealtimeVoiceVerse RealtimeVoice = "verse"
)

// RealtimeVoicesList is generated from the RealtimeVoicesList schema definition.
type RealtimeVoicesList struct {
	DefaultV1 RealtimeVoice   `json:"defaultV1"`
	DefaultV2 RealtimeVoice   `json:"defaultV2"`
	V1        []RealtimeVoice `json:"v1"`
	V2        []RealtimeVoice `json:"v2"`
}

// ReasoningEffort represents a non-empty reasoning effort value advertised by the model.
type ReasoningEffort string

// ReasoningEffortOption is generated from the ReasoningEffortOption schema definition.
type ReasoningEffortOption struct {
	Description     string          `json:"description"`
	ReasoningEffort ReasoningEffort `json:"reasoningEffort"`
}

// ReasoningItemContent is generated from the ReasoningItemContent schema definition.
type ReasoningItemContent interface {
	isReasoningItemContent()
}

// RawReasoningItemContent preserves an uninterpreted ReasoningItemContent JSON value.
type RawReasoningItemContent jsontext.Value

func (RawReasoningItemContent) isReasoningItemContent() {}

var (
	_ json.MarshalerTo     = RawReasoningItemContent{}
	_ json.UnmarshalerFrom = (*RawReasoningItemContent)(nil)
)

func (value RawReasoningItemContent) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawReasoningItemContent) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// ReasoningTextReasoningItemContent is generated from the ReasoningTextReasoningItemContent schema definition.
type ReasoningTextReasoningItemContent struct {
	Text string `json:"text"`
	Type string `json:"type"`
}

func (ReasoningTextReasoningItemContent) isReasoningItemContent() {}

// TextReasoningItemContent is generated from the TextReasoningItemContent schema definition.
type TextReasoningItemContent struct {
	Text string `json:"text"`
	Type string `json:"type"`
}

func (TextReasoningItemContent) isReasoningItemContent() {}

func decodeGeneratedReasoningItemContent(raw jsontext.Value) (ReasoningItemContent, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "reasoning_text":
			var value ReasoningTextReasoningItemContent
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "text":
			var value TextReasoningItemContent
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawReasoningItemContent(raw), nil
	}
	return RawReasoningItemContent(raw), nil
}

// SummaryTextReasoningItemReasoningSummary is generated from the SummaryTextReasoningItemReasoningSummary schema definition.
type SummaryTextReasoningItemReasoningSummary struct {
	Text string `json:"text"`
	Type string `json:"type"`
}

// ReasoningItemReasoningSummary is generated from the ReasoningItemReasoningSummary schema definition.
type ReasoningItemReasoningSummary = SummaryTextReasoningItemReasoningSummary

func (SummaryTextReasoningItemReasoningSummary) isReasoningItemReasoningSummary() {}

// ReasoningSummary represents a summary of the reasoning performed by the model. This can be useful for debugging and understanding the model's reasoning process. See https://platform.openai.com/docs/guides/reasoning?api-mode=responses#reasoning-summaries.
type ReasoningSummary interface {
	isReasoningSummary()
}

// RawReasoningSummary preserves an uninterpreted ReasoningSummary JSON value.
type RawReasoningSummary jsontext.Value

func (RawReasoningSummary) isReasoningSummary() {}

var (
	_ json.MarshalerTo     = RawReasoningSummary{}
	_ json.UnmarshalerFrom = (*RawReasoningSummary)(nil)
)

func (value RawReasoningSummary) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawReasoningSummary) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// ReasoningSummaryValue is a string-valued ReasoningSummary variant.
type ReasoningSummaryValue string

func (ReasoningSummaryValue) isReasoningSummary() {}

const (
	// ReasoningSummaryValueAuto is the "auto" ReasoningSummaryValue value.
	ReasoningSummaryValueAuto ReasoningSummaryValue = "auto"
	// ReasoningSummaryValueConcise is the "concise" ReasoningSummaryValue value.
	ReasoningSummaryValueConcise ReasoningSummaryValue = "concise"
	// ReasoningSummaryValueDetailed is the "detailed" ReasoningSummaryValue value.
	ReasoningSummaryValueDetailed ReasoningSummaryValue = "detailed"
	// ReasoningSummaryValueNone is the "none" ReasoningSummaryValue value.
	ReasoningSummaryValueNone ReasoningSummaryValue = "none"
)

func decodeGeneratedReasoningSummary(raw jsontext.Value) (ReasoningSummary, error) {
	if raw == nil {
		return nil, nil
	}
	var text string
	if err := json.Unmarshal(raw, &text); err == nil {
		switch text {
		case "auto":
			return ReasoningSummaryValue(text), nil
		case "concise":
			return ReasoningSummaryValue(text), nil
		case "detailed":
			return ReasoningSummaryValue(text), nil
		case "none":
			return ReasoningSummaryValue(text), nil
		}
		return RawReasoningSummary(raw), nil
	}
	return RawReasoningSummary(raw), nil
}

// ReasoningSummaryPartAddedNotification is generated from the ReasoningSummaryPartAddedNotification schema definition.
type ReasoningSummaryPartAddedNotification struct {
	ItemID       string `json:"itemId"`
	SummaryIndex int64  `json:"summaryIndex"`
	ThreadID     string `json:"threadId"`
	TurnID       string `json:"turnId"`
}

// ReasoningSummaryTextDeltaNotification is generated from the ReasoningSummaryTextDeltaNotification schema definition.
type ReasoningSummaryTextDeltaNotification struct {
	Delta        string `json:"delta"`
	ItemID       string `json:"itemId"`
	SummaryIndex int64  `json:"summaryIndex"`
	ThreadID     string `json:"threadId"`
	TurnID       string `json:"turnId"`
}

// ReasoningTextDeltaNotification is generated from the ReasoningTextDeltaNotification schema definition.
type ReasoningTextDeltaNotification struct {
	ContentIndex int64  `json:"contentIndex"`
	Delta        string `json:"delta"`
	ItemID       string `json:"itemId"`
	ThreadID     string `json:"threadId"`
	TurnID       string `json:"turnId"`
}

// RemoteControlClient is generated from the RemoteControlClient schema definition.
type RemoteControlClient struct {
	AppVersion  string `json:"appVersion,omitzero"`
	ClientID    string `json:"clientId"`
	DeviceModel string `json:"deviceModel,omitzero"`
	DeviceType  string `json:"deviceType,omitzero"`
	DisplayName string `json:"displayName,omitzero"`
	LastSeenAt  int64  `json:"lastSeenAt,omitzero"`
	OsVersion   string `json:"osVersion,omitzero"`
	Platform    string `json:"platform,omitzero"`
}

// RemoteControlClientsListOrder is generated from the RemoteControlClientsListOrder schema definition.
type RemoteControlClientsListOrder string

const (
	// RemoteControlClientsListOrderAsc is the "asc" RemoteControlClientsListOrder value.
	RemoteControlClientsListOrderAsc RemoteControlClientsListOrder = "asc"
	// RemoteControlClientsListOrderDesc is the "desc" RemoteControlClientsListOrder value.
	RemoteControlClientsListOrderDesc RemoteControlClientsListOrder = "desc"
)

// RemoteControlClientsListParams is generated from the RemoteControlClientsListParams schema definition.
type RemoteControlClientsListParams struct {
	Cursor        string                        `json:"cursor,omitzero"`
	EnvironmentID string                        `json:"environmentId"`
	Limit         int32                         `json:"limit,omitzero"`
	Order         RemoteControlClientsListOrder `json:"order,omitzero"`
}

// RemoteControlClientsListResponse is generated from the RemoteControlClientsListResponse schema definition.
type RemoteControlClientsListResponse struct {
	Data       []RemoteControlClient `json:"data"`
	NextCursor string                `json:"nextCursor,omitzero"`
}

// RemoteControlClientsRevokeParams is generated from the RemoteControlClientsRevokeParams schema definition.
type RemoteControlClientsRevokeParams struct {
	ClientID      string `json:"clientId"`
	EnvironmentID string `json:"environmentId"`
}

// RemoteControlClientsRevokeResponse is generated from the RemoteControlClientsRevokeResponse schema definition.
type RemoteControlClientsRevokeResponse struct{}

// RemoteControlConnectionStatus is generated from the RemoteControlConnectionStatus schema definition.
type RemoteControlConnectionStatus string

const (
	// RemoteControlConnectionStatusDisabled is the "disabled" RemoteControlConnectionStatus value.
	RemoteControlConnectionStatusDisabled RemoteControlConnectionStatus = "disabled"
	// RemoteControlConnectionStatusConnecting is the "connecting" RemoteControlConnectionStatus value.
	RemoteControlConnectionStatusConnecting RemoteControlConnectionStatus = "connecting"
	// RemoteControlConnectionStatusConnected is the "connected" RemoteControlConnectionStatus value.
	RemoteControlConnectionStatusConnected RemoteControlConnectionStatus = "connected"
	// RemoteControlConnectionStatusErrored is the "errored" RemoteControlConnectionStatus value.
	RemoteControlConnectionStatusErrored RemoteControlConnectionStatus = "errored"
)

// RemoteControlDisableParams is generated from the RemoteControlDisableParams schema definition.
type RemoteControlDisableParams struct {
	Ephemeral bool `json:"ephemeral,omitzero"`
}

// RemoteControlDisableResponse is generated from the RemoteControlDisableResponse schema definition.
type RemoteControlDisableResponse struct {
	EnvironmentID  string                        `json:"environmentId,omitzero"`
	InstallationID string                        `json:"installationId"`
	ServerName     string                        `json:"serverName"`
	Status         RemoteControlConnectionStatus `json:"status"`
}

// RemoteControlEnableParams is generated from the RemoteControlEnableParams schema definition.
type RemoteControlEnableParams struct {
	Ephemeral bool `json:"ephemeral,omitzero"`
}

// RemoteControlEnableResponse is generated from the RemoteControlEnableResponse schema definition.
type RemoteControlEnableResponse struct {
	EnvironmentID  string                        `json:"environmentId,omitzero"`
	InstallationID string                        `json:"installationId"`
	ServerName     string                        `json:"serverName"`
	Status         RemoteControlConnectionStatus `json:"status"`
}

// RemoteControlPairingStartParams is generated from the RemoteControlPairingStartParams schema definition.
type RemoteControlPairingStartParams struct {
	ManualCode bool `json:"manualCode,omitzero"`
}

// RemoteControlPairingStartResponse is generated from the RemoteControlPairingStartResponse schema definition.
type RemoteControlPairingStartResponse struct {
	EnvironmentID     string `json:"environmentId"`
	ExpiresAt         int64  `json:"expiresAt"`
	ManualPairingCode string `json:"manualPairingCode,omitzero"`
	PairingCode       string `json:"pairingCode"`
}

// RemoteControlPairingStatusParams is generated from the RemoteControlPairingStatusParams schema definition.
type RemoteControlPairingStatusParams struct {
	ManualPairingCode string `json:"manualPairingCode,omitzero"`
	PairingCode       string `json:"pairingCode,omitzero"`
}

// RemoteControlPairingStatusResponse is generated from the RemoteControlPairingStatusResponse schema definition.
type RemoteControlPairingStatusResponse struct {
	Claimed bool `json:"claimed"`
}

// RemoteControlStatusChangedNotification current remote-control connection status and remote identity exposed to clients.
type RemoteControlStatusChangedNotification struct {
	EnvironmentID  string                        `json:"environmentId,omitzero"`
	InstallationID string                        `json:"installationId"`
	ServerName     string                        `json:"serverName"`
	Status         RemoteControlConnectionStatus `json:"status"`
}

// RemoteControlStatusReadResponse is generated from the RemoteControlStatusReadResponse schema definition.
type RemoteControlStatusReadResponse struct {
	EnvironmentID  string                        `json:"environmentId,omitzero"`
	InstallationID string                        `json:"installationId"`
	ServerName     string                        `json:"serverName"`
	Status         RemoteControlConnectionStatus `json:"status"`
}

// RequestID is generated from the RequestId schema definition.
type RequestID = string

// RequestPermissionProfile is generated from the RequestPermissionProfile schema definition.
type RequestPermissionProfile struct {
	FileSystem AdditionalFileSystemPermissions `json:"fileSystem,omitzero"`
	Network    AdditionalNetworkPermissions    `json:"network,omitzero"`
}

// ResidencyRequirement is generated from the ResidencyRequirement schema definition.
type ResidencyRequirement string

const (
	// ResidencyRequirementUs is the "us" ResidencyRequirement value.
	ResidencyRequirementUs ResidencyRequirement = "us"
)

// Resource represents a known resource that the server is capable of reading.
type Resource struct {
	Meta        jsontext.Value   `json:"_meta,omitzero"`
	Annotations jsontext.Value   `json:"annotations,omitzero"`
	Description string           `json:"description,omitzero"`
	Icons       []jsontext.Value `json:"icons,omitzero"`
	MimeType    string           `json:"mimeType,omitzero"`
	Name        string           `json:"name"`
	Size        int64            `json:"size,omitzero"`
	Title       string           `json:"title,omitzero"`
	URI         string           `json:"uri"`
}

// ResourceContent contents returned when reading a resource from an MCP server.
type ResourceContent interface {
	isResourceContent()
}

// RawResourceContent preserves an uninterpreted ResourceContent JSON value.
type RawResourceContent jsontext.Value

func (RawResourceContent) isResourceContent() {}

var (
	_ json.MarshalerTo     = RawResourceContent{}
	_ json.UnmarshalerFrom = (*RawResourceContent)(nil)
)

func (value RawResourceContent) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawResourceContent) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// ResourceContentText is generated from the ResourceContentText schema definition.
type ResourceContentText struct {
	Meta     jsontext.Value `json:"_meta,omitzero"`
	MimeType string         `json:"mimeType,omitzero"`
	Text     string         `json:"text"`

	// URI represents an URI of this resource.
	URI string `json:"uri"`
}

func (ResourceContentText) isResourceContent() {}

// ResourceContentBlob is generated from the ResourceContentBlob schema definition.
type ResourceContentBlob struct {
	Meta     jsontext.Value `json:"_meta,omitzero"`
	Blob     string         `json:"blob"`
	MimeType string         `json:"mimeType,omitzero"`

	// URI represents an URI of this resource.
	URI string `json:"uri"`
}

func (ResourceContentBlob) isResourceContent() {}

func decodeGeneratedResourceContent(raw jsontext.Value) (ResourceContent, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Text jsontext.Value `json:"text"`
		Blob jsontext.Value `json:"blob"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		if object.Text != nil {
			var value ResourceContentText
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		if object.Blob != nil {
			var value ResourceContentBlob
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawResourceContent(raw), nil
	}
	return RawResourceContent(raw), nil
}

// ResourceTemplate represents a template description for resources available on the server.
type ResourceTemplate struct {
	Annotations jsontext.Value `json:"annotations,omitzero"`
	Description string         `json:"description,omitzero"`
	MimeType    string         `json:"mimeType,omitzero"`
	Name        string         `json:"name"`
	Title       string         `json:"title,omitzero"`
	URITemplate string         `json:"uriTemplate"`
}

// ResponseItem is generated from the ResponseItem schema definition.
type ResponseItem interface {
	isResponseItem()
}

// RawResponseItem preserves an uninterpreted ResponseItem JSON value.
type RawResponseItem jsontext.Value

func (RawResponseItem) isResponseItem() {}

var (
	_ json.MarshalerTo     = RawResponseItem{}
	_ json.UnmarshalerFrom = (*RawResponseItem)(nil)
)

func (value RawResponseItem) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawResponseItem) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// MessageResponseItem is generated from the MessageResponseItem schema definition.
type MessageResponseItem struct {
	Content                                []ContentItem                          `json:"content"`
	ID                                     string                                 `json:"id,omitzero"`
	InternalChatMessageMetadataPassthrough InternalChatMessageMetadataPassthrough `json:"internal_chat_message_metadata_passthrough,omitzero"`
	Phase                                  MessagePhase                           `json:"phase,omitzero"`
	Role                                   string                                 `json:"role"`
	Type                                   string                                 `json:"type"`
}

func (value *MessageResponseItem) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Content                                []jsontext.Value                       `json:"content"`
		ID                                     string                                 `json:"id,omitzero"`
		InternalChatMessageMetadataPassthrough InternalChatMessageMetadataPassthrough `json:"internal_chat_message_metadata_passthrough,omitzero"`
		Phase                                  jsontext.Value                         `json:"phase,omitzero"`
		Role                                   string                                 `json:"role"`
		Type                                   string                                 `json:"type"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	if raw.Content == nil {
		value.Content = nil
	} else {
		value.Content = make([]ContentItem, len(raw.Content))
		for i, item := range raw.Content {
			if item != nil {
				decodedContent, err := decodeGeneratedContentItem(item)
				if err != nil {
					return err
				}
				value.Content[i] = decodedContent
			}
		}
	}
	value.ID = raw.ID
	value.InternalChatMessageMetadataPassthrough = raw.InternalChatMessageMetadataPassthrough
	if raw.Phase == nil {
		value.Phase = nil
	} else {
		decodedPhase, err := decodeGeneratedMessagePhase(raw.Phase)
		if err != nil {
			return err
		}
		value.Phase = decodedPhase
	}
	value.Role = raw.Role
	value.Type = raw.Type
	return nil
}

func (MessageResponseItem) isResponseItem() {}

// AgentMessageResponseItem is generated from the AgentMessageResponseItem schema definition.
type AgentMessageResponseItem struct {
	Author                                 string                                 `json:"author"`
	Content                                []AgentMessageInputContent             `json:"content"`
	ID                                     string                                 `json:"id,omitzero"`
	InternalChatMessageMetadataPassthrough InternalChatMessageMetadataPassthrough `json:"internal_chat_message_metadata_passthrough,omitzero"`
	Recipient                              string                                 `json:"recipient"`
	Type                                   string                                 `json:"type"`
}

func (value *AgentMessageResponseItem) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Author                                 string                                 `json:"author"`
		Content                                []jsontext.Value                       `json:"content"`
		ID                                     string                                 `json:"id,omitzero"`
		InternalChatMessageMetadataPassthrough InternalChatMessageMetadataPassthrough `json:"internal_chat_message_metadata_passthrough,omitzero"`
		Recipient                              string                                 `json:"recipient"`
		Type                                   string                                 `json:"type"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.Author = raw.Author
	if raw.Content == nil {
		value.Content = nil
	} else {
		value.Content = make([]AgentMessageInputContent, len(raw.Content))
		for i, item := range raw.Content {
			if item != nil {
				decodedContent, err := decodeGeneratedAgentMessageInputContent(item)
				if err != nil {
					return err
				}
				value.Content[i] = decodedContent
			}
		}
	}
	value.ID = raw.ID
	value.InternalChatMessageMetadataPassthrough = raw.InternalChatMessageMetadataPassthrough
	value.Recipient = raw.Recipient
	value.Type = raw.Type
	return nil
}

func (AgentMessageResponseItem) isResponseItem() {}

// ReasoningResponseItem is generated from the ReasoningResponseItem schema definition.
type ReasoningResponseItem struct {
	Content                                []ReasoningItemContent                 `json:"content,omitzero"`
	EncryptedContent                       string                                 `json:"encrypted_content,omitzero"`
	ID                                     string                                 `json:"id,omitzero"`
	InternalChatMessageMetadataPassthrough InternalChatMessageMetadataPassthrough `json:"internal_chat_message_metadata_passthrough,omitzero"`
	Summary                                []ReasoningItemReasoningSummary        `json:"summary"`
	Type                                   string                                 `json:"type"`
}

func (value *ReasoningResponseItem) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Content                                []jsontext.Value                       `json:"content,omitzero"`
		EncryptedContent                       string                                 `json:"encrypted_content,omitzero"`
		ID                                     string                                 `json:"id,omitzero"`
		InternalChatMessageMetadataPassthrough InternalChatMessageMetadataPassthrough `json:"internal_chat_message_metadata_passthrough,omitzero"`
		Summary                                []ReasoningItemReasoningSummary        `json:"summary"`
		Type                                   string                                 `json:"type"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	if raw.Content == nil {
		value.Content = nil
	} else {
		value.Content = make([]ReasoningItemContent, len(raw.Content))
		for i, item := range raw.Content {
			if item != nil {
				decodedContent, err := decodeGeneratedReasoningItemContent(item)
				if err != nil {
					return err
				}
				value.Content[i] = decodedContent
			}
		}
	}
	value.EncryptedContent = raw.EncryptedContent
	value.ID = raw.ID
	value.InternalChatMessageMetadataPassthrough = raw.InternalChatMessageMetadataPassthrough
	value.Summary = raw.Summary
	value.Type = raw.Type
	return nil
}

func (ReasoningResponseItem) isResponseItem() {}

// LocalShellCallResponseItem is generated from the LocalShellCallResponseItem schema definition.
type LocalShellCallResponseItem struct {
	Action LocalShellAction `json:"action"`

	// CallID sets when using the Responses API.
	CallID string `json:"call_id,omitzero"`

	// ID legacy id field retained for compatibility with older payloads.
	ID                                     string                                 `json:"id,omitzero"`
	InternalChatMessageMetadataPassthrough InternalChatMessageMetadataPassthrough `json:"internal_chat_message_metadata_passthrough,omitzero"`
	Status                                 LocalShellStatus                       `json:"status"`
	Type                                   string                                 `json:"type"`
}

func (LocalShellCallResponseItem) isResponseItem() {}

// FunctionCallResponseItem is generated from the FunctionCallResponseItem schema definition.
type FunctionCallResponseItem struct {
	Arguments                              string                                 `json:"arguments"`
	CallID                                 string                                 `json:"call_id"`
	EncryptedFunctionArgs                  []string                               `json:"encrypted_function_args,omitzero"`
	ID                                     string                                 `json:"id,omitzero"`
	InternalChatMessageMetadataPassthrough InternalChatMessageMetadataPassthrough `json:"internal_chat_message_metadata_passthrough,omitzero"`
	Name                                   string                                 `json:"name"`
	Namespace                              string                                 `json:"namespace,omitzero"`
	Type                                   string                                 `json:"type"`
}

func (FunctionCallResponseItem) isResponseItem() {}

// ToolSearchCallResponseItem is generated from the ToolSearchCallResponseItem schema definition.
type ToolSearchCallResponseItem struct {
	Arguments                              jsontext.Value                         `json:"arguments"`
	CallID                                 string                                 `json:"call_id,omitzero"`
	Execution                              string                                 `json:"execution"`
	ID                                     string                                 `json:"id,omitzero"`
	InternalChatMessageMetadataPassthrough InternalChatMessageMetadataPassthrough `json:"internal_chat_message_metadata_passthrough,omitzero"`
	Status                                 string                                 `json:"status,omitzero"`
	Type                                   string                                 `json:"type"`
}

func (ToolSearchCallResponseItem) isResponseItem() {}

// FunctionCallOutputResponseItem is generated from the FunctionCallOutputResponseItem schema definition.
type FunctionCallOutputResponseItem struct {
	CallID                                 string                                 `json:"call_id"`
	ID                                     string                                 `json:"id,omitzero"`
	InternalChatMessageMetadataPassthrough InternalChatMessageMetadataPassthrough `json:"internal_chat_message_metadata_passthrough,omitzero"`
	Output                                 FunctionCallOutputBody                 `json:"output"`
	Type                                   string                                 `json:"type"`
}

func (value *FunctionCallOutputResponseItem) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		CallID                                 string                                 `json:"call_id"`
		ID                                     string                                 `json:"id,omitzero"`
		InternalChatMessageMetadataPassthrough InternalChatMessageMetadataPassthrough `json:"internal_chat_message_metadata_passthrough,omitzero"`
		Output                                 jsontext.Value                         `json:"output"`
		Type                                   string                                 `json:"type"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.CallID = raw.CallID
	value.ID = raw.ID
	value.InternalChatMessageMetadataPassthrough = raw.InternalChatMessageMetadataPassthrough
	if raw.Output == nil {
		value.Output = nil
	} else {
		decodedOutput, err := decodeGeneratedFunctionCallOutputBody(raw.Output)
		if err != nil {
			return err
		}
		value.Output = decodedOutput
	}
	value.Type = raw.Type
	return nil
}

func (FunctionCallOutputResponseItem) isResponseItem() {}

// CustomToolCallResponseItem is generated from the CustomToolCallResponseItem schema definition.
type CustomToolCallResponseItem struct {
	CallID                                 string                                 `json:"call_id"`
	ID                                     string                                 `json:"id,omitzero"`
	Input                                  string                                 `json:"input"`
	InternalChatMessageMetadataPassthrough InternalChatMessageMetadataPassthrough `json:"internal_chat_message_metadata_passthrough,omitzero"`
	Name                                   string                                 `json:"name"`
	Namespace                              string                                 `json:"namespace,omitzero"`
	Status                                 string                                 `json:"status,omitzero"`
	Type                                   string                                 `json:"type"`
}

func (CustomToolCallResponseItem) isResponseItem() {}

// CustomToolCallOutputResponseItem is generated from the CustomToolCallOutputResponseItem schema definition.
type CustomToolCallOutputResponseItem struct {
	CallID                                 string                                 `json:"call_id"`
	ID                                     string                                 `json:"id,omitzero"`
	InternalChatMessageMetadataPassthrough InternalChatMessageMetadataPassthrough `json:"internal_chat_message_metadata_passthrough,omitzero"`
	Name                                   string                                 `json:"name,omitzero"`
	Output                                 FunctionCallOutputBody                 `json:"output"`
	Type                                   string                                 `json:"type"`
}

func (value *CustomToolCallOutputResponseItem) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		CallID                                 string                                 `json:"call_id"`
		ID                                     string                                 `json:"id,omitzero"`
		InternalChatMessageMetadataPassthrough InternalChatMessageMetadataPassthrough `json:"internal_chat_message_metadata_passthrough,omitzero"`
		Name                                   string                                 `json:"name,omitzero"`
		Output                                 jsontext.Value                         `json:"output"`
		Type                                   string                                 `json:"type"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.CallID = raw.CallID
	value.ID = raw.ID
	value.InternalChatMessageMetadataPassthrough = raw.InternalChatMessageMetadataPassthrough
	value.Name = raw.Name
	if raw.Output == nil {
		value.Output = nil
	} else {
		decodedOutput, err := decodeGeneratedFunctionCallOutputBody(raw.Output)
		if err != nil {
			return err
		}
		value.Output = decodedOutput
	}
	value.Type = raw.Type
	return nil
}

func (CustomToolCallOutputResponseItem) isResponseItem() {}

// ToolSearchOutputResponseItem is generated from the ToolSearchOutputResponseItem schema definition.
type ToolSearchOutputResponseItem struct {
	CallID                                 string                                 `json:"call_id,omitzero"`
	Execution                              string                                 `json:"execution"`
	ID                                     string                                 `json:"id,omitzero"`
	InternalChatMessageMetadataPassthrough InternalChatMessageMetadataPassthrough `json:"internal_chat_message_metadata_passthrough,omitzero"`
	Status                                 string                                 `json:"status"`
	Tools                                  []jsontext.Value                       `json:"tools"`
	Type                                   string                                 `json:"type"`
}

func (ToolSearchOutputResponseItem) isResponseItem() {}

// WebSearchCallResponseItem is generated from the WebSearchCallResponseItem schema definition.
type WebSearchCallResponseItem struct {
	Action                                 ResponsesAPIWebSearchAction            `json:"action,omitzero"`
	ID                                     string                                 `json:"id,omitzero"`
	InternalChatMessageMetadataPassthrough InternalChatMessageMetadataPassthrough `json:"internal_chat_message_metadata_passthrough,omitzero"`
	Status                                 string                                 `json:"status,omitzero"`
	Type                                   string                                 `json:"type"`
}

func (value *WebSearchCallResponseItem) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Action                                 jsontext.Value                         `json:"action,omitzero"`
		ID                                     string                                 `json:"id,omitzero"`
		InternalChatMessageMetadataPassthrough InternalChatMessageMetadataPassthrough `json:"internal_chat_message_metadata_passthrough,omitzero"`
		Status                                 string                                 `json:"status,omitzero"`
		Type                                   string                                 `json:"type"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	if raw.Action == nil {
		value.Action = nil
	} else {
		decodedAction, err := decodeGeneratedResponsesAPIWebSearchAction(raw.Action)
		if err != nil {
			return err
		}
		value.Action = decodedAction
	}
	value.ID = raw.ID
	value.InternalChatMessageMetadataPassthrough = raw.InternalChatMessageMetadataPassthrough
	value.Status = raw.Status
	value.Type = raw.Type
	return nil
}

func (WebSearchCallResponseItem) isResponseItem() {}

// ImageGenerationCallResponseItem is generated from the ImageGenerationCallResponseItem schema definition.
type ImageGenerationCallResponseItem struct {
	ID                                     string                                 `json:"id,omitzero"`
	InternalChatMessageMetadataPassthrough InternalChatMessageMetadataPassthrough `json:"internal_chat_message_metadata_passthrough,omitzero"`
	Result                                 string                                 `json:"result"`
	RevisedPrompt                          string                                 `json:"revised_prompt,omitzero"`
	Status                                 string                                 `json:"status"`
	Type                                   string                                 `json:"type"`
}

func (ImageGenerationCallResponseItem) isResponseItem() {}

// CompactionResponseItem is generated from the CompactionResponseItem schema definition.
type CompactionResponseItem struct {
	EncryptedContent                       string                                 `json:"encrypted_content"`
	ID                                     string                                 `json:"id,omitzero"`
	InternalChatMessageMetadataPassthrough InternalChatMessageMetadataPassthrough `json:"internal_chat_message_metadata_passthrough,omitzero"`
	Type                                   string                                 `json:"type"`
}

func (CompactionResponseItem) isResponseItem() {}

// CompactionTriggerResponseItem is generated from the CompactionTriggerResponseItem schema definition.
type CompactionTriggerResponseItem struct {
	Type string `json:"type"`
}

func (CompactionTriggerResponseItem) isResponseItem() {}

// ContextCompactionResponseItem is generated from the ContextCompactionResponseItem schema definition.
type ContextCompactionResponseItem struct {
	EncryptedContent                       string                                 `json:"encrypted_content,omitzero"`
	ID                                     string                                 `json:"id,omitzero"`
	InternalChatMessageMetadataPassthrough InternalChatMessageMetadataPassthrough `json:"internal_chat_message_metadata_passthrough,omitzero"`
	Type                                   string                                 `json:"type"`
}

func (ContextCompactionResponseItem) isResponseItem() {}

// OtherResponseItem is generated from the OtherResponseItem schema definition.
type OtherResponseItem struct {
	Type string `json:"type"`
}

func (OtherResponseItem) isResponseItem() {}

func decodeGeneratedResponseItem(raw jsontext.Value) (ResponseItem, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "message":
			var value MessageResponseItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "agent_message":
			var value AgentMessageResponseItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "reasoning":
			var value ReasoningResponseItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "local_shell_call":
			var value LocalShellCallResponseItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "function_call":
			var value FunctionCallResponseItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "tool_search_call":
			var value ToolSearchCallResponseItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "function_call_output":
			var value FunctionCallOutputResponseItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "custom_tool_call":
			var value CustomToolCallResponseItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "custom_tool_call_output":
			var value CustomToolCallOutputResponseItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "tool_search_output":
			var value ToolSearchOutputResponseItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "web_search_call":
			var value WebSearchCallResponseItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "image_generation_call":
			var value ImageGenerationCallResponseItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "compaction":
			var value CompactionResponseItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "compaction_trigger":
			var value CompactionTriggerResponseItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "context_compaction":
			var value ContextCompactionResponseItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "other":
			var value OtherResponseItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawResponseItem(raw), nil
	}
	return RawResponseItem(raw), nil
}

// ResponsesAPIWebSearchAction is generated from the ResponsesAPIWebSearchAction schema definition.
type ResponsesAPIWebSearchAction interface {
	isResponsesAPIWebSearchAction()
}

// RawResponsesAPIWebSearchAction preserves an uninterpreted ResponsesAPIWebSearchAction JSON value.
type RawResponsesAPIWebSearchAction jsontext.Value

func (RawResponsesAPIWebSearchAction) isResponsesAPIWebSearchAction() {}

var (
	_ json.MarshalerTo     = RawResponsesAPIWebSearchAction{}
	_ json.UnmarshalerFrom = (*RawResponsesAPIWebSearchAction)(nil)
)

func (value RawResponsesAPIWebSearchAction) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawResponsesAPIWebSearchAction) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// SearchResponsesAPIWebSearchAction is generated from the SearchResponsesApiWebSearchAction schema definition.
type SearchResponsesAPIWebSearchAction struct {
	Queries []string `json:"queries,omitzero"`
	Query   string   `json:"query,omitzero"`
	Type    string   `json:"type"`
}

func (SearchResponsesAPIWebSearchAction) isResponsesAPIWebSearchAction() {}

// OpenPageResponsesAPIWebSearchAction is generated from the OpenPageResponsesApiWebSearchAction schema definition.
type OpenPageResponsesAPIWebSearchAction struct {
	Type string `json:"type"`
	URL  string `json:"url,omitzero"`
}

func (OpenPageResponsesAPIWebSearchAction) isResponsesAPIWebSearchAction() {}

// FindInPageResponsesAPIWebSearchAction is generated from the FindInPageResponsesApiWebSearchAction schema definition.
type FindInPageResponsesAPIWebSearchAction struct {
	Pattern string `json:"pattern,omitzero"`
	Type    string `json:"type"`
	URL     string `json:"url,omitzero"`
}

func (FindInPageResponsesAPIWebSearchAction) isResponsesAPIWebSearchAction() {}

// OtherResponsesAPIWebSearchAction is generated from the OtherResponsesApiWebSearchAction schema definition.
type OtherResponsesAPIWebSearchAction struct {
	Type string `json:"type"`
}

func (OtherResponsesAPIWebSearchAction) isResponsesAPIWebSearchAction() {}

func decodeGeneratedResponsesAPIWebSearchAction(raw jsontext.Value) (ResponsesAPIWebSearchAction, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "search":
			var value SearchResponsesAPIWebSearchAction
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "open_page":
			var value OpenPageResponsesAPIWebSearchAction
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "find_in_page":
			var value FindInPageResponsesAPIWebSearchAction
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "other":
			var value OtherResponsesAPIWebSearchAction
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawResponsesAPIWebSearchAction(raw), nil
	}
	return RawResponsesAPIWebSearchAction(raw), nil
}

// ReviewDelivery is generated from the ReviewDelivery schema definition.
type ReviewDelivery string

const (
	// ReviewDeliveryInline is the "inline" ReviewDelivery value.
	ReviewDeliveryInline ReviewDelivery = "inline"
	// ReviewDeliveryDetached is the "detached" ReviewDelivery value.
	ReviewDeliveryDetached ReviewDelivery = "detached"
)

// ReviewStartParams is generated from the ReviewStartParams schema definition.
type ReviewStartParams struct {
	// Delivery where to run the review: inline (default) on the current thread or detached on a new thread (returned in `reviewThreadId`).
	Delivery ReviewDelivery `json:"delivery,omitzero"`
	Target   ReviewTarget   `json:"target"`
	ThreadID string         `json:"threadId"`
}

func (value *ReviewStartParams) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Delivery ReviewDelivery `json:"delivery,omitzero"`
		Target   jsontext.Value `json:"target"`
		ThreadID string         `json:"threadId"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.Delivery = raw.Delivery
	if raw.Target == nil {
		value.Target = nil
	} else {
		decodedTarget, err := decodeGeneratedReviewTarget(raw.Target)
		if err != nil {
			return err
		}
		value.Target = decodedTarget
	}
	value.ThreadID = raw.ThreadID
	return nil
}

// ReviewStartResponse is generated from the ReviewStartResponse schema definition.
type ReviewStartResponse struct {
	// ReviewThreadID identifies the thread where the review runs.
	//
	// For inline reviews, this is the original thread id. For detached reviews, this is the id of the new review thread.
	ReviewThreadID string `json:"reviewThreadId"`
	Turn           Turn   `json:"turn"`
}

// ReviewTarget is generated from the ReviewTarget schema definition.
type ReviewTarget interface {
	isReviewTarget()
}

// RawReviewTarget preserves an uninterpreted ReviewTarget JSON value.
type RawReviewTarget jsontext.Value

func (RawReviewTarget) isReviewTarget() {}

var (
	_ json.MarshalerTo     = RawReviewTarget{}
	_ json.UnmarshalerFrom = (*RawReviewTarget)(nil)
)

func (value RawReviewTarget) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawReviewTarget) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// UncommittedChangesReviewTarget reviews the working tree: staged, unstaged, and untracked files.
type UncommittedChangesReviewTarget struct {
	Type string `json:"type"`
}

func (UncommittedChangesReviewTarget) isReviewTarget() {}

// BaseBranchReviewTarget reviews changes between the current branch and the given base branch.
type BaseBranchReviewTarget struct {
	Branch string `json:"branch"`
	Type   string `json:"type"`
}

func (BaseBranchReviewTarget) isReviewTarget() {}

// CommitReviewTarget reviews the changes introduced by a specific commit.
type CommitReviewTarget struct {
	Sha string `json:"sha"`

	// Title optional human-readable label (e.g., commit subject) for UIs.
	Title string `json:"title,omitzero"`
	Type  string `json:"type"`
}

func (CommitReviewTarget) isReviewTarget() {}

// CustomReviewTarget arbitrary instructions, equivalent to the old free-form prompt.
type CustomReviewTarget struct {
	Instructions string `json:"instructions"`
	Type         string `json:"type"`
}

func (CustomReviewTarget) isReviewTarget() {}

func decodeGeneratedReviewTarget(raw jsontext.Value) (ReviewTarget, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "uncommittedChanges":
			var value UncommittedChangesReviewTarget
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "baseBranch":
			var value BaseBranchReviewTarget
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "commit":
			var value CommitReviewTarget
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "custom":
			var value CustomReviewTarget
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawReviewTarget(raw), nil
	}
	return RawReviewTarget(raw), nil
}

// SandboxMode is generated from the SandboxMode schema definition.
type SandboxMode string

const (
	// SandboxModeReadOnly is the "read-only" SandboxMode value.
	SandboxModeReadOnly SandboxMode = "read-only"
	// SandboxModeWorkspaceWrite is the "workspace-write" SandboxMode value.
	SandboxModeWorkspaceWrite SandboxMode = "workspace-write"
	// SandboxModeDangerFullAccess is the "danger-full-access" SandboxMode value.
	SandboxModeDangerFullAccess SandboxMode = "danger-full-access"
)

// SandboxPolicy is generated from the SandboxPolicy schema definition.
type SandboxPolicy interface {
	isSandboxPolicy()
}

// RawSandboxPolicy preserves an uninterpreted SandboxPolicy JSON value.
type RawSandboxPolicy jsontext.Value

func (RawSandboxPolicy) isSandboxPolicy() {}

var (
	_ json.MarshalerTo     = RawSandboxPolicy{}
	_ json.UnmarshalerFrom = (*RawSandboxPolicy)(nil)
)

func (value RawSandboxPolicy) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawSandboxPolicy) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// DangerFullAccessSandboxPolicy is generated from the DangerFullAccessSandboxPolicy schema definition.
type DangerFullAccessSandboxPolicy struct {
	Type string `json:"type"`
}

func (DangerFullAccessSandboxPolicy) isSandboxPolicy() {}

// ReadOnlySandboxPolicy is generated from the ReadOnlySandboxPolicy schema definition.
type ReadOnlySandboxPolicy struct {
	NetworkAccess bool   `json:"networkAccess,omitzero"`
	Type          string `json:"type"`
}

func (ReadOnlySandboxPolicy) isSandboxPolicy() {}

// ExternalSandboxSandboxPolicy is generated from the ExternalSandboxSandboxPolicy schema definition.
type ExternalSandboxSandboxPolicy struct {
	NetworkAccess NetworkAccess `json:"networkAccess,omitzero"`
	Type          string        `json:"type"`
}

func (ExternalSandboxSandboxPolicy) isSandboxPolicy() {}

// WorkspaceWriteSandboxPolicy is generated from the WorkspaceWriteSandboxPolicy schema definition.
type WorkspaceWriteSandboxPolicy struct {
	ExcludeSlashTmp     bool     `json:"excludeSlashTmp,omitzero"`
	ExcludeTmpdirEnvVar bool     `json:"excludeTmpdirEnvVar,omitzero"`
	NetworkAccess       bool     `json:"networkAccess,omitzero"`
	Type                string   `json:"type"`
	WritableRoots       []string `json:"writableRoots,omitzero"`
}

func (WorkspaceWriteSandboxPolicy) isSandboxPolicy() {}

func decodeGeneratedSandboxPolicy(raw jsontext.Value) (SandboxPolicy, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "dangerFullAccess":
			var value DangerFullAccessSandboxPolicy
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "readOnly":
			var value ReadOnlySandboxPolicy
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "externalSandbox":
			var value ExternalSandboxSandboxPolicy
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "workspaceWrite":
			var value WorkspaceWriteSandboxPolicy
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawSandboxPolicy(raw), nil
	}
	return RawSandboxPolicy(raw), nil
}

// SandboxWorkspaceWrite is generated from the SandboxWorkspaceWrite schema definition.
type SandboxWorkspaceWrite struct {
	ExcludeSlashTmp     bool     `json:"exclude_slash_tmp,omitzero"`
	ExcludeTmpdirEnvVar bool     `json:"exclude_tmpdir_env_var,omitzero"`
	NetworkAccess       bool     `json:"network_access,omitzero"`
	WritableRoots       []string `json:"writable_roots,omitzero"`
}

// ScheduledTaskSchedule is generated from the ScheduledTaskSchedule schema definition.
type ScheduledTaskSchedule interface {
	isScheduledTaskSchedule()
}

// RawScheduledTaskSchedule preserves an uninterpreted ScheduledTaskSchedule JSON value.
type RawScheduledTaskSchedule jsontext.Value

func (RawScheduledTaskSchedule) isScheduledTaskSchedule() {}

var (
	_ json.MarshalerTo     = RawScheduledTaskSchedule{}
	_ json.UnmarshalerFrom = (*RawScheduledTaskSchedule)(nil)
)

func (value RawScheduledTaskSchedule) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawScheduledTaskSchedule) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// HourlyScheduledTaskSchedule is generated from the HourlyScheduledTaskSchedule schema definition.
type HourlyScheduledTaskSchedule struct {
	Days          []ScheduledTaskWeekday `json:"days,omitzero"`
	IntervalHours int32                  `json:"intervalHours"`
	Type          string                 `json:"type"`
}

func (HourlyScheduledTaskSchedule) isScheduledTaskSchedule() {}

// DailyScheduledTaskSchedule is generated from the DailyScheduledTaskSchedule schema definition.
type DailyScheduledTaskSchedule struct {
	Time string `json:"time"`
	Type string `json:"type"`
}

func (DailyScheduledTaskSchedule) isScheduledTaskSchedule() {}

// WeekdaysScheduledTaskSchedule is generated from the WeekdaysScheduledTaskSchedule schema definition.
type WeekdaysScheduledTaskSchedule struct {
	Time string `json:"time"`
	Type string `json:"type"`
}

func (WeekdaysScheduledTaskSchedule) isScheduledTaskSchedule() {}

// WeeklyScheduledTaskSchedule is generated from the WeeklyScheduledTaskSchedule schema definition.
type WeeklyScheduledTaskSchedule struct {
	Days []ScheduledTaskWeekday `json:"days"`
	Time string                 `json:"time"`
	Type string                 `json:"type"`
}

func (WeeklyScheduledTaskSchedule) isScheduledTaskSchedule() {}

func decodeGeneratedScheduledTaskSchedule(raw jsontext.Value) (ScheduledTaskSchedule, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "hourly":
			var value HourlyScheduledTaskSchedule
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "daily":
			var value DailyScheduledTaskSchedule
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "weekdays":
			var value WeekdaysScheduledTaskSchedule
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "weekly":
			var value WeeklyScheduledTaskSchedule
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawScheduledTaskSchedule(raw), nil
	}
	return RawScheduledTaskSchedule(raw), nil
}

// ScheduledTaskSummary is generated from the ScheduledTaskSummary schema definition.
type ScheduledTaskSummary struct {
	Key      string                `json:"key"`
	Name     string                `json:"name"`
	Prompt   string                `json:"prompt"`
	Schedule ScheduledTaskSchedule `json:"schedule"`
}

func (value *ScheduledTaskSummary) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Key      string         `json:"key"`
		Name     string         `json:"name"`
		Prompt   string         `json:"prompt"`
		Schedule jsontext.Value `json:"schedule"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.Key = raw.Key
	value.Name = raw.Name
	value.Prompt = raw.Prompt
	if raw.Schedule == nil {
		value.Schedule = nil
	} else {
		decodedSchedule, err := decodeGeneratedScheduledTaskSchedule(raw.Schedule)
		if err != nil {
			return err
		}
		value.Schedule = decodedSchedule
	}
	return nil
}

// ScheduledTaskWeekday is generated from the ScheduledTaskWeekday schema definition.
type ScheduledTaskWeekday string

const (
	// ScheduledTaskWeekdayMO is the "MO" ScheduledTaskWeekday value.
	ScheduledTaskWeekdayMO ScheduledTaskWeekday = "MO"
	// ScheduledTaskWeekdayTU is the "TU" ScheduledTaskWeekday value.
	ScheduledTaskWeekdayTU ScheduledTaskWeekday = "TU"
	// ScheduledTaskWeekdayWE is the "WE" ScheduledTaskWeekday value.
	ScheduledTaskWeekdayWE ScheduledTaskWeekday = "WE"
	// ScheduledTaskWeekdayTH is the "TH" ScheduledTaskWeekday value.
	ScheduledTaskWeekdayTH ScheduledTaskWeekday = "TH"
	// ScheduledTaskWeekdayFR is the "FR" ScheduledTaskWeekday value.
	ScheduledTaskWeekdayFR ScheduledTaskWeekday = "FR"
	// ScheduledTaskWeekdaySA is the "SA" ScheduledTaskWeekday value.
	ScheduledTaskWeekdaySA ScheduledTaskWeekday = "SA"
	// ScheduledTaskWeekdaySU is the "SU" ScheduledTaskWeekday value.
	ScheduledTaskWeekdaySU ScheduledTaskWeekday = "SU"
)

// SelectedCapabilityRoot represents an user-selected root that can expose one or more runtime capabilities.
type SelectedCapabilityRoot struct {
	// ID stable identifier supplied by the capability selection platform.
	ID string `json:"id"`

	// Location where the selected root can be resolved.
	Location CapabilityRootLocation `json:"location"`
}

// SendAddCreditsNudgeEmailParams is generated from the SendAddCreditsNudgeEmailParams schema definition.
type SendAddCreditsNudgeEmailParams struct {
	CreditType AddCreditsNudgeCreditType `json:"creditType"`
}

// SendAddCreditsNudgeEmailResponse is generated from the SendAddCreditsNudgeEmailResponse schema definition.
type SendAddCreditsNudgeEmailResponse struct {
	Status AddCreditsNudgeEmailStatus `json:"status"`
}

// ServerNotification notification sent from the server to the client.
type ServerNotification interface {
	isServerNotification()
}

// RawServerNotification preserves an uninterpreted ServerNotification JSON value.
type RawServerNotification jsontext.Value

func (RawServerNotification) isServerNotification() {}

var (
	_ json.MarshalerTo     = RawServerNotification{}
	_ json.UnmarshalerFrom = (*RawServerNotification)(nil)
)

func (value RawServerNotification) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawServerNotification) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

const (
	// NotificationMethodError is the "error" ServerNotification method.
	NotificationMethodError = "error"
	// NotificationMethodThreadStarted is the "thread/started" ServerNotification method.
	NotificationMethodThreadStarted = "thread/started"
	// NotificationMethodThreadStatusChanged is the "thread/status/changed" ServerNotification method.
	NotificationMethodThreadStatusChanged = "thread/status/changed"
	// NotificationMethodThreadArchived is the "thread/archived" ServerNotification method.
	NotificationMethodThreadArchived = "thread/archived"
	// NotificationMethodThreadDeleted is the "thread/deleted" ServerNotification method.
	NotificationMethodThreadDeleted = "thread/deleted"
	// NotificationMethodThreadUnarchived is the "thread/unarchived" ServerNotification method.
	NotificationMethodThreadUnarchived = "thread/unarchived"
	// NotificationMethodThreadClosed is the "thread/closed" ServerNotification method.
	NotificationMethodThreadClosed = "thread/closed"
	// NotificationMethodSkillsChanged is the "skills/changed" ServerNotification method.
	NotificationMethodSkillsChanged = "skills/changed"
	// NotificationMethodThreadNameUpdated is the "thread/name/updated" ServerNotification method.
	NotificationMethodThreadNameUpdated = "thread/name/updated"
	// NotificationMethodThreadGoalUpdated is the "thread/goal/updated" ServerNotification method.
	NotificationMethodThreadGoalUpdated = "thread/goal/updated"
	// NotificationMethodThreadGoalCleared is the "thread/goal/cleared" ServerNotification method.
	NotificationMethodThreadGoalCleared = "thread/goal/cleared"
	// NotificationMethodThreadEnvironmentConnected is the "thread/environment/connected" ServerNotification method.
	NotificationMethodThreadEnvironmentConnected = "thread/environment/connected"
	// NotificationMethodThreadEnvironmentDisconnected is the "thread/environment/disconnected" ServerNotification method.
	NotificationMethodThreadEnvironmentDisconnected = "thread/environment/disconnected"
	// NotificationMethodThreadSettingsUpdated is the "thread/settings/updated" ServerNotification method.
	NotificationMethodThreadSettingsUpdated = "thread/settings/updated"
	// NotificationMethodThreadTokenUsageUpdated is the "thread/tokenUsage/updated" ServerNotification method.
	NotificationMethodThreadTokenUsageUpdated = "thread/tokenUsage/updated"
	// NotificationMethodTurnStarted is the "turn/started" ServerNotification method.
	NotificationMethodTurnStarted = "turn/started"
	// NotificationMethodHookStarted is the "hook/started" ServerNotification method.
	NotificationMethodHookStarted = "hook/started"
	// NotificationMethodTurnCompleted is the "turn/completed" ServerNotification method.
	NotificationMethodTurnCompleted = "turn/completed"
	// NotificationMethodHookCompleted is the "hook/completed" ServerNotification method.
	NotificationMethodHookCompleted = "hook/completed"
	// NotificationMethodTurnDiffUpdated is the "turn/diff/updated" ServerNotification method.
	NotificationMethodTurnDiffUpdated = "turn/diff/updated"
	// NotificationMethodTurnPlanUpdated is the "turn/plan/updated" ServerNotification method.
	NotificationMethodTurnPlanUpdated = "turn/plan/updated"
	// NotificationMethodItemStarted is the "item/started" ServerNotification method.
	NotificationMethodItemStarted = "item/started"
	// NotificationMethodItemAutoApprovalReviewStarted is the "item/autoApprovalReview/started" ServerNotification method.
	NotificationMethodItemAutoApprovalReviewStarted = "item/autoApprovalReview/started"
	// NotificationMethodItemAutoApprovalReviewCompleted is the "item/autoApprovalReview/completed" ServerNotification method.
	NotificationMethodItemAutoApprovalReviewCompleted = "item/autoApprovalReview/completed"
	// NotificationMethodItemCompleted is the "item/completed" ServerNotification method.
	NotificationMethodItemCompleted = "item/completed"
	// NotificationMethodItemAgentMessageDelta is the "item/agentMessage/delta" ServerNotification method.
	NotificationMethodItemAgentMessageDelta = "item/agentMessage/delta"
	// NotificationMethodItemPlanDelta is the "item/plan/delta" ServerNotification method.
	NotificationMethodItemPlanDelta = "item/plan/delta"
	// NotificationMethodCommandExecOutputDelta is the "command/exec/outputDelta" ServerNotification method.
	NotificationMethodCommandExecOutputDelta = "command/exec/outputDelta"
	// NotificationMethodProcessOutputDelta is the "process/outputDelta" ServerNotification method.
	NotificationMethodProcessOutputDelta = "process/outputDelta"
	// NotificationMethodProcessExited is the "process/exited" ServerNotification method.
	NotificationMethodProcessExited = "process/exited"
	// NotificationMethodItemCommandExecutionOutputDelta is the "item/commandExecution/outputDelta" ServerNotification method.
	NotificationMethodItemCommandExecutionOutputDelta = "item/commandExecution/outputDelta"
	// NotificationMethodItemCommandExecutionTerminalInteraction is the "item/commandExecution/terminalInteraction" ServerNotification method.
	NotificationMethodItemCommandExecutionTerminalInteraction = "item/commandExecution/terminalInteraction"
	// NotificationMethodItemFileChangeOutputDelta is the "item/fileChange/outputDelta" ServerNotification method.
	NotificationMethodItemFileChangeOutputDelta = "item/fileChange/outputDelta"
	// NotificationMethodItemFileChangePatchUpdated is the "item/fileChange/patchUpdated" ServerNotification method.
	NotificationMethodItemFileChangePatchUpdated = "item/fileChange/patchUpdated"
	// NotificationMethodServerRequestResolved is the "serverRequest/resolved" ServerNotification method.
	NotificationMethodServerRequestResolved = "serverRequest/resolved"
	// NotificationMethodItemMCPToolCallProgress is the "item/mcpToolCall/progress" ServerNotification method.
	NotificationMethodItemMCPToolCallProgress = "item/mcpToolCall/progress"
	// NotificationMethodMCPServerOAuthLoginCompleted is the "mcpServer/oauthLogin/completed" ServerNotification method.
	NotificationMethodMCPServerOAuthLoginCompleted = "mcpServer/oauthLogin/completed"
	// NotificationMethodMCPServerStartupStatusUpdated is the "mcpServer/startupStatus/updated" ServerNotification method.
	NotificationMethodMCPServerStartupStatusUpdated = "mcpServer/startupStatus/updated"
	// NotificationMethodAccountUpdated is the "account/updated" ServerNotification method.
	NotificationMethodAccountUpdated = "account/updated"
	// NotificationMethodAccountRateLimitsUpdated is the "account/rateLimits/updated" ServerNotification method.
	NotificationMethodAccountRateLimitsUpdated = "account/rateLimits/updated"
	// NotificationMethodAppListUpdated is the "app/list/updated" ServerNotification method.
	NotificationMethodAppListUpdated = "app/list/updated"
	// NotificationMethodRemoteControlStatusChanged is the "remoteControl/status/changed" ServerNotification method.
	NotificationMethodRemoteControlStatusChanged = "remoteControl/status/changed"
	// NotificationMethodExternalAgentConfigImportProgress is the "externalAgentConfig/import/progress" ServerNotification method.
	NotificationMethodExternalAgentConfigImportProgress = "externalAgentConfig/import/progress"
	// NotificationMethodExternalAgentConfigImportCompleted is the "externalAgentConfig/import/completed" ServerNotification method.
	NotificationMethodExternalAgentConfigImportCompleted = "externalAgentConfig/import/completed"
	// NotificationMethodFSChanged is the "fs/changed" ServerNotification method.
	NotificationMethodFSChanged = "fs/changed"
	// NotificationMethodItemReasoningSummaryTextDelta is the "item/reasoning/summaryTextDelta" ServerNotification method.
	NotificationMethodItemReasoningSummaryTextDelta = "item/reasoning/summaryTextDelta"
	// NotificationMethodItemReasoningSummaryPartAdded is the "item/reasoning/summaryPartAdded" ServerNotification method.
	NotificationMethodItemReasoningSummaryPartAdded = "item/reasoning/summaryPartAdded"
	// NotificationMethodItemReasoningTextDelta is the "item/reasoning/textDelta" ServerNotification method.
	NotificationMethodItemReasoningTextDelta = "item/reasoning/textDelta"
	// NotificationMethodThreadCompacted is the "thread/compacted" ServerNotification method.
	NotificationMethodThreadCompacted = "thread/compacted"
	// NotificationMethodModelRerouted is the "model/rerouted" ServerNotification method.
	NotificationMethodModelRerouted = "model/rerouted"
	// NotificationMethodModelVerification is the "model/verification" ServerNotification method.
	NotificationMethodModelVerification = "model/verification"
	// NotificationMethodTurnModerationMetadata is the "turn/moderationMetadata" ServerNotification method.
	NotificationMethodTurnModerationMetadata = "turn/moderationMetadata"
	// NotificationMethodModelSafetyBufferingUpdated is the "model/safetyBuffering/updated" ServerNotification method.
	NotificationMethodModelSafetyBufferingUpdated = "model/safetyBuffering/updated"
	// NotificationMethodWarning is the "warning" ServerNotification method.
	NotificationMethodWarning = "warning"
	// NotificationMethodGuardianWarning is the "guardianWarning" ServerNotification method.
	NotificationMethodGuardianWarning = "guardianWarning"
	// NotificationMethodDeprecationNotice is the "deprecationNotice" ServerNotification method.
	NotificationMethodDeprecationNotice = "deprecationNotice"
	// NotificationMethodConfigWarning is the "configWarning" ServerNotification method.
	NotificationMethodConfigWarning = "configWarning"
	// NotificationMethodFuzzyFileSearchSessionUpdated is the "fuzzyFileSearch/sessionUpdated" ServerNotification method.
	NotificationMethodFuzzyFileSearchSessionUpdated = "fuzzyFileSearch/sessionUpdated"
	// NotificationMethodFuzzyFileSearchSessionCompleted is the "fuzzyFileSearch/sessionCompleted" ServerNotification method.
	NotificationMethodFuzzyFileSearchSessionCompleted = "fuzzyFileSearch/sessionCompleted"
	// NotificationMethodThreadRealtimeStarted is the "thread/realtime/started" ServerNotification method.
	NotificationMethodThreadRealtimeStarted = "thread/realtime/started"
	// NotificationMethodThreadRealtimeItemAdded is the "thread/realtime/itemAdded" ServerNotification method.
	NotificationMethodThreadRealtimeItemAdded = "thread/realtime/itemAdded"
	// NotificationMethodThreadRealtimeTranscriptDelta is the "thread/realtime/transcript/delta" ServerNotification method.
	NotificationMethodThreadRealtimeTranscriptDelta = "thread/realtime/transcript/delta"
	// NotificationMethodThreadRealtimeTranscriptDone is the "thread/realtime/transcript/done" ServerNotification method.
	NotificationMethodThreadRealtimeTranscriptDone = "thread/realtime/transcript/done"
	// NotificationMethodThreadRealtimeOutputAudioDelta is the "thread/realtime/outputAudio/delta" ServerNotification method.
	NotificationMethodThreadRealtimeOutputAudioDelta = "thread/realtime/outputAudio/delta"
	// NotificationMethodThreadRealtimeSDP is the "thread/realtime/sdp" ServerNotification method.
	NotificationMethodThreadRealtimeSDP = "thread/realtime/sdp"
	// NotificationMethodThreadRealtimeError is the "thread/realtime/error" ServerNotification method.
	NotificationMethodThreadRealtimeError = "thread/realtime/error"
	// NotificationMethodThreadRealtimeClosed is the "thread/realtime/closed" ServerNotification method.
	NotificationMethodThreadRealtimeClosed = "thread/realtime/closed"
	// NotificationMethodWindowsWorldWritableWarning is the "windows/worldWritableWarning" ServerNotification method.
	NotificationMethodWindowsWorldWritableWarning = "windows/worldWritableWarning"
	// NotificationMethodWindowsSandboxSetupCompleted is the "windowsSandbox/setupCompleted" ServerNotification method.
	NotificationMethodWindowsSandboxSetupCompleted = "windowsSandbox/setupCompleted"
	// NotificationMethodAccountLoginCompleted is the "account/login/completed" ServerNotification method.
	NotificationMethodAccountLoginCompleted = "account/login/completed"
)

func (ErrorNotification) isServerNotification() {}

// ThreadStartedNotification2 is generated from the Thread/startedNotification schema definition.
type ThreadStartedNotification2 struct {
	Method string                    `json:"method"`
	Params ThreadStartedNotification `json:"params"`
}

func (ThreadStartedNotification2) isServerNotification() {}

// ThreadStatusChangedNotification2 is generated from the Thread/status/changedNotification schema definition.
type ThreadStatusChangedNotification2 struct {
	Method string                          `json:"method"`
	Params ThreadStatusChangedNotification `json:"params"`
}

func (ThreadStatusChangedNotification2) isServerNotification() {}

// ThreadArchivedNotification2 is generated from the Thread/archivedNotification schema definition.
type ThreadArchivedNotification2 struct {
	Method string                     `json:"method"`
	Params ThreadArchivedNotification `json:"params"`
}

func (ThreadArchivedNotification2) isServerNotification() {}

// ThreadDeletedNotification2 is generated from the Thread/deletedNotification schema definition.
type ThreadDeletedNotification2 struct {
	Method string                    `json:"method"`
	Params ThreadDeletedNotification `json:"params"`
}

func (ThreadDeletedNotification2) isServerNotification() {}

// ThreadUnarchivedNotification2 is generated from the Thread/unarchivedNotification schema definition.
type ThreadUnarchivedNotification2 struct {
	Method string                       `json:"method"`
	Params ThreadUnarchivedNotification `json:"params"`
}

func (ThreadUnarchivedNotification2) isServerNotification() {}

// ThreadClosedNotification2 is generated from the Thread/closedNotification schema definition.
type ThreadClosedNotification2 struct {
	Method string                   `json:"method"`
	Params ThreadClosedNotification `json:"params"`
}

func (ThreadClosedNotification2) isServerNotification() {}

// SkillsChangedNotification2 is generated from the Skills/changedNotification schema definition.
type SkillsChangedNotification2 struct {
	Method string                    `json:"method"`
	Params SkillsChangedNotification `json:"params"`
}

func (SkillsChangedNotification2) isServerNotification() {}

// ThreadNameUpdatedNotification2 is generated from the Thread/name/updatedNotification schema definition.
type ThreadNameUpdatedNotification2 struct {
	Method string                        `json:"method"`
	Params ThreadNameUpdatedNotification `json:"params"`
}

func (ThreadNameUpdatedNotification2) isServerNotification() {}

// ThreadGoalUpdatedNotification2 is generated from the Thread/goal/updatedNotification schema definition.
type ThreadGoalUpdatedNotification2 struct {
	Method string                        `json:"method"`
	Params ThreadGoalUpdatedNotification `json:"params"`
}

func (ThreadGoalUpdatedNotification2) isServerNotification() {}

// ThreadGoalClearedNotification2 is generated from the Thread/goal/clearedNotification schema definition.
type ThreadGoalClearedNotification2 struct {
	Method string                        `json:"method"`
	Params ThreadGoalClearedNotification `json:"params"`
}

func (ThreadGoalClearedNotification2) isServerNotification() {}

// ThreadEnvironmentConnectedNotification is generated from the Thread/environment/connectedNotification schema definition.
type ThreadEnvironmentConnectedNotification struct {
	Method string                            `json:"method"`
	Params EnvironmentConnectionNotification `json:"params"`
}

func (ThreadEnvironmentConnectedNotification) isServerNotification() {}

// ThreadEnvironmentDisconnectedNotification is generated from the Thread/environment/disconnectedNotification schema definition.
type ThreadEnvironmentDisconnectedNotification struct {
	Method string                            `json:"method"`
	Params EnvironmentConnectionNotification `json:"params"`
}

func (ThreadEnvironmentDisconnectedNotification) isServerNotification() {}

// ThreadSettingsUpdatedNotification2 is generated from the Thread/settings/updatedNotification schema definition.
type ThreadSettingsUpdatedNotification2 struct {
	Method string                            `json:"method"`
	Params ThreadSettingsUpdatedNotification `json:"params"`
}

func (ThreadSettingsUpdatedNotification2) isServerNotification() {}

// ThreadTokenUsageUpdatedNotification2 is generated from the Thread/tokenUsage/updatedNotification schema definition.
type ThreadTokenUsageUpdatedNotification2 struct {
	Method string                              `json:"method"`
	Params ThreadTokenUsageUpdatedNotification `json:"params"`
}

func (ThreadTokenUsageUpdatedNotification2) isServerNotification() {}

// TurnStartedNotification2 is generated from the Turn/startedNotification schema definition.
type TurnStartedNotification2 struct {
	Method string                  `json:"method"`
	Params TurnStartedNotification `json:"params"`
}

func (TurnStartedNotification2) isServerNotification() {}

// HookStartedNotification2 is generated from the Hook/startedNotification schema definition.
type HookStartedNotification2 struct {
	Method string                  `json:"method"`
	Params HookStartedNotification `json:"params"`
}

func (HookStartedNotification2) isServerNotification() {}

// TurnCompletedNotification2 is generated from the Turn/completedNotification schema definition.
type TurnCompletedNotification2 struct {
	Method string                    `json:"method"`
	Params TurnCompletedNotification `json:"params"`
}

func (TurnCompletedNotification2) isServerNotification() {}

// HookCompletedNotification2 is generated from the Hook/completedNotification schema definition.
type HookCompletedNotification2 struct {
	Method string                    `json:"method"`
	Params HookCompletedNotification `json:"params"`
}

func (HookCompletedNotification2) isServerNotification() {}

// TurnDiffUpdatedNotification2 is generated from the Turn/diff/updatedNotification schema definition.
type TurnDiffUpdatedNotification2 struct {
	Method string                      `json:"method"`
	Params TurnDiffUpdatedNotification `json:"params"`
}

func (TurnDiffUpdatedNotification2) isServerNotification() {}

// TurnPlanUpdatedNotification2 is generated from the Turn/plan/updatedNotification schema definition.
type TurnPlanUpdatedNotification2 struct {
	Method string                      `json:"method"`
	Params TurnPlanUpdatedNotification `json:"params"`
}

func (TurnPlanUpdatedNotification2) isServerNotification() {}

// ItemStartedNotification2 is generated from the Item/startedNotification schema definition.
type ItemStartedNotification2 struct {
	Method string                  `json:"method"`
	Params ItemStartedNotification `json:"params"`
}

func (ItemStartedNotification2) isServerNotification() {}

// ItemAutoApprovalReviewStartedNotification is generated from the Item/autoApprovalReview/startedNotification schema definition.
type ItemAutoApprovalReviewStartedNotification struct {
	Method string                                        `json:"method"`
	Params ItemGuardianApprovalReviewStartedNotification `json:"params"`
}

func (ItemAutoApprovalReviewStartedNotification) isServerNotification() {}

// ItemAutoApprovalReviewCompletedNotification is generated from the Item/autoApprovalReview/completedNotification schema definition.
type ItemAutoApprovalReviewCompletedNotification struct {
	Method string                                          `json:"method"`
	Params ItemGuardianApprovalReviewCompletedNotification `json:"params"`
}

func (ItemAutoApprovalReviewCompletedNotification) isServerNotification() {}

// ItemCompletedNotification2 is generated from the Item/completedNotification schema definition.
type ItemCompletedNotification2 struct {
	Method string                    `json:"method"`
	Params ItemCompletedNotification `json:"params"`
}

func (ItemCompletedNotification2) isServerNotification() {}

// ItemAgentMessageDeltaNotification is generated from the Item/agentMessage/deltaNotification schema definition.
type ItemAgentMessageDeltaNotification struct {
	Method string                        `json:"method"`
	Params AgentMessageDeltaNotification `json:"params"`
}

func (ItemAgentMessageDeltaNotification) isServerNotification() {}

// ItemPlanDeltaNotification proposed plan streaming deltas for plan items.
type ItemPlanDeltaNotification struct {
	Method string                `json:"method"`
	Params PlanDeltaNotification `json:"params"`
}

func (ItemPlanDeltaNotification) isServerNotification() {}

// CommandExecOutputDeltaNotification2 streams base64-encoded stdout/stderr chunks for a running `command/exec` session.
type CommandExecOutputDeltaNotification2 struct {
	Method string                             `json:"method"`
	Params CommandExecOutputDeltaNotification `json:"params"`
}

func (CommandExecOutputDeltaNotification2) isServerNotification() {}

// ProcessOutputDeltaNotification2 streams base64-encoded stdout/stderr chunks for a running `process/spawn` session.
type ProcessOutputDeltaNotification2 struct {
	Method string                         `json:"method"`
	Params ProcessOutputDeltaNotification `json:"params"`
}

func (ProcessOutputDeltaNotification2) isServerNotification() {}

// ProcessExitedNotification2 final exit notification for a `process/spawn` session.
type ProcessExitedNotification2 struct {
	Method string                    `json:"method"`
	Params ProcessExitedNotification `json:"params"`
}

func (ProcessExitedNotification2) isServerNotification() {}

// ItemCommandExecutionOutputDeltaNotification is generated from the Item/commandExecution/outputDeltaNotification schema definition.
type ItemCommandExecutionOutputDeltaNotification struct {
	Method string                                  `json:"method"`
	Params CommandExecutionOutputDeltaNotification `json:"params"`
}

func (ItemCommandExecutionOutputDeltaNotification) isServerNotification() {}

// ItemCommandExecutionTerminalInteractionNotification is generated from the Item/commandExecution/terminalInteractionNotification schema definition.
type ItemCommandExecutionTerminalInteractionNotification struct {
	Method string                          `json:"method"`
	Params TerminalInteractionNotification `json:"params"`
}

func (ItemCommandExecutionTerminalInteractionNotification) isServerNotification() {}

// ItemFileChangeOutputDeltaNotification deprecated legacy apply_patch output stream notification.
type ItemFileChangeOutputDeltaNotification struct {
	Method string                            `json:"method"`
	Params FileChangeOutputDeltaNotification `json:"params"`
}

func (ItemFileChangeOutputDeltaNotification) isServerNotification() {}

// ItemFileChangePatchUpdatedNotification is generated from the Item/fileChange/patchUpdatedNotification schema definition.
type ItemFileChangePatchUpdatedNotification struct {
	Method string                             `json:"method"`
	Params FileChangePatchUpdatedNotification `json:"params"`
}

func (ItemFileChangePatchUpdatedNotification) isServerNotification() {}

// ServerRequestResolvedNotification2 is generated from the ServerRequest/resolvedNotification schema definition.
type ServerRequestResolvedNotification2 struct {
	Method string                            `json:"method"`
	Params ServerRequestResolvedNotification `json:"params"`
}

func (ServerRequestResolvedNotification2) isServerNotification() {}

// ItemMCPToolCallProgressNotification is generated from the Item/mcpToolCall/progressNotification schema definition.
type ItemMCPToolCallProgressNotification struct {
	Method string                          `json:"method"`
	Params MCPToolCallProgressNotification `json:"params"`
}

func (ItemMCPToolCallProgressNotification) isServerNotification() {}

// MCPServerOAuthLoginCompletedNotification2 is generated from the McpServer/oauthLogin/completedNotification schema definition.
type MCPServerOAuthLoginCompletedNotification2 struct {
	Method string                                   `json:"method"`
	Params MCPServerOAuthLoginCompletedNotification `json:"params"`
}

func (MCPServerOAuthLoginCompletedNotification2) isServerNotification() {}

// MCPServerStartupStatusUpdatedNotification is generated from the McpServer/startupStatus/updatedNotification schema definition.
type MCPServerStartupStatusUpdatedNotification struct {
	Method string                             `json:"method"`
	Params MCPServerStatusUpdatedNotification `json:"params"`
}

func (MCPServerStartupStatusUpdatedNotification) isServerNotification() {}

// AccountUpdatedNotification2 is generated from the Account/updatedNotification schema definition.
type AccountUpdatedNotification2 struct {
	Method string                     `json:"method"`
	Params AccountUpdatedNotification `json:"params"`
}

func (AccountUpdatedNotification2) isServerNotification() {}

// AccountRateLimitsUpdatedNotification2 is generated from the Account/rateLimits/updatedNotification schema definition.
type AccountRateLimitsUpdatedNotification2 struct {
	Method string                               `json:"method"`
	Params AccountRateLimitsUpdatedNotification `json:"params"`
}

func (AccountRateLimitsUpdatedNotification2) isServerNotification() {}

// AppListUpdatedNotification2 is generated from the App/list/updatedNotification schema definition.
type AppListUpdatedNotification2 struct {
	Method string                     `json:"method"`
	Params AppListUpdatedNotification `json:"params"`
}

func (AppListUpdatedNotification2) isServerNotification() {}

// RemoteControlStatusChangedNotification2 is generated from the RemoteControl/status/changedNotification schema definition.
type RemoteControlStatusChangedNotification2 struct {
	Method string                                 `json:"method"`
	Params RemoteControlStatusChangedNotification `json:"params"`
}

func (RemoteControlStatusChangedNotification2) isServerNotification() {}

// ExternalAgentConfigImportProgressNotification2 is generated from the ExternalAgentConfig/import/progressNotification schema definition.
type ExternalAgentConfigImportProgressNotification2 struct {
	Method string                                        `json:"method"`
	Params ExternalAgentConfigImportProgressNotification `json:"params"`
}

func (ExternalAgentConfigImportProgressNotification2) isServerNotification() {}

// ExternalAgentConfigImportCompletedNotification2 is generated from the ExternalAgentConfig/import/completedNotification schema definition.
type ExternalAgentConfigImportCompletedNotification2 struct {
	Method string                                         `json:"method"`
	Params ExternalAgentConfigImportCompletedNotification `json:"params"`
}

func (ExternalAgentConfigImportCompletedNotification2) isServerNotification() {}

// FSChangedNotification2 is generated from the Fs/changedNotification schema definition.
type FSChangedNotification2 struct {
	Method string                `json:"method"`
	Params FSChangedNotification `json:"params"`
}

func (FSChangedNotification2) isServerNotification() {}

// ItemReasoningSummaryTextDeltaNotification is generated from the Item/reasoning/summaryTextDeltaNotification schema definition.
type ItemReasoningSummaryTextDeltaNotification struct {
	Method string                                `json:"method"`
	Params ReasoningSummaryTextDeltaNotification `json:"params"`
}

func (ItemReasoningSummaryTextDeltaNotification) isServerNotification() {}

// ItemReasoningSummaryPartAddedNotification is generated from the Item/reasoning/summaryPartAddedNotification schema definition.
type ItemReasoningSummaryPartAddedNotification struct {
	Method string                                `json:"method"`
	Params ReasoningSummaryPartAddedNotification `json:"params"`
}

func (ItemReasoningSummaryPartAddedNotification) isServerNotification() {}

// ItemReasoningTextDeltaNotification is generated from the Item/reasoning/textDeltaNotification schema definition.
type ItemReasoningTextDeltaNotification struct {
	Method string                         `json:"method"`
	Params ReasoningTextDeltaNotification `json:"params"`
}

func (ItemReasoningTextDeltaNotification) isServerNotification() {}

// ThreadCompactedNotification deprecated: Use `ContextCompaction` item type instead.
type ThreadCompactedNotification struct {
	Method string                       `json:"method"`
	Params ContextCompactedNotification `json:"params"`
}

func (ThreadCompactedNotification) isServerNotification() {}

// ModelReroutedNotification2 is generated from the Model/reroutedNotification schema definition.
type ModelReroutedNotification2 struct {
	Method string                    `json:"method"`
	Params ModelReroutedNotification `json:"params"`
}

func (ModelReroutedNotification2) isServerNotification() {}

// ModelVerificationNotification2 is generated from the Model/verificationNotification schema definition.
type ModelVerificationNotification2 struct {
	Method string                        `json:"method"`
	Params ModelVerificationNotification `json:"params"`
}

func (ModelVerificationNotification2) isServerNotification() {}

// TurnModerationMetadataNotification2 is generated from the Turn/moderationMetadataNotification schema definition.
type TurnModerationMetadataNotification2 struct {
	Method string                             `json:"method"`
	Params TurnModerationMetadataNotification `json:"params"`
}

func (TurnModerationMetadataNotification2) isServerNotification() {}

// ModelSafetyBufferingUpdatedNotification2 is generated from the Model/safetyBuffering/updatedNotification schema definition.
type ModelSafetyBufferingUpdatedNotification2 struct {
	Method string                                  `json:"method"`
	Params ModelSafetyBufferingUpdatedNotification `json:"params"`
}

func (ModelSafetyBufferingUpdatedNotification2) isServerNotification() {}

func (GuardianWarningNotification) isServerNotification() {}

func (DeprecationNoticeNotification) isServerNotification() {}

func (ConfigWarningNotification) isServerNotification() {}

// FuzzyFileSearchSessionUpdatedNotification2 is generated from the FuzzyFileSearch/sessionUpdatedNotification schema definition.
type FuzzyFileSearchSessionUpdatedNotification2 struct {
	Method string                                    `json:"method"`
	Params FuzzyFileSearchSessionUpdatedNotification `json:"params"`
}

func (FuzzyFileSearchSessionUpdatedNotification2) isServerNotification() {}

// FuzzyFileSearchSessionCompletedNotification2 is generated from the FuzzyFileSearch/sessionCompletedNotification schema definition.
type FuzzyFileSearchSessionCompletedNotification2 struct {
	Method string                                      `json:"method"`
	Params FuzzyFileSearchSessionCompletedNotification `json:"params"`
}

func (FuzzyFileSearchSessionCompletedNotification2) isServerNotification() {}

// ThreadRealtimeStartedNotification2 is generated from the Thread/realtime/startedNotification schema definition.
type ThreadRealtimeStartedNotification2 struct {
	Method string                            `json:"method"`
	Params ThreadRealtimeStartedNotification `json:"params"`
}

func (ThreadRealtimeStartedNotification2) isServerNotification() {}

// ThreadRealtimeItemAddedNotification2 is generated from the Thread/realtime/itemAddedNotification schema definition.
type ThreadRealtimeItemAddedNotification2 struct {
	Method string                              `json:"method"`
	Params ThreadRealtimeItemAddedNotification `json:"params"`
}

func (ThreadRealtimeItemAddedNotification2) isServerNotification() {}

// ThreadRealtimeTranscriptDeltaNotification2 is generated from the Thread/realtime/transcript/deltaNotification schema definition.
type ThreadRealtimeTranscriptDeltaNotification2 struct {
	Method string                                    `json:"method"`
	Params ThreadRealtimeTranscriptDeltaNotification `json:"params"`
}

func (ThreadRealtimeTranscriptDeltaNotification2) isServerNotification() {}

// ThreadRealtimeTranscriptDoneNotification2 is generated from the Thread/realtime/transcript/doneNotification schema definition.
type ThreadRealtimeTranscriptDoneNotification2 struct {
	Method string                                   `json:"method"`
	Params ThreadRealtimeTranscriptDoneNotification `json:"params"`
}

func (ThreadRealtimeTranscriptDoneNotification2) isServerNotification() {}

// ThreadRealtimeOutputAudioDeltaNotification2 is generated from the Thread/realtime/outputAudio/deltaNotification schema definition.
type ThreadRealtimeOutputAudioDeltaNotification2 struct {
	Method string                                     `json:"method"`
	Params ThreadRealtimeOutputAudioDeltaNotification `json:"params"`
}

func (ThreadRealtimeOutputAudioDeltaNotification2) isServerNotification() {}

// ThreadRealtimeSDPNotification2 is generated from the Thread/realtime/sdpNotification schema definition.
type ThreadRealtimeSDPNotification2 struct {
	Method string                        `json:"method"`
	Params ThreadRealtimeSDPNotification `json:"params"`
}

func (ThreadRealtimeSDPNotification2) isServerNotification() {}

// ThreadRealtimeErrorNotification2 is generated from the Thread/realtime/errorNotification schema definition.
type ThreadRealtimeErrorNotification2 struct {
	Method string                          `json:"method"`
	Params ThreadRealtimeErrorNotification `json:"params"`
}

func (ThreadRealtimeErrorNotification2) isServerNotification() {}

// ThreadRealtimeClosedNotification2 is generated from the Thread/realtime/closedNotification schema definition.
type ThreadRealtimeClosedNotification2 struct {
	Method string                           `json:"method"`
	Params ThreadRealtimeClosedNotification `json:"params"`
}

func (ThreadRealtimeClosedNotification2) isServerNotification() {}

// WindowsWorldWritableWarningNotification2 notifies the user of world-writable directories on Windows, which cannot be protected by the sandbox.
type WindowsWorldWritableWarningNotification2 struct {
	Method string                                  `json:"method"`
	Params WindowsWorldWritableWarningNotification `json:"params"`
}

func (WindowsWorldWritableWarningNotification2) isServerNotification() {}

// WindowsSandboxSetupCompletedNotification2 is generated from the WindowsSandbox/setupCompletedNotification schema definition.
type WindowsSandboxSetupCompletedNotification2 struct {
	Method string                                   `json:"method"`
	Params WindowsSandboxSetupCompletedNotification `json:"params"`
}

func (WindowsSandboxSetupCompletedNotification2) isServerNotification() {}

// AccountLoginCompletedNotification2 is generated from the Account/login/completedNotification schema definition.
type AccountLoginCompletedNotification2 struct {
	Method string                            `json:"method"`
	Params AccountLoginCompletedNotification `json:"params"`
}

func (AccountLoginCompletedNotification2) isServerNotification() {}

func decodeGeneratedServerNotification(raw jsontext.Value) (ServerNotification, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Method string `json:"method"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Method {
		case NotificationMethodError:
			var value ErrorNotification
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodThreadStarted:
			var value ThreadStartedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodThreadStatusChanged:
			var value ThreadStatusChangedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodThreadArchived:
			var value ThreadArchivedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodThreadDeleted:
			var value ThreadDeletedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodThreadUnarchived:
			var value ThreadUnarchivedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodThreadClosed:
			var value ThreadClosedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodSkillsChanged:
			var value SkillsChangedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodThreadNameUpdated:
			var value ThreadNameUpdatedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodThreadGoalUpdated:
			var value ThreadGoalUpdatedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodThreadGoalCleared:
			var value ThreadGoalClearedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodThreadEnvironmentConnected:
			var value ThreadEnvironmentConnectedNotification
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodThreadEnvironmentDisconnected:
			var value ThreadEnvironmentDisconnectedNotification
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodThreadSettingsUpdated:
			var value ThreadSettingsUpdatedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodThreadTokenUsageUpdated:
			var value ThreadTokenUsageUpdatedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodTurnStarted:
			var value TurnStartedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodHookStarted:
			var value HookStartedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodTurnCompleted:
			var value TurnCompletedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodHookCompleted:
			var value HookCompletedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodTurnDiffUpdated:
			var value TurnDiffUpdatedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodTurnPlanUpdated:
			var value TurnPlanUpdatedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodItemStarted:
			var value ItemStartedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodItemAutoApprovalReviewStarted:
			var value ItemAutoApprovalReviewStartedNotification
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodItemAutoApprovalReviewCompleted:
			var value ItemAutoApprovalReviewCompletedNotification
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodItemCompleted:
			var value ItemCompletedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodItemAgentMessageDelta:
			var value ItemAgentMessageDeltaNotification
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodItemPlanDelta:
			var value ItemPlanDeltaNotification
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodCommandExecOutputDelta:
			var value CommandExecOutputDeltaNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodProcessOutputDelta:
			var value ProcessOutputDeltaNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodProcessExited:
			var value ProcessExitedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodItemCommandExecutionOutputDelta:
			var value ItemCommandExecutionOutputDeltaNotification
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodItemCommandExecutionTerminalInteraction:
			var value ItemCommandExecutionTerminalInteractionNotification
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodItemFileChangeOutputDelta:
			var value ItemFileChangeOutputDeltaNotification
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodItemFileChangePatchUpdated:
			var value ItemFileChangePatchUpdatedNotification
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodServerRequestResolved:
			var value ServerRequestResolvedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodItemMCPToolCallProgress:
			var value ItemMCPToolCallProgressNotification
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodMCPServerOAuthLoginCompleted:
			var value MCPServerOAuthLoginCompletedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodMCPServerStartupStatusUpdated:
			var value MCPServerStartupStatusUpdatedNotification
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodAccountUpdated:
			var value AccountUpdatedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodAccountRateLimitsUpdated:
			var value AccountRateLimitsUpdatedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodAppListUpdated:
			var value AppListUpdatedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodRemoteControlStatusChanged:
			var value RemoteControlStatusChangedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodExternalAgentConfigImportProgress:
			var value ExternalAgentConfigImportProgressNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodExternalAgentConfigImportCompleted:
			var value ExternalAgentConfigImportCompletedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodFSChanged:
			var value FSChangedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodItemReasoningSummaryTextDelta:
			var value ItemReasoningSummaryTextDeltaNotification
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodItemReasoningSummaryPartAdded:
			var value ItemReasoningSummaryPartAddedNotification
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodItemReasoningTextDelta:
			var value ItemReasoningTextDeltaNotification
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodThreadCompacted:
			var value ThreadCompactedNotification
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodModelRerouted:
			var value ModelReroutedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodModelVerification:
			var value ModelVerificationNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodTurnModerationMetadata:
			var value TurnModerationMetadataNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodModelSafetyBufferingUpdated:
			var value ModelSafetyBufferingUpdatedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodWarning:
			var value WarningNotification
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodGuardianWarning:
			var value GuardianWarningNotification
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodDeprecationNotice:
			var value DeprecationNoticeNotification
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodConfigWarning:
			var value ConfigWarningNotification
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodFuzzyFileSearchSessionUpdated:
			var value FuzzyFileSearchSessionUpdatedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodFuzzyFileSearchSessionCompleted:
			var value FuzzyFileSearchSessionCompletedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodThreadRealtimeStarted:
			var value ThreadRealtimeStartedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodThreadRealtimeItemAdded:
			var value ThreadRealtimeItemAddedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodThreadRealtimeTranscriptDelta:
			var value ThreadRealtimeTranscriptDeltaNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodThreadRealtimeTranscriptDone:
			var value ThreadRealtimeTranscriptDoneNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodThreadRealtimeOutputAudioDelta:
			var value ThreadRealtimeOutputAudioDeltaNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodThreadRealtimeSDP:
			var value ThreadRealtimeSDPNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodThreadRealtimeError:
			var value ThreadRealtimeErrorNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodThreadRealtimeClosed:
			var value ThreadRealtimeClosedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodWindowsWorldWritableWarning:
			var value WindowsWorldWritableWarningNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodWindowsSandboxSetupCompleted:
			var value WindowsSandboxSetupCompletedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case NotificationMethodAccountLoginCompleted:
			var value AccountLoginCompletedNotification2
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawServerNotification(raw), nil
	}
	return RawServerNotification(raw), nil
}

// ServerRequestResolvedNotification is generated from the ServerRequestResolvedNotification schema definition.
type ServerRequestResolvedNotification struct {
	RequestID string `json:"requestId"`
	ThreadID  string `json:"threadId"`
}

// SessionMigration is generated from the SessionMigration schema definition.
type SessionMigration struct {
	Cwd   string `json:"cwd"`
	Path  string `json:"path"`
	Title string `json:"title,omitzero"`
}

// SessionSource is generated from the SessionSource schema definition.
type SessionSource interface {
	isSessionSource()
}

// RawSessionSource preserves an uninterpreted SessionSource JSON value.
type RawSessionSource jsontext.Value

func (RawSessionSource) isSessionSource() {}

var (
	_ json.MarshalerTo     = RawSessionSource{}
	_ json.UnmarshalerFrom = (*RawSessionSource)(nil)
)

func (value RawSessionSource) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawSessionSource) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// SessionSourceValue is a string-valued SessionSource variant.
type SessionSourceValue string

func (SessionSourceValue) isSessionSource() {}

const (
	// SessionSourceValueAppServer is the "appServer" SessionSourceValue value.
	SessionSourceValueAppServer SessionSourceValue = "appServer"
	// SessionSourceValueCli is the "cli" SessionSourceValue value.
	SessionSourceValueCli SessionSourceValue = "cli"
	// SessionSourceValueExec is the "exec" SessionSourceValue value.
	SessionSourceValueExec SessionSourceValue = "exec"
	// SessionSourceValueUnknown is the "unknown" SessionSourceValue value.
	SessionSourceValueUnknown SessionSourceValue = "unknown"
	// SessionSourceValueVscode is the "vscode" SessionSourceValue value.
	SessionSourceValueVscode SessionSourceValue = "vscode"
)

// CustomSessionSource is generated from the CustomSessionSource schema definition.
type CustomSessionSource struct {
	Custom string `json:"custom"`
}

func (CustomSessionSource) isSessionSource() {}

// SubAgentSessionSource is generated from the SubAgentSessionSource schema definition.
type SubAgentSessionSource struct {
	SubAgent SubAgentSource `json:"subAgent"`
}

func (value *SubAgentSessionSource) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		SubAgent jsontext.Value `json:"subAgent"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	if raw.SubAgent == nil {
		value.SubAgent = nil
	} else {
		decodedSubAgent, err := decodeGeneratedSubAgentSource(raw.SubAgent)
		if err != nil {
			return err
		}
		value.SubAgent = decodedSubAgent
	}
	return nil
}

func (SubAgentSessionSource) isSessionSource() {}

func decodeGeneratedSessionSource(raw jsontext.Value) (SessionSource, error) {
	if raw == nil {
		return nil, nil
	}
	var text string
	if err := json.Unmarshal(raw, &text); err == nil {
		switch text {
		case "appServer":
			return SessionSourceValue(text), nil
		case "cli":
			return SessionSourceValue(text), nil
		case "exec":
			return SessionSourceValue(text), nil
		case "unknown":
			return SessionSourceValue(text), nil
		case "vscode":
			return SessionSourceValue(text), nil
		}
		return RawSessionSource(raw), nil
	}
	var object struct {
		Custom   jsontext.Value `json:"custom"`
		SubAgent jsontext.Value `json:"subAgent"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		if object.Custom != nil {
			var value CustomSessionSource
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		if object.SubAgent != nil {
			var value SubAgentSessionSource
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawSessionSource(raw), nil
	}
	return RawSessionSource(raw), nil
}

// Settings for a collaboration mode.
type Settings struct {
	DeveloperInstructions string           `json:"developer_instructions,omitzero"`
	Model                 string           `json:"model"`
	ReasoningEffort       *ReasoningEffort `json:"reasoning_effort,omitzero"`
}

// SkillDependencies is generated from the SkillDependencies schema definition.
type SkillDependencies struct {
	Tools []SkillToolDependency `json:"tools"`
}

// SkillErrorInfo is generated from the SkillErrorInfo schema definition.
type SkillErrorInfo struct {
	Message string `json:"message"`
	Path    string `json:"path"`
}

// SkillInterface is generated from the SkillInterface schema definition.
type SkillInterface struct {
	BrandColor    string `json:"brandColor,omitzero"`
	DefaultPrompt string `json:"defaultPrompt,omitzero"`
	DisplayName   string `json:"displayName,omitzero"`
	IconLarge     string `json:"iconLarge,omitzero"`

	// IconLargeURL remote large icon URL from the plugin catalog.
	IconLargeURL string `json:"iconLargeUrl,omitzero"`
	IconSmall    string `json:"iconSmall,omitzero"`

	// IconSmallURL remote small icon URL from the plugin catalog.
	IconSmallURL     string `json:"iconSmallUrl,omitzero"`
	ShortDescription string `json:"shortDescription,omitzero"`
}

// SkillMetadata is generated from the SkillMetadata schema definition.
type SkillMetadata struct {
	Dependencies   SkillDependencies `json:"dependencies,omitzero"`
	Description    string            `json:"description"`
	Enabled        bool              `json:"enabled"`
	InterfaceValue SkillInterface    `json:"interface,omitzero"`
	Name           string            `json:"name"`
	Path           string            `json:"path"`
	Scope          SkillScope        `json:"scope"`

	// ShortDescription legacy short_description from SKILL.md. Prefer SKILL.json interface.short_description.
	ShortDescription string `json:"shortDescription,omitzero"`
}

// SkillMigration is generated from the SkillMigration schema definition.
type SkillMigration struct {
	Name string `json:"name"`
}

// SkillScope is generated from the SkillScope schema definition.
type SkillScope string

const (
	// SkillScopeUser is the "user" SkillScope value.
	SkillScopeUser SkillScope = "user"
	// SkillScopeRepo is the "repo" SkillScope value.
	SkillScopeRepo SkillScope = "repo"
	// SkillScopeSystem is the "system" SkillScope value.
	SkillScopeSystem SkillScope = "system"
	// SkillScopeAdmin is the "admin" SkillScope value.
	SkillScopeAdmin SkillScope = "admin"
)

// SkillSummary is generated from the SkillSummary schema definition.
type SkillSummary struct {
	Description      string         `json:"description"`
	Enabled          bool           `json:"enabled"`
	InterfaceValue   SkillInterface `json:"interface,omitzero"`
	Name             string         `json:"name"`
	Path             string         `json:"path,omitzero"`
	ShortDescription string         `json:"shortDescription,omitzero"`
}

// SkillToolDependency is generated from the SkillToolDependency schema definition.
type SkillToolDependency struct {
	Command     string `json:"command,omitzero"`
	Description string `json:"description,omitzero"`
	Transport   string `json:"transport,omitzero"`
	Type        string `json:"type"`
	URL         string `json:"url,omitzero"`
	Value       string `json:"value"`
}

// SkillsChangedNotification notification emitted when watched local skill files change.
//
// Treat this as an invalidation signal and re-run `skills/list` with the client's current parameters when refreshed skill metadata is needed.
type SkillsChangedNotification struct{}

// SkillsConfigWriteParams is generated from the SkillsConfigWriteParams schema definition.
type SkillsConfigWriteParams struct {
	Enabled bool `json:"enabled"`

	// Name name-based selector.
	Name string `json:"name,omitzero"`

	// Path path-based selector.
	Path string `json:"path,omitzero"`
}

// SkillsConfigWriteResponse is generated from the SkillsConfigWriteResponse schema definition.
type SkillsConfigWriteResponse struct {
	EffectiveEnabled bool `json:"effectiveEnabled"`
}

// SkillsExtraRootsSetParams is generated from the SkillsExtraRootsSetParams schema definition.
type SkillsExtraRootsSetParams struct {
	ExtraRoots []string `json:"extraRoots"`
}

// SkillsExtraRootsSetResponse is generated from the SkillsExtraRootsSetResponse schema definition.
type SkillsExtraRootsSetResponse struct{}

// SkillsListEntry is generated from the SkillsListEntry schema definition.
type SkillsListEntry struct {
	Cwd    string           `json:"cwd"`
	Errors []SkillErrorInfo `json:"errors"`
	Skills []SkillMetadata  `json:"skills"`
}

// SkillsListParams is generated from the SkillsListParams schema definition.
type SkillsListParams struct {
	// Cwds when empty, defaults to the current session working directory.
	Cwds []string `json:"cwds,omitzero"`

	// ForceReload when true, bypass the skills cache and re-scan skills from disk.
	ForceReload bool `json:"forceReload,omitzero"`
}

// SkillsListResponse is generated from the SkillsListResponse schema definition.
type SkillsListResponse struct {
	Data []SkillsListEntry `json:"data"`
}

// SortDirection is generated from the SortDirection schema definition.
type SortDirection string

const (
	// SortDirectionAsc is the "asc" SortDirection value.
	SortDirectionAsc SortDirection = "asc"
	// SortDirectionDesc is the "desc" SortDirection value.
	SortDirectionDesc SortDirection = "desc"
)

// SpendControlLimitSnapshot is generated from the SpendControlLimitSnapshot schema definition.
type SpendControlLimitSnapshot struct {
	Limit            string `json:"limit"`
	RemainingPercent int32  `json:"remainingPercent"`
	ResetsAt         int64  `json:"resetsAt"`
	Used             string `json:"used"`
}

// SubAgentActivityKind is generated from the SubAgentActivityKind schema definition.
type SubAgentActivityKind string

const (
	// SubAgentActivityKindStarted is the "started" SubAgentActivityKind value.
	SubAgentActivityKindStarted SubAgentActivityKind = "started"
	// SubAgentActivityKindInteracted is the "interacted" SubAgentActivityKind value.
	SubAgentActivityKindInteracted SubAgentActivityKind = "interacted"
	// SubAgentActivityKindInterrupted is the "interrupted" SubAgentActivityKind value.
	SubAgentActivityKindInterrupted SubAgentActivityKind = "interrupted"
)

// SubAgentSource is generated from the SubAgentSource schema definition.
type SubAgentSource interface {
	isSubAgentSource()
}

// RawSubAgentSource preserves an uninterpreted SubAgentSource JSON value.
type RawSubAgentSource jsontext.Value

func (RawSubAgentSource) isSubAgentSource() {}

var (
	_ json.MarshalerTo     = RawSubAgentSource{}
	_ json.UnmarshalerFrom = (*RawSubAgentSource)(nil)
)

func (value RawSubAgentSource) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawSubAgentSource) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// SubAgentSourceValue is a string-valued SubAgentSource variant.
type SubAgentSourceValue string

func (SubAgentSourceValue) isSubAgentSource() {}

const (
	// SubAgentSourceValueCompact is the "compact" SubAgentSourceValue value.
	SubAgentSourceValueCompact SubAgentSourceValue = "compact"
	// SubAgentSourceValueMemoryConsolidation is the "memory_consolidation" SubAgentSourceValue value.
	SubAgentSourceValueMemoryConsolidation SubAgentSourceValue = "memory_consolidation"
	// SubAgentSourceValueReview is the "review" SubAgentSourceValue value.
	SubAgentSourceValueReview SubAgentSourceValue = "review"
)

// ThreadSpawn is generated from the ThreadSpawn schema definition.
type ThreadSpawn struct {
	AgentNickname  string     `json:"agent_nickname,omitzero"`
	AgentPath      *AgentPath `json:"agent_path,omitzero"`
	AgentRole      string     `json:"agent_role,omitzero"`
	Depth          int32      `json:"depth"`
	ParentThreadID ThreadID   `json:"parent_thread_id"`
}

// ThreadSpawnSubAgentSource is generated from the ThreadSpawnSubAgentSource schema definition.
type ThreadSpawnSubAgentSource struct {
	ThreadSpawn ThreadSpawn `json:"thread_spawn"`
}

func (ThreadSpawnSubAgentSource) isSubAgentSource() {}

// OtherSubAgentSource is generated from the OtherSubAgentSource schema definition.
type OtherSubAgentSource struct {
	Other string `json:"other"`
}

func (OtherSubAgentSource) isSubAgentSource() {}

func decodeGeneratedSubAgentSource(raw jsontext.Value) (SubAgentSource, error) {
	if raw == nil {
		return nil, nil
	}
	var text string
	if err := json.Unmarshal(raw, &text); err == nil {
		switch text {
		case "compact":
			return SubAgentSourceValue(text), nil
		case "memory_consolidation":
			return SubAgentSourceValue(text), nil
		case "review":
			return SubAgentSourceValue(text), nil
		}
		return RawSubAgentSource(raw), nil
	}
	var object struct {
		ThreadSpawn jsontext.Value `json:"thread_spawn"`
		Other       jsontext.Value `json:"other"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		if object.ThreadSpawn != nil {
			var value ThreadSpawnSubAgentSource
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		if object.Other != nil {
			var value OtherSubAgentSource
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawSubAgentSource(raw), nil
	}
	return RawSubAgentSource(raw), nil
}

// SubagentMigration is generated from the SubagentMigration schema definition.
type SubagentMigration struct {
	Name string `json:"name"`
}

// TerminalInteractionNotification is generated from the TerminalInteractionNotification schema definition.
type TerminalInteractionNotification struct {
	ItemID    string `json:"itemId"`
	ProcessID string `json:"processId"`
	Stdin     string `json:"stdin"`
	ThreadID  string `json:"threadId"`
	TurnID    string `json:"turnId"`
}

// TextElement is generated from the TextElement schema definition.
type TextElement struct {
	// ByteRange byte range in the parent `text` buffer that this element occupies.
	ByteRange ByteRange `json:"byteRange"`

	// Placeholder optional human-readable placeholder for the element, displayed in the UI.
	Placeholder string `json:"placeholder,omitzero"`
}

// TextPosition is generated from the TextPosition schema definition.
type TextPosition struct {
	// Column 1-based column number (in Unicode scalar values).
	Column uint `json:"column"`

	// Line 1-based line number.
	Line uint `json:"line"`
}

// TextRange is generated from the TextRange schema definition.
type TextRange struct {
	End   TextPosition `json:"end"`
	Start TextPosition `json:"start"`
}

// ThreadPayload is generated from the Thread schema definition.
type ThreadPayload struct {
	// AgentNickname optional random unique nickname assigned to an AgentControl-spawned sub-agent.
	AgentNickname string `json:"agentNickname,omitzero"`

	// AgentRole optional role (agent_role) assigned to an AgentControl-spawned sub-agent.
	AgentRole string `json:"agentRole,omitzero"`

	// CanAcceptDirectInput reports whether the app server accepts direct turn input for this loaded thread. `None` means the capability is unavailable, such as for an unloaded stored thread.
	CanAcceptDirectInput bool `json:"canAcceptDirectInput,omitzero"`

	// CliVersion version of the CLI that created the thread.
	CliVersion string `json:"cliVersion"`

	// CreatedAt unix timestamp (in seconds) when the thread was created.
	CreatedAt int64 `json:"createdAt"`

	// Cwd working directory captured for the thread.
	Cwd string `json:"cwd"`

	// Ephemeral reports whether the thread is ephemeral and should not be materialized on disk.
	Ephemeral bool `json:"ephemeral"`

	// Extra optional implementation-specific thread data.
	Extra ThreadExtra `json:"extra,omitzero"`

	// ForkedFromID source thread id when this thread was created by forking another thread.
	ForkedFromID string `json:"forkedFromId,omitzero"`

	// GitInfo optional Git metadata captured when the thread was created.
	GitInfo GitInfo `json:"gitInfo,omitzero"`

	// HistoryMode persisted thread history contract selected when this thread was created.
	HistoryMode ThreadHistoryMode `json:"historyMode,omitzero"`

	// ID identifier for this thread. Codex-generated thread IDs are UUIDv7.
	ID string `json:"id"`

	// ModelProvider model provider used for this thread (for example, 'openai').
	ModelProvider string `json:"modelProvider"`

	// Name optional user-facing thread title.
	Name string `json:"name,omitzero"`

	// ParentThreadID represents an ID of the parent thread. This will only be set if this thread is a subagent.
	ParentThreadID string `json:"parentThreadId,omitzero"`

	// Path [UNSTABLE] Path to the thread on disk.
	Path string `json:"path,omitzero"`

	// Preview usually the first user message in the thread, if available.
	Preview string `json:"preview"`

	// RecencyAt unix timestamp (in seconds) used for thread recency ordering.
	RecencyAt int64 `json:"recencyAt,omitzero"`

	// Section represents an independently persisted section selected for this thread, if any.
	Section ThreadSection `json:"section,omitzero"`

	// SectionEnteredAt unix timestamp in seconds when the thread entered its current section.
	SectionEnteredAt int64 `json:"sectionEnteredAt,omitzero"`

	// SessionID session id shared by threads that belong to the same session tree.
	SessionID string `json:"sessionId"`

	// Source origin of the thread (CLI, VSCode, codex exec, codex app-server, etc.).
	Source SessionSource `json:"source"`

	// Status current runtime status for the thread.
	Status ThreadStatus `json:"status"`

	// ThreadSource optional analytics source classification for this thread.
	ThreadSource *ThreadSource `json:"threadSource,omitzero"`

	// Turns only populated on `thread/resume`, `thread/rollback`, `thread/fork`, and `thread/read` (when `includeTurns` is true) responses. For all other responses and notifications returning a Thread, the turns field will be an empty list.
	Turns []Turn `json:"turns"`

	// UpdatedAt unix timestamp (in seconds) when the thread was last updated.
	UpdatedAt int64 `json:"updatedAt"`
}

func (value *ThreadPayload) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		AgentNickname        string            `json:"agentNickname,omitzero"`
		AgentRole            string            `json:"agentRole,omitzero"`
		CanAcceptDirectInput bool              `json:"canAcceptDirectInput,omitzero"`
		CliVersion           string            `json:"cliVersion"`
		CreatedAt            int64             `json:"createdAt"`
		Cwd                  string            `json:"cwd"`
		Ephemeral            bool              `json:"ephemeral"`
		Extra                ThreadExtra       `json:"extra,omitzero"`
		ForkedFromID         string            `json:"forkedFromId,omitzero"`
		GitInfo              GitInfo           `json:"gitInfo,omitzero"`
		HistoryMode          ThreadHistoryMode `json:"historyMode,omitzero"`
		ID                   string            `json:"id"`
		ModelProvider        string            `json:"modelProvider"`
		Name                 string            `json:"name,omitzero"`
		ParentThreadID       string            `json:"parentThreadId,omitzero"`
		Path                 string            `json:"path,omitzero"`
		Preview              string            `json:"preview"`
		RecencyAt            int64             `json:"recencyAt,omitzero"`
		Section              ThreadSection     `json:"section,omitzero"`
		SectionEnteredAt     int64             `json:"sectionEnteredAt,omitzero"`
		SessionID            string            `json:"sessionId"`
		Source               jsontext.Value    `json:"source"`
		Status               jsontext.Value    `json:"status"`
		ThreadSource         *ThreadSource     `json:"threadSource,omitzero"`
		Turns                []Turn            `json:"turns"`
		UpdatedAt            int64             `json:"updatedAt"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.AgentNickname = raw.AgentNickname
	value.AgentRole = raw.AgentRole
	value.CanAcceptDirectInput = raw.CanAcceptDirectInput
	value.CliVersion = raw.CliVersion
	value.CreatedAt = raw.CreatedAt
	value.Cwd = raw.Cwd
	value.Ephemeral = raw.Ephemeral
	value.Extra = raw.Extra
	value.ForkedFromID = raw.ForkedFromID
	value.GitInfo = raw.GitInfo
	value.HistoryMode = raw.HistoryMode
	value.ID = raw.ID
	value.ModelProvider = raw.ModelProvider
	value.Name = raw.Name
	value.ParentThreadID = raw.ParentThreadID
	value.Path = raw.Path
	value.Preview = raw.Preview
	value.RecencyAt = raw.RecencyAt
	value.Section = raw.Section
	value.SectionEnteredAt = raw.SectionEnteredAt
	value.SessionID = raw.SessionID
	if raw.Source == nil {
		value.Source = nil
	} else {
		decodedSource, err := decodeGeneratedSessionSource(raw.Source)
		if err != nil {
			return err
		}
		value.Source = decodedSource
	}
	if raw.Status == nil {
		value.Status = nil
	} else {
		decodedStatus, err := decodeGeneratedThreadStatus(raw.Status)
		if err != nil {
			return err
		}
		value.Status = decodedStatus
	}
	value.ThreadSource = raw.ThreadSource
	value.Turns = raw.Turns
	value.UpdatedAt = raw.UpdatedAt
	return nil
}

// ThreadActiveFlag is generated from the ThreadActiveFlag schema definition.
type ThreadActiveFlag string

const (
	// ThreadActiveFlagWaitingOnApproval is the "waitingOnApproval" ThreadActiveFlag value.
	ThreadActiveFlagWaitingOnApproval ThreadActiveFlag = "waitingOnApproval"
	// ThreadActiveFlagWaitingOnUserInput is the "waitingOnUserInput" ThreadActiveFlag value.
	ThreadActiveFlagWaitingOnUserInput ThreadActiveFlag = "waitingOnUserInput"
)

// ThreadApproveGuardianDeniedActionParams is generated from the ThreadApproveGuardianDeniedActionParams schema definition.
type ThreadApproveGuardianDeniedActionParams struct {
	// Event serialized `codex_protocol::protocol::GuardianAssessmentEvent`.
	Event    jsontext.Value `json:"event"`
	ThreadID string         `json:"threadId"`
}

// ThreadApproveGuardianDeniedActionResponse is generated from the ThreadApproveGuardianDeniedActionResponse schema definition.
type ThreadApproveGuardianDeniedActionResponse struct{}

// ThreadArchiveParams is generated from the ThreadArchiveParams schema definition.
type ThreadArchiveParams struct {
	ThreadID string `json:"threadId"`
}

// ThreadArchiveResponse is generated from the ThreadArchiveResponse schema definition.
type ThreadArchiveResponse struct{}

// ThreadArchivedNotification is generated from the ThreadArchivedNotification schema definition.
type ThreadArchivedNotification struct {
	ThreadID string `json:"threadId"`
}

// ThreadBackgroundTerminal is generated from the ThreadBackgroundTerminal schema definition.
type ThreadBackgroundTerminal struct {
	Command    string              `json:"command"`
	CPUPercent float64             `json:"cpuPercent,omitzero"`
	Cwd        LegacyAppPathString `json:"cwd"`
	ItemID     string              `json:"itemId"`
	OsPid      int32               `json:"osPid,omitzero"`
	ProcessID  string              `json:"processId"`
	RssKb      uint64              `json:"rssKb,omitzero"`
}

// ThreadBackgroundTerminalsCleanParams is generated from the ThreadBackgroundTerminalsCleanParams schema definition.
type ThreadBackgroundTerminalsCleanParams struct {
	ThreadID string `json:"threadId"`
}

// ThreadBackgroundTerminalsCleanResponse is generated from the ThreadBackgroundTerminalsCleanResponse schema definition.
type ThreadBackgroundTerminalsCleanResponse struct{}

// ThreadBackgroundTerminalsListParams is generated from the ThreadBackgroundTerminalsListParams schema definition.
type ThreadBackgroundTerminalsListParams struct {
	// Cursor opaque pagination cursor returned by a previous call.
	Cursor string `json:"cursor,omitzero"`

	// Limit optional page size.
	Limit    int32  `json:"limit,omitzero"`
	ThreadID string `json:"threadId"`
}

// ThreadBackgroundTerminalsListResponse is generated from the ThreadBackgroundTerminalsListResponse schema definition.
type ThreadBackgroundTerminalsListResponse struct {
	Data []ThreadBackgroundTerminal `json:"data"`

	// NextCursor opaque cursor to pass to the next call to continue after the last item. If None, there are no more items to return.
	NextCursor string `json:"nextCursor,omitzero"`
}

// ThreadBackgroundTerminalsTerminateParams is generated from the ThreadBackgroundTerminalsTerminateParams schema definition.
type ThreadBackgroundTerminalsTerminateParams struct {
	ProcessID string `json:"processId"`
	ThreadID  string `json:"threadId"`
}

// ThreadBackgroundTerminalsTerminateResponse is generated from the ThreadBackgroundTerminalsTerminateResponse schema definition.
type ThreadBackgroundTerminalsTerminateResponse struct {
	Terminated bool `json:"terminated"`
}

// ThreadClosedNotification is generated from the ThreadClosedNotification schema definition.
type ThreadClosedNotification struct {
	ThreadID string `json:"threadId"`
}

// ThreadCompactStartParams is generated from the ThreadCompactStartParams schema definition.
type ThreadCompactStartParams struct {
	ThreadID string `json:"threadId"`
}

// ThreadCompactStartResponse is generated from the ThreadCompactStartResponse schema definition.
type ThreadCompactStartResponse struct{}

// ThreadDecrementElicitationParams parameters for `thread/decrement_elicitation`.
type ThreadDecrementElicitationParams struct {
	// ThreadID thread whose out-of-band elicitation counter should be decremented.
	ThreadID string `json:"threadId"`
}

// ThreadDecrementElicitationResponse response for `thread/decrement_elicitation`.
type ThreadDecrementElicitationResponse struct {
	// Count current out-of-band elicitation count after the decrement.
	Count int64 `json:"count"`

	// Paused reports whether timeout accounting remains paused after applying the decrement.
	Paused bool `json:"paused"`
}

// ThreadDeleteParams is generated from the ThreadDeleteParams schema definition.
type ThreadDeleteParams struct {
	ThreadID string `json:"threadId"`
}

// ThreadDeleteResponse is generated from the ThreadDeleteResponse schema definition.
type ThreadDeleteResponse struct{}

// ThreadDeletedNotification is generated from the ThreadDeletedNotification schema definition.
type ThreadDeletedNotification struct {
	ThreadID string `json:"threadId"`
}

// ThreadExtra extra app-server data for a thread.
type ThreadExtra struct{}

// ThreadForkParams there are two ways to fork a thread: 1. By thread_id: load the thread from disk by thread_id and fork it into a new thread. 2. By path: load the thread from disk by path and fork it into a new thread.
//
// If using a non-empty path, the thread_id param will be ignored. Empty string path values are treated as absent.
//
// Prefer using thread_id whenever possible.
type ThreadForkParams struct {
	ApprovalPolicy AskForApproval `json:"approvalPolicy,omitzero"`

	// ApprovalsReviewer overrides where approval requests are routed for review on this thread and subsequent turns.
	ApprovalsReviewer ApprovalsReviewer `json:"approvalsReviewer,omitzero"`
	BaseInstructions  string            `json:"baseInstructions,omitzero"`

	// BeforeTurnID optional turn id to fork before, excluding that turn and all later turns. Cannot be combined with `last_turn_id`.
	BeforeTurnID string                    `json:"beforeTurnId,omitzero"`
	Config       map[string]jsontext.Value `json:"config,omitzero"`
	Cwd          string                    `json:"cwd,omitzero"`

	// DeferGoalContinuation when true, carry the source thread's current goal into the fork without starting its initial automatic continuation. The next explicit turn owns the goal lifecycle, and normal automatic continuation resumes after it.
	DeferGoalContinuation bool   `json:"deferGoalContinuation,omitzero"`
	DeveloperInstructions string `json:"developerInstructions,omitzero"`
	Ephemeral             bool   `json:"ephemeral,omitzero"`

	// ExcludeTurns when true, return only thread metadata and live fork state without populating `thread.turns`. This is useful when the client plans to call `thread/turns/list` immediately after forking.
	ExcludeTurns bool `json:"excludeTurns,omitzero"`

	// LastTurnID optional last turn id to fork through, inclusive.
	//
	// When specified, turns after `last_turn_id` are omitted from the fork. The referenced turn cannot be in progress.
	LastTurnID string `json:"lastTurnId,omitzero"`

	// Model configuration overrides for the forked thread, if any.
	Model         string `json:"model,omitzero"`
	ModelProvider string `json:"modelProvider,omitzero"`

	// Path [UNSTABLE] Specify the rollout path to fork from. If specified, the thread_id param will be ignored.
	Path string `json:"path,omitzero"`

	// Permissions named profile id for the forked thread. Cannot be combined with `sandbox`.
	Permissions string `json:"permissions,omitzero"`

	// RuntimeWorkspaceRoots replaces the thread's runtime workspace roots. Paths must be absolute.
	RuntimeWorkspaceRoots []string    `json:"runtimeWorkspaceRoots,omitzero"`
	Sandbox               SandboxMode `json:"sandbox,omitzero"`
	ServiceTier           string      `json:"serviceTier,omitzero"`
	ThreadID              string      `json:"threadId"`

	// ThreadSource optional client-supplied analytics source classification for this forked thread.
	ThreadSource *ThreadSource `json:"threadSource,omitzero"`
}

func (value *ThreadForkParams) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		ApprovalPolicy        jsontext.Value            `json:"approvalPolicy,omitzero"`
		ApprovalsReviewer     ApprovalsReviewer         `json:"approvalsReviewer,omitzero"`
		BaseInstructions      string                    `json:"baseInstructions,omitzero"`
		BeforeTurnID          string                    `json:"beforeTurnId,omitzero"`
		Config                map[string]jsontext.Value `json:"config,omitzero"`
		Cwd                   string                    `json:"cwd,omitzero"`
		DeferGoalContinuation bool                      `json:"deferGoalContinuation,omitzero"`
		DeveloperInstructions string                    `json:"developerInstructions,omitzero"`
		Ephemeral             bool                      `json:"ephemeral,omitzero"`
		ExcludeTurns          bool                      `json:"excludeTurns,omitzero"`
		LastTurnID            string                    `json:"lastTurnId,omitzero"`
		Model                 string                    `json:"model,omitzero"`
		ModelProvider         string                    `json:"modelProvider,omitzero"`
		Path                  string                    `json:"path,omitzero"`
		Permissions           string                    `json:"permissions,omitzero"`
		RuntimeWorkspaceRoots []string                  `json:"runtimeWorkspaceRoots,omitzero"`
		Sandbox               SandboxMode               `json:"sandbox,omitzero"`
		ServiceTier           string                    `json:"serviceTier,omitzero"`
		ThreadID              string                    `json:"threadId"`
		ThreadSource          *ThreadSource             `json:"threadSource,omitzero"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	if raw.ApprovalPolicy == nil {
		value.ApprovalPolicy = nil
	} else {
		decodedApprovalPolicy, err := decodeGeneratedAskForApproval(raw.ApprovalPolicy)
		if err != nil {
			return err
		}
		value.ApprovalPolicy = decodedApprovalPolicy
	}
	value.ApprovalsReviewer = raw.ApprovalsReviewer
	value.BaseInstructions = raw.BaseInstructions
	value.BeforeTurnID = raw.BeforeTurnID
	value.Config = raw.Config
	value.Cwd = raw.Cwd
	value.DeferGoalContinuation = raw.DeferGoalContinuation
	value.DeveloperInstructions = raw.DeveloperInstructions
	value.Ephemeral = raw.Ephemeral
	value.ExcludeTurns = raw.ExcludeTurns
	value.LastTurnID = raw.LastTurnID
	value.Model = raw.Model
	value.ModelProvider = raw.ModelProvider
	value.Path = raw.Path
	value.Permissions = raw.Permissions
	value.RuntimeWorkspaceRoots = raw.RuntimeWorkspaceRoots
	value.Sandbox = raw.Sandbox
	value.ServiceTier = raw.ServiceTier
	value.ThreadID = raw.ThreadID
	value.ThreadSource = raw.ThreadSource
	return nil
}

// ThreadForkResponse is generated from the ThreadForkResponse schema definition.
type ThreadForkResponse struct {
	// ActivePermissionProfile named or implicit built-in profile that produced the active permissions, when known.
	ActivePermissionProfile ActivePermissionProfile `json:"activePermissionProfile,omitzero"`
	ApprovalPolicy          AskForApproval          `json:"approvalPolicy"`

	// ApprovalsReviewer reviewer currently used for approval requests on this thread.
	ApprovalsReviewer ApprovalsReviewer `json:"approvalsReviewer"`
	Cwd               string            `json:"cwd"`

	// InstructionSources environment-native paths to instruction source files currently loaded for this thread.
	InstructionSources []LegacyAppPathString `json:"instructionSources,omitzero"`
	Model              string                `json:"model"`
	ModelProvider      string                `json:"modelProvider"`

	// MultiAgentMode @deprecated Always `explicitRequestOnly`. Use `reasoningEffort` for Ultra behavior.
	MultiAgentMode  MultiAgentMode   `json:"multiAgentMode,omitzero"`
	ReasoningEffort *ReasoningEffort `json:"reasoningEffort,omitzero"`

	// RuntimeWorkspaceRoots thread-scoped runtime workspace roots used to materialize `:workspace_roots`.
	RuntimeWorkspaceRoots []string `json:"runtimeWorkspaceRoots,omitzero"`

	// Sandbox legacy sandbox policy retained for compatibility. Experimental clients should prefer `activePermissionProfile` for profile provenance.
	Sandbox     SandboxPolicy `json:"sandbox"`
	ServiceTier string        `json:"serviceTier,omitzero"`
	Thread      ThreadPayload `json:"thread"`
}

func (value *ThreadForkResponse) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		ActivePermissionProfile ActivePermissionProfile `json:"activePermissionProfile,omitzero"`
		ApprovalPolicy          jsontext.Value          `json:"approvalPolicy"`
		ApprovalsReviewer       ApprovalsReviewer       `json:"approvalsReviewer"`
		Cwd                     string                  `json:"cwd"`
		InstructionSources      []LegacyAppPathString   `json:"instructionSources,omitzero"`
		Model                   string                  `json:"model"`
		ModelProvider           string                  `json:"modelProvider"`
		MultiAgentMode          jsontext.Value          `json:"multiAgentMode,omitzero"`
		ReasoningEffort         *ReasoningEffort        `json:"reasoningEffort,omitzero"`
		RuntimeWorkspaceRoots   []string                `json:"runtimeWorkspaceRoots,omitzero"`
		Sandbox                 jsontext.Value          `json:"sandbox"`
		ServiceTier             string                  `json:"serviceTier,omitzero"`
		Thread                  ThreadPayload           `json:"thread"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.ActivePermissionProfile = raw.ActivePermissionProfile
	if raw.ApprovalPolicy == nil {
		value.ApprovalPolicy = nil
	} else {
		decodedApprovalPolicy, err := decodeGeneratedAskForApproval(raw.ApprovalPolicy)
		if err != nil {
			return err
		}
		value.ApprovalPolicy = decodedApprovalPolicy
	}
	value.ApprovalsReviewer = raw.ApprovalsReviewer
	value.Cwd = raw.Cwd
	value.InstructionSources = raw.InstructionSources
	value.Model = raw.Model
	value.ModelProvider = raw.ModelProvider
	if raw.MultiAgentMode == nil {
		value.MultiAgentMode = nil
	} else {
		decodedMultiAgentMode, err := decodeGeneratedMultiAgentMode(raw.MultiAgentMode)
		if err != nil {
			return err
		}
		value.MultiAgentMode = decodedMultiAgentMode
	}
	value.ReasoningEffort = raw.ReasoningEffort
	value.RuntimeWorkspaceRoots = raw.RuntimeWorkspaceRoots
	if raw.Sandbox == nil {
		value.Sandbox = nil
	} else {
		decodedSandbox, err := decodeGeneratedSandboxPolicy(raw.Sandbox)
		if err != nil {
			return err
		}
		value.Sandbox = decodedSandbox
	}
	value.ServiceTier = raw.ServiceTier
	value.Thread = raw.Thread
	return nil
}

// ThreadGoal is generated from the ThreadGoal schema definition.
type ThreadGoal struct {
	CreatedAt       int64            `json:"createdAt"`
	Objective       string           `json:"objective"`
	Status          ThreadGoalStatus `json:"status"`
	ThreadID        string           `json:"threadId"`
	TimeUsedSeconds int64            `json:"timeUsedSeconds"`
	TokenBudget     int64            `json:"tokenBudget,omitzero"`
	TokensUsed      int64            `json:"tokensUsed"`
	UpdatedAt       int64            `json:"updatedAt"`
}

// ThreadGoalClearParams is generated from the ThreadGoalClearParams schema definition.
type ThreadGoalClearParams struct {
	ThreadID string `json:"threadId"`
}

// ThreadGoalClearResponse is generated from the ThreadGoalClearResponse schema definition.
type ThreadGoalClearResponse struct {
	Cleared bool `json:"cleared"`
}

// ThreadGoalClearedNotification is generated from the ThreadGoalClearedNotification schema definition.
type ThreadGoalClearedNotification struct {
	ThreadID string `json:"threadId"`
}

// ThreadGoalGetParams is generated from the ThreadGoalGetParams schema definition.
type ThreadGoalGetParams struct {
	ThreadID string `json:"threadId"`
}

// ThreadGoalGetResponse is generated from the ThreadGoalGetResponse schema definition.
type ThreadGoalGetResponse struct {
	Goal ThreadGoal `json:"goal,omitzero"`
}

// ThreadGoalSetParams is generated from the ThreadGoalSetParams schema definition.
type ThreadGoalSetParams struct {
	Objective   string           `json:"objective,omitzero"`
	Status      ThreadGoalStatus `json:"status,omitzero"`
	ThreadID    string           `json:"threadId"`
	TokenBudget int64            `json:"tokenBudget,omitzero"`
}

// ThreadGoalSetResponse is generated from the ThreadGoalSetResponse schema definition.
type ThreadGoalSetResponse struct {
	Goal ThreadGoal `json:"goal"`
}

// ThreadGoalStatus is generated from the ThreadGoalStatus schema definition.
type ThreadGoalStatus string

const (
	// ThreadGoalStatusActive is the "active" ThreadGoalStatus value.
	ThreadGoalStatusActive ThreadGoalStatus = "active"
	// ThreadGoalStatusPaused is the "paused" ThreadGoalStatus value.
	ThreadGoalStatusPaused ThreadGoalStatus = "paused"
	// ThreadGoalStatusBlocked is the "blocked" ThreadGoalStatus value.
	ThreadGoalStatusBlocked ThreadGoalStatus = "blocked"
	// ThreadGoalStatusUsageLimited is the "usageLimited" ThreadGoalStatus value.
	ThreadGoalStatusUsageLimited ThreadGoalStatus = "usageLimited"
	// ThreadGoalStatusBudgetLimited is the "budgetLimited" ThreadGoalStatus value.
	ThreadGoalStatusBudgetLimited ThreadGoalStatus = "budgetLimited"
	// ThreadGoalStatusComplete is the "complete" ThreadGoalStatus value.
	ThreadGoalStatusComplete ThreadGoalStatus = "complete"
)

// ThreadGoalUpdatedNotification is generated from the ThreadGoalUpdatedNotification schema definition.
type ThreadGoalUpdatedNotification struct {
	Goal     ThreadGoal `json:"goal"`
	ThreadID string     `json:"threadId"`
	TurnID   string     `json:"turnId,omitzero"`
}

// ThreadHistoryMode is generated from the ThreadHistoryMode schema definition.
type ThreadHistoryMode string

const (
	// ThreadHistoryModeLegacy is the "legacy" ThreadHistoryMode value.
	ThreadHistoryModeLegacy ThreadHistoryMode = "legacy"
	// ThreadHistoryModePaginated is the "paginated" ThreadHistoryMode value.
	ThreadHistoryModePaginated ThreadHistoryMode = "paginated"
)

// ThreadID is generated from the ThreadId schema definition.
type ThreadID = string

// ThreadIncrementElicitationParams parameters for `thread/increment_elicitation`.
type ThreadIncrementElicitationParams struct {
	// ThreadID thread whose out-of-band elicitation counter should be incremented.
	ThreadID string `json:"threadId"`
}

// ThreadIncrementElicitationResponse response for `thread/increment_elicitation`.
type ThreadIncrementElicitationResponse struct {
	// Count current out-of-band elicitation count after the increment.
	Count int64 `json:"count"`

	// Paused reports whether timeout accounting is paused after applying the increment.
	Paused bool `json:"paused"`
}

// ThreadInjectItemsParams is generated from the ThreadInjectItemsParams schema definition.
type ThreadInjectItemsParams struct {
	// Items raw Responses API items to append to the thread's model-visible history.
	Items    []jsontext.Value `json:"items"`
	ThreadID string           `json:"threadId"`
}

// ThreadInjectItemsResponse is generated from the ThreadInjectItemsResponse schema definition.
type ThreadInjectItemsResponse struct{}

// ThreadItem is generated from the ThreadItem schema definition.
type ThreadItem interface {
	isThreadItem()
}

// RawThreadItem preserves an uninterpreted ThreadItem JSON value.
type RawThreadItem jsontext.Value

func (RawThreadItem) isThreadItem() {}

var (
	_ json.MarshalerTo     = RawThreadItem{}
	_ json.UnmarshalerFrom = (*RawThreadItem)(nil)
)

func (value RawThreadItem) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawThreadItem) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// UserMessageThreadItem is generated from the UserMessageThreadItem schema definition.
type UserMessageThreadItem struct {
	ClientID string      `json:"clientId,omitzero"`
	Content  []UserInput `json:"content"`
	ID       string      `json:"id"`
	Type     string      `json:"type"`
}

func (value *UserMessageThreadItem) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		ClientID string           `json:"clientId,omitzero"`
		Content  []jsontext.Value `json:"content"`
		ID       string           `json:"id"`
		Type     string           `json:"type"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.ClientID = raw.ClientID
	if raw.Content == nil {
		value.Content = nil
	} else {
		value.Content = make([]UserInput, len(raw.Content))
		for i, item := range raw.Content {
			if item != nil {
				decodedContent, err := decodeGeneratedUserInput(item)
				if err != nil {
					return err
				}
				value.Content[i] = decodedContent
			}
		}
	}
	value.ID = raw.ID
	value.Type = raw.Type
	return nil
}

func (UserMessageThreadItem) isThreadItem() {}

// HookPromptThreadItem is generated from the HookPromptThreadItem schema definition.
type HookPromptThreadItem struct {
	Fragments []HookPromptFragment `json:"fragments"`
	ID        string               `json:"id"`
	Type      string               `json:"type"`
}

func (HookPromptThreadItem) isThreadItem() {}

// AgentMessageThreadItem is generated from the AgentMessageThreadItem schema definition.
type AgentMessageThreadItem struct {
	ID             string         `json:"id"`
	MemoryCitation MemoryCitation `json:"memoryCitation,omitzero"`
	Phase          MessagePhase   `json:"phase,omitzero"`
	Text           string         `json:"text"`
	Type           string         `json:"type"`
}

func (value *AgentMessageThreadItem) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		ID             string         `json:"id"`
		MemoryCitation MemoryCitation `json:"memoryCitation,omitzero"`
		Phase          jsontext.Value `json:"phase,omitzero"`
		Text           string         `json:"text"`
		Type           string         `json:"type"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.ID = raw.ID
	value.MemoryCitation = raw.MemoryCitation
	if raw.Phase == nil {
		value.Phase = nil
	} else {
		decodedPhase, err := decodeGeneratedMessagePhase(raw.Phase)
		if err != nil {
			return err
		}
		value.Phase = decodedPhase
	}
	value.Text = raw.Text
	value.Type = raw.Type
	return nil
}

func (AgentMessageThreadItem) isThreadItem() {}

// PlanThreadItem proposed plan item content. The completed plan item is authoritative and may not match the concatenation of `PlanDelta` text.
type PlanThreadItem struct {
	ID   string `json:"id"`
	Text string `json:"text"`
	Type string `json:"type"`
}

func (PlanThreadItem) isThreadItem() {}

// ReasoningThreadItem is generated from the ReasoningThreadItem schema definition.
type ReasoningThreadItem struct {
	Content []string `json:"content,omitzero"`
	ID      string   `json:"id"`
	Summary []string `json:"summary,omitzero"`
	Type    string   `json:"type"`
}

func (ReasoningThreadItem) isThreadItem() {}

// CommandExecutionThreadItem is generated from the CommandExecutionThreadItem schema definition.
type CommandExecutionThreadItem struct {
	// AggregatedOutput represents a command's output, aggregated from stdout and stderr.
	AggregatedOutput string `json:"aggregatedOutput,omitzero"`

	// Command represents a command to be executed.
	Command string `json:"command"`

	// CommandActions represents a best-effort parsing of the command to understand the action(s) it will perform. This returns a list of CommandAction objects because a single shell command may be composed of many commands piped together.
	CommandActions []CommandAction `json:"commandActions"`

	// Cwd represents a command's working directory.
	Cwd LegacyAppPathString `json:"cwd"`

	// DurationMs represents a duration of the command execution in milliseconds.
	DurationMs int64 `json:"durationMs,omitzero"`

	// ExitCode represents a command's exit code.
	ExitCode int32  `json:"exitCode,omitzero"`
	ID       string `json:"id"`

	// PluginID trusted first-party plugin id when this command resolves to one plugin script.
	PluginID string `json:"pluginId,omitzero"`

	// ProcessID identifier for the underlying PTY process (when available).
	ProcessID string `json:"processId,omitzero"`

	// ScriptPath safe plugin-relative path when this command resolves to one plugin script.
	ScriptPath string                 `json:"scriptPath,omitzero"`
	Source     CommandExecutionSource `json:"source,omitzero"`
	Status     CommandExecutionStatus `json:"status"`
	Type       string                 `json:"type"`
}

func (value *CommandExecutionThreadItem) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		AggregatedOutput string                 `json:"aggregatedOutput,omitzero"`
		Command          string                 `json:"command"`
		CommandActions   []jsontext.Value       `json:"commandActions"`
		Cwd              LegacyAppPathString    `json:"cwd"`
		DurationMs       int64                  `json:"durationMs,omitzero"`
		ExitCode         int32                  `json:"exitCode,omitzero"`
		ID               string                 `json:"id"`
		PluginID         string                 `json:"pluginId,omitzero"`
		ProcessID        string                 `json:"processId,omitzero"`
		ScriptPath       string                 `json:"scriptPath,omitzero"`
		Source           CommandExecutionSource `json:"source,omitzero"`
		Status           CommandExecutionStatus `json:"status"`
		Type             string                 `json:"type"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.AggregatedOutput = raw.AggregatedOutput
	value.Command = raw.Command
	if raw.CommandActions == nil {
		value.CommandActions = nil
	} else {
		value.CommandActions = make([]CommandAction, len(raw.CommandActions))
		for i, item := range raw.CommandActions {
			if item != nil {
				decodedCommandActions, err := decodeGeneratedCommandAction(item)
				if err != nil {
					return err
				}
				value.CommandActions[i] = decodedCommandActions
			}
		}
	}
	value.Cwd = raw.Cwd
	value.DurationMs = raw.DurationMs
	value.ExitCode = raw.ExitCode
	value.ID = raw.ID
	value.PluginID = raw.PluginID
	value.ProcessID = raw.ProcessID
	value.ScriptPath = raw.ScriptPath
	value.Source = raw.Source
	value.Status = raw.Status
	value.Type = raw.Type
	return nil
}

func (CommandExecutionThreadItem) isThreadItem() {}

// FileChangeThreadItem is generated from the FileChangeThreadItem schema definition.
type FileChangeThreadItem struct {
	Changes []FileUpdateChange `json:"changes"`
	ID      string             `json:"id"`
	Status  PatchApplyStatus   `json:"status"`
	Type    string             `json:"type"`
}

func (FileChangeThreadItem) isThreadItem() {}

// MCPToolCallThreadItem is generated from the McpToolCallThreadItem schema definition.
type MCPToolCallThreadItem struct {
	AppContext MCPToolCallAppContext `json:"appContext,omitzero"`
	Arguments  jsontext.Value        `json:"arguments"`

	// DurationMs represents a duration of the MCP tool call in milliseconds.
	DurationMs int64            `json:"durationMs,omitzero"`
	Error      MCPToolCallError `json:"error,omitzero"`
	ID         string           `json:"id"`

	// MCPAppResourceURI deprecated: use `appContext.resourceUri` instead.
	MCPAppResourceURI string            `json:"mcpAppResourceUri,omitzero"`
	PluginID          string            `json:"pluginId,omitzero"`
	ReadOnlyHint      bool              `json:"readOnlyHint,omitzero"`
	Result            MCPToolCallResult `json:"result,omitzero"`
	Server            string            `json:"server"`
	Status            MCPToolCallStatus `json:"status"`
	Tool              string            `json:"tool"`
	Type              string            `json:"type"`
}

func (MCPToolCallThreadItem) isThreadItem() {}

// DynamicToolCallThreadItem is generated from the DynamicToolCallThreadItem schema definition.
type DynamicToolCallThreadItem struct {
	Arguments    jsontext.Value                     `json:"arguments"`
	ContentItems []DynamicToolCallOutputContentItem `json:"contentItems,omitzero"`

	// DurationMs represents a duration of the dynamic tool call in milliseconds.
	DurationMs int64                 `json:"durationMs,omitzero"`
	ID         string                `json:"id"`
	Namespace  string                `json:"namespace,omitzero"`
	Status     DynamicToolCallStatus `json:"status"`
	Success    bool                  `json:"success,omitzero"`
	Tool       string                `json:"tool"`
	Type       string                `json:"type"`
}

func (value *DynamicToolCallThreadItem) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Arguments    jsontext.Value        `json:"arguments"`
		ContentItems []jsontext.Value      `json:"contentItems,omitzero"`
		DurationMs   int64                 `json:"durationMs,omitzero"`
		ID           string                `json:"id"`
		Namespace    string                `json:"namespace,omitzero"`
		Status       DynamicToolCallStatus `json:"status"`
		Success      bool                  `json:"success,omitzero"`
		Tool         string                `json:"tool"`
		Type         string                `json:"type"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.Arguments = raw.Arguments
	if raw.ContentItems == nil {
		value.ContentItems = nil
	} else {
		value.ContentItems = make([]DynamicToolCallOutputContentItem, len(raw.ContentItems))
		for i, item := range raw.ContentItems {
			if item != nil {
				decodedContentItems, err := decodeGeneratedDynamicToolCallOutputContentItem(item)
				if err != nil {
					return err
				}
				value.ContentItems[i] = decodedContentItems
			}
		}
	}
	value.DurationMs = raw.DurationMs
	value.ID = raw.ID
	value.Namespace = raw.Namespace
	value.Status = raw.Status
	value.Success = raw.Success
	value.Tool = raw.Tool
	value.Type = raw.Type
	return nil
}

func (DynamicToolCallThreadItem) isThreadItem() {}

// CollabAgentToolCallThreadItem is generated from the CollabAgentToolCallThreadItem schema definition.
type CollabAgentToolCallThreadItem struct {
	// AgentsStates last known status of the target agents, when available.
	AgentsStates map[string]CollabAgentState `json:"agentsStates"`

	// ID unique identifier for this collab tool call.
	ID string `json:"id"`

	// Model requested for the spawned agent, when applicable.
	Model string `json:"model,omitzero"`

	// Prompt text sent as part of the collab tool call, when available.
	Prompt string `json:"prompt,omitzero"`

	// ReasoningEffort reasoning effort requested for the spawned agent, when applicable.
	ReasoningEffort *ReasoningEffort `json:"reasoningEffort,omitzero"`

	// ReceiverThreadIDs thread ID of the receiving agent, when applicable. In case of spawn operation, this corresponds to the newly spawned agent.
	ReceiverThreadIDs []string `json:"receiverThreadIds"`

	// SenderThreadID thread ID of the agent issuing the collab request.
	SenderThreadID string `json:"senderThreadId"`

	// Status current status of the collab tool call.
	Status CollabAgentToolCallStatus `json:"status"`

	// Tool names of the collab tool that was invoked.
	Tool CollabAgentTool `json:"tool"`
	Type string          `json:"type"`
}

func (CollabAgentToolCallThreadItem) isThreadItem() {}

// SubAgentActivityThreadItem is generated from the SubAgentActivityThreadItem schema definition.
type SubAgentActivityThreadItem struct {
	AgentPath     string               `json:"agentPath"`
	AgentThreadID string               `json:"agentThreadId"`
	ID            string               `json:"id"`
	Kind          SubAgentActivityKind `json:"kind"`
	Type          string               `json:"type"`
}

func (SubAgentActivityThreadItem) isThreadItem() {}

// WebSearchThreadItem is generated from the WebSearchThreadItem schema definition.
type WebSearchThreadItem struct {
	Action WebSearchAction `json:"action,omitzero"`
	ID     string          `json:"id"`
	Query  string          `json:"query"`

	// Results structured search results returned out-of-band by standalone web search.
	//
	// These stay as opaque JSON at the extension/app-server boundary so new result fields and result types can pass through without a Codex release.
	Results []jsontext.Value `json:"results,omitzero"`
	Type    string           `json:"type"`
}

func (value *WebSearchThreadItem) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Action  jsontext.Value   `json:"action,omitzero"`
		ID      string           `json:"id"`
		Query   string           `json:"query"`
		Results []jsontext.Value `json:"results,omitzero"`
		Type    string           `json:"type"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	if raw.Action == nil {
		value.Action = nil
	} else {
		decodedAction, err := decodeGeneratedWebSearchAction(raw.Action)
		if err != nil {
			return err
		}
		value.Action = decodedAction
	}
	value.ID = raw.ID
	value.Query = raw.Query
	value.Results = raw.Results
	value.Type = raw.Type
	return nil
}

func (WebSearchThreadItem) isThreadItem() {}

// ImageViewThreadItem is generated from the ImageViewThreadItem schema definition.
type ImageViewThreadItem struct {
	ID   string              `json:"id"`
	Path LegacyAppPathString `json:"path"`
	Type string              `json:"type"`
}

func (ImageViewThreadItem) isThreadItem() {}

// SleepThreadItem displays item emitted by the interruptible `clock.sleep` tool.
type SleepThreadItem struct {
	DurationMs uint64 `json:"durationMs"`
	ID         string `json:"id"`
	Type       string `json:"type"`
}

func (SleepThreadItem) isThreadItem() {}

// ImageGenerationThreadItem is generated from the ImageGenerationThreadItem schema definition.
type ImageGenerationThreadItem struct {
	ID            string `json:"id"`
	Result        string `json:"result"`
	RevisedPrompt string `json:"revisedPrompt,omitzero"`
	SavedPath     string `json:"savedPath,omitzero"`
	Status        string `json:"status"`
	Type          string `json:"type"`
}

func (ImageGenerationThreadItem) isThreadItem() {}

// EnteredReviewModeThreadItem is generated from the EnteredReviewModeThreadItem schema definition.
type EnteredReviewModeThreadItem struct {
	ID     string `json:"id"`
	Review string `json:"review"`
	Type   string `json:"type"`
}

func (EnteredReviewModeThreadItem) isThreadItem() {}

// ExitedReviewModeThreadItem is generated from the ExitedReviewModeThreadItem schema definition.
type ExitedReviewModeThreadItem struct {
	ID     string `json:"id"`
	Review string `json:"review"`
	Type   string `json:"type"`
}

func (ExitedReviewModeThreadItem) isThreadItem() {}

// ContextCompactionThreadItem is generated from the ContextCompactionThreadItem schema definition.
type ContextCompactionThreadItem struct {
	ID   string `json:"id"`
	Type string `json:"type"`
}

func (ContextCompactionThreadItem) isThreadItem() {}

func decodeGeneratedThreadItem(raw jsontext.Value) (ThreadItem, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "userMessage":
			var value UserMessageThreadItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "hookPrompt":
			var value HookPromptThreadItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "agentMessage":
			var value AgentMessageThreadItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "plan":
			var value PlanThreadItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "reasoning":
			var value ReasoningThreadItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "commandExecution":
			var value CommandExecutionThreadItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "fileChange":
			var value FileChangeThreadItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "mcpToolCall":
			var value MCPToolCallThreadItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "dynamicToolCall":
			var value DynamicToolCallThreadItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "collabAgentToolCall":
			var value CollabAgentToolCallThreadItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "subAgentActivity":
			var value SubAgentActivityThreadItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "webSearch":
			var value WebSearchThreadItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "imageView":
			var value ImageViewThreadItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "sleep":
			var value SleepThreadItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "imageGeneration":
			var value ImageGenerationThreadItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "enteredReviewMode":
			var value EnteredReviewModeThreadItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "exitedReviewMode":
			var value ExitedReviewModeThreadItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "contextCompaction":
			var value ContextCompactionThreadItem
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawThreadItem(raw), nil
	}
	return RawThreadItem(raw), nil
}

// ThreadItemEntry is generated from the ThreadItemEntry schema definition.
type ThreadItemEntry struct {
	Item ThreadItem `json:"item"`

	// TurnID turn containing this item.
	TurnID string `json:"turnId"`
}

func (value *ThreadItemEntry) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Item   jsontext.Value `json:"item"`
		TurnID string         `json:"turnId"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	if raw.Item == nil {
		value.Item = nil
	} else {
		decodedItem, err := decodeGeneratedThreadItem(raw.Item)
		if err != nil {
			return err
		}
		value.Item = decodedItem
	}
	value.TurnID = raw.TurnID
	return nil
}

// ThreadItemsListParams is generated from the ThreadItemsListParams schema definition.
type ThreadItemsListParams struct {
	// Cursor opaque cursor to pass to the next call to continue after the last item.
	Cursor string `json:"cursor,omitzero"`

	// Limit optional item page size.
	Limit int32 `json:"limit,omitzero"`

	// SortDirection optional item pagination direction; defaults to ascending.
	SortDirection SortDirection `json:"sortDirection,omitzero"`
	ThreadID      string        `json:"threadId"`

	// TurnID optional turn id to filter by. When omitted, returns items across the thread.
	TurnID string `json:"turnId,omitzero"`
}

// ThreadItemsListResponse is generated from the ThreadItemsListResponse schema definition.
type ThreadItemsListResponse struct {
	// BackwardsCursor opaque cursor to pass as `cursor` when reversing `sortDirection`. This is only populated when the page contains at least one item.
	BackwardsCursor string            `json:"backwardsCursor,omitzero"`
	Data            []ThreadItemEntry `json:"data"`

	// NextCursor opaque cursor to pass to the next call to continue after the last item. if None, there are no more items to return.
	NextCursor string `json:"nextCursor,omitzero"`
}

// ThreadListCwdFilter is generated from the ThreadListCwdFilter schema definition.
type ThreadListCwdFilter interface {
	isThreadListCwdFilter()
}

// RawThreadListCwdFilter preserves an uninterpreted ThreadListCwdFilter JSON value.
type RawThreadListCwdFilter jsontext.Value

func (RawThreadListCwdFilter) isThreadListCwdFilter() {}

var (
	_ json.MarshalerTo     = RawThreadListCwdFilter{}
	_ json.UnmarshalerFrom = (*RawThreadListCwdFilter)(nil)
)

func (value RawThreadListCwdFilter) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawThreadListCwdFilter) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// ThreadListCwdFilterString is a ThreadListCwdFilter variant.
type ThreadListCwdFilterString string

func (ThreadListCwdFilterString) isThreadListCwdFilter() {}

// ThreadListCwdFilterItems is a ThreadListCwdFilter variant.
type ThreadListCwdFilterItems []string

func (ThreadListCwdFilterItems) isThreadListCwdFilter() {}

func decodeGeneratedThreadListCwdFilter(raw jsontext.Value) (ThreadListCwdFilter, error) {
	if raw == nil {
		return nil, nil
	}
	var text string
	if err := json.Unmarshal(raw, &text); err == nil {
		return ThreadListCwdFilterString(text), nil
	}
	var value ThreadListCwdFilterItems
	if err := json.Unmarshal(raw, &value); err == nil {
		return value, nil
	}
	return RawThreadListCwdFilter(raw), nil
}

// ThreadListParams is generated from the ThreadListParams schema definition.
type ThreadListParams struct {
	// AncestorThreadID optional ancestor thread filter. Returns spawned descendants at any depth, excluding the ancestor itself. Mutually exclusive with `parentThreadId`.
	AncestorThreadID string `json:"ancestorThreadId,omitzero"`

	// Archived optional archived filter; when set to true, only archived threads are returned. If false or null, only non-archived threads are returned.
	Archived bool `json:"archived,omitzero"`

	// Cursor opaque pagination cursor returned by a previous call.
	Cursor string `json:"cursor,omitzero"`

	// Cwd optional cwd filter or filters; when set, only threads whose session cwd exactly matches one of these paths are returned.
	Cwd ThreadListCwdFilter `json:"cwd,omitzero"`

	// Limit optional page size; defaults to a reasonable server-side value.
	Limit int32 `json:"limit,omitzero"`

	// ModelProviders optional provider filter; when set, only sessions recorded under these providers are returned. When present but empty, includes all providers.
	ModelProviders []string `json:"modelProviders,omitzero"`

	// ParentThreadID optional direct parent thread filter. Mutually exclusive with `ancestorThreadId`.
	ParentThreadID string `json:"parentThreadId,omitzero"`

	// SearchTerm optional substring filter for the extracted thread title.
	SearchTerm string `json:"searchTerm,omitzero"`

	// SectionID omits to include every section, set to `null` for unsectioned threads, or provide a section ID to return only threads in that section.
	SectionID string `json:"sectionId,omitzero"`

	// SortDirection optional sort direction; defaults to descending (newest first).
	SortDirection SortDirection `json:"sortDirection,omitzero"`

	// SortKey optional sort key; defaults to created_at.
	SortKey ThreadSortKey `json:"sortKey,omitzero"`

	// SourceKinds optional source filter; when set, only sessions from these source kinds are returned. When omitted or empty, defaults to interactive sources.
	SourceKinds []ThreadSourceKind `json:"sourceKinds,omitzero"`

	// UseStateDbOnly if true, return from the state DB without scanning JSONL rollouts to repair thread metadata. Omitted or false preserves scan-and-repair behavior.
	UseStateDbOnly bool `json:"useStateDbOnly,omitzero"`
}

func (value *ThreadListParams) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		AncestorThreadID string             `json:"ancestorThreadId,omitzero"`
		Archived         bool               `json:"archived,omitzero"`
		Cursor           string             `json:"cursor,omitzero"`
		Cwd              jsontext.Value     `json:"cwd,omitzero"`
		Limit            int32              `json:"limit,omitzero"`
		ModelProviders   []string           `json:"modelProviders,omitzero"`
		ParentThreadID   string             `json:"parentThreadId,omitzero"`
		SearchTerm       string             `json:"searchTerm,omitzero"`
		SectionID        string             `json:"sectionId,omitzero"`
		SortDirection    SortDirection      `json:"sortDirection,omitzero"`
		SortKey          ThreadSortKey      `json:"sortKey,omitzero"`
		SourceKinds      []ThreadSourceKind `json:"sourceKinds,omitzero"`
		UseStateDbOnly   bool               `json:"useStateDbOnly,omitzero"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.AncestorThreadID = raw.AncestorThreadID
	value.Archived = raw.Archived
	value.Cursor = raw.Cursor
	if raw.Cwd == nil {
		value.Cwd = nil
	} else {
		decodedCwd, err := decodeGeneratedThreadListCwdFilter(raw.Cwd)
		if err != nil {
			return err
		}
		value.Cwd = decodedCwd
	}
	value.Limit = raw.Limit
	value.ModelProviders = raw.ModelProviders
	value.ParentThreadID = raw.ParentThreadID
	value.SearchTerm = raw.SearchTerm
	value.SectionID = raw.SectionID
	value.SortDirection = raw.SortDirection
	value.SortKey = raw.SortKey
	value.SourceKinds = raw.SourceKinds
	value.UseStateDbOnly = raw.UseStateDbOnly
	return nil
}

// ThreadListResponse is generated from the ThreadListResponse schema definition.
type ThreadListResponse struct {
	// BackwardsCursor opaque cursor to pass as `cursor` when reversing `sortDirection`. This is only populated when the page contains at least one thread. Use it with the opposite `sortDirection`; for timestamp sorts it anchors at the start of the page timestamp so same-second updates are not skipped.
	BackwardsCursor string          `json:"backwardsCursor,omitzero"`
	Data            []ThreadPayload `json:"data"`

	// NextCursor opaque cursor to pass to the next call to continue after the last item. if None, there are no more items to return.
	NextCursor string `json:"nextCursor,omitzero"`
}

// ThreadLoadedListParams is generated from the ThreadLoadedListParams schema definition.
type ThreadLoadedListParams struct {
	// Cursor opaque pagination cursor returned by a previous call.
	Cursor string `json:"cursor,omitzero"`

	// Limit optional page size; defaults to no limit.
	Limit int32 `json:"limit,omitzero"`
}

// ThreadLoadedListResponse is generated from the ThreadLoadedListResponse schema definition.
type ThreadLoadedListResponse struct {
	// Data thread ids for sessions currently loaded in memory.
	Data []string `json:"data"`

	// NextCursor opaque cursor to pass to the next call to continue after the last item. if None, there are no more items to return.
	NextCursor string `json:"nextCursor,omitzero"`
}

// ThreadMemoryMode is generated from the ThreadMemoryMode schema definition.
type ThreadMemoryMode string

const (
	// ThreadMemoryModeEnabled is the "enabled" ThreadMemoryMode value.
	ThreadMemoryModeEnabled ThreadMemoryMode = "enabled"
	// ThreadMemoryModeDisabled is the "disabled" ThreadMemoryMode value.
	ThreadMemoryModeDisabled ThreadMemoryMode = "disabled"
)

// ThreadMemoryModeSetParams is generated from the ThreadMemoryModeSetParams schema definition.
type ThreadMemoryModeSetParams struct {
	Mode     ThreadMemoryMode `json:"mode"`
	ThreadID string           `json:"threadId"`
}

// ThreadMemoryModeSetResponse is generated from the ThreadMemoryModeSetResponse schema definition.
type ThreadMemoryModeSetResponse struct{}

// ThreadMetadataGitInfoUpdateParams is generated from the ThreadMetadataGitInfoUpdateParams schema definition.
type ThreadMetadataGitInfoUpdateParams struct {
	// Branch omits to leave the stored branch unchanged, set to `null` to clear it, or provide a non-empty string to replace it.
	Branch string `json:"branch,omitzero"`

	// OriginURL omits to leave the stored origin URL unchanged, set to `null` to clear it, or provide a non-empty string to replace it.
	OriginURL string `json:"originUrl,omitzero"`

	// Sha omits to leave the stored commit unchanged, set to `null` to clear it, or provide a non-empty string to replace it.
	Sha string `json:"sha,omitzero"`
}

// ThreadMetadataUpdateParams is generated from the ThreadMetadataUpdateParams schema definition.
type ThreadMetadataUpdateParams struct {
	// GitInfo patch the stored Git metadata for this thread. Omit a field to leave it unchanged, set it to `null` to clear it, or provide a string to replace the stored value.
	GitInfo  ThreadMetadataGitInfoUpdateParams `json:"gitInfo,omitzero"`
	ThreadID string                            `json:"threadId"`
}

// ThreadMetadataUpdateResponse is generated from the ThreadMetadataUpdateResponse schema definition.
type ThreadMetadataUpdateResponse struct {
	Thread ThreadPayload `json:"thread"`
}

// ThreadNameUpdatedNotification is generated from the ThreadNameUpdatedNotification schema definition.
type ThreadNameUpdatedNotification struct {
	ThreadID   string `json:"threadId"`
	ThreadName string `json:"threadName,omitzero"`
}

// ThreadReadParams is generated from the ThreadReadParams schema definition.
type ThreadReadParams struct {
	// IncludeTurns when true, include turns and their items from rollout history.
	IncludeTurns bool   `json:"includeTurns,omitzero"`
	ThreadID     string `json:"threadId"`
}

// ThreadReadResponse is generated from the ThreadReadResponse schema definition.
type ThreadReadResponse struct {
	Thread ThreadPayload `json:"thread"`
}

// ThreadRealtimeAppendAudioParams append audio input to thread realtime.
type ThreadRealtimeAppendAudioParams struct {
	Audio    ThreadRealtimeAudioChunk `json:"audio"`
	ThreadID string                   `json:"threadId"`
}

// ThreadRealtimeAppendAudioResponse response for appending realtime audio input.
type ThreadRealtimeAppendAudioResponse struct{}

// ThreadRealtimeAppendSpeechParams append speakable text to thread realtime.
type ThreadRealtimeAppendSpeechParams struct {
	Text     string `json:"text"`
	ThreadID string `json:"threadId"`
}

// ThreadRealtimeAppendSpeechResponse response for appending realtime speech.
type ThreadRealtimeAppendSpeechResponse struct{}

// ThreadRealtimeAppendTextParams append text input to thread realtime.
type ThreadRealtimeAppendTextParams struct {
	Role     ConversationTextRole `json:"role,omitzero"`
	Text     string               `json:"text"`
	ThreadID string               `json:"threadId"`
}

// ThreadRealtimeAppendTextResponse response for appending realtime text input.
type ThreadRealtimeAppendTextResponse struct{}

// ThreadRealtimeAudioChunk thread realtime audio chunk.
type ThreadRealtimeAudioChunk struct {
	Data              string `json:"data"`
	ItemID            string `json:"itemId,omitzero"`
	NumChannels       int64  `json:"numChannels"`
	SampleRate        int32  `json:"sampleRate"`
	SamplesPerChannel int32  `json:"samplesPerChannel,omitzero"`
}

// ThreadRealtimeClosedNotification emitted when thread realtime transport closes.
type ThreadRealtimeClosedNotification struct {
	Reason   string `json:"reason,omitzero"`
	ThreadID string `json:"threadId"`
}

// ThreadRealtimeErrorNotification emitted when thread realtime encounters an error.
type ThreadRealtimeErrorNotification struct {
	Message  string `json:"message"`
	ThreadID string `json:"threadId"`
}

// ThreadRealtimeInitialItem role-bearing text item included when a realtime V3 session starts.
type ThreadRealtimeInitialItem struct {
	Role ConversationTextRole `json:"role"`
	Text string               `json:"text"`
}

// ThreadRealtimeItemAddedNotification raw non-audio thread realtime item emitted by the backend.
type ThreadRealtimeItemAddedNotification struct {
	Item     jsontext.Value `json:"item"`
	ThreadID string         `json:"threadId"`
}

// ThreadRealtimeListVoicesParams list voices supported by thread realtime.
type ThreadRealtimeListVoicesParams struct{}

// ThreadRealtimeListVoicesResponse response for listing supported realtime voices.
type ThreadRealtimeListVoicesResponse struct {
	Voices RealtimeVoicesList `json:"voices"`
}

// ThreadRealtimeOutputAudioDeltaNotification streamed output audio emitted by thread realtime.
type ThreadRealtimeOutputAudioDeltaNotification struct {
	Audio    ThreadRealtimeAudioChunk `json:"audio"`
	ThreadID string                   `json:"threadId"`
}

// ThreadRealtimeSDPNotification emitted with the remote SDP for a WebRTC realtime session.
type ThreadRealtimeSDPNotification struct {
	SDP      string `json:"sdp"`
	ThreadID string `json:"threadId"`
}

// ThreadRealtimeStartParams start a thread-scoped realtime session.
type ThreadRealtimeStartParams struct {
	// ClientManagedHandoffs leaves Codex response handoffs to the client's explicit append calls instead of forwarding them automatically. Defaults to false.
	ClientManagedHandoffs bool `json:"clientManagedHandoffs,omitzero"`

	// CodexResponseHandoffChannelPrefixes overrides BEM channel prefixes by `analysis`, `commentary`, or `final`. Omitted channels retain their default uppercase bracketed prefixes.
	CodexResponseHandoffChannelPrefixes map[string][]string `json:"codexResponseHandoffChannelPrefixes,omitzero"`

	// CodexResponseHandoffMode selects how automatic Codex responses are routed in Frameless Bidi sessions. Omitted values default to `thinking`. Realtime V1 and V2 ignore this setting.
	CodexResponseHandoffMode CodexResponseHandoffMode `json:"codexResponseHandoffMode,omitzero"`

	// CodexResponseItemPrefix optional prefix added to automatic Codex response items when `codexResponsesAsItems` is true.
	CodexResponseItemPrefix string `json:"codexResponseItemPrefix,omitzero"`

	// CodexResponsesAsItems sends automatic Codex responses as realtime conversation items instead of handoff appends.
	CodexResponsesAsItems bool `json:"codexResponsesAsItems,omitzero"`

	// FlushTranscriptTailOnSessionEnd routes any transcript tail remaining at session end through Codex. Defaults to false. TODO: Remove this rollout knob once transcript-tail flushing is always enabled.
	FlushTranscriptTailOnSessionEnd bool `json:"flushTranscriptTailOnSessionEnd,omitzero"`

	// IncludeStartupContext sets to false to start without Codex's startup context. Omitted or null includes it.
	IncludeStartupContext bool `json:"includeStartupContext,omitzero"`

	// InitialItems adds complete role-bearing text items to the initial Frameless Bidi session history. This is only supported by realtime V3 and is sent during session startup. Requests are limited to 128 items and 8,192 estimated text tokens in total.
	InitialItems []ThreadRealtimeInitialItem `json:"initialItems,omitzero"`

	// Model overrides the configured realtime model for this session only.
	Model string `json:"model,omitzero"`

	// OutputModality selects text or audio output for the realtime session. Transport and voice stay independent so clients can choose how they connect separately from what the model emits.
	OutputModality    RealtimeOutputModality       `json:"outputModality"`
	Prompt            string                       `json:"prompt,omitzero"`
	RealtimeSessionID string                       `json:"realtimeSessionId,omitzero"`
	ThreadID          string                       `json:"threadId"`
	Transport         ThreadRealtimeStartTransport `json:"transport,omitzero"`

	// Version overrides the configured realtime protocol version for this session only.
	Version RealtimeConversationVersion `json:"version,omitzero"`
	Voice   RealtimeVoice               `json:"voice,omitzero"`
}

func (value *ThreadRealtimeStartParams) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		ClientManagedHandoffs               bool                        `json:"clientManagedHandoffs,omitzero"`
		CodexResponseHandoffChannelPrefixes map[string][]string         `json:"codexResponseHandoffChannelPrefixes,omitzero"`
		CodexResponseHandoffMode            CodexResponseHandoffMode    `json:"codexResponseHandoffMode,omitzero"`
		CodexResponseItemPrefix             string                      `json:"codexResponseItemPrefix,omitzero"`
		CodexResponsesAsItems               bool                        `json:"codexResponsesAsItems,omitzero"`
		FlushTranscriptTailOnSessionEnd     bool                        `json:"flushTranscriptTailOnSessionEnd,omitzero"`
		IncludeStartupContext               bool                        `json:"includeStartupContext,omitzero"`
		InitialItems                        []ThreadRealtimeInitialItem `json:"initialItems,omitzero"`
		Model                               string                      `json:"model,omitzero"`
		OutputModality                      RealtimeOutputModality      `json:"outputModality"`
		Prompt                              string                      `json:"prompt,omitzero"`
		RealtimeSessionID                   string                      `json:"realtimeSessionId,omitzero"`
		ThreadID                            string                      `json:"threadId"`
		Transport                           jsontext.Value              `json:"transport,omitzero"`
		Version                             RealtimeConversationVersion `json:"version,omitzero"`
		Voice                               RealtimeVoice               `json:"voice,omitzero"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.ClientManagedHandoffs = raw.ClientManagedHandoffs
	value.CodexResponseHandoffChannelPrefixes = raw.CodexResponseHandoffChannelPrefixes
	value.CodexResponseHandoffMode = raw.CodexResponseHandoffMode
	value.CodexResponseItemPrefix = raw.CodexResponseItemPrefix
	value.CodexResponsesAsItems = raw.CodexResponsesAsItems
	value.FlushTranscriptTailOnSessionEnd = raw.FlushTranscriptTailOnSessionEnd
	value.IncludeStartupContext = raw.IncludeStartupContext
	value.InitialItems = raw.InitialItems
	value.Model = raw.Model
	value.OutputModality = raw.OutputModality
	value.Prompt = raw.Prompt
	value.RealtimeSessionID = raw.RealtimeSessionID
	value.ThreadID = raw.ThreadID
	if raw.Transport == nil {
		value.Transport = nil
	} else {
		decodedTransport, err := decodeGeneratedThreadRealtimeStartTransport(raw.Transport)
		if err != nil {
			return err
		}
		value.Transport = decodedTransport
	}
	value.Version = raw.Version
	value.Voice = raw.Voice
	return nil
}

// ThreadRealtimeStartResponse response for starting thread realtime.
type ThreadRealtimeStartResponse struct{}

// ThreadRealtimeStartTransport transport used by thread realtime.
type ThreadRealtimeStartTransport interface {
	isThreadRealtimeStartTransport()
}

// RawThreadRealtimeStartTransport preserves an uninterpreted ThreadRealtimeStartTransport JSON value.
type RawThreadRealtimeStartTransport jsontext.Value

func (RawThreadRealtimeStartTransport) isThreadRealtimeStartTransport() {}

var (
	_ json.MarshalerTo     = RawThreadRealtimeStartTransport{}
	_ json.UnmarshalerFrom = (*RawThreadRealtimeStartTransport)(nil)
)

func (value RawThreadRealtimeStartTransport) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawThreadRealtimeStartTransport) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// WebsocketThreadRealtimeStartTransport is generated from the WebsocketThreadRealtimeStartTransport schema definition.
type WebsocketThreadRealtimeStartTransport struct {
	Type string `json:"type"`
}

func (WebsocketThreadRealtimeStartTransport) isThreadRealtimeStartTransport() {}

// WebrtcThreadRealtimeStartTransport is generated from the WebrtcThreadRealtimeStartTransport schema definition.
type WebrtcThreadRealtimeStartTransport struct {
	// SDP offer generated by a WebRTC RTCPeerConnection after configuring audio and the realtime events data channel.
	SDP  string `json:"sdp"`
	Type string `json:"type"`
}

func (WebrtcThreadRealtimeStartTransport) isThreadRealtimeStartTransport() {}

func decodeGeneratedThreadRealtimeStartTransport(raw jsontext.Value) (ThreadRealtimeStartTransport, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "websocket":
			var value WebsocketThreadRealtimeStartTransport
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "webrtc":
			var value WebrtcThreadRealtimeStartTransport
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawThreadRealtimeStartTransport(raw), nil
	}
	return RawThreadRealtimeStartTransport(raw), nil
}

// ThreadRealtimeStartedNotification emitted when thread realtime startup is accepted.
type ThreadRealtimeStartedNotification struct {
	RealtimeSessionID string                      `json:"realtimeSessionId,omitzero"`
	ThreadID          string                      `json:"threadId"`
	Version           RealtimeConversationVersion `json:"version"`
}

// ThreadRealtimeStopParams stop thread realtime.
type ThreadRealtimeStopParams struct {
	ThreadID string `json:"threadId"`
}

// ThreadRealtimeStopResponse response for stopping thread realtime.
type ThreadRealtimeStopResponse struct{}

// ThreadRealtimeTranscriptDeltaNotification flat transcript delta emitted whenever realtime transcript text changes.
type ThreadRealtimeTranscriptDeltaNotification struct {
	// Delta live transcript delta from the realtime event.
	Delta    string `json:"delta"`
	Role     string `json:"role"`
	ThreadID string `json:"threadId"`
}

// ThreadRealtimeTranscriptDoneNotification final transcript text emitted when realtime completes a transcript part.
type ThreadRealtimeTranscriptDoneNotification struct {
	Role string `json:"role"`

	// Text final complete text for the transcript part.
	Text     string `json:"text"`
	ThreadID string `json:"threadId"`
}

// ThreadResumeInitialTurnsPageParams is generated from the ThreadResumeInitialTurnsPageParams schema definition.
type ThreadResumeInitialTurnsPageParams struct {
	// ItemsView how much item detail to include for each returned turn; defaults to summary.
	ItemsView TurnItemsView `json:"itemsView,omitzero"`

	// Limit optional turn page size.
	Limit int32 `json:"limit,omitzero"`

	// SortDirection optional turn pagination direction; defaults to descending.
	SortDirection SortDirection `json:"sortDirection,omitzero"`
}

func (value *ThreadResumeInitialTurnsPageParams) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		ItemsView     jsontext.Value `json:"itemsView,omitzero"`
		Limit         int32          `json:"limit,omitzero"`
		SortDirection SortDirection  `json:"sortDirection,omitzero"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	if raw.ItemsView == nil {
		value.ItemsView = nil
	} else {
		decodedItemsView, err := decodeGeneratedTurnItemsView(raw.ItemsView)
		if err != nil {
			return err
		}
		value.ItemsView = decodedItemsView
	}
	value.Limit = raw.Limit
	value.SortDirection = raw.SortDirection
	return nil
}

// ThreadResumeParams there are three ways to resume a thread: 1. By thread_id: load the thread from disk by thread_id and resume it. 2. By history: instantiate the thread from memory and resume it. 3. By path: load the thread from disk by path and resume it.
//
// For non-running threads, the precedence is: history > non-empty path > thread_id. If using history or a non-empty path for a non-running thread, the thread_id param will be ignored.
//
// If thread_id identifies a running thread, app-server rejoins that thread and treats a non-empty path as a consistency check against the active rollout path. Empty string path values are treated as absent.
//
// Prefer using thread_id whenever possible.
type ThreadResumeParams struct {
	ApprovalPolicy AskForApproval `json:"approvalPolicy,omitzero"`

	// ApprovalsReviewer overrides where approval requests are routed for review on this thread and subsequent turns.
	ApprovalsReviewer     ApprovalsReviewer         `json:"approvalsReviewer,omitzero"`
	BaseInstructions      string                    `json:"baseInstructions,omitzero"`
	Config                map[string]jsontext.Value `json:"config,omitzero"`
	Cwd                   string                    `json:"cwd,omitzero"`
	DeveloperInstructions string                    `json:"developerInstructions,omitzero"`

	// ExcludeTurns when true, return only thread metadata and live-resume state without populating `thread.turns`. This is useful when the client plans to call `thread/turns/list` immediately after resuming.
	ExcludeTurns bool `json:"excludeTurns,omitzero"`

	// History [UNSTABLE] FOR CODEX CLOUD - DO NOT USE. If specified, the thread will be resumed with the provided history instead of loaded from disk.
	History []ResponseItem `json:"history,omitzero"`

	// InitialTurnsPage when present, include a `thread/turns/list` page in the resume response so clients can bootstrap recent turns without a second request.
	InitialTurnsPage ThreadResumeInitialTurnsPageParams `json:"initialTurnsPage,omitzero"`

	// Model configuration overrides for the resumed thread, if any.
	Model         string `json:"model,omitzero"`
	ModelProvider string `json:"modelProvider,omitzero"`

	// Path [UNSTABLE] Specify the rollout path to resume from. If specified for a non-running thread, the thread_id param will be ignored. If thread_id identifies a running thread, the path must match the active rollout path.
	Path string `json:"path,omitzero"`

	// Permissions named profile id for the resumed thread. Cannot be combined with `sandbox`.
	Permissions string      `json:"permissions,omitzero"`
	Personality Personality `json:"personality,omitzero"`

	// RuntimeWorkspaceRoots replaces the thread's runtime workspace roots. Paths must be absolute.
	RuntimeWorkspaceRoots []string    `json:"runtimeWorkspaceRoots,omitzero"`
	Sandbox               SandboxMode `json:"sandbox,omitzero"`
	ServiceTier           string      `json:"serviceTier,omitzero"`
	ThreadID              string      `json:"threadId"`
}

func (value *ThreadResumeParams) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		ApprovalPolicy        jsontext.Value                     `json:"approvalPolicy,omitzero"`
		ApprovalsReviewer     ApprovalsReviewer                  `json:"approvalsReviewer,omitzero"`
		BaseInstructions      string                             `json:"baseInstructions,omitzero"`
		Config                map[string]jsontext.Value          `json:"config,omitzero"`
		Cwd                   string                             `json:"cwd,omitzero"`
		DeveloperInstructions string                             `json:"developerInstructions,omitzero"`
		ExcludeTurns          bool                               `json:"excludeTurns,omitzero"`
		History               []jsontext.Value                   `json:"history,omitzero"`
		InitialTurnsPage      ThreadResumeInitialTurnsPageParams `json:"initialTurnsPage,omitzero"`
		Model                 string                             `json:"model,omitzero"`
		ModelProvider         string                             `json:"modelProvider,omitzero"`
		Path                  string                             `json:"path,omitzero"`
		Permissions           string                             `json:"permissions,omitzero"`
		Personality           Personality                        `json:"personality,omitzero"`
		RuntimeWorkspaceRoots []string                           `json:"runtimeWorkspaceRoots,omitzero"`
		Sandbox               SandboxMode                        `json:"sandbox,omitzero"`
		ServiceTier           string                             `json:"serviceTier,omitzero"`
		ThreadID              string                             `json:"threadId"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	if raw.ApprovalPolicy == nil {
		value.ApprovalPolicy = nil
	} else {
		decodedApprovalPolicy, err := decodeGeneratedAskForApproval(raw.ApprovalPolicy)
		if err != nil {
			return err
		}
		value.ApprovalPolicy = decodedApprovalPolicy
	}
	value.ApprovalsReviewer = raw.ApprovalsReviewer
	value.BaseInstructions = raw.BaseInstructions
	value.Config = raw.Config
	value.Cwd = raw.Cwd
	value.DeveloperInstructions = raw.DeveloperInstructions
	value.ExcludeTurns = raw.ExcludeTurns
	if raw.History == nil {
		value.History = nil
	} else {
		value.History = make([]ResponseItem, len(raw.History))
		for i, item := range raw.History {
			if item != nil {
				decodedHistory, err := decodeGeneratedResponseItem(item)
				if err != nil {
					return err
				}
				value.History[i] = decodedHistory
			}
		}
	}
	value.InitialTurnsPage = raw.InitialTurnsPage
	value.Model = raw.Model
	value.ModelProvider = raw.ModelProvider
	value.Path = raw.Path
	value.Permissions = raw.Permissions
	value.Personality = raw.Personality
	value.RuntimeWorkspaceRoots = raw.RuntimeWorkspaceRoots
	value.Sandbox = raw.Sandbox
	value.ServiceTier = raw.ServiceTier
	value.ThreadID = raw.ThreadID
	return nil
}

// ThreadResumeResponse is generated from the ThreadResumeResponse schema definition.
type ThreadResumeResponse struct {
	// ActivePermissionProfile named or implicit built-in profile that produced the active permissions, when known.
	ActivePermissionProfile ActivePermissionProfile `json:"activePermissionProfile,omitzero"`
	ApprovalPolicy          AskForApproval          `json:"approvalPolicy"`

	// ApprovalsReviewer reviewer currently used for approval requests on this thread.
	ApprovalsReviewer ApprovalsReviewer `json:"approvalsReviewer"`
	Cwd               string            `json:"cwd"`

	// InitialTurnsPage `thread/turns/list` page returned when requested by `initialTurnsPage`.
	InitialTurnsPage TurnsPage `json:"initialTurnsPage,omitzero"`

	// InstructionSources environment-native paths to instruction source files currently loaded for this thread.
	InstructionSources []LegacyAppPathString `json:"instructionSources,omitzero"`

	// ItemsBackwardsCursor opaque head cursor for hydrating paginated items backwards.
	//
	// Pass this as `cursor` to `thread/items/list` with `sortDirection: "desc"`. The first page includes the cursor's head item.
	ItemsBackwardsCursor string `json:"itemsBackwardsCursor,omitzero"`
	Model                string `json:"model"`
	ModelProvider        string `json:"modelProvider"`

	// MultiAgentMode @deprecated Always `explicitRequestOnly`. Use `reasoningEffort` for Ultra behavior.
	MultiAgentMode  MultiAgentMode   `json:"multiAgentMode,omitzero"`
	ReasoningEffort *ReasoningEffort `json:"reasoningEffort,omitzero"`

	// RuntimeWorkspaceRoots thread-scoped runtime workspace roots used to materialize `:workspace_roots`.
	RuntimeWorkspaceRoots []string `json:"runtimeWorkspaceRoots,omitzero"`

	// Sandbox legacy sandbox policy retained for compatibility. Experimental clients should prefer `activePermissionProfile` for profile provenance.
	Sandbox     SandboxPolicy `json:"sandbox"`
	ServiceTier string        `json:"serviceTier,omitzero"`
	Thread      ThreadPayload `json:"thread"`

	// TurnsBackwardsCursor opaque head cursor for hydrating paginated turns backwards.
	//
	// Pass this as `cursor` to `thread/turns/list` with `sortDirection: "desc"`. The first page includes the cursor's head turn.
	TurnsBackwardsCursor string `json:"turnsBackwardsCursor,omitzero"`
}

func (value *ThreadResumeResponse) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		ActivePermissionProfile ActivePermissionProfile `json:"activePermissionProfile,omitzero"`
		ApprovalPolicy          jsontext.Value          `json:"approvalPolicy"`
		ApprovalsReviewer       ApprovalsReviewer       `json:"approvalsReviewer"`
		Cwd                     string                  `json:"cwd"`
		InitialTurnsPage        TurnsPage               `json:"initialTurnsPage,omitzero"`
		InstructionSources      []LegacyAppPathString   `json:"instructionSources,omitzero"`
		ItemsBackwardsCursor    string                  `json:"itemsBackwardsCursor,omitzero"`
		Model                   string                  `json:"model"`
		ModelProvider           string                  `json:"modelProvider"`
		MultiAgentMode          jsontext.Value          `json:"multiAgentMode,omitzero"`
		ReasoningEffort         *ReasoningEffort        `json:"reasoningEffort,omitzero"`
		RuntimeWorkspaceRoots   []string                `json:"runtimeWorkspaceRoots,omitzero"`
		Sandbox                 jsontext.Value          `json:"sandbox"`
		ServiceTier             string                  `json:"serviceTier,omitzero"`
		Thread                  ThreadPayload           `json:"thread"`
		TurnsBackwardsCursor    string                  `json:"turnsBackwardsCursor,omitzero"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.ActivePermissionProfile = raw.ActivePermissionProfile
	if raw.ApprovalPolicy == nil {
		value.ApprovalPolicy = nil
	} else {
		decodedApprovalPolicy, err := decodeGeneratedAskForApproval(raw.ApprovalPolicy)
		if err != nil {
			return err
		}
		value.ApprovalPolicy = decodedApprovalPolicy
	}
	value.ApprovalsReviewer = raw.ApprovalsReviewer
	value.Cwd = raw.Cwd
	value.InitialTurnsPage = raw.InitialTurnsPage
	value.InstructionSources = raw.InstructionSources
	value.ItemsBackwardsCursor = raw.ItemsBackwardsCursor
	value.Model = raw.Model
	value.ModelProvider = raw.ModelProvider
	if raw.MultiAgentMode == nil {
		value.MultiAgentMode = nil
	} else {
		decodedMultiAgentMode, err := decodeGeneratedMultiAgentMode(raw.MultiAgentMode)
		if err != nil {
			return err
		}
		value.MultiAgentMode = decodedMultiAgentMode
	}
	value.ReasoningEffort = raw.ReasoningEffort
	value.RuntimeWorkspaceRoots = raw.RuntimeWorkspaceRoots
	if raw.Sandbox == nil {
		value.Sandbox = nil
	} else {
		decodedSandbox, err := decodeGeneratedSandboxPolicy(raw.Sandbox)
		if err != nil {
			return err
		}
		value.Sandbox = decodedSandbox
	}
	value.ServiceTier = raw.ServiceTier
	value.Thread = raw.Thread
	value.TurnsBackwardsCursor = raw.TurnsBackwardsCursor
	return nil
}

// ThreadRollbackParams DEPRECATED: `thread/rollback` will be removed soon.
type ThreadRollbackParams struct {
	// NumTurns represents a number of turns to drop from the end of the thread. Must be >= 1.
	//
	// This only modifies the thread's history and does not revert local file changes that have been made by the agent. Clients are responsible for reverting these changes.
	NumTurns int32  `json:"numTurns"`
	ThreadID string `json:"threadId"`
}

// ThreadRollbackResponse is generated from the ThreadRollbackResponse schema definition.
type ThreadRollbackResponse struct {
	// Thread represents an updated thread after applying the rollback, with `turns` populated.
	//
	// The ThreadItems stored in each Turn are lossy since we explicitly do not persist all agent interactions, such as command executions. This is the same behavior as `thread/resume`.
	Thread ThreadPayload `json:"thread"`
}

// ThreadSearchOccurrence one visible message occurrence returned by [`ThreadSearchOccurrencesResponse`].
type ThreadSearchOccurrence struct {
	ItemID  string `json:"itemId"`
	Snippet string `json:"snippet"`

	// SnippetMatchRange matches range within `snippet`, in UTF-16 code units.
	SnippetMatchRange ThreadSearchTextRange `json:"snippetMatchRange"`

	// TurnCursor opaque inclusive cursor accepted by `thread/turns/list` for this turn.
	TurnCursor string `json:"turnCursor"`
	TurnID     string `json:"turnId"`
}

// ThreadSearchOccurrencesParams parameters for searching visible message occurrences within one paginated thread.
type ThreadSearchOccurrencesParams struct {
	// Cursor opaque cursor returned by a previous call for the same thread and search term.
	Cursor string `json:"cursor,omitzero"`

	// Limit optional occurrence page size.
	Limit int32 `json:"limit,omitzero"`

	// SearchTerm case-insensitive literal substring to find in visible user messages and final assistant messages.
	SearchTerm string `json:"searchTerm"`
	ThreadID   string `json:"threadId"`
}

// ThreadSearchOccurrencesResponse is generated from the ThreadSearchOccurrencesResponse schema definition.
type ThreadSearchOccurrencesResponse struct {
	// Data occurrences in chronological message order.
	Data []ThreadSearchOccurrence `json:"data"`

	// NextCursor opaque cursor to continue after the last returned occurrence.
	NextCursor string `json:"nextCursor,omitzero"`
}

// ThreadSearchParams is generated from the ThreadSearchParams schema definition.
type ThreadSearchParams struct {
	// Archived optional archived filter; when set to true, only archived threads are returned. If false or null, only non-archived threads are returned.
	Archived bool `json:"archived,omitzero"`

	// Cursor opaque pagination cursor returned by a previous call.
	Cursor string `json:"cursor,omitzero"`

	// Limit optional page size; defaults to a reasonable server-side value.
	Limit int32 `json:"limit,omitzero"`

	// SearchTerm required substring/full-text query for thread search.
	SearchTerm string `json:"searchTerm"`

	// SortDirection optional sort direction; defaults to descending (newest first).
	SortDirection SortDirection `json:"sortDirection,omitzero"`

	// SortKey optional sort key; defaults to created_at.
	SortKey ThreadSearchSortKey `json:"sortKey,omitzero"`

	// SourceKinds optional source filter; when set, only sessions from these source kinds are returned. When omitted or empty, defaults to interactive sources.
	SourceKinds []ThreadSourceKind `json:"sourceKinds,omitzero"`
}

// ThreadSearchResponse is generated from the ThreadSearchResponse schema definition.
type ThreadSearchResponse struct {
	// BackwardsCursor opaque cursor to pass as `cursor` when reversing `sortDirection`. This is only populated when the page contains at least one thread. Use it with the opposite `sortDirection`; for timestamp sorts it anchors at the start of the page timestamp so same-second updates are not skipped.
	BackwardsCursor string               `json:"backwardsCursor,omitzero"`
	Data            []ThreadSearchResult `json:"data"`

	// NextCursor opaque cursor to pass to the next call to continue after the last item. if None, there are no more items to return.
	NextCursor string `json:"nextCursor,omitzero"`
}

// ThreadSearchResult is generated from the ThreadSearchResult schema definition.
type ThreadSearchResult struct {
	Snippet string        `json:"snippet"`
	Thread  ThreadPayload `json:"thread"`
}

// ThreadSearchSortKey is generated from the ThreadSearchSortKey schema definition.
type ThreadSearchSortKey string

const (
	// ThreadSearchSortKeyCreatedAt is the "created_at" ThreadSearchSortKey value.
	ThreadSearchSortKeyCreatedAt ThreadSearchSortKey = "created_at"
	// ThreadSearchSortKeyUpdatedAt is the "updated_at" ThreadSearchSortKey value.
	ThreadSearchSortKeyUpdatedAt ThreadSearchSortKey = "updated_at"
	// ThreadSearchSortKeyRecencyAt is the "recency_at" ThreadSearchSortKey value.
	ThreadSearchSortKeyRecencyAt ThreadSearchSortKey = "recency_at"
)

// ThreadSearchTextRange UTF-16 code-unit range within `snippet`.
type ThreadSearchTextRange struct {
	// End exclusive UTF-16 code-unit offset.
	End int32 `json:"end"`

	// Start inclusive UTF-16 code-unit offset.
	Start int32 `json:"start"`
}

// ThreadSection represents an independently persisted, user-visible thread section.
type ThreadSection struct {
	// ID opaque UUIDv7 identity that remains stable when the section is renamed.
	ID string `json:"id"`

	// Name represents a current user-visible section name.
	Name string `json:"name"`
}

// ThreadSectionListParams parameters for listing independently persisted thread sections.
type ThreadSectionListParams struct {
	// Cursor opaque pagination cursor returned by a previous call.
	Cursor string `json:"cursor,omitzero"`

	// Limit maximum number of sections to return.
	Limit int32 `json:"limit,omitzero"`
}

// ThreadSectionListResponse one page of independently persisted thread sections.
type ThreadSectionListResponse struct {
	Data []ThreadSection `json:"data"`

	// NextCursor opaque cursor for the next page, or `null` when no sections remain.
	NextCursor string `json:"nextCursor,omitzero"`
}

// ThreadSectionMoveParams parameters for moving a thread within a server-owned section ordering.
type ThreadSectionMoveParams struct {
	// BeforeThreadID existing thread to insert before; omission or null appends to the section.
	BeforeThreadID string `json:"beforeThreadId,omitzero"`

	// SectionID destination section, or `null` to remove the thread from its section.
	SectionID *string `json:"sectionId"`

	// ThreadID thread to move into, within, or out of a section.
	ThreadID string `json:"threadId"`
}

// ThreadSectionMoveResponse is generated from the ThreadSectionMoveResponse schema definition.
type ThreadSectionMoveResponse struct{}

// ThreadSetNameParams is generated from the ThreadSetNameParams schema definition.
type ThreadSetNameParams struct {
	Name     string `json:"name"`
	ThreadID string `json:"threadId"`
}

// ThreadSetNameResponse is generated from the ThreadSetNameResponse schema definition.
type ThreadSetNameResponse struct{}

// ThreadSettings is generated from the ThreadSettings schema definition.
type ThreadSettings struct {
	ActivePermissionProfile ActivePermissionProfile `json:"activePermissionProfile,omitzero"`
	ApprovalPolicy          AskForApproval          `json:"approvalPolicy"`
	ApprovalsReviewer       ApprovalsReviewer       `json:"approvalsReviewer"`
	CollaborationMode       CollaborationMode       `json:"collaborationMode"`
	Cwd                     string                  `json:"cwd"`
	Effort                  *ReasoningEffort        `json:"effort,omitzero"`
	Model                   string                  `json:"model"`
	ModelProvider           string                  `json:"modelProvider"`

	// MultiAgentMode @deprecated Always `explicitRequestOnly`. Use `effort` for Ultra behavior.
	MultiAgentMode MultiAgentMode   `json:"multiAgentMode,omitzero"`
	Personality    Personality      `json:"personality,omitzero"`
	SandboxPolicy  SandboxPolicy    `json:"sandboxPolicy"`
	ServiceTier    string           `json:"serviceTier,omitzero"`
	Summary        ReasoningSummary `json:"summary,omitzero"`
}

func (value *ThreadSettings) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		ActivePermissionProfile ActivePermissionProfile `json:"activePermissionProfile,omitzero"`
		ApprovalPolicy          jsontext.Value          `json:"approvalPolicy"`
		ApprovalsReviewer       ApprovalsReviewer       `json:"approvalsReviewer"`
		CollaborationMode       CollaborationMode       `json:"collaborationMode"`
		Cwd                     string                  `json:"cwd"`
		Effort                  *ReasoningEffort        `json:"effort,omitzero"`
		Model                   string                  `json:"model"`
		ModelProvider           string                  `json:"modelProvider"`
		MultiAgentMode          jsontext.Value          `json:"multiAgentMode,omitzero"`
		Personality             Personality             `json:"personality,omitzero"`
		SandboxPolicy           jsontext.Value          `json:"sandboxPolicy"`
		ServiceTier             string                  `json:"serviceTier,omitzero"`
		Summary                 jsontext.Value          `json:"summary,omitzero"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.ActivePermissionProfile = raw.ActivePermissionProfile
	if raw.ApprovalPolicy == nil {
		value.ApprovalPolicy = nil
	} else {
		decodedApprovalPolicy, err := decodeGeneratedAskForApproval(raw.ApprovalPolicy)
		if err != nil {
			return err
		}
		value.ApprovalPolicy = decodedApprovalPolicy
	}
	value.ApprovalsReviewer = raw.ApprovalsReviewer
	value.CollaborationMode = raw.CollaborationMode
	value.Cwd = raw.Cwd
	value.Effort = raw.Effort
	value.Model = raw.Model
	value.ModelProvider = raw.ModelProvider
	if raw.MultiAgentMode == nil {
		value.MultiAgentMode = nil
	} else {
		decodedMultiAgentMode, err := decodeGeneratedMultiAgentMode(raw.MultiAgentMode)
		if err != nil {
			return err
		}
		value.MultiAgentMode = decodedMultiAgentMode
	}
	value.Personality = raw.Personality
	if raw.SandboxPolicy == nil {
		value.SandboxPolicy = nil
	} else {
		decodedSandboxPolicy, err := decodeGeneratedSandboxPolicy(raw.SandboxPolicy)
		if err != nil {
			return err
		}
		value.SandboxPolicy = decodedSandboxPolicy
	}
	value.ServiceTier = raw.ServiceTier
	if raw.Summary == nil {
		value.Summary = nil
	} else {
		decodedSummary, err := decodeGeneratedReasoningSummary(raw.Summary)
		if err != nil {
			return err
		}
		value.Summary = decodedSummary
	}
	return nil
}

// ThreadSettingsUpdateParams is generated from the ThreadSettingsUpdateParams schema definition.
type ThreadSettingsUpdateParams struct {
	// ApprovalPolicy overrides the approval policy for subsequent turns.
	ApprovalPolicy AskForApproval `json:"approvalPolicy,omitzero"`

	// ApprovalsReviewer overrides where approval requests are routed for subsequent turns.
	ApprovalsReviewer ApprovalsReviewer `json:"approvalsReviewer,omitzero"`

	// CollaborationMode sets a pre-set collaboration mode for subsequent turns.
	//
	// For `collaboration_mode.settings.developer_instructions`, `null` means "use the built-in instructions for the selected mode".
	CollaborationMode CollaborationMode `json:"collaborationMode,omitzero"`

	// Cwd overrides the working directory for subsequent turns.
	Cwd string `json:"cwd,omitzero"`

	// Effort overrides the reasoning effort for subsequent turns.
	Effort *ReasoningEffort `json:"effort,omitzero"`

	// Model overrides the model for subsequent turns.
	Model string `json:"model,omitzero"`

	// MultiAgentMode @deprecated Ignored. Use `effort: "ultra"` for proactive multi-agent behavior.
	MultiAgentMode MultiAgentMode `json:"multiAgentMode,omitzero"`

	// Permissions selects a named permissions profile id for subsequent turns. Cannot be combined with `sandboxPolicy`.
	Permissions string `json:"permissions,omitzero"`

	// Personality overrides the personality for subsequent turns.
	Personality Personality `json:"personality,omitzero"`

	// SandboxPolicy overrides the sandbox policy for subsequent turns.
	SandboxPolicy SandboxPolicy `json:"sandboxPolicy,omitzero"`

	// ServiceTier overrides the service tier for subsequent turns. `null` clears the current service tier; omission leaves it unchanged.
	ServiceTier string `json:"serviceTier,omitzero"`

	// Summary overrides the reasoning summary for subsequent turns.
	Summary  ReasoningSummary `json:"summary,omitzero"`
	ThreadID string           `json:"threadId"`
}

func (value *ThreadSettingsUpdateParams) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		ApprovalPolicy    jsontext.Value    `json:"approvalPolicy,omitzero"`
		ApprovalsReviewer ApprovalsReviewer `json:"approvalsReviewer,omitzero"`
		CollaborationMode CollaborationMode `json:"collaborationMode,omitzero"`
		Cwd               string            `json:"cwd,omitzero"`
		Effort            *ReasoningEffort  `json:"effort,omitzero"`
		Model             string            `json:"model,omitzero"`
		MultiAgentMode    jsontext.Value    `json:"multiAgentMode,omitzero"`
		Permissions       string            `json:"permissions,omitzero"`
		Personality       Personality       `json:"personality,omitzero"`
		SandboxPolicy     jsontext.Value    `json:"sandboxPolicy,omitzero"`
		ServiceTier       string            `json:"serviceTier,omitzero"`
		Summary           jsontext.Value    `json:"summary,omitzero"`
		ThreadID          string            `json:"threadId"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	if raw.ApprovalPolicy == nil {
		value.ApprovalPolicy = nil
	} else {
		decodedApprovalPolicy, err := decodeGeneratedAskForApproval(raw.ApprovalPolicy)
		if err != nil {
			return err
		}
		value.ApprovalPolicy = decodedApprovalPolicy
	}
	value.ApprovalsReviewer = raw.ApprovalsReviewer
	value.CollaborationMode = raw.CollaborationMode
	value.Cwd = raw.Cwd
	value.Effort = raw.Effort
	value.Model = raw.Model
	if raw.MultiAgentMode == nil {
		value.MultiAgentMode = nil
	} else {
		decodedMultiAgentMode, err := decodeGeneratedMultiAgentMode(raw.MultiAgentMode)
		if err != nil {
			return err
		}
		value.MultiAgentMode = decodedMultiAgentMode
	}
	value.Permissions = raw.Permissions
	value.Personality = raw.Personality
	if raw.SandboxPolicy == nil {
		value.SandboxPolicy = nil
	} else {
		decodedSandboxPolicy, err := decodeGeneratedSandboxPolicy(raw.SandboxPolicy)
		if err != nil {
			return err
		}
		value.SandboxPolicy = decodedSandboxPolicy
	}
	value.ServiceTier = raw.ServiceTier
	if raw.Summary == nil {
		value.Summary = nil
	} else {
		decodedSummary, err := decodeGeneratedReasoningSummary(raw.Summary)
		if err != nil {
			return err
		}
		value.Summary = decodedSummary
	}
	value.ThreadID = raw.ThreadID
	return nil
}

// ThreadSettingsUpdateResponse is generated from the ThreadSettingsUpdateResponse schema definition.
type ThreadSettingsUpdateResponse struct{}

// ThreadSettingsUpdatedNotification is generated from the ThreadSettingsUpdatedNotification schema definition.
type ThreadSettingsUpdatedNotification struct {
	ThreadID       string         `json:"threadId"`
	ThreadSettings ThreadSettings `json:"threadSettings"`
}

// ThreadShellCommandParams is generated from the ThreadShellCommandParams schema definition.
type ThreadShellCommandParams struct {
	// Command shell command string evaluated by the thread's configured shell. Unlike `command/exec`, this intentionally preserves shell syntax such as pipes, redirects, and quoting. This runs unsandboxed with full access rather than inheriting the thread sandbox policy.
	Command  string `json:"command"`
	ThreadID string `json:"threadId"`
}

// ThreadShellCommandResponse is generated from the ThreadShellCommandResponse schema definition.
type ThreadShellCommandResponse struct{}

// ThreadSortKey is generated from the ThreadSortKey schema definition.
type ThreadSortKey string

const (
	// ThreadSortKeyCreatedAt is the "created_at" ThreadSortKey value.
	ThreadSortKeyCreatedAt ThreadSortKey = "created_at"
	// ThreadSortKeyUpdatedAt is the "updated_at" ThreadSortKey value.
	ThreadSortKeyUpdatedAt ThreadSortKey = "updated_at"
	// ThreadSortKeyRecencyAt is the "recency_at" ThreadSortKey value.
	ThreadSortKeyRecencyAt ThreadSortKey = "recency_at"
	// ThreadSortKeySectionPosition is the "section_position" ThreadSortKey value.
	ThreadSortKeySectionPosition ThreadSortKey = "section_position"
)

// ThreadSource is generated from the ThreadSource schema definition.
type ThreadSource string

// ThreadSourceKind is generated from the ThreadSourceKind schema definition.
type ThreadSourceKind string

const (
	// ThreadSourceKindCli is the "cli" ThreadSourceKind value.
	ThreadSourceKindCli ThreadSourceKind = "cli"
	// ThreadSourceKindVscode is the "vscode" ThreadSourceKind value.
	ThreadSourceKindVscode ThreadSourceKind = "vscode"
	// ThreadSourceKindExec is the "exec" ThreadSourceKind value.
	ThreadSourceKindExec ThreadSourceKind = "exec"
	// ThreadSourceKindAppServer is the "appServer" ThreadSourceKind value.
	ThreadSourceKindAppServer ThreadSourceKind = "appServer"
	// ThreadSourceKindSubAgent is the "subAgent" ThreadSourceKind value.
	ThreadSourceKindSubAgent ThreadSourceKind = "subAgent"
	// ThreadSourceKindSubAgentReview is the "subAgentReview" ThreadSourceKind value.
	ThreadSourceKindSubAgentReview ThreadSourceKind = "subAgentReview"
	// ThreadSourceKindSubAgentCompact is the "subAgentCompact" ThreadSourceKind value.
	ThreadSourceKindSubAgentCompact ThreadSourceKind = "subAgentCompact"
	// ThreadSourceKindSubAgentThreadSpawn is the "subAgentThreadSpawn" ThreadSourceKind value.
	ThreadSourceKindSubAgentThreadSpawn ThreadSourceKind = "subAgentThreadSpawn"
	// ThreadSourceKindSubAgentOther is the "subAgentOther" ThreadSourceKind value.
	ThreadSourceKindSubAgentOther ThreadSourceKind = "subAgentOther"
	// ThreadSourceKindUnknown is the "unknown" ThreadSourceKind value.
	ThreadSourceKindUnknown ThreadSourceKind = "unknown"
)

// ThreadStartParams is generated from the ThreadStartParams schema definition.
type ThreadStartParams struct {
	// AllowProviderModelFallback allows a provider with an authoritative static model catalog to replace an unavailable requested model with its default.
	AllowProviderModelFallback bool           `json:"allowProviderModelFallback,omitzero"`
	ApprovalPolicy             AskForApproval `json:"approvalPolicy,omitzero"`

	// ApprovalsReviewer overrides where approval requests are routed for review on this thread and subsequent turns.
	ApprovalsReviewer     ApprovalsReviewer         `json:"approvalsReviewer,omitzero"`
	BaseInstructions      string                    `json:"baseInstructions,omitzero"`
	Config                map[string]jsontext.Value `json:"config,omitzero"`
	Cwd                   string                    `json:"cwd,omitzero"`
	DeveloperInstructions string                    `json:"developerInstructions,omitzero"`
	DynamicTools          []DynamicToolSpec         `json:"dynamicTools,omitzero"`

	// Environments optional sticky environments for this thread.
	//
	// Omitted selects the default environment when environment access is enabled. Empty disables environment access for turns that do not provide a turn override. Non-empty selects the first environment as the current turn environment.
	Environments []TurnEnvironmentParams `json:"environments,omitzero"`
	Ephemeral    bool                    `json:"ephemeral,omitzero"`

	// ExperimentalRawEvents if true, opt into emitting raw Responses API items on the event stream. This is for internal use only (e.g. Codex Cloud).
	ExperimentalRawEvents bool `json:"experimentalRawEvents,omitzero"`

	// HistoryMode persisted thread history contract to use for this new thread.
	HistoryMode ThreadHistoryMode `json:"historyMode,omitzero"`

	// MockExperimentalField test-only experimental field used to validate experimental gating and schema filtering behavior in a stable way.
	MockExperimentalField string `json:"mockExperimentalField,omitzero"`
	Model                 string `json:"model,omitzero"`
	ModelProvider         string `json:"modelProvider,omitzero"`

	// MultiAgentMode @deprecated Ignored. Use Ultra reasoning effort for proactive multi-agent behavior.
	MultiAgentMode MultiAgentMode `json:"multiAgentMode,omitzero"`

	// Permissions named profile id for this thread. Cannot be combined with `sandbox`.
	Permissions string      `json:"permissions,omitzero"`
	Personality Personality `json:"personality,omitzero"`

	// RuntimeWorkspaceRoots replaces the thread's runtime workspace roots. Paths must be absolute.
	RuntimeWorkspaceRoots []string    `json:"runtimeWorkspaceRoots,omitzero"`
	Sandbox               SandboxMode `json:"sandbox,omitzero"`

	// SelectedCapabilityRoots capability roots selected for this thread by the hosting platform.
	SelectedCapabilityRoots []SelectedCapabilityRoot `json:"selectedCapabilityRoots,omitzero"`
	ServiceName             string                   `json:"serviceName,omitzero"`
	ServiceTier             string                   `json:"serviceTier,omitzero"`
	SessionStartSource      ThreadStartSource        `json:"sessionStartSource,omitzero"`

	// ThreadSource optional client-supplied analytics source classification for this thread.
	ThreadSource *ThreadSource `json:"threadSource,omitzero"`
}

func (value *ThreadStartParams) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		AllowProviderModelFallback bool                      `json:"allowProviderModelFallback,omitzero"`
		ApprovalPolicy             jsontext.Value            `json:"approvalPolicy,omitzero"`
		ApprovalsReviewer          ApprovalsReviewer         `json:"approvalsReviewer,omitzero"`
		BaseInstructions           string                    `json:"baseInstructions,omitzero"`
		Config                     map[string]jsontext.Value `json:"config,omitzero"`
		Cwd                        string                    `json:"cwd,omitzero"`
		DeveloperInstructions      string                    `json:"developerInstructions,omitzero"`
		DynamicTools               []jsontext.Value          `json:"dynamicTools,omitzero"`
		Environments               []TurnEnvironmentParams   `json:"environments,omitzero"`
		Ephemeral                  bool                      `json:"ephemeral,omitzero"`
		ExperimentalRawEvents      bool                      `json:"experimentalRawEvents,omitzero"`
		HistoryMode                ThreadHistoryMode         `json:"historyMode,omitzero"`
		MockExperimentalField      string                    `json:"mockExperimentalField,omitzero"`
		Model                      string                    `json:"model,omitzero"`
		ModelProvider              string                    `json:"modelProvider,omitzero"`
		MultiAgentMode             jsontext.Value            `json:"multiAgentMode,omitzero"`
		Permissions                string                    `json:"permissions,omitzero"`
		Personality                Personality               `json:"personality,omitzero"`
		RuntimeWorkspaceRoots      []string                  `json:"runtimeWorkspaceRoots,omitzero"`
		Sandbox                    SandboxMode               `json:"sandbox,omitzero"`
		SelectedCapabilityRoots    []SelectedCapabilityRoot  `json:"selectedCapabilityRoots,omitzero"`
		ServiceName                string                    `json:"serviceName,omitzero"`
		ServiceTier                string                    `json:"serviceTier,omitzero"`
		SessionStartSource         ThreadStartSource         `json:"sessionStartSource,omitzero"`
		ThreadSource               *ThreadSource             `json:"threadSource,omitzero"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.AllowProviderModelFallback = raw.AllowProviderModelFallback
	if raw.ApprovalPolicy == nil {
		value.ApprovalPolicy = nil
	} else {
		decodedApprovalPolicy, err := decodeGeneratedAskForApproval(raw.ApprovalPolicy)
		if err != nil {
			return err
		}
		value.ApprovalPolicy = decodedApprovalPolicy
	}
	value.ApprovalsReviewer = raw.ApprovalsReviewer
	value.BaseInstructions = raw.BaseInstructions
	value.Config = raw.Config
	value.Cwd = raw.Cwd
	value.DeveloperInstructions = raw.DeveloperInstructions
	if raw.DynamicTools == nil {
		value.DynamicTools = nil
	} else {
		value.DynamicTools = make([]DynamicToolSpec, len(raw.DynamicTools))
		for i, item := range raw.DynamicTools {
			if item != nil {
				decodedDynamicTools, err := decodeGeneratedDynamicToolSpec(item)
				if err != nil {
					return err
				}
				value.DynamicTools[i] = decodedDynamicTools
			}
		}
	}
	value.Environments = raw.Environments
	value.Ephemeral = raw.Ephemeral
	value.ExperimentalRawEvents = raw.ExperimentalRawEvents
	value.HistoryMode = raw.HistoryMode
	value.MockExperimentalField = raw.MockExperimentalField
	value.Model = raw.Model
	value.ModelProvider = raw.ModelProvider
	if raw.MultiAgentMode == nil {
		value.MultiAgentMode = nil
	} else {
		decodedMultiAgentMode, err := decodeGeneratedMultiAgentMode(raw.MultiAgentMode)
		if err != nil {
			return err
		}
		value.MultiAgentMode = decodedMultiAgentMode
	}
	value.Permissions = raw.Permissions
	value.Personality = raw.Personality
	value.RuntimeWorkspaceRoots = raw.RuntimeWorkspaceRoots
	value.Sandbox = raw.Sandbox
	value.SelectedCapabilityRoots = raw.SelectedCapabilityRoots
	value.ServiceName = raw.ServiceName
	value.ServiceTier = raw.ServiceTier
	value.SessionStartSource = raw.SessionStartSource
	value.ThreadSource = raw.ThreadSource
	return nil
}

// ThreadStartResponse is generated from the ThreadStartResponse schema definition.
type ThreadStartResponse struct {
	// ActivePermissionProfile named or implicit built-in profile that produced the active permissions, when known.
	ActivePermissionProfile ActivePermissionProfile `json:"activePermissionProfile,omitzero"`
	ApprovalPolicy          AskForApproval          `json:"approvalPolicy"`

	// ApprovalsReviewer reviewer currently used for approval requests on this thread.
	ApprovalsReviewer ApprovalsReviewer `json:"approvalsReviewer"`
	Cwd               string            `json:"cwd"`

	// InstructionSources environment-native paths to instruction source files currently loaded for this thread.
	InstructionSources []LegacyAppPathString `json:"instructionSources,omitzero"`
	Model              string                `json:"model"`
	ModelProvider      string                `json:"modelProvider"`

	// MultiAgentMode @deprecated Always `explicitRequestOnly`. Use `reasoningEffort` for Ultra behavior.
	MultiAgentMode  MultiAgentMode   `json:"multiAgentMode,omitzero"`
	ReasoningEffort *ReasoningEffort `json:"reasoningEffort,omitzero"`

	// RuntimeWorkspaceRoots thread-scoped runtime workspace roots used to materialize `:workspace_roots`.
	RuntimeWorkspaceRoots []string `json:"runtimeWorkspaceRoots,omitzero"`

	// Sandbox legacy sandbox policy retained for compatibility. Experimental clients should prefer `activePermissionProfile` for profile provenance.
	Sandbox     SandboxPolicy `json:"sandbox"`
	ServiceTier string        `json:"serviceTier,omitzero"`
	Thread      ThreadPayload `json:"thread"`
}

func (value *ThreadStartResponse) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		ActivePermissionProfile ActivePermissionProfile `json:"activePermissionProfile,omitzero"`
		ApprovalPolicy          jsontext.Value          `json:"approvalPolicy"`
		ApprovalsReviewer       ApprovalsReviewer       `json:"approvalsReviewer"`
		Cwd                     string                  `json:"cwd"`
		InstructionSources      []LegacyAppPathString   `json:"instructionSources,omitzero"`
		Model                   string                  `json:"model"`
		ModelProvider           string                  `json:"modelProvider"`
		MultiAgentMode          jsontext.Value          `json:"multiAgentMode,omitzero"`
		ReasoningEffort         *ReasoningEffort        `json:"reasoningEffort,omitzero"`
		RuntimeWorkspaceRoots   []string                `json:"runtimeWorkspaceRoots,omitzero"`
		Sandbox                 jsontext.Value          `json:"sandbox"`
		ServiceTier             string                  `json:"serviceTier,omitzero"`
		Thread                  ThreadPayload           `json:"thread"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.ActivePermissionProfile = raw.ActivePermissionProfile
	if raw.ApprovalPolicy == nil {
		value.ApprovalPolicy = nil
	} else {
		decodedApprovalPolicy, err := decodeGeneratedAskForApproval(raw.ApprovalPolicy)
		if err != nil {
			return err
		}
		value.ApprovalPolicy = decodedApprovalPolicy
	}
	value.ApprovalsReviewer = raw.ApprovalsReviewer
	value.Cwd = raw.Cwd
	value.InstructionSources = raw.InstructionSources
	value.Model = raw.Model
	value.ModelProvider = raw.ModelProvider
	if raw.MultiAgentMode == nil {
		value.MultiAgentMode = nil
	} else {
		decodedMultiAgentMode, err := decodeGeneratedMultiAgentMode(raw.MultiAgentMode)
		if err != nil {
			return err
		}
		value.MultiAgentMode = decodedMultiAgentMode
	}
	value.ReasoningEffort = raw.ReasoningEffort
	value.RuntimeWorkspaceRoots = raw.RuntimeWorkspaceRoots
	if raw.Sandbox == nil {
		value.Sandbox = nil
	} else {
		decodedSandbox, err := decodeGeneratedSandboxPolicy(raw.Sandbox)
		if err != nil {
			return err
		}
		value.Sandbox = decodedSandbox
	}
	value.ServiceTier = raw.ServiceTier
	value.Thread = raw.Thread
	return nil
}

// ThreadStartSource is generated from the ThreadStartSource schema definition.
type ThreadStartSource string

const (
	// ThreadStartSourceStartup is the "startup" ThreadStartSource value.
	ThreadStartSourceStartup ThreadStartSource = "startup"
	// ThreadStartSourceClear is the "clear" ThreadStartSource value.
	ThreadStartSourceClear ThreadStartSource = "clear"
)

// ThreadStartedNotification is generated from the ThreadStartedNotification schema definition.
type ThreadStartedNotification struct {
	Thread ThreadPayload `json:"thread"`
}

// ThreadStatus is generated from the ThreadStatus schema definition.
type ThreadStatus interface {
	isThreadStatus()
}

// RawThreadStatus preserves an uninterpreted ThreadStatus JSON value.
type RawThreadStatus jsontext.Value

func (RawThreadStatus) isThreadStatus() {}

var (
	_ json.MarshalerTo     = RawThreadStatus{}
	_ json.UnmarshalerFrom = (*RawThreadStatus)(nil)
)

func (value RawThreadStatus) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawThreadStatus) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// NotLoadedThreadStatus is generated from the NotLoadedThreadStatus schema definition.
type NotLoadedThreadStatus struct {
	Type string `json:"type"`
}

func (NotLoadedThreadStatus) isThreadStatus() {}

// IDleThreadStatus is generated from the IdleThreadStatus schema definition.
type IDleThreadStatus struct {
	Type string `json:"type"`
}

func (IDleThreadStatus) isThreadStatus() {}

// SystemErrorThreadStatus is generated from the SystemErrorThreadStatus schema definition.
type SystemErrorThreadStatus struct {
	Type string `json:"type"`
}

func (SystemErrorThreadStatus) isThreadStatus() {}

// ActiveThreadStatus is generated from the ActiveThreadStatus schema definition.
type ActiveThreadStatus struct {
	ActiveFlags []ThreadActiveFlag `json:"activeFlags"`
	Type        string             `json:"type"`
}

func (ActiveThreadStatus) isThreadStatus() {}

func decodeGeneratedThreadStatus(raw jsontext.Value) (ThreadStatus, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "notLoaded":
			var value NotLoadedThreadStatus
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "idle":
			var value IDleThreadStatus
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "systemError":
			var value SystemErrorThreadStatus
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "active":
			var value ActiveThreadStatus
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawThreadStatus(raw), nil
	}
	return RawThreadStatus(raw), nil
}

// ThreadStatusChangedNotification is generated from the ThreadStatusChangedNotification schema definition.
type ThreadStatusChangedNotification struct {
	Status   ThreadStatus `json:"status"`
	ThreadID string       `json:"threadId"`
}

func (value *ThreadStatusChangedNotification) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Status   jsontext.Value `json:"status"`
		ThreadID string         `json:"threadId"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	if raw.Status == nil {
		value.Status = nil
	} else {
		decodedStatus, err := decodeGeneratedThreadStatus(raw.Status)
		if err != nil {
			return err
		}
		value.Status = decodedStatus
	}
	value.ThreadID = raw.ThreadID
	return nil
}

// ThreadTokenUsage is generated from the ThreadTokenUsage schema definition.
type ThreadTokenUsage struct {
	Last               TokenUsageBreakdown `json:"last"`
	ModelContextWindow int64               `json:"modelContextWindow,omitzero"`
	Total              TokenUsageBreakdown `json:"total"`
}

// ThreadTokenUsageUpdatedNotification is generated from the ThreadTokenUsageUpdatedNotification schema definition.
type ThreadTokenUsageUpdatedNotification struct {
	ThreadID   string           `json:"threadId"`
	TokenUsage ThreadTokenUsage `json:"tokenUsage"`
	TurnID     string           `json:"turnId"`
}

// ThreadTurnsListParams is generated from the ThreadTurnsListParams schema definition.
type ThreadTurnsListParams struct {
	// Cursor opaque cursor to pass to the next call to continue after the last turn.
	Cursor string `json:"cursor,omitzero"`

	// ItemsView how much item detail to include for each returned turn; defaults to summary.
	ItemsView TurnItemsView `json:"itemsView,omitzero"`

	// Limit optional turn page size.
	Limit int32 `json:"limit,omitzero"`

	// SortDirection optional turn pagination direction; defaults to descending.
	SortDirection SortDirection `json:"sortDirection,omitzero"`
	ThreadID      string        `json:"threadId"`
}

func (value *ThreadTurnsListParams) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		Cursor        string         `json:"cursor,omitzero"`
		ItemsView     jsontext.Value `json:"itemsView,omitzero"`
		Limit         int32          `json:"limit,omitzero"`
		SortDirection SortDirection  `json:"sortDirection,omitzero"`
		ThreadID      string         `json:"threadId"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.Cursor = raw.Cursor
	if raw.ItemsView == nil {
		value.ItemsView = nil
	} else {
		decodedItemsView, err := decodeGeneratedTurnItemsView(raw.ItemsView)
		if err != nil {
			return err
		}
		value.ItemsView = decodedItemsView
	}
	value.Limit = raw.Limit
	value.SortDirection = raw.SortDirection
	value.ThreadID = raw.ThreadID
	return nil
}

// ThreadTurnsListResponse is generated from the ThreadTurnsListResponse schema definition.
type ThreadTurnsListResponse struct {
	// BackwardsCursor opaque cursor to pass as `cursor` when reversing `sortDirection`. This is only populated when the page contains at least one turn. Use it with the opposite `sortDirection` to include the anchor turn again and catch updates to that turn.
	BackwardsCursor string `json:"backwardsCursor,omitzero"`
	Data            []Turn `json:"data"`

	// NextCursor opaque cursor to pass to the next call to continue after the last turn. if None, there are no more turns to return.
	NextCursor string `json:"nextCursor,omitzero"`
}

// ThreadUnarchiveParams is generated from the ThreadUnarchiveParams schema definition.
type ThreadUnarchiveParams struct {
	ThreadID string `json:"threadId"`
}

// ThreadUnarchiveResponse is generated from the ThreadUnarchiveResponse schema definition.
type ThreadUnarchiveResponse struct {
	Thread ThreadPayload `json:"thread"`
}

// ThreadUnarchivedNotification is generated from the ThreadUnarchivedNotification schema definition.
type ThreadUnarchivedNotification struct {
	ThreadID string `json:"threadId"`
}

// ThreadUnsubscribeParams is generated from the ThreadUnsubscribeParams schema definition.
type ThreadUnsubscribeParams struct {
	ThreadID string `json:"threadId"`
}

// ThreadUnsubscribeResponse is generated from the ThreadUnsubscribeResponse schema definition.
type ThreadUnsubscribeResponse struct {
	Status ThreadUnsubscribeStatus `json:"status"`
}

// ThreadUnsubscribeStatus is generated from the ThreadUnsubscribeStatus schema definition.
type ThreadUnsubscribeStatus string

const (
	// ThreadUnsubscribeStatusNotLoaded is the "notLoaded" ThreadUnsubscribeStatus value.
	ThreadUnsubscribeStatusNotLoaded ThreadUnsubscribeStatus = "notLoaded"
	// ThreadUnsubscribeStatusNotSubscribed is the "notSubscribed" ThreadUnsubscribeStatus value.
	ThreadUnsubscribeStatusNotSubscribed ThreadUnsubscribeStatus = "notSubscribed"
	// ThreadUnsubscribeStatusUnsubscribed is the "unsubscribed" ThreadUnsubscribeStatus value.
	ThreadUnsubscribeStatusUnsubscribed ThreadUnsubscribeStatus = "unsubscribed"
)

// TokenUsageBreakdown is generated from the TokenUsageBreakdown schema definition.
type TokenUsageBreakdown struct {
	CacheWriteInputTokens int64 `json:"cacheWriteInputTokens,omitzero"`
	CachedInputTokens     int64 `json:"cachedInputTokens"`
	InputTokens           int64 `json:"inputTokens"`
	OutputTokens          int64 `json:"outputTokens"`
	ReasoningOutputTokens int64 `json:"reasoningOutputTokens"`
	TotalTokens           int64 `json:"totalTokens"`
}

// Tool definition for a tool the client can call.
type Tool struct {
	Meta         jsontext.Value   `json:"_meta,omitzero"`
	Annotations  jsontext.Value   `json:"annotations,omitzero"`
	Description  string           `json:"description,omitzero"`
	Icons        []jsontext.Value `json:"icons,omitzero"`
	InputSchema  jsontext.Value   `json:"inputSchema"`
	Name         string           `json:"name"`
	OutputSchema jsontext.Value   `json:"outputSchema,omitzero"`
	Title        string           `json:"title,omitzero"`
}

// ToolsV2 is generated from the ToolsV2 schema definition.
type ToolsV2 struct {
	WebSearch WebSearchToolConfig `json:"web_search,omitzero"`
}

// Turn is generated from the Turn schema definition.
type Turn struct {
	// CompletedAt unix timestamp (in seconds) when the turn completed.
	CompletedAt int64 `json:"completedAt,omitzero"`

	// DurationMs duration between turn start and completion in milliseconds, if known.
	DurationMs int64 `json:"durationMs,omitzero"`

	// Error only populated when the Turn's status is failed.
	Error TurnError `json:"error,omitzero"`

	// ID identifier for this turn. Codex-generated turn IDs are UUIDv7.
	ID string `json:"id"`

	// Items thread items currently included in this turn payload.
	Items []ThreadItem `json:"items"`

	// ItemsView describes how much of `items` has been loaded for this turn.
	ItemsView TurnItemsView `json:"itemsView,omitzero"`

	// StartedAt unix timestamp (in seconds) when the turn started.
	StartedAt int64      `json:"startedAt,omitzero"`
	Status    TurnStatus `json:"status"`
}

func (value *Turn) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		CompletedAt int64            `json:"completedAt,omitzero"`
		DurationMs  int64            `json:"durationMs,omitzero"`
		Error       TurnError        `json:"error,omitzero"`
		ID          string           `json:"id"`
		Items       []jsontext.Value `json:"items"`
		ItemsView   jsontext.Value   `json:"itemsView,omitzero"`
		StartedAt   int64            `json:"startedAt,omitzero"`
		Status      TurnStatus       `json:"status"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.CompletedAt = raw.CompletedAt
	value.DurationMs = raw.DurationMs
	value.Error = raw.Error
	value.ID = raw.ID
	if raw.Items == nil {
		value.Items = nil
	} else {
		value.Items = make([]ThreadItem, len(raw.Items))
		for i, item := range raw.Items {
			if item != nil {
				decodedItems, err := decodeGeneratedThreadItem(item)
				if err != nil {
					return err
				}
				value.Items[i] = decodedItems
			}
		}
	}
	if raw.ItemsView == nil {
		value.ItemsView = nil
	} else {
		decodedItemsView, err := decodeGeneratedTurnItemsView(raw.ItemsView)
		if err != nil {
			return err
		}
		value.ItemsView = decodedItemsView
	}
	value.StartedAt = raw.StartedAt
	value.Status = raw.Status
	return nil
}

// TurnCompletedNotification is generated from the TurnCompletedNotification schema definition.
type TurnCompletedNotification struct {
	ThreadID string `json:"threadId"`
	Turn     Turn   `json:"turn"`
}

// TurnDiffUpdatedNotification notification that the turn-level unified diff has changed. Contains the latest aggregated diff across all file changes in the turn.
type TurnDiffUpdatedNotification struct {
	Diff     string `json:"diff"`
	ThreadID string `json:"threadId"`
	TurnID   string `json:"turnId"`
}

// TurnEnvironmentParams is generated from the TurnEnvironmentParams schema definition.
type TurnEnvironmentParams struct {
	Cwd           LegacyAppPathString `json:"cwd"`
	EnvironmentID string              `json:"environmentId"`

	// RuntimeWorkspaceRoots environment-native runtime workspace roots. Omitted defaults to `cwd`.
	RuntimeWorkspaceRoots []LegacyAppPathString `json:"runtimeWorkspaceRoots,omitzero"`
}

// TurnError is generated from the TurnError schema definition.
type TurnError struct {
	AdditionalDetails string         `json:"additionalDetails,omitzero"`
	CodexErrorInfo    CodexErrorInfo `json:"codexErrorInfo,omitzero"`
	Message           string         `json:"message"`
}

func (value *TurnError) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		AdditionalDetails string         `json:"additionalDetails,omitzero"`
		CodexErrorInfo    jsontext.Value `json:"codexErrorInfo,omitzero"`
		Message           string         `json:"message"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.AdditionalDetails = raw.AdditionalDetails
	if raw.CodexErrorInfo == nil {
		value.CodexErrorInfo = nil
	} else {
		decodedCodexErrorInfo, err := decodeGeneratedCodexErrorInfo(raw.CodexErrorInfo)
		if err != nil {
			return err
		}
		value.CodexErrorInfo = decodedCodexErrorInfo
	}
	value.Message = raw.Message
	return nil
}

// TurnInterruptParams is generated from the TurnInterruptParams schema definition.
type TurnInterruptParams struct {
	ThreadID string `json:"threadId"`
	TurnID   string `json:"turnId"`
}

// TurnInterruptResponse is generated from the TurnInterruptResponse schema definition.
type TurnInterruptResponse struct{}

// TurnItemsView is generated from the TurnItemsView schema definition.
type TurnItemsView interface {
	isTurnItemsView()
}

// RawTurnItemsView preserves an uninterpreted TurnItemsView JSON value.
type RawTurnItemsView jsontext.Value

func (RawTurnItemsView) isTurnItemsView() {}

var (
	_ json.MarshalerTo     = RawTurnItemsView{}
	_ json.UnmarshalerFrom = (*RawTurnItemsView)(nil)
)

func (value RawTurnItemsView) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawTurnItemsView) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// TurnItemsViewValue is a string-valued TurnItemsView variant.
type TurnItemsViewValue string

func (TurnItemsViewValue) isTurnItemsView() {}

const (
	// TurnItemsViewValueFull is the "full" TurnItemsViewValue value.
	TurnItemsViewValueFull TurnItemsViewValue = "full"
	// TurnItemsViewValueNotLoaded is the "notLoaded" TurnItemsViewValue value.
	TurnItemsViewValueNotLoaded TurnItemsViewValue = "notLoaded"
	// TurnItemsViewValueSummary is the "summary" TurnItemsViewValue value.
	TurnItemsViewValueSummary TurnItemsViewValue = "summary"
)

func decodeGeneratedTurnItemsView(raw jsontext.Value) (TurnItemsView, error) {
	if raw == nil {
		return nil, nil
	}
	var text string
	if err := json.Unmarshal(raw, &text); err == nil {
		switch text {
		case "full":
			return TurnItemsViewValue(text), nil
		case "notLoaded":
			return TurnItemsViewValue(text), nil
		case "summary":
			return TurnItemsViewValue(text), nil
		}
		return RawTurnItemsView(raw), nil
	}
	return RawTurnItemsView(raw), nil
}

// TurnModerationMetadataNotification is generated from the TurnModerationMetadataNotification schema definition.
type TurnModerationMetadataNotification struct {
	Metadata jsontext.Value `json:"metadata"`
	ThreadID string         `json:"threadId"`
	TurnID   string         `json:"turnId"`
}

// TurnPlanStep is generated from the TurnPlanStep schema definition.
type TurnPlanStep struct {
	Status TurnPlanStepStatus `json:"status"`
	Step   string             `json:"step"`
}

// TurnPlanStepStatus is generated from the TurnPlanStepStatus schema definition.
type TurnPlanStepStatus string

const (
	// TurnPlanStepStatusPending is the "pending" TurnPlanStepStatus value.
	TurnPlanStepStatusPending TurnPlanStepStatus = "pending"
	// TurnPlanStepStatusInProgress is the "inProgress" TurnPlanStepStatus value.
	TurnPlanStepStatusInProgress TurnPlanStepStatus = "inProgress"
	// TurnPlanStepStatusCompleted is the "completed" TurnPlanStepStatus value.
	TurnPlanStepStatusCompleted TurnPlanStepStatus = "completed"
)

// TurnPlanUpdatedNotification is generated from the TurnPlanUpdatedNotification schema definition.
type TurnPlanUpdatedNotification struct {
	Explanation string         `json:"explanation,omitzero"`
	Plan        []TurnPlanStep `json:"plan"`
	ThreadID    string         `json:"threadId"`
	TurnID      string         `json:"turnId"`
}

// TurnStartParams is generated from the TurnStartParams schema definition.
type TurnStartParams struct {
	// AdditionalContext optional client-provided context fragments keyed by an opaque source identifier.
	AdditionalContext map[string]AdditionalContextEntry `json:"additionalContext,omitzero"`

	// ApprovalPolicy overrides the approval policy for this turn and subsequent turns.
	ApprovalPolicy AskForApproval `json:"approvalPolicy,omitzero"`

	// ApprovalsReviewer overrides where approval requests are routed for review on this turn and subsequent turns.
	ApprovalsReviewer   ApprovalsReviewer `json:"approvalsReviewer,omitzero"`
	ClientUserMessageID string            `json:"clientUserMessageId,omitzero"`

	// CollaborationMode sets a pre-set collaboration mode. Takes precedence over model, reasoning_effort, and developer instructions if set.
	//
	// For `collaboration_mode.settings.developer_instructions`, `null` means "use the built-in instructions for the selected mode".
	CollaborationMode CollaborationMode `json:"collaborationMode,omitzero"`

	// Cwd overrides the working directory for this turn and subsequent turns.
	Cwd string `json:"cwd,omitzero"`

	// Effort overrides the reasoning effort for this turn and subsequent turns.
	Effort *ReasoningEffort `json:"effort,omitzero"`

	// Environments optional environments for this turn and subsequent turns.
	//
	// Omitted uses the thread sticky environments. Empty disables environment access for this turn. Non-empty selects the first environment as the current turn environment for this turn.
	Environments []TurnEnvironmentParams `json:"environments,omitzero"`
	Input        []UserInput             `json:"input"`

	// Model overrides the model for this turn and subsequent turns.
	Model string `json:"model,omitzero"`

	// MultiAgentMode @deprecated Ignored. Use `effort: "ultra"` for proactive multi-agent behavior.
	MultiAgentMode MultiAgentMode `json:"multiAgentMode,omitzero"`

	// OutputSchema optional JSON Schema used to constrain the final assistant message for this turn.
	OutputSchema jsontext.Value `json:"outputSchema,omitzero"`

	// Permissions selects a named permissions profile id for this turn and subsequent turns. Cannot be combined with `sandboxPolicy`.
	Permissions string `json:"permissions,omitzero"`

	// Personality overrides the personality for this turn and subsequent turns.
	Personality Personality `json:"personality,omitzero"`

	// ResponsesapiClientMetadata optional metadata to enrich Codex's ResponsesAPI turn metadata.
	//
	// Entries are flattened into the JSON string sent as `client_metadata["x-codex-turn-metadata"]` on ResponsesAPI HTTP and websocket requests.
	//
	// They are not sent as top-level ResponsesAPI `client_metadata` keys, and reserved keys such as `session_id`, `thread_id`, `turn_id`, and `window_id` cannot be overridden.
	ResponsesapiClientMetadata map[string]string `json:"responsesapiClientMetadata,omitzero"`

	// RuntimeWorkspaceRoots replaces the thread's runtime workspace roots for this turn and subsequent turns. Paths must be absolute.
	RuntimeWorkspaceRoots []string `json:"runtimeWorkspaceRoots,omitzero"`

	// SandboxPolicy overrides the sandbox policy for this turn and subsequent turns.
	SandboxPolicy SandboxPolicy `json:"sandboxPolicy,omitzero"`

	// ServiceTier overrides the service tier for this turn and subsequent turns.
	ServiceTier string `json:"serviceTier,omitzero"`

	// Summary overrides the reasoning summary for this turn and subsequent turns.
	Summary  ReasoningSummary `json:"summary,omitzero"`
	ThreadID string           `json:"threadId"`
}

func (value *TurnStartParams) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		AdditionalContext          map[string]AdditionalContextEntry `json:"additionalContext,omitzero"`
		ApprovalPolicy             jsontext.Value                    `json:"approvalPolicy,omitzero"`
		ApprovalsReviewer          ApprovalsReviewer                 `json:"approvalsReviewer,omitzero"`
		ClientUserMessageID        string                            `json:"clientUserMessageId,omitzero"`
		CollaborationMode          CollaborationMode                 `json:"collaborationMode,omitzero"`
		Cwd                        string                            `json:"cwd,omitzero"`
		Effort                     *ReasoningEffort                  `json:"effort,omitzero"`
		Environments               []TurnEnvironmentParams           `json:"environments,omitzero"`
		Input                      []jsontext.Value                  `json:"input"`
		Model                      string                            `json:"model,omitzero"`
		MultiAgentMode             jsontext.Value                    `json:"multiAgentMode,omitzero"`
		OutputSchema               jsontext.Value                    `json:"outputSchema,omitzero"`
		Permissions                string                            `json:"permissions,omitzero"`
		Personality                Personality                       `json:"personality,omitzero"`
		ResponsesapiClientMetadata map[string]string                 `json:"responsesapiClientMetadata,omitzero"`
		RuntimeWorkspaceRoots      []string                          `json:"runtimeWorkspaceRoots,omitzero"`
		SandboxPolicy              jsontext.Value                    `json:"sandboxPolicy,omitzero"`
		ServiceTier                string                            `json:"serviceTier,omitzero"`
		Summary                    jsontext.Value                    `json:"summary,omitzero"`
		ThreadID                   string                            `json:"threadId"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.AdditionalContext = raw.AdditionalContext
	if raw.ApprovalPolicy == nil {
		value.ApprovalPolicy = nil
	} else {
		decodedApprovalPolicy, err := decodeGeneratedAskForApproval(raw.ApprovalPolicy)
		if err != nil {
			return err
		}
		value.ApprovalPolicy = decodedApprovalPolicy
	}
	value.ApprovalsReviewer = raw.ApprovalsReviewer
	value.ClientUserMessageID = raw.ClientUserMessageID
	value.CollaborationMode = raw.CollaborationMode
	value.Cwd = raw.Cwd
	value.Effort = raw.Effort
	value.Environments = raw.Environments
	if raw.Input == nil {
		value.Input = nil
	} else {
		value.Input = make([]UserInput, len(raw.Input))
		for i, item := range raw.Input {
			if item != nil {
				decodedInput, err := decodeGeneratedUserInput(item)
				if err != nil {
					return err
				}
				value.Input[i] = decodedInput
			}
		}
	}
	value.Model = raw.Model
	if raw.MultiAgentMode == nil {
		value.MultiAgentMode = nil
	} else {
		decodedMultiAgentMode, err := decodeGeneratedMultiAgentMode(raw.MultiAgentMode)
		if err != nil {
			return err
		}
		value.MultiAgentMode = decodedMultiAgentMode
	}
	value.OutputSchema = raw.OutputSchema
	value.Permissions = raw.Permissions
	value.Personality = raw.Personality
	value.ResponsesapiClientMetadata = raw.ResponsesapiClientMetadata
	value.RuntimeWorkspaceRoots = raw.RuntimeWorkspaceRoots
	if raw.SandboxPolicy == nil {
		value.SandboxPolicy = nil
	} else {
		decodedSandboxPolicy, err := decodeGeneratedSandboxPolicy(raw.SandboxPolicy)
		if err != nil {
			return err
		}
		value.SandboxPolicy = decodedSandboxPolicy
	}
	value.ServiceTier = raw.ServiceTier
	if raw.Summary == nil {
		value.Summary = nil
	} else {
		decodedSummary, err := decodeGeneratedReasoningSummary(raw.Summary)
		if err != nil {
			return err
		}
		value.Summary = decodedSummary
	}
	value.ThreadID = raw.ThreadID
	return nil
}

// TurnStartResponse is generated from the TurnStartResponse schema definition.
type TurnStartResponse struct {
	Turn Turn `json:"turn"`
}

// TurnStartedNotification is generated from the TurnStartedNotification schema definition.
type TurnStartedNotification struct {
	ThreadID string `json:"threadId"`
	Turn     Turn   `json:"turn"`
}

// TurnStatus is generated from the TurnStatus schema definition.
type TurnStatus string

const (
	// TurnStatusCompleted is the "completed" TurnStatus value.
	TurnStatusCompleted TurnStatus = "completed"
	// TurnStatusInterrupted is the "interrupted" TurnStatus value.
	TurnStatusInterrupted TurnStatus = "interrupted"
	// TurnStatusFailed is the "failed" TurnStatus value.
	TurnStatusFailed TurnStatus = "failed"
	// TurnStatusInProgress is the "inProgress" TurnStatus value.
	TurnStatusInProgress TurnStatus = "inProgress"
)

// TurnSteerParams is generated from the TurnSteerParams schema definition.
type TurnSteerParams struct {
	// AdditionalContext optional client-provided context fragments keyed by an opaque source identifier.
	AdditionalContext   map[string]AdditionalContextEntry `json:"additionalContext,omitzero"`
	ClientUserMessageID string                            `json:"clientUserMessageId,omitzero"`

	// ExpectedTurnID required active turn id precondition. The request fails when it does not match the currently active turn.
	ExpectedTurnID string      `json:"expectedTurnId"`
	Input          []UserInput `json:"input"`

	// ResponsesapiClientMetadata optional metadata to enrich Codex's ResponsesAPI turn metadata.
	//
	// Entries are flattened into the JSON string sent as `client_metadata["x-codex-turn-metadata"]` on ResponsesAPI HTTP and websocket requests.
	//
	// They are not sent as top-level ResponsesAPI `client_metadata` keys, and reserved keys such as `session_id`, `thread_id`, `turn_id`, and `window_id` cannot be overridden.
	ResponsesapiClientMetadata map[string]string `json:"responsesapiClientMetadata,omitzero"`
	ThreadID                   string            `json:"threadId"`
}

func (value *TurnSteerParams) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	var raw struct {
		AdditionalContext          map[string]AdditionalContextEntry `json:"additionalContext,omitzero"`
		ClientUserMessageID        string                            `json:"clientUserMessageId,omitzero"`
		ExpectedTurnID             string                            `json:"expectedTurnId"`
		Input                      []jsontext.Value                  `json:"input"`
		ResponsesapiClientMetadata map[string]string                 `json:"responsesapiClientMetadata,omitzero"`
		ThreadID                   string                            `json:"threadId"`
	}
	if err := json.UnmarshalDecode(dec, &raw); err != nil {
		return err
	}
	value.AdditionalContext = raw.AdditionalContext
	value.ClientUserMessageID = raw.ClientUserMessageID
	value.ExpectedTurnID = raw.ExpectedTurnID
	if raw.Input == nil {
		value.Input = nil
	} else {
		value.Input = make([]UserInput, len(raw.Input))
		for i, item := range raw.Input {
			if item != nil {
				decodedInput, err := decodeGeneratedUserInput(item)
				if err != nil {
					return err
				}
				value.Input[i] = decodedInput
			}
		}
	}
	value.ResponsesapiClientMetadata = raw.ResponsesapiClientMetadata
	value.ThreadID = raw.ThreadID
	return nil
}

// TurnSteerResponse is generated from the TurnSteerResponse schema definition.
type TurnSteerResponse struct {
	TurnID string `json:"turnId"`
}

// TurnsPage is generated from the TurnsPage schema definition.
type TurnsPage struct {
	BackwardsCursor string `json:"backwardsCursor,omitzero"`
	Data            []Turn `json:"data"`
	NextCursor      string `json:"nextCursor,omitzero"`
}

// UserInput is generated from the UserInput schema definition.
type UserInput interface {
	isUserInput()
}

// RawUserInput preserves an uninterpreted UserInput JSON value.
type RawUserInput jsontext.Value

func (RawUserInput) isUserInput() {}

var (
	_ json.MarshalerTo     = RawUserInput{}
	_ json.UnmarshalerFrom = (*RawUserInput)(nil)
)

func (value RawUserInput) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawUserInput) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// TextUserInput is generated from the TextUserInput schema definition.
type TextUserInput struct {
	Text string `json:"text"`

	// TextElements uI-defined spans within `text` used to render or persist special elements.
	TextElements []TextElement `json:"text_elements,omitzero"`
	Type         string        `json:"type"`
}

func (TextUserInput) isUserInput() {}

// ImageUserInput is generated from the ImageUserInput schema definition.
type ImageUserInput struct {
	Detail ImageDetail `json:"detail,omitzero"`
	Type   string      `json:"type"`
	URL    string      `json:"url"`
}

func (ImageUserInput) isUserInput() {}

// LocalImageUserInput is generated from the LocalImageUserInput schema definition.
type LocalImageUserInput struct {
	Detail ImageDetail `json:"detail,omitzero"`
	Path   string      `json:"path"`
	Type   string      `json:"type"`
}

func (LocalImageUserInput) isUserInput() {}

// AudioUserInput is generated from the AudioUserInput schema definition.
type AudioUserInput struct {
	Type string `json:"type"`
	URL  string `json:"url"`
}

func (AudioUserInput) isUserInput() {}

// LocalAudioUserInput is generated from the LocalAudioUserInput schema definition.
type LocalAudioUserInput struct {
	Path string `json:"path"`
	Type string `json:"type"`
}

func (LocalAudioUserInput) isUserInput() {}

// SkillUserInput is generated from the SkillUserInput schema definition.
type SkillUserInput struct {
	Name string `json:"name"`
	Path string `json:"path"`
	Type string `json:"type"`
}

func (SkillUserInput) isUserInput() {}

// MentionUserInput is generated from the MentionUserInput schema definition.
type MentionUserInput struct {
	Name string `json:"name"`
	Path string `json:"path"`
	Type string `json:"type"`
}

func (MentionUserInput) isUserInput() {}

func decodeGeneratedUserInput(raw jsontext.Value) (UserInput, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "text":
			var value TextUserInput
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "image":
			var value ImageUserInput
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "localImage":
			var value LocalImageUserInput
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "audio":
			var value AudioUserInput
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "localAudio":
			var value LocalAudioUserInput
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "skill":
			var value SkillUserInput
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "mention":
			var value MentionUserInput
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawUserInput(raw), nil
	}
	return RawUserInput(raw), nil
}

// Verbosity controls output length/detail on GPT-5 models via the Responses API. Serialized with lowercase values to match the OpenAI API.
type Verbosity string

const (
	// VerbosityLow is the "low" Verbosity value.
	VerbosityLow Verbosity = "low"
	// VerbosityMedium is the "medium" Verbosity value.
	VerbosityMedium Verbosity = "medium"
	// VerbosityHigh is the "high" Verbosity value.
	VerbosityHigh Verbosity = "high"
)

// WarningNotification is generated from the WarningNotification schema definition.
type WarningNotification struct {
	// Message concise warning message for the user.
	Message string `json:"message"`

	// ThreadID optional thread target when the warning applies to a specific thread.
	ThreadID string `json:"threadId,omitzero"`
}

// WebSearchAction is generated from the WebSearchAction schema definition.
type WebSearchAction interface {
	isWebSearchAction()
}

// RawWebSearchAction preserves an uninterpreted WebSearchAction JSON value.
type RawWebSearchAction jsontext.Value

func (RawWebSearchAction) isWebSearchAction() {}

var (
	_ json.MarshalerTo     = RawWebSearchAction{}
	_ json.UnmarshalerFrom = (*RawWebSearchAction)(nil)
)

func (value RawWebSearchAction) MarshalJSONTo(enc *jsontext.Encoder) error {
	return enc.WriteValue(jsontext.Value(value))
}

func (value *RawWebSearchAction) UnmarshalJSONFrom(dec *jsontext.Decoder) error {
	return json.UnmarshalDecode(dec, (*jsontext.Value)(value))
}

// SearchWebSearchAction is generated from the SearchWebSearchAction schema definition.
type SearchWebSearchAction struct {
	Queries []string `json:"queries,omitzero"`
	Query   string   `json:"query,omitzero"`
	Type    string   `json:"type"`
}

func (SearchWebSearchAction) isWebSearchAction() {}

// OpenPageWebSearchAction is generated from the OpenPageWebSearchAction schema definition.
type OpenPageWebSearchAction struct {
	Type string `json:"type"`
	URL  string `json:"url,omitzero"`
}

func (OpenPageWebSearchAction) isWebSearchAction() {}

// FindInPageWebSearchAction is generated from the FindInPageWebSearchAction schema definition.
type FindInPageWebSearchAction struct {
	Pattern string `json:"pattern,omitzero"`
	Type    string `json:"type"`
	URL     string `json:"url,omitzero"`
}

func (FindInPageWebSearchAction) isWebSearchAction() {}

// OtherWebSearchAction is generated from the OtherWebSearchAction schema definition.
type OtherWebSearchAction struct {
	Type string `json:"type"`
}

func (OtherWebSearchAction) isWebSearchAction() {}

func decodeGeneratedWebSearchAction(raw jsontext.Value) (WebSearchAction, error) {
	if raw == nil {
		return nil, nil
	}
	var object struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(raw, &object); err == nil {
		switch object.Type {
		case "search":
			var value SearchWebSearchAction
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "openPage":
			var value OpenPageWebSearchAction
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "findInPage":
			var value FindInPageWebSearchAction
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		case "other":
			var value OtherWebSearchAction
			if err := json.Unmarshal(raw, &value); err != nil {
				return nil, err
			}
			return value, nil
		}
		return RawWebSearchAction(raw), nil
	}
	return RawWebSearchAction(raw), nil
}

// WebSearchContextSize is generated from the WebSearchContextSize schema definition.
type WebSearchContextSize string

const (
	// WebSearchContextSizeLow is the "low" WebSearchContextSize value.
	WebSearchContextSizeLow WebSearchContextSize = "low"
	// WebSearchContextSizeMedium is the "medium" WebSearchContextSize value.
	WebSearchContextSizeMedium WebSearchContextSize = "medium"
	// WebSearchContextSizeHigh is the "high" WebSearchContextSize value.
	WebSearchContextSizeHigh WebSearchContextSize = "high"
)

// WebSearchLocation is generated from the WebSearchLocation schema definition.
type WebSearchLocation struct {
	City     string `json:"city,omitzero"`
	Country  string `json:"country,omitzero"`
	Region   string `json:"region,omitzero"`
	Timezone string `json:"timezone,omitzero"`
}

// WebSearchMode is generated from the WebSearchMode schema definition.
type WebSearchMode string

const (
	// WebSearchModeDisabled is the "disabled" WebSearchMode value.
	WebSearchModeDisabled WebSearchMode = "disabled"
	// WebSearchModeCached is the "cached" WebSearchMode value.
	WebSearchModeCached WebSearchMode = "cached"
	// WebSearchModeIndexed is the "indexed" WebSearchMode value.
	WebSearchModeIndexed WebSearchMode = "indexed"
	// WebSearchModeLive is the "live" WebSearchMode value.
	WebSearchModeLive WebSearchMode = "live"
)

// WebSearchToolConfig is generated from the WebSearchToolConfig schema definition.
type WebSearchToolConfig struct {
	AllowedDomains []string             `json:"allowed_domains,omitzero"`
	ContextSize    WebSearchContextSize `json:"context_size,omitzero"`
	Location       WebSearchLocation    `json:"location,omitzero"`
}

// WindowsSandboxReadiness is generated from the WindowsSandboxReadiness schema definition.
type WindowsSandboxReadiness string

const (
	// WindowsSandboxReadinessReady is the "ready" WindowsSandboxReadiness value.
	WindowsSandboxReadinessReady WindowsSandboxReadiness = "ready"
	// WindowsSandboxReadinessNotConfigured is the "notConfigured" WindowsSandboxReadiness value.
	WindowsSandboxReadinessNotConfigured WindowsSandboxReadiness = "notConfigured"
	// WindowsSandboxReadinessUpdateRequired is the "updateRequired" WindowsSandboxReadiness value.
	WindowsSandboxReadinessUpdateRequired WindowsSandboxReadiness = "updateRequired"
)

// WindowsSandboxReadinessResponse is generated from the WindowsSandboxReadinessResponse schema definition.
type WindowsSandboxReadinessResponse struct {
	Status WindowsSandboxReadiness `json:"status"`
}

// WindowsSandboxSetupCompletedNotification is generated from the WindowsSandboxSetupCompletedNotification schema definition.
type WindowsSandboxSetupCompletedNotification struct {
	Error   string                  `json:"error,omitzero"`
	Mode    WindowsSandboxSetupMode `json:"mode"`
	Success bool                    `json:"success"`
}

// WindowsSandboxSetupMode is generated from the WindowsSandboxSetupMode schema definition.
type WindowsSandboxSetupMode string

const (
	// WindowsSandboxSetupModeElevated is the "elevated" WindowsSandboxSetupMode value.
	WindowsSandboxSetupModeElevated WindowsSandboxSetupMode = "elevated"
	// WindowsSandboxSetupModeUnelevated is the "unelevated" WindowsSandboxSetupMode value.
	WindowsSandboxSetupModeUnelevated WindowsSandboxSetupMode = "unelevated"
)

// WindowsSandboxSetupStartParams is generated from the WindowsSandboxSetupStartParams schema definition.
type WindowsSandboxSetupStartParams struct {
	Cwd  string                  `json:"cwd,omitzero"`
	Mode WindowsSandboxSetupMode `json:"mode"`
}

// WindowsSandboxSetupStartResponse is generated from the WindowsSandboxSetupStartResponse schema definition.
type WindowsSandboxSetupStartResponse struct {
	Started bool `json:"started"`
}

// WindowsWorldWritableWarningNotification is generated from the WindowsWorldWritableWarningNotification schema definition.
type WindowsWorldWritableWarningNotification struct {
	ExtraCount  uint     `json:"extraCount"`
	FailedScan  bool     `json:"failedScan"`
	SamplePaths []string `json:"samplePaths"`
}

// WorkspaceMessage is generated from the WorkspaceMessage schema definition.
type WorkspaceMessage struct {
	// ArchivedAt unix timestamp (in seconds) when the message was archived.
	ArchivedAt int64 `json:"archivedAt,omitzero"`

	// CreatedAt unix timestamp (in seconds) when the message was created.
	CreatedAt   int64                `json:"createdAt,omitzero"`
	MessageBody string               `json:"messageBody"`
	MessageID   string               `json:"messageId"`
	MessageType WorkspaceMessageType `json:"messageType"`
}

// WorkspaceMessageType is generated from the WorkspaceMessageType schema definition.
type WorkspaceMessageType string

const (
	// WorkspaceMessageTypeHeadline is the "headline" WorkspaceMessageType value.
	WorkspaceMessageTypeHeadline WorkspaceMessageType = "headline"
	// WorkspaceMessageTypeAnnouncement is the "announcement" WorkspaceMessageType value.
	WorkspaceMessageTypeAnnouncement WorkspaceMessageType = "announcement"
	// WorkspaceMessageTypeUnknown is the "unknown" WorkspaceMessageType value.
	WorkspaceMessageTypeUnknown WorkspaceMessageType = "unknown"
)

// WriteStatus is generated from the WriteStatus schema definition.
type WriteStatus string

const (
	// WriteStatusOk is the "ok" WriteStatus value.
	WriteStatusOk WriteStatus = "ok"
	// WriteStatusOkOverridden is the "okOverridden" WriteStatus value.
	WriteStatusOkOverridden WriteStatus = "okOverridden"
)

func (WarningNotification) isServerNotification() {}
